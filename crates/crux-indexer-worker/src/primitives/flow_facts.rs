...
use crate::{
    primitives::data_access::{DataAccessRef, data_access_refs_for_value},
    primitives::definition::{NativeDefinitionInput, native_static_definition, safe_id},
    primitives::flow_output::{
...
fn flow_steps(
    ...
    for call in roots.iter().flat_map(|root| function_calls(root)) {
        ...
        let source_refs = match &target_variable {
            Some(target_variable) => crate::primitives::flow_output::step_target_source_refs(
                context,
                &definition_id,
                target_variable,
            )?,
