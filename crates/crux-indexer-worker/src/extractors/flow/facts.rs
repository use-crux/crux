...
use crate::{
    ...
    extractors::data::access::{DataAccessRef, data_access_refs_for_value},
    extractors::definition::{NativeDefinitionInput, native_static_definition, safe_id},
    extractors::flow::output::{
...
fn flow_steps(
    ...
    for call in roots.iter().flat_map(|root| function_calls(root)) {
        ...
        let source_refs = match &target_variable {
            Some(target_variable) => crate::extractors::flow::output::step_target_source_refs(
                context,
                &definition_id,
                target_variable,
            )?,
