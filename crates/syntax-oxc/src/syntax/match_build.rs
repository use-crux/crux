...
use crate::{
    protocol::{StaticInitializerRecord, StaticSourceMatch, StaticSyntaxValue},
    syntax::match_interests::{CalleeMatcher, EvidenceSlice},
    syntax::semantic_imports::SemanticImportIndex,
    syntax::semantic_initializers::SemanticInitializerIndex,
    syntax::source::{SourceNeedleIndex, SourceView},
    syntax::values::{call_args, callee_record_from_expression, expression_name, object_value},
};
...
impl<'a, 'b> MatchContext<'a, 'b> {
    pub(crate) fn with_scope(self, scope_id: ScopeId) -> Self {
        Self { scope_id, ..self }
    }

    fn visible_initializers(self, span: Span) -> Vec<StaticInitializerRecord> {
        self.initializer_index.visible_before(self.scope_id, span)
    }
}
...
pub(crate) fn match_from_declarator(
    ...
    match init {
        Expression::ObjectExpression(object) => Some(StaticSourceMatch::Object {
            ...
            local_initializers: context.visible_initializers(object.span),
        }),
        Expression::CallExpression(call) => {
            ...
            Some(call_match(context, variable_name, call, exported))
        }
        Expression::NewExpression(new_expression) => {
            new_match(context, variable_name, new_expression, exported)
        }
        _ => None,
    }
}
...
pub(crate) fn call_match(
    ...
    StaticSourceMatch::Call {
        ...
        local_initializers: context.visible_initializers(call.span),
    }
}
...
pub(crate) fn new_match(
    ...
    Some(StaticSourceMatch::New {
        ...
        local_initializers: context.visible_initializers(new_expression.span),
    })
}
