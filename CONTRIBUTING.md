...
make build       # build local workers, devtools UI, embed assets, and build Crux Local
...
1. `pnpm --filter @use-crux/local-workers build` builds Node workers into `packages/local-workers/dist/`, and `pnpm --filter @use-crux/devtools build` builds the React UI into `packages/devtools/ui/dist/`.
