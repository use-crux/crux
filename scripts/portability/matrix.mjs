...
export async function loadPortabilityContext(repoRoot, options = {}) {
  ...
  for (const declaration of matrix.packages) {
    ...
    const root = stageRoot ? stagedPackages.get(declaration.name) : dirname(join(repoRoot, declaration.manifest))
    if (!root) {
    ...
    packages.set(declaration.name, { declaration, manifest, root })
  }
