...
const outputUrl = new URL(
  "../packages/core/src/project-index/fixtures/definition-coverage.json",
  import.meta.url,
);
...
function canonicalRef(kind) {
  ...
  switch (kind) {
    case "rag.knowledgeBase":
      return ref(
        "rag.knowledgeBase:connected",
        kind,
        "contributed-knowledge-base",
      );
    case "toolPolicy":
      return ref("toolPolicy:connected", kind, "contributed-tool-policy");
    case "flow.step":
      return ref("flow.step:connected:connected", kind, "invoked-flow-step");
    case "composition.parallel.branch":
      return ref(
        "composition.parallel:connected:branch:connected",
        kind,
        "invoked-composition-branch",
      );
    case "rag.recipe.step":
      return ref(
        "rag.recipe:connected:step:connected",
        kind,
        "invoked-recipe-step",
      );
    case "scorer":
      return ref("scorer:connected", kind, "invoked-scorer");
    default:
      return undefined;
  }
}
...
function expectedTreatment(descriptor) {
  if (descriptor.primary === "runtime-observed-unjoined")
    return "runtime-unjoined";
  if (
    descriptor.primary === "directly-observed" ||
    descriptor.runtimeIdentity === "definition-ref"
  )
...
if (process.argv.includes("--check")) {
  ...
  if (current !== serialized) {
    process.stderr.write(
      `stale generated fixture: ${fileURLToPath(outputUrl)}\n`,
    );
    process.exitCode = 1;
  }
} else {
