...
Attach bounded `DefinitionRef[]` evidence to observability records so prompt, context, tool, agent, flow, retriever, blackboard, and composition spans join directly to their canonical Project Index definitions without stack or name guessing. Composition APIs now require an authored `id`; the random per-execution `compositionId` remains a separate execution identity.
