...
function describeProvenance(provenance: ProjectModelProvenance): string {
  switch (provenance.kind) {
    case 'source':
      return provenance.exportName === undefined ? provenance.file : `${provenance.file}#${provenance.exportName}`
    case 'runtime':
      return provenance.traceId === undefined ? provenance.attribute : `${provenance.traceId}:${provenance.attribute}`
    case 'filesystem':
...
const root: ProjectModelField<string> = {
  value: '/repo',
  provenance: { kind: 'filesystem', path: '/repo/package.json', convention: 'nearest-package-root' },
}
...
const model: ResolvedProjectModel = {
  ...
  quality: {
    persistenceRoot: {
      ...
      provenance: { kind: 'filesystem', path: '/repo', convention: 'default-quality-root' },
    },
    includeGlobs: [
      { value: 'evals/**/*.eval.ts', provenance: { kind: 'filesystem', path: '/repo', convention: 'quality-default' } },
      { value: '**/*.eval.ts', provenance: { kind: 'filesystem', path: '/repo', convention: 'quality-default' } },
    ],
    excludeGlobs: [],
    evaluationFiles: [],
  },
  diagnostics: [
    {
      ...
      provenance: { kind: 'filesystem', path: '/repo', convention: 'no-config-source-discovery' },
    },
  ],
}
