...
`evals/**/*.eval.ts` and `**/*.eval.ts`; source-discovered
`prompt({ tests })` exports lower into `prompt:<id>` evaluations without
config registration. `crux.config.ts` is optional policy and defaults, not a
required primitive registry.
...
builds recent run rows and score series for one evaluation, while evaluation
experiment relation reads expose latest experiment summaries per evaluation and
grouped by evaluation for list/detail screens. These records are served by the
local backend so clients do not reconstruct evidence from raw experiments,
baselines, source catalogs, and traces. Direct `evaluation.run()`
...
`crux dev` generates a persistent per-project observability ingest token at
`.crux/devtools/ingest-token` and prints it as `CRUX_DEVTOOLS_TOKEN=...` on
startup. Use that token with a public tunnel origin when a cloud runtime such as
Convex needs to post records back to your local devtools server. The browser
session token embedded in tunnel links is separate and grants UI/API access;
the ingest token is accepted only for `POST /api/observability/records`.
...
This is what adapter runtimes consume internally. For SDK integration, prefer `defineProviderRuntime()` from `@use-crux/core/adapter`: use `ownership: 'single-turn'` for raw chat SDKs that look like OpenAI/Anthropic/Google, or `ownership: 'loop-owned'` for SDKs that own their own model/tool loop. Use `.resolve()` directly only for bespoke framework glue where no adapter runtime fits.
