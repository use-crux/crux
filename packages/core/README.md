...
This is what adapters call internally. Use it directly when integrating with an SDK that doesn't have an adapter. For raw chat SDKs that look like OpenAI/Anthropic/Google, prefer `defineProviderRuntime({ turn })` from `@crux/core/adapter`; it compiles provider request/response/stream hooks into a `createX()` adapter factory.
