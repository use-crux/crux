...
- Adapters use `systemBlocks` to emit provider-native cache markers:
  ...
  - `@crux/google`: `resolveGoogleSystemConfig()` plans the cacheable system prefix once for both `call()` and `stream()`. The native runtime receives a narrow cache resolver: by default `GoogleCacheManager` creates server-side `CachedContent` objects via `client.caches.create()` with SHA-256 content+TTL hashing for dedup, while custom `GoogleCachedContentPort` implementations can resolve cache names themselves. The cached prefix goes into `config.cachedContent`, the uncached remainder remains as `config.systemInstruction`, and lifecycle handling covers concurrency dedup (promise sharing), per-call TTL expiry, LRU eviction, optional failure propagation, and graceful error fallback.
