...
`project-index/project-model.ts` defines the separate config-inspection read model. `ResolvedProjectModel` explains root selection, package metadata, config files, source roots, ignored paths, discovered definitions, Quality defaults, and diagnostics with per-field provenance. It deliberately stays a shallow JSON-safe DTO: source/runtime/filesystem/config/CLI provenance is a discriminated union, diagnostics use stable reason codes, and definition/diagnostic ids are branded strings at TypeScript boundaries. Source-only discovery is informational, while selected source-shape findings such as missing stable ids and runtime-dependent tool maps are represented as Project Model diagnostics with source provenance. The resolver that fills this model belongs to the local/indexer layer: `@crux/indexer` exposes `resolveProjectModel(...)`, and `@crux/local` renders that shape through `crux config inspect`; core only owns the shared contract.
