...
└── adapter/
    ...
    ├── provider-runtime/   defineProviderRuntime() — public provider authoring compiler for singleTurn and loop runtimes
...
All adapters follow the same structure. Cross-cutting concerns (prompt resolution, middleware, safety, validation retry, tool lifecycle, fallback, hooks, and model-output normalization) are handled by the adapter execution session. Provider packages define `defineProviderRuntime({ singleTurn })` for raw chat SDKs or `defineProviderRuntime({ loop })` for SDK-owned loops. Provider-specific code stays in the runtime spec: request assembly, SDK port binding, transcript conversion/assistant extraction, response metadata normalization, stream delta extraction, settings/schema mapping, and unusual provider dependencies.
