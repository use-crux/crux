...
└── adapter/
    ...
    ├── testing.ts          Testing barrel: adapterSpecConformance() plus fakeExecutor()/executorSpecConformance()
    ├── testing/
...
Quality is one authored surface (`evaluate()` from `@crux/core/quality`) over an internal engine in `quality/internal/` — `engine.ts` (normalization, matrix execution, trials, concurrency, timeouts, aggregates, gates), `signals.ts` (trace capture and typed extraction), `expect-runtime.ts` (bound matchers and assertion recording), `assertion-callbacks.ts` (shared expect/assert callback replay and not-evaluated accounting), `score-map.ts` (post-score score map assembly), `compare.ts` (paired statistics), `baseline.ts` (promotion), `cassette.ts` (replay), `output-cache.ts` (watch cache), and `persist.ts` (records). None of it is exported publicly except `@crux/core/quality/internal/runner`, the @internal contract the first-party worker drives.
