...
Quality is one authored surface (`evaluate()` from `@use-crux/core/quality`) over an internal engine in `quality/internal/` — `engine.ts` (normalization, matrix execution, trials, concurrency, timeouts, aggregates, gates), `signals.ts` (trace capture and typed extraction), `expect-runtime.ts` (bound matchers and assertion recording), `assertion-callbacks.ts` (shared expect/afterScores callback replay and not-evaluated accounting), `programmatic-runtime.ts` (direct `.run()` source-frame and devtools-forwarding defaults), `score-map.ts` (post-score score map assembly), `compare.ts` (paired statistics), `baseline.ts` (promotion), `cassette.ts` (replay), `output-cache.ts` (watch cache), `feedback.ts` (human labels and annotations), and `persist.ts` (records). None of that engine layout is the tooling API. The only exported first-party tooling contract is `@use-crux/core/quality/internal/runner`, a narrow `createQualityRunner()` facade with `collect()`, `run()`, `promote()`, and `feedback.{add,list}()` operations plus domain-level event/result types. It hides `EvaluationDefinition`, `runEvaluation`, prompt-test lowering, persistence paths, baseline helpers, cassettes, output-cache keys, ULIDs, and source-frame timing from local-workers/devtools.
