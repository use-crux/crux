...
Local persistence follows a two-plane boundary. SQLite is the canonical runtime store for observability history: runs, spans, events, artifacts, edges, metrics, lifecycle reconciliation inputs, resource activity, and deletion. File-backed observability databases open through a WAL/busy-timeout DSN and a small connection pool so multiple in-flight HTTP flushes, web reads, and TUI reads do not serialize behind one connection; in-memory databases stay single-connection for tests. Authored source assets are discovered by convention through the Project Index and project-root cassette scan: `*.suite.ts`, `*.suite.tsx`, `*.suite.js`, `*.suite.mjs`, `*.suite.json`, and `*.cassette.json` are treated as source-controlled quality assets under the normal generated/dependency ignores. JSON/JSONL under `.crux/quality` is reserved for portable mutable workbench state: suites, cases, feedback, local cassettes, baselines, insight statuses, and insight silences. Quality read models join authored assets, mutable `.crux/quality` records, and observability history in Go services; they do not copy run trees into `.crux/quality/runs`, and local workbench records take precedence over discovered metadata for the same suite id. Run lists, dashboard read models, lifecycle reconciliation, and insights use ingest-time count/token/cost rollup columns plus lightweight run-signal aggregation so local histories of thousands of runs remain responsive. The lifecycle driver reads only running runs with `lifecycle_status IS NULL`, skips candidates whose `last_activity_at` is within 60 seconds, persists reconciled state after a presentation transition, and clears that state only for newly inserted late records. List endpoints default to a newest-first page, avoid span metric JSON scans, cap full-history devtools read-model pulls at 500 runs, and treat optional list enrichment as non-fatal on backend read deadlines; callers that truly need exact metrics should inspect a single run. Resource activity views use the family/start index with a latest-500 cap and batched artifact/edge joins. Streaming text is stored as coalesced `token.chunk` span events in a capped per-span ring; exact `RunDetail` excludes those high-frequency events and focused clients read them through the lazy span-events endpoint. Startup and 30-minute retention ticks delete at most 50 oldest runs per pass beyond the 14-day or 2000-run defaults, run incremental SQLite vacuum, and cap artifact previews to 256KB by default. Full `RunDetail` projection is used only when inspecting one run.
...
Devtools run-detail views poll briefly after a run reaches a terminal status. This keeps Convex/serverless boundary flushes visible when final artifacts or follow-up generation spans arrive just after the terminal run update. The web runs list consumes server-owned session, token, cost, and graph-count rollups from `/api/observability/runs`; focused span output streams use the lazy `/api/observability/runs/{runId}/spans/{spanId}/events?name=token.chunk` endpoint instead of loading high-frequency chunks into every run detail.
...
crux quality diff <experimentA> <experimentB> --json
crux quality promote <experimentId>
```

Quality is the sole evaluation CLI surface. On `crux quality run`, the CLI extracts the embedded `quality-runner.mjs` to `~/.cache/crux/` and runs it with `node --import tsx/esm`. The worker collects (default convention globs `evals/**/*.eval.ts` and `**/*.eval.ts`, optional `quality.include`, imported eval files, lowered `prompt({ tests })` when a registry exists), then executes selected cells through the core runner facade, streaming one NDJSON event protocol (collect:done, eval:start, cell:start/done, eval:done, promote:done, diff:done, run:done, error) on stdout. Every worker event carries a run-scoped ULID `runId`, and the worker verifies `QUALITY_RUNNER_PROTOCOL` from the project-local core before collection so binary/core skew fails as a structured exit-2 error. The worker never bundles `@use-crux/core` — it resolves the project's own core instance at runtime (via `@use-crux/core/quality/internal/runner`, the @internal tooling contract subpath) so evaluation symbols are shared with user code. Devtools auto-attach resolves the same project's `@use-crux/core/observability` subpath separately so observability globals stay shared without widening the runner facade. The engine persists Experiment records natively behind the facade; the Go side renders the reporter, exit codes, `--json` run summaries, and JUnit from the event stream.

Quality graph coverage is core-owned and Go-rendered. Runs emit `eval.run` and per-cell `eval.case` roots, cells link to their umbrella run with `eval.case_of`, cassette replays link with `replay.of`, run comparisons and arbitrary diffs emit `comparison.report` artifacts with candidate/baseline edges, and successful baseline promotion emits `baseline.promotion` against the promoted eval run. The Go server forwards these records to activity, web, and TUI read models without redefining Quality policy.
...
Quality is one authored beta surface (`evaluate()` from `@use-crux/core/quality`) over an internal engine in `quality/internal/` — `engine.ts` (normalization, matrix execution, trials, concurrency, timeouts, aggregates, gates), `signals.ts` (trace capture and typed extraction), `expect-runtime.ts` (bound matchers and assertion recording), `assertion-callbacks.ts` (shared expect/afterScores callback replay and not-evaluated accounting), `programmatic-runtime.ts` (direct `.run()` source-frame and devtools-forwarding defaults), `score-map.ts` (post-score score map assembly), `compare.ts` (paired statistics), `experiment-diff.ts` (experiment-to-experiment diff), `failure-artifacts.ts` (agent-readable failing-cell artifacts), `baseline.ts` (promotion), `cassette.ts` (replay), `output-cache.ts` (watch cache), `feedback.ts` (human labels and annotations), and `persist.ts` (records). The stable-within-0.x beta contract covers the five authoring values (`evaluate`, `target`, `scorers`, `dataset`, `cassette`), experiment/manifest schemas on `@use-crux/core/quality/schemas`, CLI JSON outputs, and exit codes. None of the engine layout is the tooling API. The exported first-party tooling contract is `@use-crux/core/quality/internal/runner`, a narrow `createQualityRunner()` facade with `collect()`, `run()`, `promote()`, `compare()`, and `feedback.{add,list}()` operations plus domain-level event/result types, but it remains internal with no stability guarantees. It hides `EvaluationDefinition`, `runEvaluation`, prompt-test lowering, persistence paths, baseline helpers, cassettes, output-cache keys, ULIDs, and source-frame timing from local-workers/devtools. The public machine-consumption subpath `@use-crux/core/quality/schemas` exports Zod schemas and JSON Schema generation for experiment records, manifests, failure artifacts, run summaries, and judge reports; it does not widen the root authoring surface.
