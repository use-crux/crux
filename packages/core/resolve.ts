 ...
 *
 * @deprecated This sync gate-only pass predates the contributor-contract
 * pipeline (use-crux/crux#29). Resolution now goes through the lowered
 * contributor driver; prefer `inspectArgs()` for exclusion inspection or
 * `createPromptResolver()` for testable resolution. Kept (with byte-identical
 * exclusion strings) until its remaining tests are ported, then removed.
 * Custom `contributor()` entries surface in the `injectables` bucket here
 * and are not gated by this pass.
 */
