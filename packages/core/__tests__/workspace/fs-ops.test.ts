...
describe('workspace filesystem operations', () => {
  it('checks whether a file exists in the selected namespace', async () => {
    ...
    await ws.write('/workspace/notes.md', 'override notes', { namespace: 'thread:override' })
    ...
    await expect(ws.exists('/workspace/notes.md', { namespace: 'thread:override' })).resolves.toBe(true)
    await expect(ws.exists('/workspace/default-only.md', { namespace: 'thread:override' })).resolves.toBe(false)
  })
  ...
  it('appends text, creates missing files, and rejects binary files', async () => {
    ...
    await ws.write('/workspace/notes.md', 'alpha', { mimeType: 'text/markdown' })
    ...
    await expect(ws.append('/outputs/report.pdf', 'nope')).rejects.toThrow(/only text files/i)
  })
  ...
  it('renames files with metadata, overwrite checks, and mount access rules', async () => {
    ...
    await expect(ws.rename('/workspace/source.md', '/workspace/existing.md')).rejects.toThrow(/already exists/i)
    const moved = await ws.rename('/workspace/source.md', '/workspace/moved.md')
    ...
    await expect(ws.read('/workspace/moved.md')).resolves.toMatchObject({ kind: 'text', content: 'source' })

    await ws.write('/workspace/next.md', 'next', { namespace: 'thread:override' })
    await ws.move('/workspace/next.md', '/workspace/done.md', { namespace: 'thread:override' })
    await expect(ws.exists('/workspace/done.md', { namespace: 'thread:override' })).resolves.toBe(true)

    await expect(ws.rename('/workspace/moved.md', '/workspace/existing.md', { overwrite: true })).resolves.toMatchObject({
      path: '/workspace/existing.md',
    })
    await expect(ws.rename('/workspace/existing.md', '/archive/existing.md')).rejects.toThrow(/read-only/i)
  })
  ...
  it('copies files while preserving readable content and metadata', async () => {
    ...
    await expect(ws.copy('/workspace/source.md', '/workspace/existing.md')).rejects.toThrow(/already exists/i)
    ...
    await expect(ws.read('/workspace/source.md')).resolves.toMatchObject({ kind: 'text', content: 'blob-backed text' })
    await expect(ws.read('/workspace/copy.md')).resolves.toMatchObject({ kind: 'text', content: 'blob-backed text' })
  })
  ...
  it('greps scoped text files with line, column, and result caps', async () => {
    ...
    await expect(ws.grep('needle', { path: '/workspace/**/*.md', maxResults: 2 })).resolves.toEqual({
      matches: [
        { path: '/workspace/a.md', line: 1, column: 7, text: 'first needle' },
        { path: '/workspace/nested/b.md', line: 1, column: 1, text: 'needle blob' },
      ],
    })
    await expect(ws.grep('needle', { path: '/workspace/a.md', ignoreCase: true })).resolves.toMatchObject({
      matches: [
        { line: 1, column: 7 },
        { line: 2, column: 8 },
      ],
  ...
  it('injects rename and grep tools with default and prefixed names', async () => {
    ...
    await tools.writeWorkspaceFile.execute?.({ path: '/workspace/source.md', content: 'needle' })
    await tools.renameWorkspaceFile.execute?.({ from: '/workspace/source.md', to: '/workspace/done.md' })

    await expect(tools.grepWorkspace.execute?.({ query: 'needle', path: '/workspace/**/*.md' })).resolves.toEqual({
      matches: [{ path: '/workspace/done.md', line: 1, column: 1, text: 'needle' }],
    })
  })
})
