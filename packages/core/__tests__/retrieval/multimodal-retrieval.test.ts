...
describe('multimodal retrieval dog proof', () => {
  it('retrieves across text and image while keeping persisted state byte-safe', async () => {
    ...
    await products.index([
      ...
      { namespace: 'products', sourceId: 'cat-faq', content: 'A cat adoption guide' },
      { namespace: 'products', sourceId: 'cat-photo', asset: otherPhoto },
    ])
    ...
    expect(imageToImageHits[0]).toEqual(
      expect.objectContaining({ source: expect.objectContaining({ id: 'rex' }) }),
    )

    await products.index([{ namespace: 'products', sourceId: 'dog-faq', content: 'A brown dog' }])
  ...
  it('supports bare media through recipes and uses dense-only hybrid provenance', async () => {
    ...
    const sparse = embedding({
      kind: 'sparse', name: 'sparse', maxInputTokens: 100, batch: { maxSize: 8 },
      embed: sparseProvider,
    })
    await indexer({
      id: 'products', namespace: 'products', storage, dense, sparse,
    }).indexDocuments([{ namespace: 'products', sourceId: 'rex', asset: dogPhoto }])
