...
describe("compilePrompt input schema", () => {
  it("merges context schemas into one object schema", () => {
    ...
    const schema = compilePrompt({
      system: "S",
      use: [ctx1, ctx2],
    } as AnyPromptConfig).inputSchema;
  ...
  it("throws on duplicate keys across contexts", () => {
    ...
    expect(() =>
      compilePrompt({ system: "S", use: [ctx1, ctx2] } as AnyPromptConfig),
  ...
  it("returns no schema when neither prompt nor contexts declare input", () => {
    ...
    expect(
      compilePrompt({ system: "S", use: [ctx] } as AnyPromptConfig).inputSchema,
    ).toBeUndefined();
  });
});
...
describe("compilePrompt resolution", () => {
  ...
  it("resolves prompt text from static strings and functions", async () => {
    const staticResult = await resolveArgs({
      system: "sys",
      prompt: "do this",
    } as AnyPromptConfig);
  ...
  it("keeps segmented system text in inspect and resolved blocks", async () => {
    ...
    const config = {
      input,
      system: ({ input }) =>
        `Workspace ${input.workspace.name} uses ${input.account.plan}.`,
      use: [ctx],
    } as AnyPromptConfig;
    ...
    expect(resolved.systemBlocks?.[0]).toMatchObject({
      source: "prompt",
      segments: inspected.system.parts[0].segments,
    });
  ...
  it("matches model id slash-prefix adaptations and wildcard fallbacks", async () => {
    ...
    expect(
      (await resolveArgs(config, { provider: "", modelId: "openai/gpt-4o" }))
        .system,
    ).toBe("S\n\nOPENAI");
    expect(
      (
        await resolveArgs(config, {
          provider: "mistral",
...
describe("compilePrompt token-aware system composition", () => {
  ...
  it("returns system blocks with provider cache metadata", async () => {
    ...
    expect(result.systemBlocks?.[1]).toMatchObject({
      source: "context:cached",
      providerCache: true,
    });
    expect(result.systemBlocks?.[2]).toMatchObject({
      source: "context:uncached",
      providerCache: false,
    });
  });
  ...
  it("excludes skipped and dropped contexts from blocks", async () => {
    ...
    const skipped = await resolveArgs({
      system: "Base.",
      use: [empty],
    } as AnyPromptConfig);
    expect(skipped.systemBlocks?.map((block) => block.source)).toEqual([
      "prompt",
    ]);
  ...
  it("caches context resolver output by declared input fields", async () => {
    const resolver = vi.fn(
      ({ input }: { input: { orgId: string } }) => `result-${input.orgId}`,
    );
    const cached = context({
      id: "cache-by-org",
