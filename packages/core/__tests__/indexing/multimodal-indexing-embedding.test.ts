...
describe('multimodal indexing embedding stage', () => {
  it('embeds media densely as documents and omits media from the sparse leg', async () => {
    ...
    const sparseProvider = vi.fn(async (texts: string[]) =>
      texts.map(() => ({ indices: [0], values: [1] })),
    )
    const docs = indexer({
      id: 'hybrid',
      namespace: 'kb',
      ...
      sparse: embedding({
        kind: 'sparse', name: 'sparse', maxInputTokens: 100,
        batch: { maxSize: 8 }, embed: sparseProvider,
      }),
    })
    ...
    const entries = (await records.list('indexer:hybrid:namespace:kb:source:rex:')).entries
  ...
  it('rejects unsupported media before provider I/O and names every offending source', async () => {
    ...
    const run = docs.indexDocuments([
      {
        namespace: 'kb', sourceId: 'photo-a',
        asset: { type: 'data', data: new Uint8Array([1]), mediaType: 'image/png' },
      },
      {
        namespace: 'kb', sourceId: 'photo-b',
  ...
  it('computes but never caches media without a locally provable byte hash', async () => {
    ...
    const input = [{
      namespace: 'kb', sourceId: 'remote-photo',
      asset: { type: 'url' as const, url: new URL('https://cdn.example/dog.png?signature=secret'), mediaType: 'image/png' },
    }]
...
describe('indexer embedding-space guard', () => {
  it('rejects a mixed-space write before embedding and permits it after clear', async () => {
    ...
    await first.clear()
    await expect(second.indexDocuments([{ namespace: 'kb', sourceId: 'two', content: 'second' }])).resolves.toMatchObject({
      chunkCount: 1,
    })
...
function textEmbedding(name: string, dimensions: number, provider = vi.fn(async () => [Array(dimensions).fill(0)])) {
  return embedding({
    kind: 'dense', name, dimensions, maxInputTokens: 100,
    batch: { maxSize: 8 }, embed: provider,
  })
}
