...
describe("duplicate module runtime state", () => {
  ...
  it("shares deployment identity across module copies and restores it on dispose", async () => {
    vi.resetModules();
    const instrumentationCore = await import("../src/index");
    const restore = instrumentationCore.configureObservability({
      identity: {
        projectId: "duplicate-module-fixture",
        manifestId: `pim_${"a".repeat(64)}`,
        deploymentId: "preview-42",
      },
    });

    vi.resetModules();
    const actionCore = await import("../src/index");
    const records: Array<{ type: string; deployment?: unknown }> = [];
    actionCore.subscribeObservability(["run:start"], (record) =>
      records.push(record),
    );

    await actionCore.observe.run(
      { name: "cross-copy identity", rootPrimitive: "custom.operation" },
      async () => undefined,
    );
    restore();
    await actionCore.observe.run(
      {
        name: "after cross-copy identity restore",
        rootPrimitive: "custom.operation",
      },
      async () => undefined,
    );

    expect(records[0]?.deployment).toEqual({
      projectId: "duplicate-module-fixture",
      manifestId: `pim_${"a".repeat(64)}`,
      deploymentId: "preview-42",
    });
    expect(records[1]).not.toHaveProperty("deployment");
  }, 30_000);
  ...
  it("clears another copy's sequencer and terminal-run state on reset", async () => {
    ...
    activeSpan.withContext(() =>
      firstCore.observe.event({ name: "before reset" }),
    );
    activeSpan.withContext(() =>
      firstCore.observe.event({ name: "before reset again" }),
    );
    ...
    activeSpan.withContext(() =>
      firstCore.observe.event({ name: "after reset" }),
    );
  ...
  it("does not reuse a legacy v1 registry that lacks the privacy fields", async () => {
    ...
    try {
      const registry = originalCurrent as {
        runtime: object;
        observability: Record<string, unknown>;
      };
      const {
        feedbackDestination: _feedback,
        redactPaths: _paths,
        ...legacy
      } = registry.observability;
      Reflect.set(globalThis, legacyKey, {
        ...registry,
        registryVersion: 1,
        runtime: { ...registry.runtime },
        observability: legacy,
      });
      Reflect.deleteProperty(globalThis, currentKey);
      vi.resetModules();

      const core = await import("../src/index");
      expect(core.currentObservabilityRedactPaths()).toEqual([]);
      expect(() =>
        core.configureObservability({ redactPaths: ["customer.email"] }),
      ).not.toThrow();
    } finally {
      Reflect.set(globalThis, legacyKey, originalLegacy);
      Reflect.set(globalThis, currentKey, originalCurrent);
      vi.resetModules();
    }
  }, 15_000);
  ...
  it("removes bad listeners without blocking valid cross-copy synchronization", async () => {
    ...
    try {
      ...
      expect(() =>
        notifyObservabilityRegistryListeners(registry),
      ).not.toThrow();
