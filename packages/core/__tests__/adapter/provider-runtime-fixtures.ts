...
export interface RuntimeRawResponse {
  ...
  readonly usage: typeof RUNTIME_USAGE
}

export interface RuntimeStream extends AsyncIterable<{ readonly text: string }> {
...
export function runtimeResponse(
  text: string,
  overrides: Partial<Omit<RuntimeRawResponse, 'id' | 'model' | 'text' | 'usage'>> = {},
): RuntimeRawResponse {
  return {
    ...
    turn: {
      bind: (client: RuntimeClient) => ({
        async call(request, mode) {
          ...
          if (!response) throw new Error('single-turn runtime script exhausted')
          return response
        },
        async stream(request) {
          client.streams.push(request)
          return streamFrom(client.streamChunks.shift() ?? ['runtime ', 'stream'])
        },
      }),
      request(args, ctx) {
        return {
      ...
      response: {
        meta(raw): NativeResponseMetadata {
          return {
            ...
            finishReason: raw.finishReason ?? (raw.toolCalls?.length ? 'tool_calls' : 'stop'),
          }
        },
      },
      stream: {
        request: (request) => ({ ...request, stream: true as const }),
        textDelta: (chunk) => (isRuntimeStreamChunk(chunk) ? chunk.text : undefined),
...
function isRuntimeStreamChunk(chunk: unknown): chunk is { readonly text: string } {
  return typeof chunk === 'object' && chunk !== null && typeof (chunk as { readonly text?: unknown }).text === 'string'
}

function isRuntimeProviderMessage(message: unknown): message is RuntimeProviderMessage {
  if (typeof message !== 'object' || message === null) return false
  const record = message as { readonly role?: unknown; readonly text?: unknown }
  return (
    (record.role === 'system' || record.role === 'user' || record.role === 'assistant' || record.role === 'tool') &&
