        ...
        results: [
          {
            ...
            toolName: "search",
            text: "hit",
            modelOutput: { type: "json", value: { ok: true } },
          },
        ],
      },
    ]);
...
describe("transcriptUnitsToMessages", () => {
  ...
  it("round-trips canonical messages through units", () => {
    ...
    expect(
      transcriptUnitsToMessages(messagesToTranscriptUnits(messages)),
    ).toEqual(messages);
  });
});
...
describe("createToolResultEncodingHelpers", () => {
  ...
  it("exposes content parts only for content model output", () => {
    ...
    expect(
      helpers.contentParts(
        result({ modelOutput: { type: "json", value: { a: 1 } } }),
      ),
    ).toBeUndefined();
  ...
  it("flags errors from error model outputs or the explicit error flag", () => {
    expect(
      helpers.errorFlag(
        result({ modelOutput: { type: "error-text", value: "x" } }),
      ),
    ).toBe(true);
    expect(
      helpers.errorFlag(
        result({ modelOutput: { type: "error-json", value: { e: 1 } } }),
      ),
    ).toBe(true);
    expect(helpers.errorFlag(result({ isError: true }))).toBe(true);
    expect(
      helpers.errorFlag(
        result({ modelOutput: { type: "execution-denied", reason: "no" } }),
      ),
    ).toBe(false);
    expect(
      helpers.errorFlag(result({ modelOutput: { type: "json", value: {} } })),
    ).toBe(false);
  });
});
...
function toolMessage(
  ...
  return {
    ...
    metadata: {
      ...
      ...(extra?.modelOutputError !== undefined
        ? { modelOutputError: extra.modelOutputError }
        : {}),
    },
  };
}
