...
describe('Workers withCrux lifecycle', () => {
  ...
  it('returns the exact response while waitUntil owns deferred work and the final drain', async () => {
    ...
    const handler = withCrux(
      async (_request: Request, _ctx: ExecutionContext) => {
        const run = observe.openRun({ name: 'workers-with-crux', rootPrimitive: 'run' })
        run.end()
        defer(async () => {
          await deferredGate
  ...
  it('rethrows the original handler error after retaining deferred work and the final drain', async () => {
    ...
    const handler = withCrux(
      async (_ctx: ExecutionContext) => {
        observe.openRun({ name: 'workers-with-crux-error', rootPrimitive: 'run' }).end()
        defer(() => {
          deferredCompleted = true
        })
        throw original
    ...
    expect(deferredCompleted).toBe(true)
    expect(deliveredRecords()).toContainEqual(
      expect.objectContaining({ type: 'run:start', name: 'workers-with-crux-error' }),
    )
  })
  ...
  it('preserves the handler result and reports a structured failure when the drain throws', async () => {
    ...
    const onDrain = vi.fn()
    const handler = withCrux(async (_ctx: ExecutionContext) => 'exact-result' as const, {
      context: (ctx) => ctx,
      invocation: () => ({ onDrain }),
    })
    const ctx = createExecutionContext()
  ...
  it('contains a throwing drain reporter without changing the result or rejecting waitUntil', async () => {
    ...
    const handler = withCrux(async (_ctx: ExecutionContext) => ({ ok: true } as const), {
      context: (ctx) => ctx,
      invocation: () => ({
        onDrain: () => {
  ...
  it('reports a truthful partial drain when the host budget expires', async () => {
    ...
    const handler = withCrux(
      async (_ctx: ExecutionContext) => {
        observe.openRun({ name: 'workers-with-crux-deadline', rootPrimitive: 'run' }).end()
        return 'response'
      },
      {
        context: (ctx) => ctx,
  ...
  it('keeps concurrent deferred callbacks attached to their own request run', async () => {
    ...
    const handler = withCrux(
      (requestId: string, _ctx: ExecutionContext) => {
        const run = observe.openRun({ name: `workers-concurrent-${requestId}`, rootPrimitive: 'run' })
        run.withContext(() => {
          defer(() => {
            observe.openSpan({ name: `deferred-${requestId}`, primitive: 'defer.run' }).end()
            run.end()
          })
        })
    ...
    const requestIds = Array.from({ length: 6 }, (_, index) => `request-${index}`)
    const contexts = requestIds.map(() => createExecutionContext())

    const responses = await Promise.all(
      requestIds.map((requestId, index) => handler(requestId, contexts[index]!)),
    )
    expect(await Promise.all(responses.map((response) => response.json()))).toEqual(
      requestIds.map((requestId) => ({ requestId })),
    )
    ...
    for (const requestId of requestIds) {
      const runStart = records.find(
        (record) => record.type === 'run:start' && record.name === `workers-concurrent-${requestId}`,
      )
