...
describe("sanitizeDefinitionSource", () => {
  ...
  it("rejects a windows drive-relative source and project root", () => {
    expect(
      sanitizeDefinitionSource(
        { file: "C:repo/private.ts", line: 1 },
        { projectRoot: "C:repo" },
      ),
    ).toBeUndefined();
  });
  ...
  it("omits a relative path that traverses upward", () => {
    expect(
      sanitizeDefinitionSource({ file: "../secret.ts", line: 1 }),
    ).toBeUndefined();
    expect(
  ...
  it("omits source without a positive line number", () => {
    expect(
      sanitizeDefinitionSource({ file: "src/a.ts", line: 0 }),
    ).toBeUndefined();
  ...
  it("drops a non-positive column but keeps the location", () => {
    expect(
      sanitizeDefinitionSource({ file: "src/a.ts", line: 4, column: 0 }),
    ).toEqual({
      file: "src/a.ts",
      line: 4,
    });
...
describe("safeDefinitionId fingerprint fallback (empty-after-sanitize)", () => {
  ...
  it.each([
    ...
    expect(compositionDefinitionRef("swarm", raw).id).toBe(
      `composition.swarm:${fingerprint}`,
    );
  });
...
describe("contributor and child definition refs", () => {
  it.each([
    [
      knowledgeBaseDefinitionRef("docs"),
      {
        id: "rag.knowledgeBase:docs",
        kind: "rag.knowledgeBase",
        role: "contributed-knowledge-base",
      },
    ],
    [
      toolPolicyDefinitionRef("approval"),
      {
        id: "toolPolicy:approval",
        kind: "toolPolicy",
        role: "contributed-tool-policy",
      },
    ],
    [
      flowStepDefinitionRef("research", "load docs"),
      {
        id: "flow.step:research:load-docs",
        kind: "flow.step",
        role: "invoked-flow-step",
      },
    ],
    [
      parallelBranchDefinitionRef("fanout", "researcher"),
      {
        id: "composition.parallel:fanout:branch:researcher",
        kind: "composition.parallel.branch",
        role: "invoked-composition-branch",
      },
    ],
    [
      recipeStepDefinitionRef("search", "rerank"),
      {
        id: "rag.recipe:search:step:rerank",
        kind: "rag.recipe.step",
        role: "invoked-recipe-step",
      },
    ],
    [
      scorerDefinitionRef("helpfulness"),
      { id: "scorer:helpfulness", kind: "scorer", role: "invoked-scorer" },
    ],
  ])(
    "builds a canonical ref from identities held by the runtime",
    (actual, expected) => {
