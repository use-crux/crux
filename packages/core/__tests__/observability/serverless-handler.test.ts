...
describe('withObservableInvocation', () => {
  ...
  it('flushes a cold-start invocation before returning', async () => {
    ...
    const handler = withObservableInvocation(async () => {
      ...
      observe.openRun({ name: 'cold start', rootPrimitive: 'custom.operation' }).end()
      return 'ok'
    })
  ...
  it('still performs a bounded final flush when the handler throws', async () => {
    ...
    const { observe } = await import('../../src/observability')
    const handler = withObservableInvocation(async () => {
      observe.openRun({ name: 'errors out', rootPrimitive: 'custom.operation' }).end()
  ...
  it('reports the drain result through onDrain instead of discarding it', async () => {
    ...
    const handler = withObservableInvocation(
      async () => {
        observe.openRun({ name: 'reported drain', rootPrimitive: 'custom.operation' }).end()
        return 'ok'
      },
  ...
  it('warns by default instead of silently discarding an incomplete drain', async () => {
    ...
    const handler = withObservableInvocation(
      async () => {
        observe.openRun({ name: 'unreported drain', rootPrimitive: 'custom.operation' }).end()
        return 'ok'
      },
      () => ({ remainingTimeMs: 5 }),
    )
    ...
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('drain did not fully complete'), expect.any(Object))
  })
  ...
  it('preserves the handler error even when the final flush also fails', async () => {
    ...
    const handler = withObservableInvocation(
      async () => {
        observe.openRun({ name: 'handler throws too', rootPrimitive: 'custom.operation' }).end()
        throw new Error('handler failed')
      },
  ...
  it('reports a synthetic failed drain result instead of masking the handler result when flush itself throws', async () => {
    const { observe } = await import('../../src/observability')
    setObservabilityTransport({ send: acceptedDeliveryReceipt }, { scheduledDelayMs: 60_000 })
    const flushSpy = vi.spyOn(observe, 'flush').mockRejectedValueOnce(new Error('flush exploded'))
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    const drains: unknown[] = []
    const handler = withObservableInvocation(
      async () => {
        observe.openRun({ name: 'flush throws', rootPrimitive: 'custom.operation' }).end()
        return 'ok'
      },
      () => ({ onDrain: (result) => drains.push(result) }),
    )
  ...
  it('does not let a throwing onDrain reporter affect the handler result', async () => {
    ...
    const handler = withObservableInvocation(
      async () => {
        observe.openRun({ name: 'reporter throws', rootPrimitive: 'custom.operation' }).end()
        return 'ok'
      },
      () => ({
        onDrain: () => {
  ...
  it('stops flushing once the derived deadline is exhausted', async () => {
    ...
    const handler = withObservableInvocation(
      async () => {
        observe.openRun({ name: 'deadline exhausted', rootPrimitive: 'custom.operation' }).end()
        return 'ok'
      },
      () => ({ remainingTimeMs: 5 }),
    )
  ...
  it('snapshots the deadline once at invocation start instead of extending it on every read', async () => {
    ...
    const handler = withObservableInvocation(
      async () => {
        observe.openRun({ name: 'moving deadline', rootPrimitive: 'custom.operation' }).end()
  ...
  it('accepts an explicit remaining-time budget instead of an absolute deadline', async () => {
    ...
    const handler = withObservableInvocation(
      async () => {
        observe.openRun({ name: 'remaining time', rootPrimitive: 'custom.operation' }).end()
        return 'ok'
      },
...
describe('withNodeObservableInvocation', () => {
  ...
  it('derives the deadline from context.getRemainingTimeInMillis()', async () => {
    ...
    const { observe } = await import('../../src/observability')
    const handler = withNodeObservableInvocation(async (event: { documentId: string }) => {
      observe.openRun({ name: 'lambda invocation', rootPrimitive: 'custom.operation' }).end()
      return event.documentId
    })

    await expect(
      handler({ documentId: 'doc_1' }, { getRemainingTimeInMillis: () => 30_000 }),
    ).resolves.toBe('doc_1')
    expect(delivered.length).toBeGreaterThan(0)
  })
  ...
  it('still flushes when the context has no getRemainingTimeInMillis()', async () => {
    ...
    const { observe } = await import('../../src/observability')
    const handler = withNodeObservableInvocation(async (event: { documentId: string }) => {
      observe.openRun({ name: 'lambda invocation no context', rootPrimitive: 'custom.operation' }).end()
      return event.documentId
    })
