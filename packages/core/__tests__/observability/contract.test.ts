...
describe('Crux observability graph contract', () => {
  ...
  it('accepts custom edge and artifact names only through the custom namespace', () => {
    ...
    expect(CruxGraphRecordSchema.safeParse({ ...edge, edgeId: 'edge_custom', edgeType: 'custom.app_relation' }).success).toBe(
      true,
    )
    expect(CruxGraphRecordSchema.safeParse({ ...artifact, artifactId: 'artifact_custom', kind: 'custom.app_payload' }).success).toBe(
      true,
    )
    expect(CruxGraphRecordSchema.safeParse({ ...edge, edgeId: 'edge_invalid', edgeType: 'app_relation' }).success).toBe(
      false,
    )
    expect(CruxGraphRecordSchema.safeParse({ ...artifact, artifactId: 'artifact_invalid', kind: 'app_payload' }).success).toBe(
      false,
    )
  })
  ...
  it('keeps primitive names mapped to their canonical families', () => {
    ...
    expect(CruxSpanStartRecordSchema.safeParse({
      schemaVersion: CRUX_OBSERVABILITY_SCHEMA_VERSION,
