...
describe("persisted message validation", () => {
  it.each([
    ...
    ["malformed MIME", { type: "url", url: "https://example.com/a", mediaType: "SECRET_MIME" }],
    ["empty provider", { type: "provider-file", provider: " ", fileId: "file-1" }],
    ["empty file ID", { type: "provider-file", provider: "openai", fileId: " " }],
    ["empty ref", { type: "asset-ref", ref: { uri: " " }, mediaType: "image/png" }],
    ["invalid asset facts", { type: "url", url: "https://example.com/a", info: { size: -1 } }],
  ] as const)("rejects %s through a safe tagged boundary", async (_label, source) => {
    ...
    expect(error).toMatchObject({ code: "invalid_media_source", path: "record.messages" });
  ...
  it.each([
    ...
    ["conflicting MIME", {
      type: "file",
      source: { type: "url", url: "https://example.com/a", mediaType: "application/pdf" },
    ...
    await expect(loadPersistedMessagesRecord({
      storage: { records },
      key: "malformed-part",
