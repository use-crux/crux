/** Node.js filesystem adapters for private Eval V3 runs and exact evidence. */
...
export interface EvalFileStoreOptions {
  /** Project root containing the private `.crux/quality` directory. */
  readonly projectRoot: string;
}

/** Loud error for corrupt committed Baseline truth. */
...
export function createEvalBaselineFileStore(options: EvalFileStoreOptions) {
  const store = {
    ...
    async read(sourceKey: { readonly relativeFile: string }): Promise<EvalBaselineV3 | undefined> {
...
/** Build and atomically set the sibling Baseline; CLI orchestration arrives later. */
...
/** Create an atomic filesystem store for terminal Eval V3 run records. */
  ...
  return {
    ...
    async read(runId) {
      ...
      try {
        ...
        if (run.runId !== runId) {
          return { status: "corrupt", error: "runId does not match its file name" };
        }
...
/** Create an atomic, restart-safe local exact-evidence store. */
export function createEvalEvidenceFileStore(
  ...
  return {
    ...
    async read(key) {
      const path = artifactPath(directory, key);
      try {
...
function baselinePath(projectRoot: string, sourceFile: string): string {
  ...
  if (file === null) throw new TypeError("Eval source path must end in .eval.ts or .eval.js");
