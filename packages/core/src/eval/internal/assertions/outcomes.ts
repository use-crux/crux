/**
 *
 * Assertion outcome helpers for the Eval engine.
 *
...
export function assertionValue(
  ...
  return {
    label,
    value,
    preview: previewText ?? previewAssertionValue(value),
    redacted: false,
  };
}
  ...
  return outcomes.map((outcome) => {
    const actual =
      outcome.actual !== undefined
        ? redactAssertionValue(outcome.actual, redactPaths)
        : undefined;
    const expected =
      outcome.expected !== undefined
        ? redactAssertionValue(outcome.expected, redactPaths)
        : undefined;
    const expression =
      outcome.expression !== undefined
        ? redactAssertionExpression(
            outcome.expression,
            { actual, expected },
            redactPaths,
          )
...
function redactAssertionValue(
  value: CellAssertionValue,
  redactPaths: readonly string[],
): CellAssertionValue {
  const redactedValue = applyRedaction(value.value, redactPaths);
  const redacted =
    value.redacted || valueChangedByRedaction(value.value, redactedValue);
  return {
    ...value,
    value: redactedValue,
...
function redactAssertionExpression(
  ...
  const left =
    values.actual ?? redactAssertionValue(expression.left, redactPaths);
  const right =
...
function valueChangedByRedaction(
  original: unknown,
  redacted: unknown,
): boolean {
