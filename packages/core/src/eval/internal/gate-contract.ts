...
export interface ScoreGate {
  ...
  max?: number
  /**
   * Minimum paired-difference delta vs the explicit Baseline (candidate −
   * Baseline, paired per Case). Requires compatible accepted Baseline evidence.
   */
  minDeltaVsBaseline?: number
}

/**
 * The declarative Gate set of an Eval. Keys under `scores` are typed by the
 * Eval's literal scorer names (plus the lowered `pass`
...
/**
 * The recorded outcome of one evaluated Gate in an Eval run.
 */
export interface GateResult {
  /** Dot-path of the gate, e.g. `'scores.helpful.min'`, `'consistency.passAllTrials'`. */
  gate: string
  /** Gates evaluate per selected Current or Variant arm. */
  variantName?: string
  threshold: number | boolean
  actual: number | boolean
  passed: boolean
  /**
   * `true` when this result could not be evaluated as blocking, for example
   * when compatible Baseline evidence is absent. Informational results never
   * turn incomplete evidence into a passing run.
   */
