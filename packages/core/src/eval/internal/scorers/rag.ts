...
function retrievedContextChunks(
  ...
  if (isRecord(args.input)) {
    ...
    if (
      ...
      declared.every((chunk) => typeof chunk === "string")
    ) {
...
export function createRagScorerRun(
  kind: RagScorerKind,
  opts: RagScorerOptions,
): ContextualScorerRun {
  const name = opts.name ?? kind;
  return async (args, context): Promise<Score> => {
    const chunks = NEEDS_CONTEXT[kind]
      ? retrievedContextChunks(args, context)
      : undefined;
    ...
    if (kind === "contextRecall" && args.expected === undefined) {
      return {
        ...
        metadata: {
          reason:
            "contextRecall needs an `expected` reference answer on the case",
        },
      };
    }
    ...
    const judgesChunks =
      kind === "contextPrecision" || kind === "contextRecall";
    ...
    const result = await judge.score(judgeInput, {
