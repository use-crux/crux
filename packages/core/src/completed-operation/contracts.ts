 ...
 */
export type CompletedOperationPayload<
  TRaw = unknown,
  TMetadata = unknown,
  TWarning = unknown,
> = Readonly<{
   ...
   */
  safety?: SafetyAudit;
  /** Logical routing decisions when a routing wrapper selected this result. */
 ...
 * instead.
 */
export type CompletedOperationResult<
  TRaw = unknown,
  TMetadata = unknown,
  TWarning = unknown,
...
export function validateOperationTimeout(
  timeout: OperationTimeout | undefined,
): void {
...
export function validateOperationExecution(
  execution: OperationExecution,
): OperationExecution {
  if (!Number.isSafeInteger(execution.calls) || execution.calls <= 0) {
    throw new RangeError(
      "Completed operation execution calls must be a positive safe integer.",
    );
  }
  if (execution.kind === "native") return Object.freeze({ ...execution });
  if (
    execution.operations.length === 0 ||
    execution.operations.some((operation) => !operation.trim())
  ) {
    throw new TypeError(
      "Composed operation execution must name every child operation.",
    );
  }
  return Object.freeze({
    ...execution,
    operations: Object.freeze([...execution.operations]),
  });
}
...
function positiveMilliseconds(value: number | undefined, name: string): void {
  if (value !== undefined && (!Number.isFinite(value) || value <= 0)) {
    throw new RangeError(
      `Completed operation timeout.${name} must be positive finite milliseconds.`,
    );
  }
}
