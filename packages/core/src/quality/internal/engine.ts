...
import {
  createQualityCellToken,
  shouldQuarantineQualityWrite,
  withQualityCellToken,
...
import { withToolExecutionMock } from "../../tools/mock";
import {
  taskToolSourceFixtureIdentities,
  taskOutputCacheBypassReason,
  type OutputCacheBypassReason,
} from "./tool-source-cache";
...
export class QualityDefinitionError extends Error {
  ...
  constructor(
    message: string,
    options: { code?: QualityDefinitionDiagnosticCode } = {},
  ) {
    super(message);
    ...
    cases.push({
      ...
      metadata: {
        ...
        datasetProvenance: {
          ...
          record.metadata.provenance !== undefined
            ? { row: record.metadata.provenance }
            : {}),
...
function validateCellSelections(
  ...
  for (const selection of selections) {
    if (
      ...
      !knownVariants.has(selection.variantName)
    ) {
      throw new QualityDefinitionError(
        `unknown variant '${selection.variantName}' in failed-cell rerun selection.`,
      );
    }
  }
}
...
function cellSelectionMatches(
  ...
  return selections.some((selection) => {
    ...
    return (
      selection.variantName === undefined ||
      selection.variantName === variantName
    );
  });
}
...
function sampleCases(
  ...
  if (!Number.isInteger(sample.size) || sample.size < 0) {
    throw new QualityDefinitionError(
      `--sample must be a non-negative integer; got ${String(sample.size)}.`,
    );
  }
  ...
  return [...cases]
    ...
    .sort(
      (left, right) =>
        ...
        left.entry.caseId.localeCompare(right.entry.caseId),
    )
    .slice(0, sample.size)
    .map(({ entry }) => entry);
}
...
function mockTools(
  ...
  for (const [name, mock] of Object.entries(mocks ?? {})) {
    const original = isRecord(merged[name])
      ...
      : {};
    merged[name] = withToolExecutionMock({
      ...original,
...
interface VariantContext {
  ...
  /** Why this task must execute live even when reusable outputs are requested. */
  outputCacheBypass?: OutputCacheBypassReason;
  /**
...
function resolveVariantContexts(input: {
  ...
  baseOutputCacheBypass?: OutputCacheBypassReason;
  setup: EngineSetup | undefined;
}): VariantContext[] {
  const {
    definition,
    ...
    return [
      {
        ...
        ...(baseOutputCacheBypass
          ? { outputCacheBypass: baseOutputCacheBypass }
          : {}),
  ...
  return declared.map(([name, overrides]) => {
    ...
    let outputCacheBypass = baseOutputCacheBypass;
    if (taskOverride !== undefined) {
      ...
      outputCacheBypass = taskOutputCacheBypassReason(taskOverride);
    }
    ...
    return {
      ...
      taskFingerprint,
      ...(outputCacheBypass ? { outputCacheBypass } : {}),
      capabilities: detected.capabilities,
  ...
  return sha256Hex(
    canonicalJson({
      ...
      datasets: definition.datasets.map((dataset) =>
        datasetIdentity(dataset, rootDir),
      ),
...
function taskFingerprintOf(task: unknown, detected: DetectedTask): string {
  ...
  const fixtureIdentities = taskToolSourceFixtureIdentities(task);
  if (fixtureIdentities.length > 0) {
    identity.toolSourceFixtures = fixtureIdentities;
  }
  if (detected.kind === "fn" && typeof task === "function") {
    ...
    identity.configFingerprint = fingerprintValue(
      (task as { config?: unknown }).config,
    );
...
function scorerIdentity(
  scorer: EvaluationDefinition["scorers"][number],
): Record<string, unknown> {
  ...
  return {
    ...
    ...(explicit !== undefined
      ? { identityFingerprint: fingerprintValue(explicit) }
      : {}),
  };
}
...
interface CellCacheContext {
  ...
  reuse: boolean;
  replayMode: string;
  bypassReason?: OutputCacheBypassReason;
}
...
async function executeCell(input: {
  ...
  const cacheKey =
    input.cache === undefined || input.cache.bypassReason !== undefined
      ? undefined
      : cellCacheKey({
  ...
  if (
    ...
    input.cache.bypassReason === undefined &&
    cacheKey !== undefined &&
    cellError === undefined &&
    !Array.isArray(rawCase.turns)
  ) {
  ...
  return assembleCell({
    ...
    traceId: run.traceId,
    cached: false,
    cacheBypassReason: input.cache?.bypassReason,
  });
}
...
async function assembleCell(args: {
  ...
  traceId?: string;
  cached: boolean;
  cacheBypassReason?: OutputCacheBypassReason;
}): Promise<ExperimentCell<unknown, unknown>> {
  const {
    input,
    rawCase,
  ...
  const redactedOutput = truncateOutput(
    safeQualitySnapshot(output, input.redactPaths),
  );

  const metadata: Record<string, unknown> = {
    ...(rawCase.metadata !== undefined
      ? (safeQualitySnapshot(rawCase.metadata, input.redactPaths) as Record<
          string,
          unknown
        >)
      : {}),
    ...
    ...(cached ? { cached: true } : {}),
    ...(cacheBypassReason ? { outputCacheBypass: cacheBypassReason } : {}),
  };
  ...
  return {
    ...
    ...(rawCase.expected !== undefined
      ? { expected: safeQualitySnapshot(rawCase.expected, input.redactPaths) }
      : {}),
    ...
    ...(cellError !== undefined
      ? { error: sanitizeMediaPreview(cellError) as CellRuntimeError }
      : {}),
    durationMs,
    ...(signals.costUsd !== undefined ? { costUsd: signals.costUsd } : {}),
...
async function runEvaluationInner(
  ...
  const outputCacheBypass = taskOutputCacheBypassReason(definition.task);
  ...
  const allVariantContexts = resolveVariantContexts({
    ...
    ...(outputCacheBypass ? { baseOutputCacheBypass: outputCacheBypass } : {}),
    setup: options.setup,
  });
  ...
  if (variantFilter !== undefined && variantFilter.length > 0) {
    ...
    variantSubsetDemotes =
      ...
      (definition.baseline === undefined ||
        !filterSet.has(definition.baseline));
  }
  const selectedVariantNames = variantContexts.map((context) => context.name);
  validateCellSelections(
    overrides?.cells,
    new Set(allVariantContexts.map((context) => context.name)),
  );
  ...
  if (
    overrides?.maxCostUsd !== undefined &&
    (!Number.isFinite(overrides.maxCostUsd) || overrides.maxCostUsd < 0)
  ) {
    throw new QualityDefinitionError(
      `--max-cost must be a non-negative finite number; got ${String(overrides.maxCostUsd)}.`,
    );
  }
  ...
  for (const variant of variantContexts) {
    for (const resolved of selected) {
      ...
      if (skip !== undefined && skip !== false) {
        ...
        if (
          !cellSelectionMatches(overrides?.cells, resolved.caseId, variant.name)
        )
          continue;
  ...
  try {
        ...
        executeCell({
          ...
          cache:
            cacheContext === undefined
              ...
              : {
                  ...cacheContext,
                  ...(plan.variant.outputCacheBypass
                    ? { bypassReason: plan.variant.outputCacheBypass }
                    : {}),
                },
    ...
    cells = await withSignalCapture(capture, async () => {
      if (overrides?.maxCostUsd === undefined) {
        return Promise.all(
          plans.map((plan) => limiter(() => executePlan(plan))),
        );
      }
