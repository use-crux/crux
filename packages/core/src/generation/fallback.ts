...
export function classifyError(error: unknown): ErrorCategory | null {
  ...
  if (isCruxAdapterError(error)) {
    ...
    if (kind === 'invalid-request' && code.endsWith('.authentication')) return 'auth_error'
    if (kind === 'provider-error' && code.includes('connection')) return 'connection_error'
    if (kind === 'provider-error' && error.providerError.retryable) return 'server_error'
    return null
  }
