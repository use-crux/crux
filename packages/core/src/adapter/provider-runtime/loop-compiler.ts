...
function portForBoundLoop(
  ...
  if (bound.replayStream)
    port.replayStream = (cached) => bound.replayStream!(cached);
  return port;
}

function describeModelFallback<TModel>(
  runtimeId: string,
  model: TModel,
): ModelInfo {
  ...
  if (typeof model === "object" && model !== null) {
    ...
    const provider =
      typeof record.provider === "string" ? record.provider : runtimeId;
    const modelId =
