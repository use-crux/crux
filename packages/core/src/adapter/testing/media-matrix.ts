...
interface MediaAdapterConformanceRow {
  ...
  readonly streamImage: string;
  readonly streamSpeech: string;
}
...
const MESSAGE_SUPPORT = {
  ...
  anthropic: [
    ...
    "unsupported",
    "native",
    "limited native file/tool output",
  ],
  "ai-sdk": [
    "model-owned",
    "model-owned",
  ...
  Record<Adapter, readonly [string, string, string, string, string]>
>;
...
function operationLabel(
  operation: "image" | "transcription" | "speech",
  adapter: Adapter,
): string {
  const rows =
    operation === "image"
  ...
  if (operation === "transcription" && adapter === "google")
    return "honest composition, no word timing/diarization";
  if (operation === "image" && adapter === "google")
    return "Imagen/Gemini native";
  if (operation === "speech" && adapter === "google")
    return "native incl. multi-speaker";
  return support;
}

/** Tested documentation rows. This remains test-only and is not a capability API. */
export const MEDIA_ADAPTER_MATRIX: readonly MediaAdapterConformanceRow[] =
  Object.freeze(
    ADAPTERS.map(([adapter, label]) => {
      ...
      return Object.freeze({
        ...
        streamImage:
          adapter === "openai" || adapter === "google" ? "native" : "absent",
        streamSpeech:
          adapter === "openai" || adapter === "google" ? "native" : "absent",
      });
    }),
...
export function mediaAdapterMatrixMarkdown(): string {
  const cell = (
    capability: keyof Omit<MediaAdapterConformanceRow, "adapter" | "label">,
    label: string,
  ) =>
  ...
  return [
    ...
    cell("streamImage", "`streamImage`"),
    cell("streamSpeech", "`streamSpeech`"),
  ].join("\n");
}
