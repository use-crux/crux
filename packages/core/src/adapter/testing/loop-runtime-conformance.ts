 ...
 * 8. A declared pre-client-tool step transform is ordered, terminal on
 *    failure, and cannot edit tool-call parts.
 *
...
export async function loopRuntimePortConformance<TModel>(
  ...
  // 1. No-tool response completes in one step.
  {
    ...
    if (outcome.status !== "complete")
      fail(
        ...
        `expected complete, got ${outcome.status}`,
      );
    else {
      if (outcome.steps !== 1)
        fail("single-step completion", `expected 1 step, got ${outcome.steps}`);
      if (outcome.response.text !== "done")
        fail(
          "single-step completion",
          `expected final text 'done', got '${outcome.response.text}'`,
        );
    }
  }
  ...
  // 2 + 4. Tool loop runs to completion; observer sees ordered steps.
  {
    const { runtime, model } = await harness.prepare({
      emissions: [
        { text: "", toolCalls: [{ name: "echo", args: { v: 1 } }] },
        { text: "finished" },
      ],
    });
    const seen: number[] = [];
    ...
    if (outcome.status !== "complete")
      fail("tool loop completion", `expected complete, got ${outcome.status}`);
    else if (outcome.steps !== 2)
      fail("tool loop completion", `expected 2 steps, got ${outcome.steps}`);
    if (
      seen.length === 0 ||
      seen[0] !== 0 ||
      ...
      fail(
        "observer ordering",
        `expected strictly increasing indexes from 0, saw [${seen.join(", ")}]`,
      );
    }
  }
  ...
  // 3. maxSteps caps the loop.
  {
    ...
    const { runtime, model } = await harness.prepare({
      emissions: [toolStep, toolStep, toolStep, { text: "late" }],
    });
    const outcome = await runtime.runTextLoop(
      baseRequest(runtime, model, { tools: { echo: echoTool }, maxSteps: 2 }),
    );
  ...
  // 4b. `stop` directive halts the loop.
  {
    ...
    const { runtime, model } = await harness.prepare({
      emissions: [toolStep, toolStep, { text: "unreachable" }],
    });
    const outcome = await runtime.runTextLoop(
      baseRequest(runtime, model, {
        tools: { echo: echoTool },
        observer: {
          onStepEnd: async () => ({ kind: "stop", reason: "conformance" }),
        },
      }),
    );
    if (outcome.status !== "complete")
      fail("stop directive", `expected complete, got ${outcome.status}`);
    else if (outcome.steps !== 1)
      fail(
        "stop directive",
        `expected loop to stop after 1 step, got ${outcome.steps}`,
      );
  }
  ...
  // 5. amend + refundStep returns the step to the budget.
  {
    const toolStep = { text: "", toolCalls: [{ name: "echo", args: {} }] };
    const { runtime, model } = await harness.prepare({
      emissions: [toolStep, toolStep, { text: "end" }],
    });
    let refunded = false;
    const outcome = await runtime.runTextLoop(
      baseRequest(runtime, model, {
    ...
    if (outcome.status !== "complete")
      fail("refundStep", `expected complete, got ${outcome.status}`);
    else {
      if (outcome.steps !== 2) {
        fail(
          "refundStep",
          `expected 2 budget-consuming steps after one refund, got ${outcome.steps}`,
        );
      }
      ...
      if (outcome.response.text !== "end") {
        fail(
          "refundStep",
          `expected final text 'end' once the refund unlocked a third step, got '${outcome.response.text}'`,
        );
      }
    }
  }
  ...
  // 6. Approval-needing tool suspends without executing.
  {
    ...
    if (outcome.status !== "suspended")
      fail("approval suspension", `expected suspended, got ${outcome.status}`);
    else {
      if (executed)
        fail("approval suspension", "tool executed despite needing approval");
      ...
      if (outcome.messages.some((message) => message.role === "assistant")) {
        fail(
          "approval suspension",
          "suspended messages must not include the approval-requesting assistant step",
        );
      }
  ...
  // 7. runStructuredAttempt: invalid is a value; valid yields the object.
  {
    ...
    try {
      ...
      if (attempt.status !== "invalid")
        fail(
          "structured invalid-as-value",
          `expected invalid, got ${attempt.status}`,
        );
    } catch (error) {
      fail(
        "structured invalid-as-value",
        `runStructuredAttempt threw on invalid output: ${String(error)}`,
      );
    }
  ...
  violations.push(...(await stepTransformConformance(harness)));
  return violations;
}
