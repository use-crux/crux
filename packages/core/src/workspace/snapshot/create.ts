...
export async function createWorkspaceSnapshot(
  ...
  return withWorkspaceMutationLock(config.workspaceId, namespace, async () => {
    assertSnapshotSelectionSupported(path, config.mounts);
    const records = selectSnapshotRecords(
      await listFileRecords(config.store, config.workspaceId, namespace).catch(
        (error: unknown) => {
    ...
    try {
      ...
      await deleteSnapshotEntries(config.store, header).catch(() => undefined);
      for (const asset of ownedAssets.reverse()) {
        await config.assets?.delete(asset).catch(() => undefined);
      }
      await cleanupSnapshotHeader(config.store, header).catch(() => undefined);
