...
import type { JsonObject } from "../types/tool";
...
import { clonePrivateJsonObject, isPrivateJsonObject } from "./json-private";
import type {
...
async function encodeParts(
  ...
  const encoded: PersistedContentPart[] = [];
...
async function encodeMediaPart(
  part: Extract<InvocationContentPart, { readonly type: "image" | "file" }>,
  path: string,
  state: EncodeState,
