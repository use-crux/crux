...
export type ImagePartInput =
  | { readonly data: string | Uint8Array | ArrayBuffer; readonly mediaType: string }
...
function base64Data(data: string | Uint8Array | ArrayBuffer): string {
  ...
  return bytesToBase64(data instanceof Uint8Array ? data : new Uint8Array(data))
}
...
function fileIdText(fileId: string | Record<string, string>): string {
  return typeof fileId === 'string' ? escapeBare(fileId) : JSON.stringify(fileId)
}
