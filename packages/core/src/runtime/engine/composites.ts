...
import type { WakeFailureInput } from "./kernel-wake-commits";
import type {
  CancelWorkInput,
  CancelWorkResult,
  EmitEventInput,
...
import type {
  ...
  ExpireDeferredScopeInput,
  ExpireDeferredScopeResult,
...
export interface RuntimeCompositeInput {
  ...
  /** Atomically accept a Signal occurrence and every required delivery. */
  readonly "signal.publish": SignalPublishCompositeInput;
...
export interface RuntimeCompositeResult {
  ...
  readonly "signal.publish": SignalPublishCompositeResult;
  /** No result. */
