...
const observabilityScope =
  createAsyncScopeFacet<ObservabilityContextFrame>("core.observability");
 ...
 */
export function withObservabilityContext<R>(
 ...
 */
export function withCruxCorrelators<R>(
  correlators: CruxCorrelators,
  fn: () => R,
): R {
  const previousFrame = currentFrame();
  const merged = mergeCruxCorrelators(previousFrame?.correlators, correlators);
  const context = previousFrame?.context
    ? { ...previousFrame.context, correlators: merged }
    : undefined;
