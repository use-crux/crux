 ...
 */
export function withObservableInvocation<TArgs extends readonly unknown[], TResult>(
  ...
  return async (...args: TArgs): Promise<TResult> => {
    ...
    try {
      outcome = { ok: true, value: await observe.withHostLifecycle(lifecycle, () => handler(...args)) }
    } catch (error) {
...
async function reportDrain(
  ...
  const flushTimeoutMs =
    ...
    remainingHostDeadlineMs(lifecycle, { safetyMarginMs: invocation?.flushSafetyMarginMs ?? 0 })
  let result: ObservabilityFlushResult
  try {
    result = await observe.flush(flushTimeoutMs === undefined ? {} : { timeoutMs: flushTimeoutMs })
  } catch (error) {
...
function resolveDeadlineMs(invocation: CruxServerlessInvocation | undefined): number | undefined {
  if (invocation?.deadlineMs !== undefined) return invocation.deadlineMs
  if (invocation?.remainingTimeMs !== undefined) return Date.now() + invocation.remainingTimeMs
  return undefined
}
