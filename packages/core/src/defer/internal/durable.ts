...
export interface DurableDeferEvidenceHooks {
  /** Open/capture the public run correlators before durable acceptance. */
...
export function createDurableDeferController(
  ...
  async function createSession(): Promise<DurableSession> {
    ...
    if (!definition) throw runtimeRequiredError({ api: 'defer(target, input)' })
  ...
  return {
    async stage(target, input) {
      ...
      // Capture public correlators and a predetermined scheduled span id before
      ...
      const correlators = evidence?.ensurePublicCorrelators?.()
      ...
      const provisionalProvenance: RuntimeNamedDeferProvenance = {
        ...
        ...(correlators?.runId ? { runId: correlators.runId } : {}),
        ...(correlators?.traceId ? { traceId: correlators.traceId } : {}),
      }
