...
export type ToolModelInputOrigin = Extract<
  ModelInputOrigin,
...
export type CanonicalModelIngressResult =
...
export async function guardModelIngress(
  options: GuardModelIngressOptions,
): Promise<CanonicalModelIngressResult> {
  if (options.input.kind === "text") {
    const bindings = inputBindingsFor(
      options.bindings,
      "model.input.text",
      options.input.origin.source,
    );
...
async function guardDocumentIngress(
  ...
  const mediaBindings = inputBindingsFor(
    options.bindings,
    "model.input.media",
    options.input.origin.source,
  );
  ...
  if (mediaBindings.length > 0) {
    const mediaSlots = options.input.slots.filter(
      (slot) => slot.kind === "media",
    );
    if (mediaSlots.length > 0) {
      await visitMedia({
        ...
        groups: [
          { id: "model-ingress", size: mediaSlots.length, minimumRetained: 0 },
        ],
        context: ({ subject }) => ({
          ...options.context,
          origin: {
  ...
  const textBindings = inputBindingsFor(
    options.bindings,
    "model.input.text",
    options.input.origin.source,
  );
  ...
  const originalProjection = projectModelIngressSlots(
    options.input.slots,
    removed,
  );
  const result = await createGuardrailPipeline(textBindings).runInput(
    originalProjection,
    {
      ...options.context,
      origin: options.input.origin,
  ...
  const text = patchModelIngressText(
    options.input.slots,
    removed,
    result.content,
  );
  if (!text) {
    ...
    throw new SafetyResultError({
      ...
      problem:
        "rewrite could not be faithfully applied to structured model input",
      message:
  ...
  for (const slot of input.slots) {
    if (keys.has(slot.key)) {
      throw new SafetyResultError({
        ...
        message:
          "Structured model input could not be guarded because its private slot keys were not unique.",
      });
    }
    keys.add(slot.key);
  }
