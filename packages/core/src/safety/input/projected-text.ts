...
import type { GuardrailAudit, GuardrailAuditEntry, GuardrailContext } from '../guardrail/types'
...
export async function guardProjectedTextInput(
  ...
  for (let index = 0; index < messages.length; index++) {
    ...
    const result = await pipeline.runInput(originalContent, options.context(messages))
  ...
  const result = await pipeline.runInput(input.prompt, options.context(input.messages))
  return {
...
function latestRewritePolicyId(entries: readonly GuardrailAuditEntry[]): string | undefined {
  for (let index = entries.length - 1; index >= 0; index--) {
    ...
    if (!entry) continue
    if (entry.action === 'redact' || entry.action === 'transform' || entry.action === 'rewrite') return entry.guard
  }
  return undefined
}
