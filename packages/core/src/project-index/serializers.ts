...
import { isPromptText, lowerPromptText } from "../prompt-text/internal";
...
function directTemplateText(value: unknown): string | undefined {
  if (typeof value === "string") return value;
  if (isPromptText(value)) return lowerPromptText(value).text;
  return undefined;
}
...
export function serializePrompt(prompt: AnyPrompt): PromptMeta {
  ...
  const systemTemplate = directTemplateText(sys);
  if (systemTemplate !== undefined) meta.systemTemplate = systemTemplate;
  else if (typeof sys === "function") meta.systemTemplate = fnSource(sys);

  // Extract prompt template
  const pr = prompt.config.prompt;
  const promptTemplate = directTemplateText(pr);
  if (promptTemplate !== undefined) meta.promptTemplate = promptTemplate;
...
export function serializeContext(
  ...
  const usedBy = prompts
    .filter((p) => p.contexts.some((c) => c === ctx))
    .map((p) => p.id);
  ...
  const meta: ContextMeta = {
    ...
    isStatic: ctx.systemKind === "static",
    usedBy,
  };

  // Only SDK-owned direct-value wrappers are safe to execute during serialization.
  if (ctx.systemKind === "static") {
    try {
      const result = ctx.systemFn({});
      meta.systemTemplate =
        directTemplateText(result) ?? fnSource(ctx.systemFn);
    } catch {
  ...
  index.prompts.forEach((prompt, index) => {
    ...
    if (!prompt.id) {
      diagnostics.push({
        ...
        suggestedFix:
          "Add a stable prompt id for history, baselines, and affected-eval tracking.",
      });
    }
  ...
  index.contexts.forEach((context, index) => {
    ...
    if (!context.id) {
      diagnostics.push({
        ...
        code: "missing-definition-id",
        message:
          "Context has no explicit id; Crux derived a local fallback id.",
        source: context.definitionSource,
        relatedDefinitionIds: [id],
        suggestedFix:
          "Add a stable context id so prompts and index relations remain durable.",
      });
    }
  });
