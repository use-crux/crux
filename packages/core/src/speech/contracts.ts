...
export type GenerateSpeechOptions<TModel = string, TVoice = string, TExtra = never> = Readonly<{
  model: TModel
  text: string
...
export type GenerateSpeechResult<TRaw = unknown, TMetadata = unknown, TWarning = unknown> =
  CompletedOperationResult<TRaw, TMetadata, TWarning> & Readonly<{ audio: DataAsset }>
...
export type GenerateSpeech<
  ...
  TMetadata = unknown,
  TWarning = unknown,
> = (options: GenerateSpeechOptions<TModel, TVoice, TExtra>) => Promise<GenerateSpeechResult<TRaw, TMetadata, TWarning>>
