...
export function validateGenerateSpeechOptions(
  options: Readonly<{
    text: string;
    outputFormat?: string;
    instructions?: string;
 ...
 */
export function createGenerateSpeechResult<
  TRaw,
  TMetadata = unknown,
  TWarning = unknown,
>(
  ...
  const mediaTypeEssence = audio.mediaType.split(";", 1)[0]?.trim() ?? "";
  if (!/^audio\/[a-z0-9!#$&^_.+-]+$/i.test(mediaTypeEssence))
    throw new TypeError("Speech audio must use an audio MIME type.");
  if (audio.data instanceof Uint8Array && audio.data.byteLength === 0)
    throw new TypeError("Speech audio bytes must not be empty.");
  if (audio.data instanceof Blob && audio.data.size === 0)
    throw new TypeError("Speech audio blob must not be empty.");
