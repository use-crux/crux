...
export function workspaceSetOptions(store: DataStore, retention: WorkspaceRetention | undefined): SetOptions | undefined {
  const ttl = retention?.ttlMs
  if (ttl === undefined || ttl <= 0 || store.supportsTtl?.() !== true) return undefined
  return { ttl }
}
...
export async function assertWorkspaceWriteAllowed(input: {
  ...
  if (limits.maxFileBytes !== undefined && input.nextSize > limits.maxFileBytes) {
    throw new Error(
  ...
  const records = await listFileRecords(input.store, input.workspaceId, input.namespace)
  const currentTotal = records.reduce((total, record) => total + record.size, 0)
  const existingSize = input.existing?.size ?? 0
  const nextTotal = currentTotal - existingSize + input.nextSize
