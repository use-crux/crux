...
import type { WorkspaceArtifact, WorkspaceArtifactsQuery, WorkspaceFinalizeOptions } from './artifact-types'
import type {
  NormalizedMount,
  WorkspaceFileRecord,
} from './types'
...
export function createWorkspaceArtifactOps(config: WorkspaceArtifactOpsConfig): WorkspaceArtifactOps {
  async function artifacts(options?: WorkspaceArtifactsQuery): Promise<readonly WorkspaceArtifact[]> {
    const namespace = options?.namespace ?? (await config.resolveNamespace())
    return instrument({ workspaceId: config.workspaceId, operation: 'artifacts', namespace, path: options?.path ?? '/outputs' }, async () => {
      const filter = artifactFilter(options)
      const records = await listFileRecords(config.store, config.workspaceId, namespace, { filter })
      return records
        .filter(isArtifactRecord)
        .map((record) => recordToArtifact(record, { workspaceId: config.workspaceId, namespace }))
    })
  }

  async function finalize(path: string, options?: WorkspaceFinalizeOptions): Promise<WorkspaceArtifact> {
    const namespace = options?.namespace ?? (await config.resolveNamespace())
    return instrument({ workspaceId: config.workspaceId, operation: 'finalize', namespace, path }, async () => {
      const normalized = normalizePath(path)
      mountForPath(normalized, config.mounts, 'write')
      const current = await getRequiredRecord(config.store, config.workspaceId, namespace, normalized)
      ...
      await config.store.set(fileKey(config.workspaceId, namespace, normalized), finalized)
      return recordToArtifact(finalized, { workspaceId: config.workspaceId, namespace })
    })
  }
...
export function recordToArtifact(
  ...
  return {
    ...
    size: record.size,
    uri: record.uri ?? inlineArtifactUri(scope.workspaceId, scope.namespace, record.path),
...
function inlineArtifactUri(workspaceId: string, namespace: string, path: string): string {
...
function artifactFilter(options: WorkspaceArtifactsQuery | undefined): Record<string, unknown> | undefined {
...
function isArtifactRecord(record: WorkspaceFileRecord): boolean {
  return record.status !== undefined || record.kind !== undefined || record.producedBy !== undefined
}
