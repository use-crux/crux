...
/**
 * Serialize a `Prompt` instance into JSON-safe metadata for the devtools catalog.
 *
...
export function serializePrompt(prompt: AnyPrompt): PromptMeta {
  const meta: PromptMeta = {
    ...
    contextIds: prompt.contexts
      .filter((c): c is Context<z.ZodType> => !!c && typeof c === 'object' && '_tag' in c && (c as { _tag: unknown })._tag === 'Context')
      .map((c) => c.id),
...
/**
 * Serialize a `Context` instance into JSON-safe metadata for the devtools catalog.
 *
...
/**
 * Serialize a `FlowToolDef` into JSON-safe metadata for the devtools catalog.
 */
...
/**
 * Serialize a full catalog of prompts, contexts, and tools.
 *
 ...
 * @param tools - Optional tool definitions to include in the catalog.
 */
...
export function catalogDefinitionsFromSnapshot(catalog: {
  prompts: PromptMeta[]
  contexts: ContextMeta[]
  tools?: ToolMeta[]
}): { definitions: ProjectDefinition[]; relations: ProjectRelation[]; diagnostics: CatalogDiagnostic[]; sources: CatalogSourceFile[] } {
  ...
  function trackSource(definitionId: string, source: { file: string } | undefined): void {
    if (!source) return
    const existing = sourceFiles.get(source.file) ?? { file: source.file, status: 'indexed' as const, definitionIds: [] }
    existing.definitionIds = [...(existing.definitionIds ?? []), definitionId]
    sourceFiles.set(source.file, existing)
  }

  catalog.prompts.forEach((prompt, index) => {
  ...
  catalog.contexts.forEach((context, index) => {
    ...
    if (!context.id) {
      diagnostics.push({
        ...
        source: context.definitionSource,
        relatedDefinitionIds: [id],
        suggestedFix: 'Add a stable context id so prompts and catalog relations remain durable.',
      })
    }
  })
...
export function serializeProjectCatalog(input: {
