...
> {
  ...
  prepare(
    ...
    | Promise<ProviderConformancePrepared<TClient, TModel, TDeps>>
    | ProviderConformancePrepared<TClient, TModel, TDeps>;
}
...
export interface ProviderRuntimeConformanceStreamHandle {
  /** Canonical native adapters expose provider-neutral text deltas here. */
  ...
  readonly completion: Promise<unknown> | (() => Promise<unknown>);
}
