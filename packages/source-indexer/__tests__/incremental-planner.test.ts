...
import type { CatalogSourceFile, ProjectCatalogSnapshot, ProjectDefinition } from '@crux/core/catalog'
...
import { catalogBoundaryFileNames, catalogCacheBoundaryFileNames } from '../indexer/incremental/boundaries'
import { normalizedCatalogStateFromSnapshot } from './helpers/catalog-equivalence'
...
describe('incremental catalog planner', () => {
      ...
      previousCatalog: catalog({
        sources: [
          { file: entry, status: 'indexed', dependencies: [prompt], dependents: [] },
          {
            ...
            dependencies: [],
            dependents: [entry],
            diagnostics: ['catalog.rich_import_failed:prompt'],
          },
        ],
        diagnostics: [
          {
            id: 'catalog.rich_import_failed:prompt',
  ...
  it('converts planner decisions into catalog invalidations', () => {
    const file = join(root, 'src/prompt.ts')
    ...
    expect(catalogInvalidationFromDecision(partial)).toEqual({
      files: [file],
      definitionIds: ['prompt:writer'],
    })
    expect(catalogInvalidationFromDecision(full)).toEqual({ all: true })
  })
  ...
  it('keeps planner and cache boundary constants aligned for compiler config files', () => {
    expect(catalogBoundaryFileNames).toEqual(expect.arrayContaining([...catalogCacheBoundaryFileNames]))
  })
  ...
  it('provides a normalized patch equivalence harness for future partial execution tests', () => {
    ...
    expect(normalizedCatalogStateFromSnapshot(snapshot)).toEqual(normalizedCatalogStateFromSnapshot(snapshot))
  })
  ...
  it('explains semantic closure vocabulary without implying execution', () => {
    ...
    expect(
      explainIncrementalDecision({
        ...
        graphConfidence: 'complete-enough-for-source-closure',
        explanation: {
          summary: 'Semantic source refs may affect catalog facts.',
          graphAvailable: true,
          fallbackUsed: false,
          traversedFiles: [file],
        },
...
function catalog(input: {
  ...
  readonly diagnostics?: ProjectCatalogSnapshot['diagnostics']
