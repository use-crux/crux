...
describe('extension extractor parity', () => {
  it.each([
    ...
    const file = join(root, 'src/catalog.ts')
...
describe('fact-first static parser', () => {
  it('projects extracted facts into catalog definitions, relations, and dependencies', async () => {
    const root = await fixtureRoot()
    const sourceFile = join(root, 'src/catalog.ts')
