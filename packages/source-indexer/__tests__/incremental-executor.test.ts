...
describe('incremental indexing executor', () => {
  it('applies a partial AST patch that matches a full source-only reindex for a leaf source change', async () => {
    ...
    const patchedState = applyCatalogPatch(
      applyCatalogPatch(emptyCatalogPatchState(), catalogPatchFromSnapshot(previousCatalog, 'ast', 'ok')),
      incremental.patches[0],
    )
    const fullUpdatedCatalog = await indexProject({ root, staticOnly: true })
    const fullUpdatedState = applyCatalogPatch(
      emptyCatalogPatchState(),
      catalogPatchFromSnapshot(fullUpdatedCatalog, 'ast', 'ok'),
    )
    ...
    expect(normalizedCatalogState(patchedState)).toEqual(normalizedCatalogState(fullUpdatedState))
  })
  ...
  it('applies partial AST and semantic patches that match full reindex for a TypeScript analyzer source change', async () => {
    ...
    const patchedState = incremental.patches.reduce(
      (state, patch) => applyCatalogPatch(state, patch),
      applyCatalogPatch(
        applyCatalogPatch(emptyCatalogPatchState(), catalogPatchFromSnapshot(previousCatalog, 'ast', 'ok')),
    ...
    const fullUpdatedCatalog = await indexProject({ root, staticOnly: true })
    const fullUpdatedSemanticPatch = await indexProjectSemantic({ root, previousCatalog: fullUpdatedCatalog })
    const fullUpdatedState = applyCatalogPatch(
      applyCatalogPatch(emptyCatalogPatchState(), catalogPatchFromSnapshot(fullUpdatedCatalog, 'ast', 'ok')),
      fullUpdatedSemanticPatch,
    )

    expect(incremental.decision.kind).toBe('source-file-reindex')
    expect(incremental.report.semanticAnalyzedFiles).toEqual([
      join(root, 'src/tool.ts'),
    ])
    expect(normalizedCatalogState(patchedState)).toEqual(normalizedCatalogState(fullUpdatedState))
  }, 15000)
  ...
  it('uses semantic source-ref support rows to partially reindex a schema support file', async () => {
    ...
    const astCatalog = await indexProject({ root, staticOnly: true })
    const enriched = await indexProjectIncremental({
      root,
      previousCatalog: astCatalog,
      files: [join(root, 'src/tool.ts')],
      mode: 'ast-and-semantic',
    })
    const enrichedCatalog = projectCatalogFromState(
      enriched.patches.reduce(
        (state, patch) => applyCatalogPatch(state, patch),
        applyCatalogPatch(emptyCatalogPatchState(), catalogPatchFromSnapshot(astCatalog, 'ast', 'ok')),
      ),
      astCatalog,
    )

    expect(enrichedCatalog.sources).toContainEqual(
    ...
    const incremental = await indexProjectIncremental({
      root,
      previousCatalog: enrichedCatalog,
      files: [join(root, 'src/schema.ts')],
      mode: 'ast-and-semantic',
    })
    const patchedState = incremental.patches.reduce(
      (state, patch) => applyCatalogPatch(state, patch),
      applyCatalogPatch(emptyCatalogPatchState(), catalogPatchFromSnapshot(enrichedCatalog, 'ast', 'ok')),
    )
    const fullUpdatedCatalog = await indexProject({ root, staticOnly: true })
    const fullUpdatedSemanticPatch = await indexProjectSemantic({ root, previousCatalog: enrichedCatalog })
    const fullUpdatedState = applyCatalogPatch(
      applyCatalogPatch(emptyCatalogPatchState(), catalogPatchFromSnapshot(fullUpdatedCatalog, 'ast', 'ok')),
      fullUpdatedSemanticPatch,
    )
...
function normalizedCatalogState(state: ReturnType<typeof applyCatalogPatch>): unknown {
  return JSON.parse(
    JSON.stringify({
...
function normalizedSourceRow(source: ReturnType<typeof applyCatalogPatch>['sources'][number]): ReturnType<typeof applyCatalogPatch>['sources'][number] {
  return {
    file: source.file,
...
function projectCatalogFromState(
  state: ReturnType<typeof applyCatalogPatch>,
