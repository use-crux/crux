...
export function fullReindexDecision(input: {
  ...
  return {
    ...
    previousCatalogDefinitionCount: input.previousCatalogDefinitionCount,
...
export function sourceFileDecision(input: {
  ...
  return {
    ...
    explanation: explanation({
      summary: 'Changed files are known source leaves in the previous catalog graph.',
      graphAvailable: true,
      fallbackUsed: false,
...
export function dependencyClosureDecision(input: {
  ...
  return {
    ...
    graphConfidence: 'complete-enough-for-source-closure',
    explanation: explanation({
      summary: 'Changed files affect known dependents through the previous catalog source graph.',
      graphAvailable: true,
      fallbackUsed: false,
      traversedFiles: input.affectedFiles,
    }),
