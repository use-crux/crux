...
import type { CatalogExtractor, UnresolvedReference } from '../extensions'
import { facts } from '../extensions'
import type { ProjectSourceRefRole } from '@crux/core/catalog'
 ...
 * unresolved references so relation resolution can connect policies to protected catalog definitions.
 */
export const safetyCatalogExtractor: CatalogExtractor = {
  name: 'safety',
  patterns: [
...
function validatorSourceRefs(
  ctx: Parameters<CatalogExtractor['extract']>[0],
  definitionId: string,
 ...
 * a catalog-id separator are treated as direct ids.
 */
function appliesToRefs(config: NonNullable<Parameters<CatalogExtractor['extract']>[0]['config']>): {
