 ...
 * This extractor normalizes those shapes into catalog definitions and folded store definitions while
 * preserving retention/schema intelligence for detail views and lint rules.
 */
export const memoryCatalogExtractor: CatalogExtractor = {
  name: 'memory',
 ...
 *
 * Blackboard facts help catalog consumers identify shared writable state and reason about merge or
 * conflict behavior without executing runtime orchestration.
 */
export const blackboardCatalogExtractor: CatalogExtractor = {
  name: 'blackboard',
 ...
 *
 * The metadata separates data/contract/runtime concerns so catalog consumers do not need to parse raw
...
/**
 * Projects one memory block object into catalog-facing metadata.
 *
...
/** Projects one configured memory block helper call into catalog-facing metadata. */
 ...
 * catalog while staying attached to the memory primitive that declared them.
 */
...
/** Projects static store configuration into catalog metadata for memory backing resources. */
