 ...
 * `createSourceIndexerExtensionRuntime(...).extractStatic(...)` so they can observe no-match, none,
...
export function extractFactsWithExtensionRegistry(
  ...
  return extractedFactsFromStaticExtractionResult(
    createSourceIndexerExtensionRuntime({ extensions: registry.extensions }).extractStatic(staticCtx),
  )
}
