...
`@crux/source-indexer` will use compiler profiles to assemble first-party Source Indexer Extensions,
compiler-owned intrinsics, and Extension Runtime instances before adding public third-party extension
loading.

**Context**

The Project Catalog Compiler already had an experimental extension boundary with extractors, relation
...
intrinsics. `createProjectCatalogCompiler(...)` creates an isolated compiler instance from a profile
...
Catalog rules now require metadata before they can run. Rule metadata includes docs, option schema,
...
The package export map is intentionally narrow. `@crux/source-indexer` exports the stable indexing
entry points, `@crux/source-indexer/extensions` exports the experimental extractor authoring surface,
and `@crux/source-indexer/source-resolver` exports the source resolver facade. Internal `indexer/*`
