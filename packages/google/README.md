...
The package exports `googleProviderRuntime` for advanced adapter composition. `createGoogle()` binds `googleProviderRuntime.create(client, { cacheResolver })` after resolving CachedContent options; adapter authors should use `defineProviderRuntime()` from `@crux/core/adapter`.
