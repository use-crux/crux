 ...
 */
const googleDialect: ProviderTranscriptDialect<
  Content,
...
export function readGoogleAssistant(
  response: GenerateContentResponse,
): GoogleAssistantTurn {
  const parts = (response.candidates?.[0]?.content?.parts ??
    []) as readonly GoogleInboundPart[];
  const toolCalls = toolCallsFromParts(parts);
  const content = assistantContentFromParts(parts);
  const projectedText = googlePartsText(parts);
  return {
    text: response.text ?? projectedText,
    content,
    toolCalls: toolCalls.length > 0 ? toolCalls : undefined,
  };
}

function assistantContentFromParts(
  ...
  return parts.flatMap((part, index): AssistantContentPart[] => {
    ...
    const content = messageContentFromGoogleParts([part]);
...
function decodeMessage(value: unknown): ProviderTranscriptUnit {
  const content = isGoogleContentLike(value)
    ? value
    : { role: "user", parts: [{ text: String(value ?? "") }] };
...
function isFunctionCall(value: unknown): value is GoogleFunctionCall {
  return (
    isRecord(value) && optionalString(value.id) && optionalString(value.name)
  );
}
...
function isGoogleContentLike(value: unknown): value is GoogleContentLike {
  return (
    isRecord(value) && (value.parts === undefined || Array.isArray(value.parts))
  );
}
