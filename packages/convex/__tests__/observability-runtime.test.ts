...
function mutationRef<TArgs extends Record<string, unknown>, TResult>(
  path: string,
): FunctionReference<"mutation", "public", TArgs, TResult> {
  return makeFunctionReference<"mutation", TArgs, TResult>(path);
}

function queryRef<TArgs extends Record<string, unknown>, TResult>(
  path: string,
): FunctionReference<"query", "public", TArgs, TResult> {
  return makeFunctionReference<"query", TArgs, TResult>(path);
}

const triagePrompt = makePrompt({
  id: "observability-runtime-triage",
  system: "Triage agent",
});
const billingPrompt = makePrompt({
  id: "observability-runtime-billing",
  system: "Billing agent",
});
...
function createMockGenerate(
  behavior: Record<
    string,
    { text: string } | { handoffTo: string; reason: string }
  >,
) {
  return async (
    prompt: { id?: string },
    ...
    const agentId =
      prompt?.id === "observability-runtime-triage"
        ? "triage"
        : prompt?.id === "observability-runtime-billing"
          ? "billing"
          : "unknown";
...
describe("@use-crux/convex observability runtime binding (Phase 8)", () => {
  ...
  describe("short bounded flush budget", () => {
    it("uses a short bounded default, not the prior 20-second window", () => {
      expect(DEFAULT_CONVEX_OBSERVABILITY_FLUSH_TIMEOUT_MS).toBeLessThanOrEqual(
        5000,
      );
  ...
  describe("structured drain inspection", () => {
    ...
    it("returns without awaiting a rejected async reporter and contains its rejection", async () => {
      setObservabilityTransport(createInMemoryObservabilityTransport());
      ...
      await expect(
        settlesBeforeReporter(flushObservability({ onDrain: () => reporter })),
      ).resolves.toEqual(
        expect.objectContaining({ status: expect.any(String) }),
      );
      rejectReporter(new Error("async Convex reporter failed"));
      await Promise.resolve();

      expect(consoleError).toHaveBeenCalledWith(
        expect.stringContaining("onDrain reporter rejected"),
        expect.objectContaining({ message: "async Convex reporter failed" }),
      );
    });
    ...
    it("warns instead of silently discarding an incomplete drain when no onDrain is supplied", async () => {
      ...
      const span = observe.openSpan({
        name: "observability-runtime-test",
        primitive: "custom.operation",
      });
      span.withContext(() =>
        observe.event({ name: "observability-runtime-test.event" }),
      );
    ...
    it("does not warn on an incomplete opportunistic (terminal: false) mid-operation flush", async () => {
      ...
      const span = observe.openSpan({
        name: "observability-runtime-opportunistic",
        primitive: "custom.operation",
      });
      span.withContext(() =>
        observe.event({ name: "observability-runtime-opportunistic.event" }),
      );
    ...
    it("still reports an incomplete opportunistic flush to an explicit onDrain callback", async () => {
      ...
      const span = observe.openSpan({
        name: "observability-runtime-opportunistic-ondrain",
        primitive: "custom.operation",
      });
      span.withContext(() =>
        observe.event({
          name: "observability-runtime-opportunistic-ondrain.event",
        }),
      );
      const onDrain = vi.fn();

      const flush = flushObservability({
        timeoutMs: 5,
        terminal: false,
        onDrain,
      });
    ...
    it("reports partial delivery (mixed accept/reject) through the structured result rather than a plain boolean", async () => {
      ...
      const span = observe.openSpan({
        name: "observability-runtime-partial",
        primitive: "custom.operation",
      });
      span.withContext(() => {
  ...
  describe("fresh-invocation segment resume", () => {
    ...
    it("resumeRun() reopens the same logical run in a fresh segment given only a carrier value (unit-level, not a Convex boundary proof)", async () => {
      ...
      const resumedRun = observe.resumeRun(carrier, {
        reason: "convex.boundary.resume",
      });
      ...
      expect(transport.records).toContainEqual(
        expect.objectContaining({
          type: "run:resume",
          runId,
          segmentId: resumedRun.segmentId,
        }),
      );
      expect(transport.records).toContainEqual(
    ...
    it("rejects a hostile/malformed carrier instead of resuming an arbitrary run", async () => {
      setObservabilityTransport(createInMemoryObservabilityTransport());
      expect(() =>
        observe.resumeRun({ crux: { runId: "" } } as never, {
          reason: "convex.boundary.resume",
        }),
      ).toThrow();
    });
    ...
    it("carries the suspended run through a real Convex durable boundary: saved via convex-test, read back by the next scheduled swarm.resume() invocation", async () => {
      ...
      const actionRun1 = observe.openRun({
        name: "convex action 1",
        rootPrimitive: "runtime.convex.action",
      });
      ...
      const swarmStart = transport.records.find(
        (record) =>
          ...
          record.rootPrimitive === "composition.swarm",
      );
      const runId =
      ...
      resetObservabilityRuntime();
      setObservabilityTransport(transport);
      const actionRun2 = observe.openRun({
        name: "convex action 2",
        rootPrimitive: "runtime.convex.action",
      });
      ...
      const resumeRecord = transport.records.find(
      ...
      expect(
        resumeRecord && "segmentId" in resumeRecord
          ? resumeRecord.segmentId
          : undefined,
      ).not.toBe(
        swarmStart && "segmentId" in swarmStart
          ? swarmStart.segmentId
          : undefined,
      );
      expect(transport.records).toContainEqual(
        expect.objectContaining({ type: "run:end", runId, status: "ok" }),
  ...
  describe("no timer dependence", () => {
    it("completes a flush without advancing fake timers when the transport settles on a microtask", async () => {
      ...
      try {
        ...
        const span = observe.openSpan({
          name: "observability-runtime-notimer",
          primitive: "custom.operation",
        });
  ...
  describe("flow suspension across a Convex boundary uses the short bounded budget", () => {
    it("does not fall back to the removed 20-second default for direct flow handler flushes", async () => {
      ...
      await expect(reviewFlow.handler({} as never, {})).resolves.toMatchObject({
        status: "completed",
      });
