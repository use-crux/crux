...
import { convexRuntimeStore, type ConvexRuntimeComponent, type ConvexRuntimeStore } from '../store'
...
export function createConvexEvalHost(options: CreateConvexEvalHostOptions): ConvexEvalHostActions {
  ...
  const bind = (ctx: ConvexEvalActionCtx) => {
    ...
    const runtime: InProcessRuntimeEngineDefinition<ConvexRuntimeStore> = Object.freeze({
      kind: 'in-process',
      id: 'convex-eval',
      store,
    ...
    (definition, runtimeOptions) =>
      bindHostRuntime(definition, {
        ...
        createWake: () => async (envelope) => {
          await ctx.scheduler.runAfter(0, options.targetExecutor ?? actions.executeEvalTarget, { envelope })
        },
  ...
  actions = {
    ...
    executeEvalTarget: internalActionGeneric({
      ...
      handler: async (ctx, { envelope }) => {
        ...
        const host = bind(actionCtx)
        try {
          return await runWithRuntimeHost({ host: 'convex', bind: hostBinder(actionCtx) }, async () => {
            const result = await host.runtime.kernel.handleWake(decodeConvexWakeEnvelope(envelope))
