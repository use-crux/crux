...
`semantic-facts-v50` hard migration, which includes bounded media streaming,
...
Agent-tool relations, Session diagnostic classification, Flow Session targets
gated by canonical `flow` export identity, subscription lineage, and observed
public method usage.
...
- Built-in **Index Rules** are evaluated by the Rust `crates/lints` implementation. TypeScript retains internal descriptor/rule contracts, but arbitrary third-party rule execution is reserved and absent from the public extension authoring barrel.
