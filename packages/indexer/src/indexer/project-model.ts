 ...
 * source indexing, package metadata, and ignored conventions.
 *
 * @module
 */
...
export async function resolveProjectModel(
  options: ResolveProjectModelOptions,
): Promise<ResolvedProjectModel> {
  const root = resolve(options.root);
  const resolutionMode =
    options.resolutionMode ?? DEFAULT_PROJECT_MODEL_RESOLUTION_MODE;
  ...
  const configFiles = projectConfigFiles(
    root,
    options.configPath,
    resolutionMode,
    diagnostics,
  ...
  return {
    ...
    resolutionMode: field(
      resolutionMode,
      runtimeProvenance("project-model.resolutionMode"),
    ),
    ...(packageName ? { packageName } : {}),
    configFiles,
    sourceRoots: [
      field(root, filesystemProvenance(root, "project source root")),
    ],
    ignoredPaths: DEFAULT_IGNORED_PATHS.map((path) =>
      field(path, filesystemProvenance(root, "default ignored path")),
    ),
    definitions,
    relations,
  ...
  return configFiles.map((configFile, index): ProjectConfigFile => {
    ...
    return {
      ...
      status: field(
        status,
        status === "ignored"
          ? filesystemProvenance(configFile, "extra config ignored")
          : pathProvenance,
      ),
    };
  });
...
function projectModelDefinition(
  ...
  return {
    ...
    ...(definition.path
      ? { path: field([...definition.path], provenance) }
      : {}),
...
function projectModelRelation(
  ...
  const provenance = source
    ? sourceProvenance(source, exportName)
    : runtimeProvenance("project-index.relation");
...
function packageNameField(root: string): ProjectModelField<string> | undefined {
  ...
  return field(
    parsed.name,
    filesystemProvenance(packageJson, "package.json name"),
  );
}
...
function field<T>(
  value: T,
  provenance: ProjectModelProvenance,
): ProjectModelField<T> {
