...
registerHooks({
  resolve(specifier, context, nextResolve) {
    ...
    const session =
      importSessionStorage.getStore() ?? importSessionFrom(context.parentURL);
...
export async function withUserImportSession<T>(
  task: () => Promise<T>,
): Promise<T> {
...
function fingerprintResolution<T extends { readonly url: string }>(
  resolved: T,
  session: string,
): T {
