...
export function schemaPropertySourceRefs(input: {
  ...
  const schema =
    staticRecordSchemaProperty(
      input.object,
      input.property,
      input.initializerValues,
    ) ??
    (resolved
      ? staticSyntaxValueToJsonSchema(resolved.value, resolved.initializers)
      : undefined);
...
function nestedSchemaSourceRefs(input: {
  ...
  const visit = (value: StaticSyntaxValue): void => {
    if (value.kind === "identifier") {
      ...
      if (
        resolved &&
        !seen.has(resolved.symbol) &&
        schemaKind(resolved.value)
      ) {
  ...
  switch (value.kind) {
    ...
    case "object":
      return value.properties.flatMap((property) =>
        property.spread ? [] : [property.value],
      );
    case "call":
    ...
    case "tagged-template":
      return value.expressions.map((expression) => expression.value);
    default:
      return [];
  }
}
...
function containsRootNamespace(
  value: StaticSyntaxValue,
  name: "z" | "v",
): boolean {
  ...
  if (value.kind === "array")
    return value.elements.some((item) => containsRootNamespace(item, name));
  if (value.kind === "object") {
    return value.properties.some(
      (property) =>
        !property.spread && containsRootNamespace(property.value, name),
    );
  }
  return false;
}
