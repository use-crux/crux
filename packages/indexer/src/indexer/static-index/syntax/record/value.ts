...
export function staticStringValue(
  ...
  return resolved?.kind === "literal" && typeof resolved.value === "string"
    ? resolved.value
    : undefined;
}
...
export function staticNumberValue(
  ...
  const resolved = resolveStaticSyntaxValue(value, initializers);
  return resolved?.kind === "literal" && typeof resolved.value === "number"
    ? resolved.value
    : undefined;
}
...
export function staticBooleanValue(
  ...
  const resolved = resolveStaticSyntaxValue(value, initializers);
  return resolved?.kind === "literal" && typeof resolved.value === "boolean"
    ? resolved.value
    : undefined;
}
...
export function staticReferenceName(
  value: StaticSyntaxValue | undefined,
): string | undefined {
  if (!value) return undefined;
  if (value.kind === "identifier" || value.kind === "property-access")
    return value.name;
  ...
  switch (resolved.kind) {
    ...
    case "array":
      return resolved.elements.map((element) =>
        staticSyntaxValueJson(element, initializers),
      );
    case "object":
      return Object.fromEntries(
        resolved.properties
          ...
          .map((property) => [
            property.name,
            staticSyntaxValueJson(property.value, initializers),
          ]),
      );
    case "identifier":
...
function resolveStaticSyntaxValueWithSeen(
  ...
  seen.add(value.name);
  return resolveStaticSyntaxValueWithSeen(
    initializers.get(value.name) ?? value,
    initializers,
    seen,
  );
}
