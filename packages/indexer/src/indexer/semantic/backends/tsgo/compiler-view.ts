...
type TsgoSemanticCompilerSourceFile = TsgoSourceFile &
  SemanticCompilerSourceFile;
...
export function createTsgoCompilerView(
  ...
  return {
    ...
    resolvedSymbols(nodes) {
      return nodes.map((node) =>
        cached(resolvedSymbolCache, nativeNodeKey(node), () => {
          const symbol =
            cachedShorthandAssignmentValueSymbol(
              project,
              node,
              shorthandSymbolCache,
            ) ?? resolvedSymbol(project, node);
          if (isIdentifier(node) && isPropertyAccessName(node)) {
            return symbol && isUsablePropertyAccessSymbol(node, symbol)
              ? symbol
              : undefined;
          }
          return (
            (symbol ? sourceImportAlias(sourceLookup, symbol) : undefined) ??
            sourceIdentifierSymbol(sourceLookup, node) ??
            symbol
          );
        }),
      );
    ...
    typesAt(nodes) {
      return cachedNativeResults(nodes, typeCache, (nativeNodes) =>
    ...
    declarationsOf(symbols) {
      return symbols.map((symbol) =>
        isResolvedDeclarationSymbol(symbol)
          ? symbol.declarations
          : cached(declarationsCache, String(symbol.id), () =>
              sourceLookup.declarationsForSymbol(symbol),
            ),
      );
    },
    canonicalExportIdentity(node, moduleName, exportName) {
      return sourceLookup.isCanonicalExport(node, moduleName, exportName)
...
function resolvedSymbol(
  ...
  if (isIdentifier(node)) {
    return (
      project.checker.getResolvedSymbol(node as TsgoIdentifier) ??
...
function cachedShorthandAssignmentValueSymbol(
  ...
  return cached(cache, nativeNodeKey(node), () =>
    shorthandAssignmentValueSymbol(project, node),
  );
}
...
function sourceImportAlias(
  ...
  const declarations = symbol.declarations.flatMap((handle) => {
    const declaration = sourceLookup.declarationForHandle(handle);
    return declaration
      ? sourceLookup.importedDeclarations(
          declaration.getSourceFile().fileName,
          declaration.pos,
          declaration.end,
          formatSyntaxKind(declaration.kind),
        )
      : [];
  });
  return declarations.length > 0
    ? { kind: "resolved-declarations", name: symbol.name, declarations }
    : undefined;
}
...
function sourceIdentifierSymbol(
  ...
  const declarations = sourceLookup.sourceDeclarations(
    node.getSourceFile().fileName,
    node.text,
    node.pos,
  );
  return declarations.length > 0
    ? { kind: "resolved-declarations", name: node.text, declarations }
    : undefined;
}
...
function isPropertyAccessName(node: TsgoIdentifier): boolean {
  const parent = node.parent;
  return Boolean(
    parent && isPropertyAccessExpression(parent) && parent.name === node,
  );
}

function isUsablePropertyAccessSymbol(
  node: TsgoIdentifier,
  symbol: TsgoSymbol,
): boolean {
  if (isNamespaceImportPropertyAccessName(node)) return true;
  return symbol.declarations.some((declaration) =>
    isPropertyLikeDeclarationKind(formatSyntaxKind(declaration.kind)),
  );
}
...
function isResolvedDeclarationSymbol(
  symbol: TsgoSemanticSymbol,
): symbol is TsgoResolvedDeclarationSymbol {
