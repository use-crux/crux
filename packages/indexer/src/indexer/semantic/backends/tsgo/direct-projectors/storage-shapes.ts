...
export function storageDefinitionSource(
  expression: Expression,
): Expression | ObjectLiteralExpression | undefined {
  if (!isCallExpression(expression)) return undefined;
  const [firstArg] = nativeNodeList(expression.arguments);
  if (
    storageCallName(expression) === "storage" &&
    firstArg &&
    isObjectLiteralExpression(firstArg)
  )
    return firstArg;
  return expression;
}
...
export function storageBundleObject(
...
export function storagePropertyExpression(
  ...
  return isShorthandPropertyAssignment(property) && isIdentifier(property.name)
    ? property.name
    : undefined;
}
...
export function storageDependencies(
...
export function expressionIdentifier(
  expression: Expression,
): string | undefined {
