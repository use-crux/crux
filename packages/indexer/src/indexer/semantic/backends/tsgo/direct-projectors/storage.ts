...
export function isNativeDirectStorageInitializer(
  variable: NativeVariable,
): boolean {
  return Boolean(
    nativeStorageDefinition(variable) || nativeStorageBundleObject(variable),
  );
}
...
export function nativeDirectStorageEvidence(
  ...
  bindingsByFile: ReadonlyMap<
    SourceFile,
    ReadonlyMap<string, NativeSourceBinding>
  >,
  ...
  const definitionsByVariable = new Map(
    definitions.map((definition) => [definition.variable.name, definition]),
  );
  const facts = definitions.map((definition) =>
    storageDefinitionFact(definition, definitionsByVariable),
  );
  if (!facts.every((fact): fact is DefinitionFact => Boolean(fact)))
    return undefined;

  const relations = definitions.flatMap((definition) =>
    storageRelations(definition, definitionsByVariable),
  );
  const refGroups = definitions.map((definition) => {
    const bindings = bindingsByFile.get(definition.variable.file);
    return bindings
      ? storageSourceRefs(definition, definitionsByVariable, bindings)
      : undefined;
  });
  if (!refGroups.every((entry): entry is SourceRefFact[] => Boolean(entry)))
    return undefined;
...
function nativeStorageDefinition(
  ...
  if (descriptor) {
    return {
      ...
      descriptor,
      source:
        storageDefinitionSource(variable.initializer) ?? variable.initializer,
    };
  }
  return nativeStorageBundleObject(variable);
}

function nativeStorageBundleObject(
  variable: NativeVariable,
): NativeStorageDefinition | undefined {
  if (
    !variable.name.toLowerCase().includes("storage") ||
    !isObjectLiteralExpression(variable.initializer)
  )
    return undefined;
...
function storageDefinitionFact(
  ...
  const scope =
    definition.kind === "storage.scope"
      ? storageScopeReference(definition, definitionsByVariable)
      : undefined;
...
function storageRelations(
  ...
  if (definition.kind === "storage.bundle") {
    return (storageReferences(definition, definitionsByVariable) ?? []).flatMap(
      (ref) => {
...
function storageSourceRefs(
  ...
  const refs = [
    ...(storageReferences(definition, definitionsByVariable) ?? []),
  ];
  const scope =
    definition.kind === "storage.scope"
      ? storageScopeReference(definition, definitionsByVariable)
  ...
  return projected.every((ref): ref is SourceRefFact => Boolean(ref))
    ? projected
    : undefined;
}
...
function storageReferences(
  ...
  return object
    ? storageObjectReferences(object, definitionsByVariable)
    : undefined;
}
...
function storageScopeReference(
  ...
  const [storageExpression, prefixExpression] = nativeNodeList(
    definition.variable.initializer.arguments,
  );
  const variable = storageExpression
    ? expressionIdentifier(storageExpression)
    : undefined;
  const target = variable ? definitionsByVariable.get(variable) : undefined;
  if (
    !storageExpression ||
    !target ||
    !storagePropertyMatches("storage", target)
  )
    return undefined;
  return {
    property: "storage",
    ...
    target,
    ...(prefixExpression && isStringLiteral(prefixExpression)
      ? { prefix: prefixExpression.text }
      : {}),
  };
}
