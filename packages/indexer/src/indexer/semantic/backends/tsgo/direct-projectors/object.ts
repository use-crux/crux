...
export function nativeCruxCall(
  expression: Expression,
): NativeCruxCall | undefined {
  if (!isCallExpression(expression) || !isIdentifier(expression.expression))
    return undefined;
  const primitive = nativeDirectPrimitiveForCallName(
    expression.expression.text,
  );
  if (!primitive) return undefined;
  const args = nativeNodeList(expression.arguments);
  const object =
    primitive.callName === "fallback"
      ? nativeFallbackOptions(args)
      : primitive.callName === "retry"
        ? args[1]
        : args[0];
  return object && isObjectLiteralExpression(object)
    ? { primitive, object }
    : undefined;
}
...
export function propertyInitializer(
  ...
  const property = nativeNodeList(object.properties).find(
    (
      candidate,
    ): candidate is PropertyAssignment | ShorthandPropertyAssignment =>
      (isPropertyAssignment(candidate) ||
        isShorthandPropertyAssignment(candidate)) &&
...
export function stringProperty(
  ...
  return initializer && isStringLiteral(initializer)
    ? initializer.text
    : undefined;
}
...
function nativeFallbackOptions(
  ...
  const hasOptionsShape = Boolean(
    ...
    propertyInitializer(last, "when") ||
    propertyInitializer(last, "onFallback") ||
