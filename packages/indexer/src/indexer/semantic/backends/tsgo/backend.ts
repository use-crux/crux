...
import {
  emitNativeSemanticCoverage,
  measureSemanticTiming,
} from "../../instrumentation";
import {
  createTsgoNativeSemanticEngine,
  type TsgoNativeSemanticEngineInput,
} from "./engine";
        ...
        analyze(analyzeInput: SemanticAnalyzeInput): SemanticAnalyzeResult {
          return semanticIndexEvidenceBatchesCached(
            ...
            analyzeInput.files,
            {
              ...
              produceEvidence: ({ validationDependencies }) =>
                nativeSemanticEvidenceBatches(
                  { ...analyzeInput, validationDependencies },
                  engineForSession(),
                  (engine) =>
                    evictEngineForIdentity(engines, input.identity, engine),
                ),
            },
          );
        },
...
async function* nativeSemanticEvidenceBatches(
  analyzeInput: import("./types").NativeSemanticEngineAnalyzeInput,
  engine: NativeSemanticEngine,
  onUncacheable: (engine: NativeSemanticEngine) => void,
  ...
  try {
    ...
    if (!analyzeInput.validationDependencies.cacheable()) {
      onUncacheable(engine);
    }
  }
}

function evictEngineForIdentity(
  engines: Map<string, NativeSemanticEngine>,
  identity: SemanticProjectSessionIdentity,
  engine: NativeSemanticEngine,
): void {
  const key = nativeSessionCacheKey(identity);
  if (engines.get(key) !== engine) return;
  engine.close();
  engines.delete(key);
}
...
function nativeSessionCacheKey(
  identity: SemanticProjectSessionIdentity,
): string {
