...
export interface SemanticSourceFileFactInput<
  TView extends SemanticAnalyzerView = TypeScriptSemanticCompilerView,
> {
...
export function createTypeScriptSemanticFactInput(
  ...
  const checker = measureSemanticTiming(
    options.instrumentation,
  ...
  const view = createTypeScriptSemanticCompilerView({
    ...
    interceptedModuleNames:
      options.programSession?.interceptedModuleNames ?? new Set(),
