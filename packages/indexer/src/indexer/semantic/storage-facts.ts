...
type StorageReferenceProperty = 'storage' | 'records' | 'vectors' | 'blobs'
...
export function semanticStorageDefinitionEnrichments(
  ...
  if (isSemanticStorageDefinitionKind(candidate.kind)) {
    return [storageDefinitionEnrichment(candidate as SemanticDefinitionCandidate & { readonly kind: SemanticStorageDefinitionKind }, view)]
  }
  if (candidate.kind === 'rag.retriever' || candidate.kind === 'workspace') {
    const refs = storageReferencesForObject(candidateConfigObject(candidate, view), view)
...
export function semanticStorageRelationsForCandidate(
  ...
  if (candidate.kind === 'storage.bundle') return storageBundleRelations(candidate, view)
  if (candidate.kind === 'storage.scope') return storageScopeRelations(candidate, view)
  if (candidate.kind === 'rag.retriever' || candidate.kind === 'workspace') {
    return primitiveStorageRelations(
      candidate as SemanticDefinitionCandidate & { readonly kind: 'rag.retriever' | 'workspace' },
...
function storageDefinitionEnrichment(
  candidate: SemanticDefinitionCandidate & { readonly kind: SemanticStorageDefinitionKind },
  view: SemanticAnalyzerView,
): SemanticDefinitionEnrichment {
  const descriptor = semanticStorageFactoryDescriptor(candidate.call ? callExpressionName(candidate.call, view) : undefined)
  const refs = candidate.kind === 'storage.bundle'
    ? storageReferencesForObject(candidateConfigObject(candidate, view) ?? candidate.object, view)
    : []
  const scope = candidate.kind === 'storage.scope' ? storageScopeReference(candidate, view) : undefined
  ...
  const metadata: ProjectDefinitionMetadata = compactRecord({
    ...
    blobsVariable: storageReferenceVariable(refs, 'blobs', view),
    baseStorageVariable: scope ? expressionVariable(scope.expression, view) : undefined,
    prefix: scope?.prefix,
    facts,
    intelligence,
  })
  ...
  return {
    ...
    sourceRefs: storageSourceRefs(candidate.definitionId, [...refs, ...(scope ? [scope] : [])], view),
  }
}
...
function storageBundleRelations(candidate: SemanticDefinitionCandidate, view: SemanticAnalyzerView): ProjectRelation[] {
  return storageReferencesForObject(candidateConfigObject(candidate, view) ?? candidate.object, view).flatMap((ref) => {
    const type = bundleRelationType(ref.property)
    return type ? [semanticRelation(candidate, type, candidate.definitionId, ref.target.id, view)] : []
  })
}

function storageScopeRelations(candidate: SemanticDefinitionCandidate, view: SemanticAnalyzerView): ProjectRelation[] {
  const scope = storageScopeReference(candidate, view)
  return scope ? [semanticRelation(candidate, 'storage.scope.wraps_storage', candidate.definitionId, scope.target.id, view)] : []
}

function primitiveStorageRelations(
  candidate: SemanticDefinitionCandidate & { readonly kind: 'rag.retriever' | 'workspace' },
  refs: readonly StorageReference[],
  view: SemanticAnalyzerView,
): ProjectRelation[] {
...
function candidateConfigObject(
  ...
  if (view.syntax.isKind(candidate.object, 'objectLiteral')) return candidate.object
  const [firstArg] = candidate.call ? view.syntax.callArguments(candidate.call) : []
  return firstArg ? semanticObjectExpression(firstArg, view, new Set()) : undefined
}
...
function storageReferencesForObject(
  ...
  return (['storage', 'records', 'vectors', 'blobs'] as const).flatMap((property) => {
    ...
    return target && storagePropertyMatchesTarget(property, target) ? [{ property, expression, target }] : []
  })
}
...
function storageScopeReference(
  ...
  const [storageExpression, prefixExpression] = view.syntax.callArguments(candidate.call)
  const target = storageExpression ? semanticTargetForExpression(storageExpression, view) : undefined
  if (!target || !storagePropertyMatchesTarget('storage', target)) return undefined
  return {
    property: 'storage',
    expression: storageExpression,
    target,
    ...(prefixExpression ? { prefix: view.syntax.stringLiteralText(unwrapExpression(prefixExpression, view)) } : {}),
  }
}

function storagePropertyMatchesTarget(property: StorageReferenceProperty, target: SemanticTarget): boolean {
  switch (property) {
    case 'storage':
...
function storageDependencies(refs: readonly StorageReference[]): DependencyFacts | undefined {
...
function storageSourceRefs(
  ...
  return refs.flatMap((ref) => {
    const sourceRef = semanticResolvedSourceRef(definitionId, ref.property, 'config', ref.expression, view, {
...
function bundleRelationType(property: StorageReferenceProperty): ProjectRelation['type'] | undefined {
  switch (property) {
    case 'records':
    ...
    case 'blobs':
      return 'storage.bundle.uses_blob_store'
    default:
      return undefined
  }
}
...
function primitiveRelationType(
  ...
  return `${ownerKind}.uses_${property === 'records' ? 'record_store' : property === 'vectors' ? 'vector_store' : property === 'blobs' ? 'blob_store' : 'storage'}`
}

function storageReferenceTarget(refs: readonly StorageReference[], property: StorageReferenceProperty): SemanticTarget | undefined {
...
function expressionVariable(expression: SemanticAnalyzerNode<SemanticAnalyzerView>, view: SemanticAnalyzerView): string | undefined {
