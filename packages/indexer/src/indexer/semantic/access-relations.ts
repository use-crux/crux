      ...
      if (target && view.syntax.propertyAccessName(target) === "step") {
        ...
        if (stepName && targetArg) {
          ...
          for (const access of semanticAccessesForExpression(targetArg, view)) {
            relations.push(
              ...semanticAccessRelation("flow.step", from, access, view),
            );
          }
        }
      }
...
function semanticAccessNodeForResolved(
  ...
  if (view.syntax.isFunctionLike(resolved.declaration))
  ...
  const visit = (child: SemanticAnalyzerNode<SemanticAnalyzerView>): void => {
    if (view.syntax.isKind(child, "callExpression")) {
      ...
      if (
        helperDepth > 0 &&
        target &&
        view.syntax.isKind(target, "identifier")
      ) {
        const resolved = resolveSemanticExpression(target, view);
        const helperNode = resolved
          ? semanticAccessNodeForResolved(resolved, view)
          : undefined;
...
function semanticAccessesForCall(
  ...
  if (!view.syntax.isKind(expression, "propertyAccessExpression")) {
    ...
    if (target?.kind === "scorer")
      return [{ kind: "score", target, node: call }];
...
function semanticInvocationKind(
  ...
  if (
    targetKind === "scorer" &&
    ["score", "judge", "run", "evaluate", "call"].includes(method)
  )
    return "score";
  if (
    isEvalKind(targetKind) &&
    ["run", "evaluate", "execute", "call"].includes(method)
  )
    return "eval";
  return undefined;
}
...
function semanticDataAccessKind(method: string): "read" | "write" | undefined {
  return dataAccessKindForMethod(method);
}
...
function semanticAccessRelationType(
  ...
  if (accessKind === "query" && targetKind === "rag.retriever")
    return `${fromKind}.queries_retriever`;
  if (accessKind === "score" && targetKind === "scorer")
    return `${fromKind}.uses_scorer`;
  if (accessKind === "eval" && isEvalKind(targetKind))
    return `${fromKind}.runs_eval`;
