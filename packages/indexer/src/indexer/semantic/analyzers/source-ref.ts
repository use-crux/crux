  ...
  return {
    ...
    analyze(candidate, context) {
      return {
        sourceRefs: [
          ...deps
            .sourceRefCandidates(candidate, context.view)
            .flatMap((refCandidate) => {
              const resolved = deps.resolveExpression(
                refCandidate.expression,
                context.view,
              );
              return resolved
                ? [
                    {
                      definitionId: refCandidate.definitionId,
                      ref: deps.sourceRef(refCandidate, resolved, context.view),
                    },
                  ]
                : [];
            }),
          ...deps
            .templateInterpolationSourceRefs(candidate, context.view)
            .map((ref) => ({
          ...
          ...deps
            .injectionConditionSourceRefs(candidate, context.view)
            .map((ref) => ({
              definitionId: candidate.definitionId,
              ref,
            })),
          ...deps
            .promptTextSourceRefs(context.root, candidate, context.view)
            .map((ref) => ({
              definitionId: candidate.definitionId,
              ref,
            })),
