...
export interface SemanticSyntaxNode<
  TKind extends SemanticSyntaxKind = SemanticSyntaxKind,
> {
 ...
 */
export interface SemanticSyntaxSourceFile<
...
export interface SemanticSyntaxView<
  TNode extends SemanticSyntaxNode = SemanticSyntaxNode,
  TSourceFile extends TNode & SemanticSyntaxSourceFile<TNode> = TNode &
...
> {
  ...
  isKind<TKind extends SemanticSyntaxKind>(
    node: TNode,
    kind: TKind,
  ): node is SemanticSyntaxNodeOf<TNode, TKind>;
  ...
  logicalAndOperands(
    node: TNode,
  ): { readonly left: TNode; readonly right: TNode } | undefined;
  ...
  /** Return every return expression owned by one prompt-text callback. */
  promptTextReturnExpressions(node: TNode): readonly TNode[];
  /** Return the tag expression from a tagged template. */
  taggedTemplateTag(node: TNode): TNode | undefined;
  /** Return the template body from a tagged template. */
  taggedTemplateBody(node: TNode): TNode | undefined;
  /** Return the result branches from a conditional expression. */
  conditionalBranches(
    node: TNode,
  ): { readonly whenTrue: TNode; readonly whenFalse: TNode } | undefined;
  ...
  /** Return the declaration-list mutability for a variable declaration. */
  variableDeclarationKind(node: TNode): "const" | "let" | "var" | undefined;
  ...
  parameterTypeReference(
    ...
    | { readonly name: string; readonly arguments: readonly TNode[] }
    | undefined;
