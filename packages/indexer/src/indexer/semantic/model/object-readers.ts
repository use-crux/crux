...
export function semanticObjectExpression(
  ...
  return semanticObjectExpression(
    resolved.expression,
    view,
    new Set([...seen, key]),
  );
...
export function objectProperty(
  ...
  return expression && view.syntax.isKind(expression, "objectLiteral")
    ? expression
    : undefined;
}
...
export function semanticObjectProperty(
  ...
  const property = propertyInitializer(object, name, view);
  return property
    ? semanticObjectExpression(toExpression(property, view), view, new Set())
    : undefined;
}
...
export function arrayProperty(
  ...
  const property = propertyInitializer(object, name, view);
  const expression = property ? toExpression(property, view) : undefined;
  return expression && view.syntax.isKind(expression, "arrayLiteral")
    ? expression
    : undefined;
}
...
export function semanticArrayProperty(
  ...
  const property = propertyInitializer(object, name, view);
  return property
    ? semanticArrayExpression(toExpression(property, view), view, new Set())
    : undefined;
}
...
export function semanticArrayExpression(
  ...
  const key = semanticResolvedKey(resolved);
  if (seen.has(key)) return undefined;
  return semanticArrayExpression(
    resolved.expression,
    view,
    new Set([...seen, key]),
  );
...
export function semanticFallbackOptions(
  ...
  const hasOptionsShape = Boolean(
    ...
    propertyInitializer(last, "when", view) ||
    propertyInitializer(last, "onFallback", view) ||
...
/** Returns the model expressions from `fallback([a, b], opts?)` or legacy positional calls. */
export function semanticFallbackModelExpressions(
  call: SemanticAnalyzerNode<SemanticAnalyzerView>,
  view: SemanticAnalyzerView,
): SemanticAnalyzerNode<SemanticAnalyzerView>[] {
  const options = semanticFallbackOptions(call, view);
  const modelArgs = view.syntax
    .callArguments(call)
    .filter((argument) => argument !== options);
  const [first] = modelArgs;
  if (first) {
    const array = semanticArrayExpression(
      toExpression(first, view),
      view,
      new Set(),
    );
    if (array)
      return view.syntax
        .arrayElements(array)
        .filter((element) => !view.syntax.spreadExpression(element));
  }
  return modelArgs;
}
