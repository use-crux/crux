  ...
  if (facts.liveFields && facts.liveFields.length > 0) {
    findings.push(
      finding(
        ...
        "Remove Request, client, credential, socket, callback, handle, poll, open, and onEvent fields from managedTransportBinding() options. Keep only id, configRef, signalId, and optional adapter/provider identities.",
      ),
    );
  }
