/**
 * Static Project Index facts for authored `stream()` transports.
 *
 * @module
 */
...
/** Projects one authored managed stream transport declaration. */
export function extractStreamStaticFacts(ctx: ExtractContext) {
  ...
  if (
    native.match.callee.importedName !== "stream" &&
    native.match.callee.name !== "stream"
  ) {
    return none();
  }
