/**
 * Static Project Index facts for authored `polling()` transports.
 *
 * @module
 */
...
export function extractPollingStaticFacts(ctx: ExtractContext) {
  ...
  if (
    native.match.callee.importedName !== "polling" &&
    native.match.callee.name !== "polling"
  ) {
    return none();
  }
  ...
  const hasPoll = Boolean(
    options?.properties.some(
      (property) => !property.spread && property.name === "poll",
    ),
  );
