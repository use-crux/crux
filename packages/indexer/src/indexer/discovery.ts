...
export async function discoverProjectDefinitions(
  ...
  if (loaded.sourceImports) {
    ...
    const resolvedRich = await discoverResolvedDefinitionsFromStaticCandidates(
      root,
      sources,
      staticFiles,
      extraction,
  ...
  const staticResult = await discoverStaticDefinitions(
    ...
    loaded,
    {
      definitions: initialFacts.definitions,
      relations: initialFacts.relations,
    },
    failedImportFiles,
    sources,
    extraction,
...
function mergeSourceGraph(target: SourceGraph, incoming: SourceGraph): void {
  for (const [file, dependencies] of incoming.dependenciesByFile) {
    target.dependenciesByFile.set(
      ...
      [
        ...new Set([
          ...(target.dependenciesByFile.get(file) ?? []),
          ...dependencies,
        ]),
      ].sort(),
    );
