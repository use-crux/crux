...
export function canonicalFactExtractors(
  ...
  for (const extractor of extractors) {
    ...
    const key = [extractor.extension?.name ?? '', extractor.extension?.version ?? '', extractor.name].join('\0')
...
export function factExtractorLabel(extractor: ProjectIndexFactExtractorProvenance): string {
...
function compareFactExtractors(
  ...
  return (
    ...
    compareUtf8(left.extension?.version ?? '', right.extension?.version ?? '') ||
    compareUtf8(left.name, right.name)
  )
}

function assertIdentityPart(value: string, label: string): void {
  if (value.length === 0 || hasControlCharacter(value) || !hasWellFormedUnicode(value)) {
