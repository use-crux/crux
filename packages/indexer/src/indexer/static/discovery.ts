...
export async function discoverResolvedDefinitionsFromStaticCandidates(
  ...
  const semanticProfileByFile = new Map<
    string,
    NonNullable<StaticFileExtraction["semanticProfile"]>
  >();
  const interfaceHashByFile = new Map<string, string>();

  for (const batch of staticDiscoveryBatches(
    staticFiles,
    options.staticFileBatches,
  )) {
    for (const file of batch.files) {
      ...
      if (parsed.semanticProfile)
        semanticProfileByFile.set(file, parsed.semanticProfile);
      if (parsed.interfaceHash)
        interfaceHashByFile.set(file, parsed.interfaceHash);
      ...
      for (const definitionItem of parsed.definitions) {
        ...
        const exportName =
          typeof definitionItem.metadata?.exportName === "string"
            ? definitionItem.metadata.exportName
            : undefined;
      ...
      await withCruxIndexMode(async () => {
        try {
          ...
          for (const [exportName, value] of Object.entries(mod)) {
            ...
            const resolved = await resolvedDefinitionFromExport(
              root,
              file,
              exportName,
              value,
          ...
          diagnostics.push(
            richImportFailedDiagnostic(root, file, errorMessage(error)),
          );
        }
      });
    }
...
export async function discoverStaticDefinitions(
  ...
  knownDefinitionIds = new Set(
    indexFacts.definitions.map((definition) => definition.id),
  ),
  staticFiles: readonly string[] = [],
  options: {
    readonly staticFileBatches?: readonly ProjectShardFileBatch[];
  ...
  for (const batch of staticDiscoveryBatches(
    ...
    parsedFiles.push(
      ...(await extractStaticDiscoveryBatch(extraction, batch.files)),
    );
  }
  ...
  for (const result of parsedFiles) {
    ...
    if ("error" in result) {
      nextSources = sourceStatus(nextSources, file, "error");
      diagnostics.push(
        staticParseFailedDiagnostic(root, file, errorMessage(result.error)),
      );
      continue;
    }
    ...
    if (parsed.definitions.length === 0 && parsed.relations.length === 0)
      continue;

    const isFallback =
      failedImportFiles.includes(file) ||
      Boolean(loaded.configFile === file && loaded.importFailed);
    nextSources = sourceStatus(
      nextSources,
      file,
      isFallback ? "partial" : "indexed",
    );
...
> {
  try {
    return (
      await extraction.extractFiles(files, {
        concurrency: STATIC_DISCOVERY_EXTRACTION_CONCURRENCY,
      })
    ).map((parsed) => ({
      file: parsed.file,
      parsed,
    ...
    return mapBounded(
      files,
      STATIC_DISCOVERY_EXTRACTION_CONCURRENCY,
      async (file) => {
        try {
...
function staticDiscoveryBatches(
  ...
  if (!batches || batches.length === 0)
    return [{ shard: { id: ".", root: "" }, files: sortedFiles }];
  ...
  return [
    ...
    ...(fallbackFiles.length > 0
      ? [{ shard: { id: "__unassigned__", root: "" }, files: fallbackFiles }]
      : []),
  ];
}
