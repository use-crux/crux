...
async function fixtureRoot(): Promise<string> {
  const root = join(
    process.cwd(),
    `.tmp-semantic-cache-${Date.now()}-${Math.random().toString(16).slice(2)}`,
  );
  roots.push(root);
...
describe("semantic facts cache", () => {
  it("uses the combined media-stream and PromptText fragment-join epoch", () => {
    expect(SEMANTIC_FACTS_CACHE_EPOCH).toBe("semantic-facts-v36");
  });

  it("does not reuse a valid artifact from the pre-fragment-join v35 namespace", async () => {
    ...
    const staleDir = join(root, ".crux/cache/index", "semantic-facts-v35");
  ...
  it("ignores JSON semantic fact caches after the hard binary cache migration", async () => {
    ...
    await writeFile(
      join(cacheDir, binaryName.replace(/\.bin$/, ".json")),
  ...
  it("uses order-insensitive source profile cache identity", async () => {
    ...
    const cached = await semanticIndexFactsCached(root, [right, left], {
      ...
      async *produceEvidence() {
        throw new Error(
          "producer should not run for equivalent semantic source profile order",
        );
      },
    });
  ...
  it("does not reuse semantic facts from incomplete source profiles round-tripped through worker events", async () => {
    ...
    const backendIdentity: SemanticBackendIdentity = {
      name: "test-cache-incomplete-profile",
      version: "v1",
    };
    const facts = cachedFacts();
    let producerCalls = 0;
    ...
    expect(timingNames).toEqual(
      expect.arrayContaining(["semantic.cache.unkeyed"]),
    );
  ...
  it("normalizes relative and absolute source profile files", async () => {
    ...
    const cached = await semanticIndexFactsCached(root, [file], {
      ...
      async *produceEvidence() {
        throw new Error(
          "producer should not run for equivalent semantic source profile paths",
        );
      },
    });
...
async function cacheFileNames(root: string): Promise<readonly string[]> {
  return (
    await readdir(join(root, ".crux/cache/index", SEMANTIC_FACTS_CACHE_EPOCH))
  ).sort();
}

function semanticSourceProfileFixture(
  root: string,
