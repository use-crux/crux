  ...
  it("resolves embedding consumers, callsites, and conclusive invalid combinations", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `export const writer = indexer({ id: 'writer', namespace: 'shared', records, storage: search, dense: text })`,
        `export const sparseWriter = indexer({ id: 'sparse-writer', namespace: 'sparse', records, storage: search, sparse })`,
        `export const reader = retriever({ id: 'reader', indexerId: 'writer', namespace: 'shared', records, storage: search, dense: vision })`,
        `export const kb = knowledgeBase({ id: 'kb', records, storage: search, embeddings: vision, sparseEmbeddings: sparse })`,
  ...
  it("omits findings for dynamic evidence and permits hybrid media queries", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const search = retriever({ id: 'search', namespace: 'shared', records, storage: searchStore, dense, sparse, mode: 'hybrid' })`,
  ...
  it("does not treat nested document metadata as media evidence", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const writer = indexer({ id: 'writer', namespace: 'shared', records, storage: search, sparse })`,
  ...
  it("retains byte-safe partial evidence for dynamic embedding references", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const writer = indexer({ id: 'writer', namespace: 'shared', records, storage: search, dense: dynamicEmbedding })`,
  ...
  it("checks sparse per-request overrides on hybrid retrievers", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `void search.retrieve(image, { mode: 'sparse' })`,
        `void search.retrieve({ input: image, mode: 'sparse' })`,
      ].join("\n"),
    );
  ...
  it("resolves imported and local aliases without widening same-name calls", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const writer = defineIndexer({ id: 'writer', namespace: 'shared', records, storage: search, dense })`,
  ...
  it("compares exact provider identities when defaults are statically proven", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `export const writer = indexer({ id: 'writer', namespace: 'shared', records, storage: search, dense: small })`,
        `export const reader = retriever({ id: 'reader', namespace: 'shared', records, storage: search, dense: large })`,
      ].join("\n"),
    );
  ...
  it("compares exact sparse identities for a shared namespace", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `export const writer = indexer({ id: 'writer', namespace: 'shared', records, storage: search, sparse: first })`,
        `export const reader = retriever({ id: 'reader', namespace: 'shared', records, storage: search, sparse: second, mode: 'sparse' })`,
      ].join("\n"),
    );
  ...
  it("projects knowledge-base defaults and checks its indexing and retrieval calls", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const sparse = embedding({ kind: 'sparse', name: 'sparse', maxInputTokens: 32, batch: { maxSize: 1 }, embed: async () => [] })`,
        `export const kb = knowledgeBase({ id: 'docs', records, storage: search, embeddings: text, sparseEmbeddings: sparse })`,
        `const documents = [{ id: 'dog', parts: [{ type: 'image', asset: { type: 'data', mediaType: 'image/png', data: bytes } }] }]`,
        `export async function run() {`,
        `  await kb.index(documents)`,
        `  await kb.retriever({ mode: 'sparse' }).retrieve({ type: 'data', mediaType: 'image/png', data: bytes } as never)`,
        `}`,
      ].join("\n"),
    );
