  ...
  it("resolves embedding consumers, callsites, and conclusive invalid combinations", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `export const writer = indexer({ id: 'writer', namespace: 'shared', records, vectors, dense: text })`,
        `export const sparseWriter = indexer({ id: 'sparse-writer', namespace: 'sparse', records, vectors, sparse })`,
        `export const reader = retriever({ id: 'reader', indexerId: 'writer', namespace: 'shared', records, vectors, dense: vision })`,
  ...
  it("omits findings for dynamic evidence and permits hybrid media queries", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const search = retriever({ id: 'search', namespace: 'shared', records, vectors, dense, sparse, mode: 'hybrid' })`,
  ...
  it("does not treat nested document metadata as media evidence", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const writer = indexer({ id: 'writer', namespace: 'shared', records, vectors, sparse })`,
  ...
  it("retains byte-safe partial evidence for dynamic embedding references", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const writer = indexer({ id: 'writer', namespace: 'shared', records, vectors, dense: dynamicEmbedding })`,
  ...
  it("checks sparse per-request overrides on hybrid retrievers", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const sparse = embedding({ kind: 'sparse', name: 'sparse', maxInputTokens: 32, batch: { maxSize: 1 }, embed: async () => [] })`,
        `const search = retriever({ id: 'search', namespace: 'shared', records, vectors, dense, sparse, search: { mode: 'hybrid' } })`,
  ...
  it("resolves imported and local aliases without widening same-name calls", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const writer = defineIndexer({ id: 'writer', namespace: 'shared', records, vectors, dense })`,
  ...
  it("compares exact provider identities when defaults are statically proven", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `export const writer = indexer({ id: 'writer', namespace: 'shared', records, vectors, dense: small })`,
        `export const reader = retriever({ id: 'reader', namespace: 'shared', records, vectors, dense: large })`,
      ].join("\n"),
    );
  ...
  it("compares exact sparse identities for a shared namespace", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `export const writer = indexer({ id: 'writer', namespace: 'shared', records, vectors, sparse: first })`,
        `export const reader = retriever({ id: 'reader', namespace: 'shared', records, vectors, sparse: second, mode: 'sparse' })`,
      ].join("\n"),
    );
  ...
  it("projects knowledge-base defaults and checks its indexing and retrieval calls", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        `const sparse = embedding({ kind: 'sparse', name: 'sparse', maxInputTokens: 32, batch: { maxSize: 1 }, embed: async () => [] })`,
        `export const kb = knowledgeBase({ id: 'docs', records, vectors, embeddings: text, sparseEmbeddings: sparse })`,
