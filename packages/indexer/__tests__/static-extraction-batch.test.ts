...
describe('static extraction batch frontend', () => {
  ...
  it('falls back to the frontend for imported records outside the batch', async () => {
    ...
    const sources = {
      [files[0]]: "import { helper } from './helper'\nexport const a = helper()",
      [files[1]]: "export const b = 'b'",
  ...
  it('uses a provided record batch reader for cache-enabled multi-file extraction misses', async () => {
    ...
    const extraction = createStaticExtraction({
      ...
      sources: memorySourceReader(sources),
      syntaxFrontend: createProvidedStaticSyntaxFrontend({ recordProvider: provider }),
    })
  ...
  it('uses validated cache hits without reading provided syntax records', async () => {
    ...
    const extraction = createStaticExtraction({
      root,
      cache: {
        get: async (key) => cachedExtraction(files.find((file) => cacheKeys.get(file) === key) ?? '/missing'),
        set: async () => undefined,
      },
      cacheHits: files.map((file) => ({ file, cacheKey: cacheKeys.get(file) ?? '' })),
      sources: memorySourceReader(sources),
  ...
  it('derives cache metadata from syntax-record imports without TypeScript AST input', async () => {
    ...
    expect(key?.sourceHash).toBe(createHash('sha256').update(sources[file] ?? '').digest('hex'))
    expect(key?.dependencies).toEqual([
      {
        file: 'src/helper.ts',
        sourceHash: createHash('sha256').update(sources[dependency] ?? '').digest('hex'),
      },
    ])
  })
    ...
    const loaded = await Promise.all([
      cache.readImportedDefinition({
        ...
        load: async () => {
          ...
          return { id: 'prompt:shared', kind: 'prompt', name: 'sharedPrompt', fidelity: 'resolved' }
        },
      }),
      cache.readImportedDefinition({
        file: '/fixture/src/shared.ts',
        ...
        load: async () => {
          loads += 1
          return { id: 'prompt:other', kind: 'prompt', name: 'other', fidelity: 'resolved' }
        },
      }),
    ])

    expect(loaded.map((definition) => definition?.id)).toEqual(['prompt:shared', 'prompt:shared'])
    expect(loads).toBe(1)
