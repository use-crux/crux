...
type RootAstHostOnlyKeys = 'nativeFactProjection' | 'providedRecordCacheSize' | 'staticCacheHits' | 'staticSyntaxFrontend'

type PublicRecordsDoNotExposeHostOnlyKeys = Expect<
  Equal<Extract<keyof IndexProjectAstFromSyntaxRecordsOptions, RootAstHostOnlyKeys>, never>
>

type PublicProviderDoNotExposeHostOnlyKeys = Expect<
  Equal<Extract<keyof IndexProjectAstFromSyntaxRecordProviderOptions, RootAstHostOnlyKeys>, never>
>

type HostRecordsExposeHostOnlyKeys = Expect<
  Equal<
    Extract<keyof IndexProjectAstFromSyntaxRecordsHostOptions, RootAstHostOnlyKeys>,
...
type HostProviderExposeHostOnlyKeys = Expect<
  Equal<
    Extract<keyof IndexProjectAstFromSyntaxRecordProviderHostOptions, RootAstHostOnlyKeys>,
    'nativeFactProjection' | 'providedRecordCacheSize' | 'staticCacheHits'
  >
>
...
type ExtractContextKeysAreFrozen = Expect<Equal<keyof ExtractContext, keyof FrozenExtractContext>>
type ExtractContextShapeIsFrozen = Expect<
  Equal<
    ExtractContext extends FrozenExtractContext ? (FrozenExtractContext extends ExtractContext ? true : false) : false,
    true
  >
>

type PublicIndexerExtensionKeysAreFrozen = Expect<
  Equal<keyof IndexerExtension, 'name' | 'version' | 'crux' | 'relations' | 'rules' | 'extractors'>
>
