...
async function fixtureRoot(): Promise<string> {
  const root = await mkdtemp(join(process.cwd(), '.tmp-semantic-native-direct-'))
  roots.push(root)
  return root
}
  ...
  it('matches TypeScript facts for routing primitives', async () => {
    ...
    await writeFile(
      ...
      `
        ...
        export const resilientWriter = fallback(writerAgent, writerPrompt, {
          id: 'resilient-writer',
          shouldFallback,
          onAttemptError,
        })
        export const qualityCascade = cascade({
          id: 'quality-routing',
          tiers: [
            { model: writerAgent, evaluate: accepted },
