...
const negativeFixtureCases: readonly NegativeFixtureCase[] = [
  {
    ...
    callNames: ['knowledgeBase', 'retriever', 'retrievalRecipe', 'retrieve', 'rerank'],
  },
  ...
  { family: 'agent', callNames: ['agent', 'convexAgent'], constructorNames: ['Agent'] },
  { family: 'composition', callNames: ['parallel', 'pipeline', 'consensus', 'swarm'] },
  { family: 'memory', callNames: ['memory', 'createMemoryId', 'workingState', 'episodes', 'durableStore'] },
  { family: 'blackboard', callNames: ['blackboard', 'createMemoryId'] },
  { family: 'routing', callNames: ['router', 'cascade', 'fallback'] },
    ...
    async () => {
      for (const testCase of negativeFixtureCases) {
        const { fallbackOut, nativeOut, record } = await extractNativeAndFallback({
          ...
          ...(testCase.constructorNames ? { constructorNames: testCase.constructorNames } : {}),
        })

        expect(nativeFactCount(record, testCase.family), `${testCase.family} native facts`).toBe(0)
...
function lookalikeSource(testCase: NegativeFixtureCase): string {
  const functionNames = testCase.callNames.map((name) => `const ${safeIdentifier(name)} = '${name}'`)
  const constructorNames = (testCase.constructorNames ?? []).map((name) => `class ${safeIdentifier(name)} {}`)
  const properties = [...testCase.callNames, ...(testCase.constructorNames ?? [])]
    .map((name) => `${safeIdentifier(name)}: '${name}'`)
    .join(', ')
  return [...functionNames, ...constructorNames, `export const lookalikes = { ${properties} }`].join('\n')
}
