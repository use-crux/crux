...
async function fixtureRoot(): Promise<string> {
  const root = await mkdtemp(
    join(process.cwd(), ".tmp-semantic-native-service-"),
  );
  roots.push(root);
  return root;
}
...
describe("native semantic index service", () => {
  it("reports tsgo and the configured executable as its compiler runtime identity", async () => {
    ...
    const backend = createNativeSemanticBackend({
    ...
    await expect(
      Promise.resolve(
        backend.compilerRuntimeIdentity?.({ root, backend: backend.identity }),
      ),
    ).resolves.toEqual({
      name: "tsgo",
  ...
  it("reuses the experimental native engine host for the same semantic project identity", async () => {
    ...
    const semanticInstrumentation = {
      onTiming: (timing: { readonly name: string }) =>
        timingNames.push(timing.name),
    };
    ...
    expect(
      timingNames.filter(
        (name) =>
          name === "semantic.native.host.create" ||
          name === "semantic.native.host.reuse",
      ),
  ...
  it("matches TypeScript facts while mixing native direct Crux projection with the shared analyzer", async () => {
    ...
    expect(coverageExtractors).toEqual([
      ["crux.direct-crux", "crux.shared-analyzer"],
    ]);
  }, 20_000);
