...
describe('static extraction engine', () => {
  ...
  it('projects extension facts from caller-provided syntax records', async () => {
    ...
    const record = await createTypeScriptStaticSyntaxFrontend({ callNames: ['defineWorkflow'] }).parseFile({
      root,
      file,
      source,
    })
  ...
  it('uses native facts from syntax records before invoking matching TypeScript extractors', async () => {
    ...
    const baseRecord = await createTypeScriptStaticSyntaxFrontend({ callNames: ['nativePrompt'] }).parseFile({
      root,
      file,
      source,
    })
    ...
    const record: StaticSyntaxFileRecord = {
      ...
      nativeFacts: [
        {
          matchIndex: 0,
          replaces: [{ extension: '@acme/native-prompts', extractor: 'native.prompt' }],
          facts: nativeFacts,
        },
      ],
    }
  ...
  it('runs third-party TS extractors for matches covered by first-party native facts', async () => {
    ...
    const baseRecord = await createTypeScriptStaticSyntaxFrontend({ callNames: ['prompt'] }).parseFile({ root, file, source })
    const match = baseRecord.matches[0]
    if (!match) throw new Error('Expected fixture match')
    const record: StaticSyntaxFileRecord = {
      ...baseRecord,
    ...
    expect(extracted.definitions.map((definition) => definition.id)).toContain('prompt:writer')
  ...
  it('projects the TypeScript extractor lane without emitting external native facts', async () => {
    ...
    expect(extracted.definitions.map((definition) => definition.id)).not.toContain('prompt:writer')
    expect(extracted.diagnostics).toEqual([
      expect.objectContaining({
        code: 'prompt-audit',
        message: 'Prompt audit extension inspected writer',
  ...
  itWithRustOxc('emits exact native routing facts from the Rust/Oxc frontend', async () => {
    ...
    const rustRecord = await createRustOxcStaticSyntaxFrontend({ callNames: ['router', 'cascade', 'fallback'] }).parseFile({
      root,
      file,
      source,
    })
    ...
    const nativeExtraction = createStaticExtraction({
      ...
      cache: 'none',
      sources: memorySourceReader({ [file]: source }),
      syntaxFrontend: createProvidedStaticSyntaxFrontend({ records: [rustRecord] }),
    })
    const fallbackExtraction = createStaticExtraction({
      root,
      cache: 'none',
      sources: memorySourceReader({ [file]: source }),
      syntaxFrontend: createProvidedStaticSyntaxFrontend({ records: [{ ...rustRecord, nativeFacts: [] }] }),
    })
  ...
  itWithRustOxc('prunes native routing match evidence without changing extracted output', async () => {
    ...
    const fullRecord = await createRustOxcStaticSyntaxFrontend({ callNames }).parseFile({ root, file, source })
    ...
    const nativeExtraction = createStaticExtraction({
      ...
      cache: 'none',
      sources: memorySourceReader({ [file]: source }),
      syntaxFrontend: createProvidedStaticSyntaxFrontend({ records: [prunedRecord] }),
    })
    const fallbackExtraction = createStaticExtraction({
      root,
      cache: 'none',
      sources: memorySourceReader({ [file]: source }),
      syntaxFrontend: createProvidedStaticSyntaxFrontend({ records: [{ ...fullRecord, nativeFacts: [] }] }),
    })
  ...
  it('keeps routing evidence when a third-party extractor matches the same call', async () => {
    ...
    const file = '/fixture/src/routing.ts'
    const source = routingFixtureSource()
    const seenOptions: Parameters<typeof createTypeScriptStaticSyntaxFrontend>[0][] = []
    const extraction = createStaticExtraction({
      root,
      cache: 'none',
    ...
    extractors: [
      {
        ...
        extract: () => {
          throw new Error('TypeScript extractor fallback should not run when native facts are present')
        },
      },
    ],
  }
...
function nativePromptFactsForTest(match: StaticSourceMatch, file: string): ExtractedFacts {
  return {
    definitions: [
      {
