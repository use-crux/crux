...
describe("static cache identity", () => {
  it("advances the static cache namespace for the combined static contract", () => {
    ...
    expect(STATIC_PARSE_CACHE_EPOCH).toBe("static-parse-v94");
    expect(STATIC_PARSE_CACHE_EPOCH).not.toBe("static-parse-v93");
    expect(STATIC_PARSE_CACHE_EPOCH).not.toBe("static-parse-v85");
  });

  it("takes the Session Flow-target and subscription evidence contract epoch", () => {
