...
describe("signal transport native static projection", () => {
  ...
  itWithRustOxc(
    "projects stream transport with hasOpen and provider transportKind",
    async () => {
      const result = await extractNativeAndFallback({
        callNames: ["signal", "stream", "signalProvider"],
        callInterests: [
          {
            name: "signal",
            importFrom: ["@use-crux/core", "@use-crux/core/signal"],
      ...
      expect(nativeFactCount(result.record, "signal.transport.stream")).toBe(
        1,
      );
      const transport = result.nativeOut.definitions.find(
        (definition) => definition.kind === "signal.transport",
  ...
  itWithRustOxc(
    "rejects live open on managedTransportBinding options",
    async () => {
      const result = await extractNativeAndFallback({
        callNames: [
          "signalProvider",
        ...
        source: [
          ...
          `const provider = signalProvider({`,
          `  id: 'orders.stream',`,
          `  transport: stream({ async *open() { throw new Error('unused') } }),`,
          `  signals: {},`,
          `  onEvent() {},`,
          `})`,
  ...
  itWithRustOxc(
    "projects sse transport with hasOpen and provider transportKind",
    async () => {
      const result = await extractNativeAndFallback({
        callNames: ["signal", "sse", "signalProvider"],
        callInterests: [
          {
            name: "signal",
            importFrom: ["@use-crux/core", "@use-crux/core/signal"],
      ...
      expect(nativeFactCount(result.record, "signal.transport.sse")).toBe(1);
      const transport = result.nativeOut.definitions.find(
        (definition) => definition.kind === "signal.transport",
      );
      expect(transport).toMatchObject({
  ...
  itWithRustOxc(
    "rejects live open on SSE managedTransportBinding options",
    async () => {
      const result = await extractNativeAndFallback({
        callNames: [
          "signalProvider",
        ...
        source: [
          ...
          `const provider = signalProvider({`,
          `  id: 'orders.sse',`,
          `  transport: sse({ async *open() { throw new Error('unused') } }),`,
          `  signals: {},`,
          `  onEvent() {},`,
          `})`,
          `export const binding = managedTransportBinding(provider, {`,
