...
import {
  indexerExtensionRegistry,
  staticIndexerCallNames,
} from "../indexer/extractors/registry";
...
describe("indexer extension boundary", () => {
  it("runs built-in Crux extractors through the extension registry", async () => {
    ...
    expect(
      indexerExtensionRegistry.extensions.map((extension) => extension.name),
    ).toContain("@use-crux/indexer/crux-core");
    expect(
      indexerExtensionRegistry.extractors.map((item) => item.extractor.name),
    ).toContain("rag.retriever");
    expect(
      indexerExtensionRegistry.extractors.map((item) => item.extractor.name),
    ).toContain("safety");
    expect(
    ...
    expect(
      indexerExtensionRegistry.extractors.filter(
        (item) => item.extractor.patterns.length === 0,
      ),
    ).toEqual([]);
          ...
          patterns: [
            {
              kind: "call",
              name: "defineWorkflow",
              importFrom: ["@fixtures/workflow-lib"],
            },
          ],
          extract: (ctx) => {
            const id = ctx.config?.string("id") ?? ctx.source.localName;
