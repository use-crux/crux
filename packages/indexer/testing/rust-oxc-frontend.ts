...
const LOCAL_PLATFORM_PACKAGES = {
  linux: {
    x64: "@use-crux/local-linux-x64",
    arm64: "@use-crux/local-linux-arm64",
  },
  darwin: {
    x64: "@use-crux/local-darwin-x64",
    arm64: "@use-crux/local-darwin-arm64",
  },
  win32: {
    x64: "@use-crux/local-win32-x64",
    arm64: "@use-crux/local-win32-arm64",
  },
} as const satisfies Partial<
  Record<NodeJS.Platform, Partial<Record<NodeJS.Architecture, string>>>
>;
const localPlatformPackages: Partial<
  Record<NodeJS.Platform, Partial<Record<NodeJS.Architecture, string>>>
> = LOCAL_PLATFORM_PACKAGES;
...
interface PendingRequest {
  readonly resolve: (
    record: StaticSyntaxFileRecord | readonly StaticSyntaxFileRecord[],
  ) => void;
...
interface RustOxcWorker {
  readonly parseFile: (
    input: Omit<RustOxcWorkerRequest, "id">,
  ) => Promise<StaticSyntaxFileRecord>;
  readonly parseFiles: (
    input: Omit<RustOxcWorkerBatchRequest, "id">,
...
export interface RustOxcPackagedWorkerOptions {
  /** Platform to resolve. Defaults to the current Node.js platform. */
  readonly platform?: NodeJS.Platform;
  /** CPU architecture to resolve. Defaults to the current Node.js architecture. */
  readonly arch?: NodeJS.Architecture;
  /** Package resolution roots. Defaults to the current working directory and indexer package. */
  readonly searchPaths?: readonly string[];
}

let rustOxcSyntaxFrontendTestStatusMemo:
...
export function rustOxcSyntaxFrontendTestStatus(): RustOxcSyntaxFrontendTestStatus {
  if (rustOxcSyntaxFrontendTestStatusMemo)
...
/**
 * Reports whether fixture tests should use the shipped Rust/Oxc worker.
 *
 * This intentionally checks only explicit worker configuration and installed
 * platform packages. Cargo fallback remains available to parity tests through
 * `rustOxcSyntaxFrontendTestStatus()`, but extension fixtures should prefer
 * binaries that users actually install.
 */
export function rustOxcFixtureSyntaxFrontendStatus(): RustOxcSyntaxFrontendTestStatus {
  const explicitWorker = workerEnvValue("CRUX_STATIC_INDEX_WORKER");
  if (explicitWorker.value)
    return { available: true, workerPath: explicitWorker.value };
  return rustOxcPackagedWorkerStatus();
}

/**
 * Resolves the Rust/Oxc worker from an installed `@use-crux/local-*` platform package.
 *
 * The worker is shipped beside the `crux` binary by `@use-crux/local`; testing
 * uses this resolver to test what ships when that optional platform package is
 * present, while retaining TypeScript fixture fallback when it is absent.
 */
export function rustOxcPackagedWorkerStatus(
  options: RustOxcPackagedWorkerOptions = {},
): RustOxcSyntaxFrontendTestStatus {
  const platform = options.platform ?? process.platform;
  const arch = options.arch ?? process.arch;
  const packageName = localPlatformPackageName(platform, arch);
  if (!packageName) {
    return {
      available: false,
      reason: `unsupported platform package target: ${platform}-${arch}`,
    };
  }
  const searchPaths = options.searchPaths ?? [
    process.cwd(),
    INDEXER_PACKAGE_ROOT,
  ];
  for (const searchPath of searchPaths) {
    const packageJsonPath = resolvePackageJson(packageName, searchPath);
    if (!packageJsonPath) continue;
    const workerPath = join(
      dirname(packageJsonPath),
      "bin",
      staticIndexWorkerBinaryName(platform),
    );
    if (existsSync(workerPath)) return { available: true, workerPath };
    return {
      available: false,
      reason: `${packageName} is installed without ${workerPath}`,
    };
  }
  return { available: false, reason: `${packageName} is not resolvable` };
}
...
function createRustOxcWorker(): RustOxcWorker {
  ...
  return {
    parseFile: async (input) =>
      sendRequest<StaticSyntaxFileRecord>(state, input),
    parseFiles: async (input) =>
      sendRequest<readonly StaticSyntaxFileRecord[]>(state, input),
  };
}

async function sendRequest<
  T extends StaticSyntaxFileRecord | readonly StaticSyntaxFileRecord[],
>(
  state: RustOxcWorkerState,
  input:
    | Omit<RustOxcWorkerRequest, "id">
    | Omit<RustOxcWorkerBatchRequest, "id">,
): Promise<T> {
...
function ensureWorkerStarted(
  ...
  child.on("exit", (code, signal) => {
    rejectAll(
      state,
      new Error(
        `Rust/Oxc indexer worker exited code=${code ?? "null"} signal=${signal ?? "null"} ${state.stderr}`,
      ),
    );
...
function handleWorkerLine(state: RustOxcWorkerState, line: string): void {
  ...
  try {
    ...
    rejectAll(
      state,
      new Error(
        `Invalid Rust/Oxc indexer worker response: ${errorMessage(error)}`,
      ),
    );
    return;
  }
  ...
  if (response.ok && "record" in response) {
    ...
    pending.reject(
      new Error(response.error ?? "Rust/Oxc indexer worker failed"),
    );
  }
...
function workerCommand(): {
  ...
  const explicitWorker = workerEnvValue("CRUX_STATIC_INDEX_WORKER");
  if (explicitWorker.value)
    return { bin: explicitWorker.value, args: ["serve"] };
  const packagedWorker = rustOxcPackagedWorkerStatus();
  if (packagedWorker.workerPath)
    return { bin: packagedWorker.workerPath, args: ["serve"] };
  const status = rustOxcSyntaxFrontendTestStatus();
  if (!status.available) {
    throw new Error(
      `Rust/Oxc indexer worker test helper is unavailable: ${status.reason ?? "unknown reason"}`,
    );
  }
  return {
    bin: "cargo",
...
function rustOxcWorkerPoolSize(): number {
  ...
  if (explicit.value !== undefined) {
    ...
    if (!Number.isInteger(parsed) || parsed <= 0) {
      throw new Error(
        `${explicit.name} must be a positive integer, received ${value}`,
      );
    }
    return parsed;
  }
...
function workerFileRequest(
  input: StaticSyntaxFileInput,
  readSourceFromDisk: boolean,
): RustOxcWorkerFileRequest {
...
function detectRustOxcSyntaxFrontendTestStatus(): RustOxcSyntaxFrontendTestStatus {
  if (rustOxcTestsSkippedByEnv()) {
    return {
      available: false,
      reason: "CRUX_INDEXER_SKIP_RUST_OXC_TESTS is enabled",
    };
  }
  const fixtureStatus = rustOxcFixtureSyntaxFrontendStatus();
  if (fixtureStatus.available) return fixtureStatus;
  ...
  if (!version)
    return {
      available: false,
      reason: `could not parse rustc version from "${rustc.output.trim()}"`,
    };
...
function localPlatformPackageName(
  platform: NodeJS.Platform,
  arch: NodeJS.Architecture,
): string | undefined {
  return localPlatformPackages[platform]?.[arch];
}

function staticIndexWorkerBinaryName(
  platform: NodeJS.Platform,
): "crux-static-index-worker" | "crux-static-index-worker.exe" {
...
function resolvePackageJson(
  ...
  try {
    return requireFromTesting.resolve(`${packageName}/package.json`, {
      paths: [searchPath],
    });
  } catch {
    return undefined;
...
function commandOutput(
  ...
  if (result.error)
    return {
      ok: false,
      reason: `${command} is unavailable: ${result.error.message}`,
    };
  if (result.status !== 0) {
    return {
      ok: false,
...
function rustOxcTestsSkippedByEnv(): boolean {
  const value =
    process.env.CRUX_INDEXER_SKIP_RUST_OXC_TESTS?.trim().toLowerCase();
  return value === "1" || value === "true" || value === "yes" || value === "on";
}

function workerEnvValue(name: string): {
  readonly name: string;
