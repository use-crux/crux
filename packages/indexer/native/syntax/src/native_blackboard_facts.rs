...
use crate::{
    native_definition::{NativeDefinitionInput, native_static_definition},
    native_memory_id::authored_memory_id,
    native_memory_store::{authored_store, authored_store_name, store_definition, store_id},
    native_record_values::{direct_string_property, property_value},
    native_routing_model::{CallParts, RoutingContext},
    native_routing_output::extracted_facts,
    native_schema::syntax_value_to_json_schema,
};
...
pub(crate) fn blackboard_facts(
    ...
    let id = format!(
        "blackboard:{}",
        crate::native_definition::safe_id(&definition_key)
    );
...
fn blackboard_fact_metadata(
    store: Option<&crate::native_memory_store::StoreInfo>,
