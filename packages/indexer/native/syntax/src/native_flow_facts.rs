...
use crate::{
    native_data_access::{DataAccessRef, data_access_refs_for_value},
    native_definition::{NativeDefinitionInput, native_static_definition, safe_id},
    native_flow_output::{
        flow_fact_metadata, flow_intelligence, flow_references, step_definitions,
    },
...
fn flow_steps(
    ...
    for call in roots.iter().flat_map(|root| function_calls(root)) {
        ...
        let source_refs = match &target_variable {
            Some(target_variable) => crate::native_flow_output::step_target_source_refs(
                context,
                &definition_id,
                target_variable,
            )?,
