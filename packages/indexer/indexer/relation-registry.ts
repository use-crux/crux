import type { ProjectDefinitionKind, ProjectRelation, SourceLocation } from '@crux/core/project-index'
...
const promptContextAccessRelationPolicies = (['prompt', 'context'] as const).flatMap(
  (owner) =>
    [
      {
...
const injectableUseRelationPolicies = (['prompt', 'context'] as const).flatMap(
  (owner) =>
    [
      {
...
const invocationAccessRelationPolicies = (['prompt', 'context', 'tool', 'agent', 'flow.step'] as const).flatMap(
  (owner) =>
    [
      {
