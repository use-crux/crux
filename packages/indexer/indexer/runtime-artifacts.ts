...
export async function generateRuntimeArtifacts(
  ...
  if (host === "next") {
    if (
      await writeGeneratedFile(
        nextFile,
        nextEntryFile(manifest, nextFile, options.root, contentHash),
        {
          protect: true,
        },
      )
  ...
  if (host === "convex") {
    ...
    if (
      await writeGeneratedFile(
        convexTargetsFile,
        convexTargetEntryFile(manifest, convexTargetsFile, options.root),
        {
          protect: true,
        },
      )
...
function convexTargetEntryFile(
  manifest: RuntimeArtifactManifest,
  outputFile: string,
  root: string,
): string {
...
async function resolveRuntimeArtifactHost(
  options: GenerateRuntimeArtifactsOptions,
...
async function validateTargetExports(
  root: string,
