...
import {
  extractFilesWithBatchFrontend,
  parseWithRecordMemo,
  semanticProfileForFile,
} from "./batch";
import {
  cacheKeyInput,
  createStaticParseCacheKeyContext,
  noStaticParseCache,
  persistentStaticParseCache,
} from "./cache";
 ...
 */
export function createStaticExtraction(
  options: StaticExtractionOptions,
): StaticExtractionEngine {
  const profile = compilerProfileWithExtensions(
  ...
  const syntaxFrontend = resolveSyntaxFrontend(options.syntaxFrontend, {
    callNames: [...callNames],
    callInterests: [
      ...staticExtractionCallInterests(profile, runtime.extensionRuntime),
    ],
    constructorInterests: [
      ...staticExtractionConstructorInterests(runtime.extensionRuntime),
    ],
    pruneNativeFactCallNames: [
      ...staticExtractionNativeFactPruneCallNames(runtime.extensionRuntime),
    ],
  });
  ...
  const cacheHitsByFile = new Map(
    (options.cacheHits ?? []).map((hit) => [hit.file, hit.cacheKey] as const),
  );
  ...
  const extractFile = async (file: string): Promise<StaticFileExtraction> => {
    return withStaticExtractionTiming(
      ...
      async () => {
        ...
        if (cacheHit) {
          ...
          if (cached) {
            return {
              ...cached,
              semanticProfile:
                cached.semanticProfile ??
                (await semanticProfileForExtractedFile(
                  instrumentation,
                  file,
                  parseMemo,
                )),
              interfaceHash:
                ...
                (await interfaceHashForExtractedFile(
                  instrumentation,
                  file,
                  parseMemo,
                )),
        ...
        const semanticProfile = await semanticProfileForExtractedFile(
          instrumentation,
          file,
          parseMemo,
        );
        const interfaceHash = await interfaceHashForExtractedFile(
          instrumentation,
          file,
          parseMemo,
        );
  ...
  const extractFiles = (
    ...
    extractOptions?: { readonly concurrency?: number },
  ) => {
...
function cacheStore(
  root: string,
  cache: StaticExtractionOptions["cache"],
): StaticParseCacheStore {
