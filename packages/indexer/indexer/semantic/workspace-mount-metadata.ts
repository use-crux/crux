...
export const RETRIEVER_WORKSPACE_MOUNT_SOURCE_HELPER = 'retrieverWorkspaceMountSource'

export const RETRIEVER_WORKSPACE_MOUNT_SOURCE_CAPABILITIES = ['list', 'read', 'grep', 'exists', 'stat'] as const
...
/**
 * Converts authored mount objects into JSON-like metadata suitable for index
 * consumers.
 */
export function workspaceMountsMetadata(mounts: readonly StaticObjectReader[]): WorkspaceMountMetadata[] | undefined {
...
export function workspaceMountIntelligenceData(mounts: readonly WorkspaceMountMetadata[] | undefined): Record<string, unknown> {
  if (!mounts || mounts.length === 0) return {}
  const mountRows = mounts
    .filter((mount): mount is WorkspaceMountMetadata & { readonly path: string } => typeof mount.path === 'string')
    .map((mount) =>
      compactRecord({
        path: mount.path,
        access: mount.access,
...
export function compactWorkspaceMountMetadata(mount: WorkspaceMountMetadata): WorkspaceMountMetadata | undefined {
...
export function compactWorkspaceMountSourceMetadata(
  ...
  return Object.keys(metadata).length > 0 ? (metadata as WorkspaceMountSourceMetadata) : undefined
}

function workspaceMountSourceMetadata(mount: StaticObjectReader): WorkspaceMountSourceMetadata | undefined {
...
function workspaceMountObjectSourceMetadata(source: StaticObjectReader): WorkspaceMountSourceMetadata | undefined {
...
function workspaceMountSourceCapabilities(source: StaticObjectReader, kind: string): readonly string[] | undefined {
  if (kind === 'retriever') return RETRIEVER_WORKSPACE_MOUNT_SOURCE_CAPABILITIES
  const capabilities = WORKSPACE_MOUNT_SOURCE_CAPABILITY_PROPERTIES.filter((property) => source.has(property))
