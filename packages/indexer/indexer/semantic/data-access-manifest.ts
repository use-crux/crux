...
export type PrimitiveDataAccessOperation = NonNullable<DataAccessFact['operation']>
export type PrimitiveDataAccessTargetKind = NonNullable<DataAccessFact['targetKind']>
...
const targetKindDeclarations = [
  ...
  { kind: 'storage.blobStore', aliases: ['blob'] },
...
export function dataAccessKindForMethod(method: string): PrimitiveDataAccessKind | undefined {
...
export function dataAccessOperationForMethod(
  ...
  if (exactOperations.has(method)) return method as PrimitiveDataAccessOperation
  if (method === 'query' || method === 'find' || method === 'search' || method === 'list') return 'query'
  if (method === 'append' || method === 'put' || method === 'save') return 'append'
...
export function dataAccessTargetKindForVariable(targetVariable: string): PrimitiveDataAccessTargetKind | undefined {
