...
import {
  ...
  tsgoNativeDirectEvidenceForFiles,
  type TsgoNativeDirectEvidenceResult,
} from './tsgo-native-direct-evidence'
import { createTsgoProjectConfig, type TsgoProjectConfig } from './tsgo-project-config'
import { createTsgoTypeScriptSourceCache } from './tsgo-source-cache'
...
export interface TsgoSemanticCompilerHost {
  ...
  analyzeNativeDirect(input: TsgoSemanticCompilerAnalyzeInput): TsgoNativeDirectEvidenceResult | undefined
...
export function createTsgoSemanticCompilerHost(input: TsgoSemanticCompilerHostInput): TsgoSemanticCompilerHost {
  ...
  return {
    analyzeNativeDirect(analyzeInput) {
      return createTsgoNativeDirectEvidence(analyzeInput)
    },
  ...
  function createTsgoNativeDirectEvidence(
    analyzeInput: TsgoSemanticCompilerAnalyzeInput,
  ): TsgoNativeDirectEvidenceResult | undefined {
    const candidateFiles = tsgoNativeDirectCandidateFiles(analyzeInput.files, analyzeInput.sourceProfile)
    ...
    try {
      ...
      const direct = project ? tsgoNativeDirectEvidenceForFiles(project, candidateFiles) : undefined
