...
func TestAnalyzeCollectsExtensionEvidenceJobs(t *testing.T) {
	analyzer := &recordingAnalyzer{
		events: []staticprotocol.AnalyzeStreamEvent{
			{
				Type:                  "extensionEvidenceJobs",
				ExtensionEvidenceJobs: []json.RawMessage{json.RawMessage(`{"id":"job:one"}`)},
			},
		},
		response: staticprotocol.AnalyzeResponse{
			Facts: []json.RawMessage{json.RawMessage(`{"definition":"native"}`)},
		},
	}
	fetchCalls := 0

	result, err := Analyze(context.Background(), analyzer, staticprotocol.AnalyzeRequest{}, func(_ context.Context, jobs []json.RawMessage) ([]json.RawMessage, error) {
		fetchCalls++
...
func TestAnalyzeReportsEvidenceFetchErrorAfterAnalyzeCompletes(t *testing.T) {
	...
	result, err := Analyze(context.Background(), analyzer, staticprotocol.AnalyzeRequest{}, func(context.Context, []json.RawMessage) ([]json.RawMessage, error) {
...
func (a *recordingAnalyzer) NativeStaticAnalyzeStream(_ context.Context, request staticprotocol.AnalyzeRequest, handle staticprotocol.AnalyzeStreamHandler) (staticprotocol.AnalyzeResponse, error) {
