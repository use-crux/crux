...
func TestSyntaxWorkerParsesFileThroughCommandWorker(t *testing.T) {
	worker := NewSyntaxWorker(shellPath(t), fakeSyntaxWorker(t))
	defer worker.Close()

	record, err := worker.ParseFile(context.Background(), SyntaxParseRequest{
		Root:             "/repo",
...
func TestSyntaxWorkerCommandPathUsesExplicitEnv(t *testing.T) {
...
func TestSyntaxWorkerCommandPathDiscoversBundledSibling(t *testing.T) {
	oldExecutable := osExecutable
	t.Cleanup(func() { osExecutable = oldExecutable })
	t.Setenv(syntaxWorkerEnv, "")
	...
	worker := filepath.Join(dir, syntaxWorkerBinaryName())
	...
	if got != worker {
		t.Fatalf("syntaxWorkerCommandPath() = %q, want bundled worker %q", got, worker)
	}
}

func TestSyntaxWorkerCommandPathMissingWhenNoEnvOrSibling(t *testing.T) {
	oldExecutable := osExecutable
	t.Cleanup(func() { osExecutable = oldExecutable })
	t.Setenv(syntaxWorkerEnv, "")
	...
	if got, ok := syntaxWorkerCommandPath(); ok {
		t.Fatalf("syntaxWorkerCommandPath() = %q, true; want no worker", got)
	}
}

func TestSyntaxWorkerPoolParsesFilesThroughCommandWorkers(t *testing.T) {
	pool := NewSyntaxWorkerPool(2, shellPath(t), fakeEchoSyntaxWorker(t))
	defer pool.Close()
	...
	for _, file := range files {
		...
		go func() {
			defer wg.Done()
			record, err := pool.ParseFile(context.Background(), SyntaxParseRequest{
				Root:      "/repo",
				File:      file,
...
func TestSyntaxWorkerParsesFilesThroughBatchCommandWorker(t *testing.T) {
	worker := NewSyntaxWorker(shellPath(t), fakeStreamingBatchSyntaxWorker(t))
	defer worker.Close()

	records, err := worker.ParseFiles(context.Background(), []SyntaxParseRequest{
		{
			Root:      "/repo",
...
func TestSyntaxWorkerStreamsBatchRecords(t *testing.T) {
	worker := NewSyntaxWorker(shellPath(t), fakeStreamingBatchSyntaxWorker(t))
	defer worker.Close()

	var indexes []int
	var records []json.RawMessage
	err := worker.ParseFilesStream(context.Background(), []SyntaxParseRequest{
		{
			Root:      "/repo",
			File:      "/repo/src/one.ts",
			Source:    "export const one = prompt({ id: 'one' })",
...
func TestSyntaxWorkerParsesFilesThroughOutOfOrderStreamingBatch(t *testing.T) {
	worker := NewSyntaxWorker(shellPath(t), fakeOutOfOrderStreamingBatchSyntaxWorker(t))
	defer worker.Close()
...
func TestSyntaxWorkerPoolStreamsBatchAcrossWorkers(t *testing.T) {
	logPath := filepath.Join(t.TempDir(), "workers.log")
	pool := NewSyntaxWorkerPool(2, shellPath(t), fakeCountingBatchSyntaxWorker(t, logPath))
	defer pool.Close()
...
func TestSyntaxWorkerPoolParsesBatchAcrossWorkers(t *testing.T) {
	logPath := filepath.Join(t.TempDir(), "workers.log")
	pool := NewSyntaxWorkerPool(2, shellPath(t), fakeCountingBatchSyntaxWorker(t, logPath))
	defer pool.Close()
