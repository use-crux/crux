...
type syntaxWorkerSyntaxBatchEventRawFields struct {
	id      json.RawMessage
	typ     json.RawMessage
...
func decodeSyntaxBatchEvent(raw json.RawMessage) (syntaxWorkerSyntaxBatchEvent, error) {
	fields, err := syntaxWorkerSyntaxBatchEventFields(raw)
	if err != nil {
		return syntaxWorkerSyntaxBatchEvent{}, err
	}
	...
	event := syntaxWorkerSyntaxBatchEvent{ID: id, Type: eventType}
	switch eventType {
	case "record":
		...
		if len(fields.record) == 0 {
			return syntaxWorkerSyntaxBatchEvent{}, fmt.Errorf("project indexer worker syntax stream event missing record")
		}
		event.Index = index
...
func syntaxWorkerSyntaxBatchEventFields(raw []byte) (syntaxWorkerSyntaxBatchEventRawFields, error) {
	var fields syntaxWorkerSyntaxBatchEventRawFields
	...
	for {
		...
		valueEnd, err := scanSyntaxEventFieldValue(raw, valueStart, key)
		if err != nil {
...
func scanSyntaxEventFieldValue(raw []byte, index int, key string) (int, error) {
	if key == "record" {
		...
		if end, ok := finalSyntaxRecordFieldEnd(raw, index); ok {
			return end, nil
		}
	}
...
func finalSyntaxRecordFieldEnd(raw []byte, index int) (int, bool) {
	index = skipJSONWhitespace(raw, index)
	if index >= len(raw) || raw[index] != '{' {
		return 0, false
	}
