...
func TestLoadManifestBuildsStaticHostManifestRequest(t *testing.T) {
	...
	if reader.request.NativeCompilerProtocolVersion != staticprotocol.Version {
		t.Fatalf("protocol version = %d, want %d", reader.request.NativeCompilerProtocolVersion, staticprotocol.Version)
	}
