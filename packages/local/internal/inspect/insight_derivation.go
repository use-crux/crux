...
func deriveInsights(in inspectInsightInputs) []inspectInsightRecord {
	...
	for _, run := range runs {
		...
		insights = append(insights, inspectInsightRecord{
			Tag:            "InspectInsight",
			InsightID:      "tool-loop-" + inspectfs.SafeFileName(run.TraceID),
	...
	for _, run := range runs {
		if (run.Status == "stale" || run.Status == "incomplete") && !suppressedRunSignals.has("lifecycle", run.TraceID) {
			...
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "run-lifecycle-" + inspectfs.SafeFileName(run.TraceID),
		...
		if (run.Status == "suspended" || run.SuspensionSignalCount > 0) && !suppressedRunSignals.has("suspension", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "run-suspended-" + inspectfs.SafeFileName(run.TraceID),
		...
		if run.DiagnosticCount > 0 && !suppressedRunSignals.has("diagnostic", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "trace-diagnostics-" + inspectfs.SafeFileName(run.TraceID),
		...
		if run.DurationMs != nil && *run.DurationMs >= 60000 && !suppressedRunSignals.has("latency", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "slow-run-" + inspectfs.SafeFileName(run.TraceID),
				Title:          "Run is slow",
		...
		if run.TokenCount >= 10000 && !suppressedRunSignals.has("tokens", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "high-token-usage-" + inspectfs.SafeFileName(run.TraceID),
		...
		if run.TokenCount > 0 && (run.Cost == nil || *run.Cost == 0) && !suppressedRunSignals.has("missing-cost", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "missing-cost-" + inspectfs.SafeFileName(run.TraceID),
		...
		if run.Cost != nil && *run.Cost >= 0.05 && !suppressedRunSignals.has("cost", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "high-cost-" + inspectfs.SafeFileName(run.TraceID),
				Title:          "Run is costly",
		...
		if run.ToolErrorCount > 0 && !suppressedRunSignals.has("tool-errors", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "tool-errors-" + inspectfs.SafeFileName(run.TraceID),
		...
		if run.RepeatedToolCount >= 5 && !suppressedRunSignals.has("repeated-tool", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "repeated-tool-" + inspectfs.SafeFileName(run.TraceID),
		...
		if run.RetrievalIssueCount > 0 && !suppressedRunSignals.has("retrieval", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "retrieval-issues-" + inspectfs.SafeFileName(run.TraceID),
		...
		if (run.InspectSignalIssueCount > 0 || run.BlockedSignalCount > 0) && !suppressedRunSignals.has("inspect-signal", run.TraceID) {
			insights = append(insights, inspectInsightRecord{
				Tag:            "InspectInsight",
				InsightID:      "inspect-signal-" + inspectfs.SafeFileName(run.TraceID),
