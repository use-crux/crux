...
type inspectSignalSet map[string]map[string]struct{}

func (s inspectSignalSet) add(signal string, traceIDs []string) {
	if s[signal] == nil {
...
func (s inspectSignalSet) has(signal string, traceID string) bool {
	if s[signal] == nil {
		return false
	}
...
func inspectPatternInsights(runs []inspectRunRecord) ([]inspectInsightRecord, inspectSignalSet) {
	type pattern struct {
		key          string
		title        string
		signal       string
	...
	add := func(key, title, signal, severity string, tags []string, run inspectRunRecord, cause, fix string) {
		p, ok := patterns[key]
		if !ok {
			p = &pattern{
				key:      key,
	...
	for _, run := range runs {
		target := inspectfs.SafeFileName(firstNonEmpty(run.TargetID, "unknown"))
		if run.TokenCount >= 10000 {
			add("pattern-high-token-"+target, "Repeated high token usage pattern", "tokens", inspectTokenSeverity(run.TokenCount), []string{"Pattern", "Tokens", "Cost"}, run, "The same target repeatedly crosses the token attention threshold.", "Inspect prompt, context, memory, retrieval, and branching behavior across linked runs to identify the repeated token source.")
			add("pattern-high-token-global", "High token usage is recurring", "tokens", inspectTokenSeverity(run.TokenCount), []string{"Pattern", "Tokens", "Cost"}, run, "Many runs are crossing the token attention threshold.", "Inspect linked runs for common prompt, context, memory, retrieval, composition, or model-selection causes.")
		}
		...
		if run.DurationMs != nil && *run.DurationMs >= 60000 {
			add("pattern-slow-"+target, "Repeated slow run pattern", "latency", inspectLatencySeverity(*run.DurationMs), []string{"Pattern", "Latency", "Performance"}, run, "The same target repeatedly exceeds the latency attention threshold.", "Compare linked run waterfalls to find recurring slow generations, tools, retrieval, or fanout branches.")
			add("pattern-slow-global", "Slow runs are recurring", "latency", inspectLatencySeverity(*run.DurationMs), []string{"Pattern", "Latency", "Performance"}, run, "Many runs are crossing the latency attention threshold.", "Compare linked run waterfalls for common slow generations, tools, retrieval stages, or fanout patterns.")
		}
		if run.Cost != nil && *run.Cost >= 0.05 {
			add("pattern-high-cost-"+target, "Repeated high cost pattern", "cost", inspectCostSeverity(*run.Cost), []string{"Pattern", "Cost", "Tokens"}, run, "The same target repeatedly crosses the cost attention threshold.", "Inspect linked runs for expensive models, long contexts, retries, or repeated composition branches.")
			add("pattern-high-cost-global", "High cost is recurring", "cost", inspectCostSeverity(*run.Cost), []string{"Pattern", "Cost", "Tokens"}, run, "Many runs are crossing the cost attention threshold.", "Inspect linked runs for common expensive models, long contexts, retries, or repeated composition branches.")
		}
		...
		for _, code := range run.DiagnosticCodes {
			codeKey := inspectfs.SafeFileName(code)
			add("pattern-diagnostic-"+target+"-"+codeKey, "Repeated observability diagnostic pattern", "diagnostic", inspectDiagnosticSeverity([]string{code}), []string{"Pattern", "Observability"}, run, "The same observability diagnostic is recurring across runs.", "Inspect the linked run diagnostics and fix the runtime boundary or primitive that repeatedly loses lifecycle data.")
			add("pattern-diagnostic-global-"+codeKey, "Observability diagnostics are recurring", "diagnostic", inspectDiagnosticSeverity([]string{code}), []string{"Pattern", "Observability"}, run, "The same observability diagnostic is recurring across runs.", "Inspect linked diagnostics and fix the runtime boundary or primitive that repeatedly loses lifecycle data.")
		}
	}
	insights := []inspectInsightRecord{}
	suppressed := inspectSignalSet{}
	for _, p := range patterns {
		...
		insights = append(insights, inspectInsightRecord{
			...
			Status:          "open",
			UpdatedAt:       inspectMillisToRFC3339(p.latestMillis),
		})
	}
