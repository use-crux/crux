...
// for the caller.
func applyRunsOptions(runs []inspectRunRecord, opts api.InspectRunsOptions) []inspectRunRecord {
	out := make([]inspectRunRecord, 0, len(runs))
	for _, r := range runs {
...
func matchesRunsOptions(r inspectRunRecord, opts api.InspectRunsOptions) bool {
...
func sortRunsByDuration(rs []inspectRunRecord, asc bool) {
...
func sortRunsByCost(rs []inspectRunRecord, asc bool) {
	sort.SliceStable(rs, func(i, j int) bool {
		a := runCost(rs[i])
		b := runCost(rs[j])
		if asc {
...
func sortRunsByTokens(rs []inspectRunRecord, asc bool) {
	sort.SliceStable(rs, func(i, j int) bool {
		if asc {
...
func runDuration(r inspectRunRecord) float64 {
	if r.DurationMs == nil {
		return 0
	}
	return *r.DurationMs
...
func runCost(r inspectRunRecord) float64 {
	if r.Cost == nil {
		return 0
	}
	return *r.Cost
...
func inspectRunTabCountsFromRuns(runs []inspectRunRecord) inspectRunTabCounts {
	counts := inspectRunTabCounts{All: len(runs)}
