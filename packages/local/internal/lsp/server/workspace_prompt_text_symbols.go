...
// PromptTextSymbols pins one current OWN/ATTACHED analyzer epoch while
// projecting identity-sensitive Markdown headings.
func (w *workspaceRuntime) PromptTextSymbols(
	ctx context.Context,
	...
	empty := lsprompttext.SymbolResult{
		Revision: document.Revision, Symbols: []protocol.DocumentSymbol{},
	}
	...
	defer cancel()
	result := w.server.promptText.Symbols(queryContext, lsprompttext.Request{
