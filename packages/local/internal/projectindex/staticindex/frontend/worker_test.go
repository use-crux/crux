...
func TestWorkerParsesFileThroughCommandWorker(t *testing.T) {
	worker := New(shellPath(t), fakeWorker(t))
	defer worker.Close()

	record, err := worker.ParseFile(context.Background(), Request{
		Root:             "/repo",
...
func TestWorkerCommandPathDiscoversBundledSibling(t *testing.T) {
...
func TestWorkerCommandPathMissingWhenNoEnvOrSibling(t *testing.T) {
	oldExecutable := osExecutable
	t.Cleanup(func() { osExecutable = oldExecutable })
	t.Setenv(syntaxWorkerEnv, "")
...
func TestPoolParsesFilesThroughCommandWorkers(t *testing.T) {
	...
	for _, file := range files {
		...
		go func() {
			defer wg.Done()
			record, err := pool.ParseFile(context.Background(), Request{
				Root:      "/repo",
				File:      file,
...
func TestWorkerParsesFilesThroughBatchCommandWorker(t *testing.T) {
	worker := New(shellPath(t), fakeStreamingBatchWorker(t))
	defer worker.Close()

	records, err := worker.ParseFiles(context.Background(), []Request{
		{
			Root:      "/repo",
...
func TestWorkerStreamsBatchRecords(t *testing.T) {
	worker := New(shellPath(t), fakeStreamingBatchWorker(t))
	defer worker.Close()

	var indexes []int
	var records []json.RawMessage
	err := worker.ParseFilesStream(context.Background(), []Request{
		{
			Root:      "/repo",
			File:      "/repo/src/one.ts",
			Source:    "export const one = prompt({ id: 'one' })",
...
func TestWorkerParsesFilesThroughOutOfOrderStreamingBatch(t *testing.T) {
	worker := New(shellPath(t), fakeOutOfOrderStreamingBatchWorker(t))
	defer worker.Close()
...
func TestPoolStreamsBatchAcrossWorkers(t *testing.T) {
	logPath := filepath.Join(t.TempDir(), "workers.log")
	pool := NewPool(2, shellPath(t), fakeCountingBatchWorker(t, logPath))
	defer pool.Close()
...
func TestPoolParsesBatchAcrossWorkers(t *testing.T) {
	logPath := filepath.Join(t.TempDir(), "workers.log")
	pool := NewPool(2, shellPath(t), fakeCountingBatchWorker(t, logPath))
	defer pool.Close()
...
func fakeEchoWorker(t *testing.T) string {
	t.Helper()
...
func fakeStreamingBatchWorker(t *testing.T) string {
	t.Helper()
...
func fakeOutOfOrderStreamingBatchWorker(t *testing.T) string {
	t.Helper()
...
func fakeCountingBatchWorker(t *testing.T, logPath string) string {
	t.Helper()
