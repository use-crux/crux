...
func writablePrimaryMisses(plan protocol.Plan) []string {
...
func sourceFileSet(files []protocol.SourceFile) map[string]bool {
	set := map[string]bool{}
...
func sourceFileMap(files []protocol.SourceFile) map[string]protocol.SourceFile {
	out := make(map[string]protocol.SourceFile, len(files))
	for _, file := range files {
		if file.File != "" {
			out[file.File] = file
		}
