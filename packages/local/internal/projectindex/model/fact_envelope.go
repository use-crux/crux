...
func FactTransactionFromPatch(patch IndexPatch) IndexFactTransaction {
...
// IndexPatchFactsFromEnvelopes decodes validated fact envelopes into the patch
// fact groups used by the Project Index read model.
func IndexPatchFactsFromEnvelopes(envelopes []IndexFactEnvelope) (IndexPatchFacts, error) {
	return indexPatchFactsFromEnvelopes(envelopes)
}
...
// AddIndexFactEnvelope decodes one validated fact envelope into its
// corresponding patch fact group.
func AddIndexFactEnvelope(facts *IndexPatchFacts, envelope IndexFactEnvelope) error {
	return addIndexFactEnvelope(facts, envelope)
}
...
// DecodeIndexFact decodes a validated envelope payload into the requested fact
// type.
func DecodeIndexFact[T any](envelope IndexFactEnvelope, out *T) error {
	return decodeIndexFact(envelope, out)
}

// ValidateIndexFactFidelity verifies that an envelope uses a supported fact
// fidelity label.
...
// ValidateIndexFactProvenance verifies that the envelope carries enough
// provenance to explain how the fact was produced.
