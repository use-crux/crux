...
func CallRaw(ctx context.Context, w *Worker, req any) (json.RawMessage, error) {
	if w == nil {
		return nil, fmt.Errorf("node process: nil worker")
	}
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *Worker) ensureSpawned() error {
	...
	if w.config.commandPath != "" {
		...
		if scriptPath == "" {
			if w.script.Name == "" {
				return fmt.Errorf("node process: script name is required")
			}
			var err error
...
func scanLine(stdout *bufio.Reader, maxBytes int) scanResult {
	if stdout == nil {
		return scanResult{err: fmt.Errorf("node process: stdout unavailable")}
	}
	var line []byte
	for {
		...
		if maxBytes > 0 && len(line) > maxBytes {
			return scanResult{err: fmt.Errorf("node process: response exceeded %d bytes", maxBytes)}
		}
		if errors.Is(err, bufio.ErrBufferFull) {
			continue
		}
		if errors.Is(err, io.EOF) && len(line) == 0 {
			return scanResult{err: fmt.Errorf("node process: no output (EOF)")}
		}
		if err != nil && !errors.Is(err, io.EOF) {
			return scanResult{err: fmt.Errorf("node process: read response: %w", err)}
		}
		break
	}
