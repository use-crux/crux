...
// the Index screen loads its data, the workbench's propagation
...
func TestWorkbenchPropagatesAffectedSuitesToSuites(t *testing.T) {
	...
	// the index data + observe the Suites side directly.
	cat := screens.NewIndex()
	w.screens["index"] = cat
	...
	// indexLoadedMsg events reaching the Index screen flow into
