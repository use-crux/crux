...
func TestWorkbenchHelpRendersActiveScreenKeybinds(t *testing.T) {
	...
	// short, well-known labels like "insights" and "runs" via the
	...
	// Overview's Keybinds() contains `{Key: "g i", Label: "insights"}`.
	if !strings.Contains(out, "insights") {
		t.Errorf("help overlay does not contain Overview screen-specific label \"insights\" — workbench is not feeding the screen's Keybinds()")
	}
