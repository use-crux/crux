...
func loadProjectIndexPatches(ctx context.Context, db *sql.DB, root string) ([]IndexPatch, error) {
	...
	for _, phase := range phaseOrderForProjection() {
		...
		if !hasPatch && !hasIndexPatchFacts(facts) {
			continue
		}
		if !hasPatch {
...
func hasIndexPatchFacts(facts IndexPatchFacts) bool {
...
func phaseOrderForProjection() []IndexPatchPhase {
	return []IndexPatchPhase{indexPatchPhaseCache, indexPatchPhaseAST, indexPatchPhaseSemantic, indexPatchPhaseRuntime, indexPatchPhaseQuality}
}
