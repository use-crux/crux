...
func projectSemanticPatchWithAstSnapshot(patch IndexPatch, index store.IndexData) IndexPatch {
	...
	if len(supportSources) > 0 {
		patch.Facts.Sources = mergeIndexSources(patch.Facts.Sources, supportSources)
	}
	return patch
}
...
func projectSemanticOwnerFileForDefinition(index store.IndexData, definitionID string) string {
	...
	for _, source := range index.Sources {
		if stringSliceContains(source.DefinitionIDs, definitionID) {
			return source.File
		}
	}
	return ""
