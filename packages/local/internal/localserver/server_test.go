...
func TestNewUsesItsScopedLoggerForResponseEncodingFailures(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPost, "/api/observability/records", strings.NewReader(`{"schemaVersion":5,"records":[]}`))
...
func TestNewMountsLocalRuntimeRouteGroups(t *testing.T) {
	...
	assertStatusAndClose(t, http.MethodPost, ts.URL+"/api/observability/records", []byte(`{"schemaVersion":5,"records":[]}`), http.StatusAccepted)
