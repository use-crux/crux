...
func TestProjectIndexWorkerCollectProjectSyntaxRecordsUsesParserConcurrency(t *testing.T) {
	root := t.TempDir()
	files := []string{
...
func TestProjectIndexWorkerCollectProjectSyntaxRecordsBatchUsesDiskSourceHandoff(t *testing.T) {
	root := t.TempDir()
	files := []string{
...
func TestProjectIndexWorkerCollectProjectSyntaxRecordsUsesFilesToParse(t *testing.T) {
	root := t.TempDir()
	files := []string{
		fileWithSource(t, root, "src/one.ts"),
		fileWithSource(t, root, "src/two.ts"),
...
func TestProjectSyntaxParseRequestsFromPlanUsesEmptyFilesToParse(t *testing.T) {
	root := t.TempDir()
	files := []string{
		filepath.Join(root, "src", "one.ts"),
		filepath.Join(root, "src", "two.ts"),
...
type recordingProjectSyntaxParser struct {
	concurrency int
	requests    []ProjectSyntaxParseRequest
}

func (p *recordingProjectSyntaxParser) ParseFile(_ context.Context, request ProjectSyntaxParseRequest) (json.RawMessage, error) {
...
func (p *recordingProjectSyntaxParser) Concurrency() int {
	return p.concurrency
}

func (p *recordingProjectSyntaxParser) Close() error {
	return nil
}

type recordingBatchProjectSyntaxParser struct {
	requests []ProjectSyntaxParseRequest
}

func (p *recordingBatchProjectSyntaxParser) ParseFile(context.Context, ProjectSyntaxParseRequest) (json.RawMessage, error) {
	return nil, fmt.Errorf("ParseFile should not be called")
}

func (p *recordingBatchProjectSyntaxParser) ParseFiles(_ context.Context, requests []ProjectSyntaxParseRequest) ([]json.RawMessage, error) {
...
func (p *recordingBatchProjectSyntaxParser) Concurrency() int {
	return 1
}

func (p *recordingBatchProjectSyntaxParser) Close() error {
	return nil
}

type blockingProjectSyntaxParser struct {
	concurrency int
	started     chan struct{}
	release     chan struct{}
...
func newBlockingProjectSyntaxParser(concurrency int, fileCount int) *blockingProjectSyntaxParser {
	return &blockingProjectSyntaxParser{
...
func (p *blockingProjectSyntaxParser) ParseFile(ctx context.Context, request ProjectSyntaxParseRequest) (json.RawMessage, error) {
...
func (p *blockingProjectSyntaxParser) Concurrency() int {
	return p.concurrency
}

func (p *blockingProjectSyntaxParser) Close() error {
	return nil
}

func (p *blockingProjectSyntaxParser) waitForStarted(t *testing.T, count int) {
	t.Helper()
	for i := 0; i < count; i++ {
		select {
		case <-p.started:
