...
func (w *ProjectIndexWorker) indexProjectAstPatchFromNativeSyntaxRecordStream(
	ctx context.Context,
	root string,
	configPath string,
	projectName string,
	plan devtools.ProjectStaticSyntaxPlan,
	parser ProjectSyntaxBatchStreamParser,
...
func (w *ProjectIndexWorker) streamNativeSyntaxRecordsToProjectIndexer(
	ctx context.Context,
	req projectIndexRequest,
	plan devtools.ProjectStaticSyntaxPlan,
	parser ProjectSyntaxBatchStreamParser,
	...
	err := nodeworker.StreamCallSession(ctx, w.worker, func(send nodeworker.StreamSender) error {
		if err := send(projectIndexWorkerStartRequest(req, requestID)); err != nil {
			return err
		}
