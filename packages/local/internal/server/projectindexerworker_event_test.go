...
func TestDecodeProjectIndexerWorkerBatchRecordEventPreservesRecordJSON(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("decodeProjectIndexerWorkerSyntaxBatchEvent error = %v", err)
	}
...
func TestDecodeProjectIndexerWorkerBatchTerminalEvents(t *testing.T) {
	done, err := decodeProjectIndexerWorkerSyntaxBatchEvent(json.RawMessage(`{"id":9,"type":"done","count":3}`))
	if err != nil {
	...
	failed, err := decodeProjectIndexerWorkerSyntaxBatchEvent(json.RawMessage(`{"id":10,"type":"error","error":"missing source"}`))
	if err != nil {
