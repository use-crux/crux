...
type projectSyntaxWorkerBatchEventRawFields struct {
	id      json.RawMessage
	typ     json.RawMessage
...
func decodeProjectSyntaxWorkerBatchEvent(raw json.RawMessage) (projectSyntaxWorkerBatchEvent, error) {
	fields, err := projectSyntaxWorkerBatchEventFields(raw)
	if err != nil {
		return projectSyntaxWorkerBatchEvent{}, err
	}
	...
	event := projectSyntaxWorkerBatchEvent{ID: id, Type: eventType}
	switch eventType {
	case "record":
		...
		if len(fields.record) == 0 {
			return projectSyntaxWorkerBatchEvent{}, fmt.Errorf("project syntax worker stream event missing record")
		}
		event.Index = index
...
func projectSyntaxWorkerBatchEventFields(raw []byte) (projectSyntaxWorkerBatchEventRawFields, error) {
	var fields projectSyntaxWorkerBatchEventRawFields
	index := skipJSONWhitespace(raw, 0)
	if index >= len(raw) || raw[index] != '{' {
		return fields, fmt.Errorf("project syntax worker stream event must be a JSON object")
	}
	index++
	for {
		index = skipJSONWhitespace(raw, index)
		if index >= len(raw) {
			return fields, fmt.Errorf("unterminated project syntax worker stream event")
		}
		if raw[index] == '}' {
			return fields, nil
		}
		...
		if err != nil {
			return fields, fmt.Errorf("decode project syntax worker stream event key: %w", err)
		}
		index = skipJSONWhitespace(raw, keyEnd)
		if index >= len(raw) || raw[index] != ':' {
			return fields, fmt.Errorf("project syntax worker stream event field %q missing colon", key)
		}
		valueStart := skipJSONWhitespace(raw, index+1)
		valueEnd, err := scanProjectSyntaxWorkerEventFieldValue(raw, valueStart, key)
		if err != nil {
			return fields, fmt.Errorf("scan project syntax worker stream event field %q: %w", key, err)
		}
		...
		case '}':
			return fields, nil
		default:
			return fields, fmt.Errorf("project syntax worker stream event field %q has invalid delimiter %q", key, raw[index])
		}
	}
}

func scanProjectSyntaxWorkerEventFieldValue(raw []byte, index int, key string) (int, error) {
	if key == "record" {
		...
		if end, ok := finalProjectSyntaxWorkerRecordFieldEnd(raw, index); ok {
			return end, nil
		}
	}
...
func finalProjectSyntaxWorkerRecordFieldEnd(raw []byte, index int) (int, bool) {
	index = skipJSONWhitespace(raw, index)
	if index >= len(raw) || raw[index] != '{' {
		return 0, false
	}
...
func decodeJSONUintField(raw json.RawMessage, name string) (uint64, error) {
	if len(raw) == 0 {
		return 0, fmt.Errorf("project syntax worker stream event missing %s", name)
	}
	var value uint64
	if err := json.Unmarshal(raw, &value); err != nil {
		return 0, fmt.Errorf("decode project syntax worker stream event %s: %w", name, err)
	}
	return value, nil
}
...
func decodeJSONStringField(raw json.RawMessage, name string) (string, error) {
	if len(raw) == 0 {
		return "", fmt.Errorf("project syntax worker stream event missing %s", name)
	}
	var value string
	if err := json.Unmarshal(raw, &value); err != nil {
		return "", fmt.Errorf("decode project syntax worker stream event %s: %w", name, err)
	}
	return value, nil
}
