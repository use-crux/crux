...
func TestProjectIndexerWorkerParsesFileThroughCommandWorker(t *testing.T) {
	worker := NewProjectIndexerWorkerProcess(shellPath(t), fakeSyntaxWorker(t))
	defer worker.Close()
...
func TestProjectIndexerWorkerCommandPathUsesExplicitEnv(t *testing.T) {
	t.Setenv(projectIndexerWorkerEnv, "/opt/crux/crux-indexer-worker")

	got, ok := projectIndexerWorkerCommandPath()
	if !ok {
		t.Fatalf("projectIndexerWorkerCommandPath() ok = false, want true")
	}
	if got != "/opt/crux/crux-indexer-worker" {
		t.Fatalf("projectIndexerWorkerCommandPath() = %q, want explicit env path", got)
	}
}

func TestProjectIndexerWorkerCommandPathDiscoversBundledSibling(t *testing.T) {
	oldExecutable := osExecutable
	t.Cleanup(func() { osExecutable = oldExecutable })
	t.Setenv(projectIndexerWorkerEnv, "")
	...
	worker := filepath.Join(dir, projectIndexerWorkerBinaryName())
	if err := os.WriteFile(worker, []byte("test"), 0o700); err != nil {
		t.Fatalf("write fake indexer worker: %v", err)
	}
	...
	if got != worker {
		t.Fatalf("projectIndexerWorkerCommandPath() = %q, want bundled worker %q", got, worker)
	}
}

func TestProjectIndexerWorkerCommandPathMissingWhenNoEnvOrSibling(t *testing.T) {
	oldExecutable := osExecutable
	t.Cleanup(func() { osExecutable = oldExecutable })
	t.Setenv(projectIndexerWorkerEnv, "")
	...
	if got, ok := projectIndexerWorkerCommandPath(); ok {
		t.Fatalf("projectIndexerWorkerCommandPath() = %q, true; want no worker", got)
	}
}

func TestProjectIndexerWorkerPoolParsesFilesThroughCommandWorkers(t *testing.T) {
	pool := NewProjectIndexerWorkerPool(2, shellPath(t), fakeEchoSyntaxWorker(t))
	defer pool.Close()
	...
	for _, file := range files {
		...
		go func() {
			...
			if !strings.Contains(string(record), file) {
				errs <- &indexerWorkerTestError{message: "record did not preserve file " + file + ": " + string(record)}
			}
		}()
	}
	wg.Wait()
...
func TestProjectIndexerWorkerParsesFilesThroughBatchCommandWorker(t *testing.T) {
	worker := NewProjectIndexerWorkerProcess(shellPath(t), fakeStreamingBatchSyntaxWorker(t))
	defer worker.Close()
...
func TestProjectIndexerWorkerStreamsBatchRecords(t *testing.T) {
	worker := NewProjectIndexerWorkerProcess(shellPath(t), fakeStreamingBatchSyntaxWorker(t))
	defer worker.Close()
...
func TestProjectIndexerWorkerParsesFilesThroughOutOfOrderStreamingBatch(t *testing.T) {
	worker := NewProjectIndexerWorkerProcess(shellPath(t), fakeOutOfOrderStreamingBatchSyntaxWorker(t))
	defer worker.Close()
...
func TestProjectIndexerWorkerPoolStreamsBatchAcrossWorkers(t *testing.T) {
	logPath := filepath.Join(t.TempDir(), "workers.log")
	pool := NewProjectIndexerWorkerPool(2, shellPath(t), fakeCountingBatchSyntaxWorker(t, logPath))
	defer pool.Close()
...
func TestProjectIndexerWorkerPoolParsesBatchAcrossWorkers(t *testing.T) {
	logPath := filepath.Join(t.TempDir(), "workers.log")
	pool := NewProjectIndexerWorkerPool(2, shellPath(t), fakeCountingBatchSyntaxWorker(t, logPath))
	defer pool.Close()
...
type indexerWorkerTestError struct {
	message string
}

func (e *indexerWorkerTestError) Error() string {
	return e.message
}
