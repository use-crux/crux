...
type RunnerDeps struct {
	...
	ExtractRunner func() (string, error)
}

// EvaluationCollector serves the spec-02 Evaluation manifests for the
...
// NewEvaluationCollector creates a collector rooted at the project.
func NewEvaluationCollector(projectRoot, configPath string, deps RunnerDeps) *EvaluationCollector {
...
func (c *EvaluationCollector) EvaluationManifests(ctx context.Context) ([]json.RawMessage, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *EvaluationCollector) collectFromWorker(ctx context.Context) ([]json.RawMessage, error) {
	nodePath, err := c.deps.FindNode()
	if err != nil {
		return nil, err
	}
	runnerPath, err := c.deps.ExtractRunner()
	if err != nil {
