...
// PromoteRequest is the POST /api/quality/promote body.
...
// RunPromote spawns the embedded worker's promote mode against the
// project and parses its NDJSON outcome.
func RunPromote(ctx context.Context, projectRoot, configPath string, deps RunnerDeps, req PromoteRequest) (api.QualityPromoteResult, error) {
	if req.ExperimentID == "" {
...
func RegisterPromote(mux *http.ServeMux, projectRoot, configPath string, deps RunnerDeps, qualityEvents *qualitysvc.EventBus) {
	mux.HandleFunc("POST /api/quality/promote", func(w http.ResponseWriter, r *http.Request) {
		...
		result, err := RunPromote(r.Context(), projectRoot, configPath, deps, req)
		if err != nil {
