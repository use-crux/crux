...
// spinnerFrames is the Braille spinner cycled one frame per event. Advancing on
// event arrival (rather than a timer) keeps the reporter single-threaded with no
// goroutine — events arrive frequently enough during a run to read as motion.
const spinnerFrames = "⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"
