...
# Verified VHS pixel → terminal cell mapping (vhs v0.11.0 + xterm.js fit).
#
# Geometry formula (FontSize 14, LetterSpacing 0, Padding 60):
#   viewport = (Width - 2*Padding) × (Height - 2*Padding)
#   xterm fit overhead ≈ 32px width, 10px height (scrollbar / chrome)
#   cell advance ≈ 8.0 px/col  (JetBrains Mono @ 14 with LetterSpacing 0;
#                                without LetterSpacing 0, default ls=1.0 ≈ 9.14 px/col)
#   cell height  ≈ 16.0 px/row (measured; not FontSize*LineHeight alone)
#   cols = floor((Width  - 2*Padding - 32) / 8)
#   rows = floor((Height - 2*Padding - 10) / 16)
#
# Empirical stty size (rows cols) for each preset below is exact.
# Before (old SIZE_PIXELS, FontSize 14, default LetterSpacing 1.0, default Padding 60):
#   160x45: 1400×700 → ~139×35   (undershot; RESPONSIVE MEDIUM, not three-pane Runs)
#   100x30:  900×480 → ~89×22
#   70x24:   640×400 → ~56×17    (below TUI minimum-size fallback)
# After (values below + LetterSpacing 0 + Padding 60):
#   160x45: 1432×850 → 160×45 exact
#   100x30:  952×610 → 100×30 exact
#   70x24:   712×514 →  70×24 exact
# Never undershoot; if a future font/Chrome change breaks exact fit, prefer +1..+2 cells.
VHS_FONT_SIZE=14
VHS_PADDING=60
VHS_LETTER_SPACING=0
...
# Geometry: ${size} cells via Width×Height ${px_w}×${px_h}, FontSize ${VHS_FONT_SIZE},
# LetterSpacing ${VHS_LETTER_SPACING}, Padding ${VHS_PADDING} (see SIZE_PIXELS comments).
...
Set LetterSpacing ${VHS_LETTER_SPACING}
Set TypingSpeed 1ms
Env CI ""
Env GITHUB_ACTIONS ""
Env NO_COLOR ""
