...
export interface ResolvedAuthoredCaseFile {
...
/**
 * Resolve one authored `caseFile()` exactly like a relative import while
 * preventing lexical and symlink escapes from the configured project root.
 */
export async function resolveAuthoredCaseFile(
  options: ResolveAuthoredCaseFileOptions,
): Promise<ResolvedAuthoredCaseFile> {
  if (isAbsolute(options.authoredPath) || win32.isAbsolute(options.authoredPath)) {
    throw pathError(options, undefined, 'absolute paths are not allowed; use a relative path from the Eval file')
  }

  const root = await realpath(options.projectRoot).catch((error: unknown) => {
    throw pathError(options, undefined, `cannot realpath the configured project root (${errorMessage(error)})`)
  })
  const source = resolve(root, options.sourceFile)
  assertContained(root, source, options, undefined, 'the Eval source is outside the project root')

  const lexicalTarget = resolve(dirname(source), options.authoredPath)
  const lexicalCanonical = projectRelative(root, lexicalTarget)
  assertContained(root, lexicalTarget, options, lexicalCanonical, 'the authored path resolves outside the project root')

  const target = await realpathOrNearest(lexicalTarget).catch((error: unknown) => {
    throw pathError(options, lexicalCanonical, `cannot resolve the target (${errorMessage(error)})`)
  })
  assertContained(root, target.path, options, projectRelative(root, target.path), 'the real target resolves outside the project root')
  const canonicalPath = projectRelative(root, target.path)
  options.registerWatchDependency?.(canonicalPath)

  const sidecar = await realpathOrNearest(resolve(root, options.sidecarFile)).catch((error: unknown) => {
    throw pathError(options, canonicalPath, `cannot resolve the automatic sibling (${errorMessage(error)})`)
  })
  ...
  const info = await stat(target.path).catch((error: unknown) => {
    throw pathError(options, canonicalPath, `cannot inspect the target (${errorMessage(error)})`)
  })
  if (!info.isFile()) {
    throw pathError(options, canonicalPath, 'target must be a regular Case file')
  }
  if (!['.json', '.jsonl', '.csv'].includes(extname(canonicalPath))) {
    throw pathError(options, canonicalPath, 'supported Case-file extensions are .json, .jsonl, and .csv')
  }
...
function pathError(
  ...
  const attempted = canonicalPath === undefined ? '' : `; canonical target '${canonicalPath}'`
