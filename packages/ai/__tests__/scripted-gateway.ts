...
function materialize(scripted: ScriptedResult, kind: 'text' | 'object'): Record<string, unknown> {
  const text = scripted.text ?? (scripted.object !== undefined ? JSON.stringify(scripted.object) : '')
  const usage = {
  ...
  const responseMessages =
    ...
    (kind === 'text' ? [{ role: 'assistant', content: [{ type: 'text', text }] }] : [])
  return {
    text,
    ...
    response: { id: 'scripted-resp', modelId: 'scripted-model', messages: responseMessages },
  }
}
...
export function scriptedGateway(config: ScriptedGatewayConfig = {}): ScriptedGateway {
  ...
  function runStream(
    ...
    const onChunk = args.onChunk as ((event: unknown) => Promise<void>) | undefined
  ...
  const gateway: SdkGateway = {
    ...
    generateText: async (args) => {
      ...
      if (scripted instanceof Error) throw scripted
      return materialize(scripted, 'text') as unknown as Awaited<ReturnType<SdkGateway['generateText']>>
    },
    generateObject: async (args) => {
      ...
      if (scripted instanceof Error) throw scripted
      return materialize(scripted, 'object') as unknown as Awaited<ReturnType<SdkGateway['generateObject']>>
    },
    streamText: (args) => {
      calls.streamText.push(args as Record<string, unknown>)
      return runStream(args as Record<string, unknown>, streamTextScripts.shift(), 'text') as unknown as ReturnType<
        SdkGateway['streamText']
      >
    },
    streamObject: (args) => {
      calls.streamObject.push(args as Record<string, unknown>)
      return runStream(args as Record<string, unknown>, streamObjectScripts.shift(), 'object') as unknown as ReturnType<
        SdkGateway['streamObject']
      >
    },
    embedMany: async (args) => {
      calls.embedMany.push(args as Record<string, unknown>)
      const scripted = embedScripts.shift() ?? { embeddings: [[0.1, 0.2]], tokens: 4 }
      return {
    ...
    rerank: async (args) => {
      ...
      return { ranking: scripted.ranking } as unknown as Awaited<ReturnType<SdkGateway['rerank']>>
    },
  }
...
export function objectGenerationError(rawText: string): Error {
  return Object.assign(new Error('response did not match the expected schema'), {
