...
export function buildBaseArgs(
  ...
  const args: LoopArgs = {
    model: resolveAiSdkNativeModel(request.model) as LanguageModel,
    ...request.settings,
  };

  const systemArg = buildSystemArg(
  ...
  if (options.includeTools) {
    ...
    if (request.activeTools && request.activeTools.length > 0)
      args.activeTools = [...request.activeTools];
  ...
  if (request.extra?.providerOptions !== undefined) {
    args.providerOptions = mergeProviderOptions(
      args.providerOptions,
      request.extra.providerOptions,
    );
  }
  if (request.extra?.headers !== undefined)
    args.headers = request.extra.headers;
  if (request.extra?.maxRetries !== undefined)
    args.maxRetries = request.extra.maxRetries;
...
export function syncToolArgs(
  ...
  const approvalTools = request.tools
    ? request.toolApproval
      ...
      : request.tools
    : {};
  const next = installToolWireSchemas(approvalTools, request.toolWireSchemas);
  if (!current) {
  ...
  for (const [name, tool] of Object.entries(tools)) {
    ...
    result[name] =
      wire && isRecord(tool)
        ? {
            ...tool,
            inputSchema: jsonSchema(wire as Parameters<typeof jsonSchema>[0]),
          }
        : tool;
  }
  return result;
...
function mergeProviderOptions(current: unknown, next: unknown): unknown {
  ...
  for (const [provider, options] of Object.entries(next)) {
    ...
    merged[provider] = isRecord(options)
      ? { ...existing, ...options }
      : options;
  }
  return merged;
