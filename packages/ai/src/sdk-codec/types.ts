...
export interface AiSdkCallPlan<TMethod extends keyof SdkGateway, TDecoded> {
  ...
  decode(raw: Awaited<ReturnType<SdkGateway[TMethod]>>): TDecoded | Promise<TDecoded>
}
...
export interface AiSdkCodec {
  ...
  loop(request: ExecutorRequest<LanguageModel>): AiSdkCallPlan<'generateText', ExecutorOutcome<SdkLoopResultLike>>
  /** Plan one SDK structured-output attempt. */
  structured(request: StructuredRequest<LanguageModel>): Promise<AiSdkStructuredPlan>
  /** Plan one SDK stream call, text or structured. */
  stream(request: ExecutorRequest<LanguageModel> & { readonly schema?: z.ZodType }): Promise<AiSdkStreamPlan>
  /** Recreate an SDK-shaped stream handle from a cached result. */
  replayStream(cached: CachedStreamPayload): ExecutorStreamHandle<SdkStreamResultLike>
}
 ...
 * @internal
 */
export interface AiSdkStructuredPlan extends AiSdkCallPlan<'generateObject', StructuredAttempt<SdkLoopResultLike>> {
  /** Decode SDK validation/parse errors into core's invalid-attempt variant. */
  decodeError(error: unknown): Promise<StructuredAttempt<SdkLoopResultLike> | undefined>
}
...
export type AiSdkStreamPlan =
  | {
      ...
      attach(raw: ReturnType<SdkGateway['streamText']>): ExecutorStreamHandle<SdkStreamResultLike>
    }
  | {
      readonly method: 'streamObject'
      readonly args: Parameters<SdkGateway['streamObject']>[0]
      attach(raw: ReturnType<SdkGateway['streamObject']>): ExecutorStreamHandle<SdkStreamResultLike>
    }
