...
- **Treat the backend payload as authoritative**, per the existing UI rules in `packages/devtools/CLAUDE.md`. The UI should never reconstruct memory state, workspace file lists, plan versions, etc. from raw events client-side once these endpoints exist.
