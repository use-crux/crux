...
export type { ResolvedFnSource, ResolvedLocation, SourceLocation } from '@/shared/services/sourceResolver'
...
function scheduleBatch(): void {
  ...
  queueMicrotask(async () => {
    ...
    try {
      const locations = await sourceResolverService.resolveSources([...uniqueMap.values()])
      if (locations) {
 ...
 */
export function useResolvedSource(source: SourceLocation | undefined): ResolvedLocation | undefined {
 ...
 */
export function useResolvedSources(sources: Array<SourceLocation | undefined>): Array<ResolvedLocation | undefined> {
  const [, forceUpdate] = useState(0)
...
export function useResolvedFnSource(
  ...
  const [result, setResult] = useState<ResolvedFnSource | null | undefined>(undefined)
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      const value = await sourceResolverService.resolveFnSource({ file, line, column })
