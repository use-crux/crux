...
import { catchUpActionFromDelta, decideOnObservabilityRevisionEvent } from './runs-revision'
...
describe('catchUpActionFromDelta', () => {
  it('invalidates on an expired catch-up window', () => {
    expect(catchUpActionFromDelta({ revision: 10, changes: [], expired: true })).toBe('invalidate')
  })

  it('invalidates when the bounded delta reports real changes', () => {
    expect(
      catchUpActionFromDelta({ revision: 10, changes: [{ entity: 'run', id: 'run_1', revision: 10 }], expired: false }),
    ).toBe('invalidate')
  })

  it('is a no-op when the client is already current', () => {
    expect(catchUpActionFromDelta({ revision: 10, changes: [], expired: false })).toBe('noop')
  })
