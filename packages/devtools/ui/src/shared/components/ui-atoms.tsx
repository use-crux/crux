...
export function fmt(n: number | null | undefined, type: 'tok' | '$' | 'ms'): string {
  if (n == null) return '-'
  switch (type) {
    case 'tok':
...
export function Metric({
  ...
  return (
    <div>
      <div className="text-[11px] text-(--qw-fg-faint) leading-tight">{label}</div>
      <div className={`mt-1 text-sm font-medium tabular-nums leading-tight ${color}`}>
        {value}
        {sub && <span className="ml-1 text-[11px] font-normal text-(--qw-fg-faint)">{sub}</span>}
      </div>
    </div>
  )
}
...
export function SectionHead({ children }: { children: ReactNode }) {
  return <h4 className="mb-3 text-sm font-medium text-(--qw-fg)">{children}</h4>
}
...
export function StatCard({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <div className="border border-(--qw-border) bg-(--qw-bg-elev) p-4">
      ...
      <div className="mt-1 text-2xl font-semibold tabular-nums text-(--qw-fg)">{value}</div>
