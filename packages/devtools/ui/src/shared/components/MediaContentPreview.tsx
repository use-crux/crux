...
export function MediaContentPreview({ descriptor, label }: MediaContentPreviewProps) {
  ...
  const visible = [
    ...
    descriptor.sizeBytes !== undefined ? formatBytes(descriptor.sizeBytes) : undefined,
    dimensions(descriptor),
    descriptor.durationSeconds !== undefined ? `${formatNumber(descriptor.durationSeconds)}s` : undefined,
    descriptor.pageCount !== undefined ? `${formatNumber(descriptor.pageCount)} pages` : undefined,
  ...
  const accessible = [
    ...
    descriptor.durationSeconds !== undefined ? `${formatNumber(descriptor.durationSeconds)} seconds` : undefined,
    descriptor.pageCount !== undefined ? `${formatNumber(descriptor.pageCount)} pages` : undefined,
  ...
  return (
    <span aria-label={ariaLabel} className="inline-flex items-center gap-1 rounded border border-(--qw-border) px-1.5 py-0.5 text-(--qw-fg-muted)">
...
function accessibleDimensions(value: MediaContentDescriptor): string | undefined {
  if (value.width === undefined || value.height === undefined) return undefined
