...
export async function fetchJsonOr404<T>(path: string, signal?: AbortSignal): Promise<T | null> {
  const res = await fetch(apiUrl(path), { signal })
...
export async function deleteJson<TBody>(path: string, body?: TBody): Promise<Response> {
  return fetch(apiUrl(path), {
    method: 'DELETE',
