...
import {
  normalizeKind,
  stripRoot,
} from "@/features/index/components/IndexTree";
...
export function LintSeverityChip({
  severity,
  count,
}: {
  severity: Severity;
  ...
  return (
    ...
    >
      <span
        ...
        style={{ width: 5, height: 5, background: c.fg }}
      />
      {c.label}
      {count != null && (
        <span className="ml-0.5 font-mono text-[10.5px] opacity-80">
          {count}
        </span>
      )}
    </span>
  ...
  if (finding.maturity !== "stable") {
    const tone =
      finding.maturity === "preview"
        ? {
            fg: "var(--qw-warn)",
            border: "color-mix(in srgb, var(--qw-warn) 35%, transparent)",
          }
        : {
            fg: "var(--qw-iris)",
            border: "color-mix(in srgb, var(--qw-iris) 35%, transparent)",
          };
    items.push(
      <MetaChip
        key="mat"
...
export function resolveSuppressDirective(
