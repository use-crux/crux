...
export function IndexHealth({ index, indexedAt, connected, tabs }: IndexHealthProps) {
  const { navigate } = useNavigation()

  const findings = index.healthFindings
  const active = findings.filter((f) => !f.suppressed)
  const suppressedCount = findings.length - active.length
  const affected = new Set(active.map((f) => f.primaryDefinitionId).filter(Boolean)).size
  const warnCount = active.filter((f) => f.severity === 'warning' || f.severity === 'error').length
  ...
  return (
    ...
    >
      <div style={{ background: T.bg }}>
        <div style={{ maxWidth: 1180, margin: '0 auto', padding: '26px 32px 90px' }}>
