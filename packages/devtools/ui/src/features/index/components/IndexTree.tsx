              ...
              >
                ...
                {rollup.worstSeverity && rollup.count > 0 && (
                  ...
                  >
                    ...
                    <span className="text-[10px]" style={{ color: LINT_SEV_COLOR[rollup.worstSeverity].fg }}>
                      {rollup.count}
                    </span>
                  </span>
                )}
  ...
  return (
    <RowErrorBoundary rowKey={def.id}>
      ...
      >
        ...
        >
          {role && (
            <span className="shrink-0 text-[9px] uppercase tracking-[0.08em]" style={{ color: 'var(--qw-fg-faint)' }}>
              {role}
            </span>
          )}
...
export function foldedParentId(def: ProjectDefinition, defsById: Map<string, ProjectDefinition>): string | undefined {
...
export function buildFoldMap(
  ...
  for (const arr of m.values()) {
    arr.sort((a, b) => (a.metadata?.indexPresentation?.order ?? 0) - (b.metadata?.indexPresentation?.order ?? 0))
  }
  return m
}
