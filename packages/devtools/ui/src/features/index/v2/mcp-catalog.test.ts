...
describe("MCP Catalog projection", () => {
  ...
  it("projects exact MCP tool origin, schemas, fingerprints, and untrusted annotations", () => {
    ...
    expect(view).toHaveProperty(
      "inputSchema.properties.invoiceId.type",
      "string",
    );
  });
  ...
  it("distinguishes never-observed servers and never invents an owner join", () => {
    ...
    expect(
      mcpCatalogView(index.byId("mcp.server:billing")!, index),
    ).toMatchObject({
      kind: "server",
    ...
    expect(
      mcpCatalogView(toolIndex.byId("tool:billing_lookup")!, toolIndex),
    ).toMatchObject({
      kind: "tool",
  ...
  it("maps runtime availability to current and removed without a shadow state", () => {
    const currentIndex = buildIndex({
      ...
      definitions: indexData.definitions.map((definition) =>
        definition.id === "tool:billing_lookup"
          ? { ...definition, status: "active" as const }
          : definition,
      ),
    });
    expect(
      mcpCatalogView(currentIndex.byId("tool:billing_lookup")!, currentIndex),
    ).toMatchObject({
      kind: "tool",
      state: "current",
    });
    ...
    const removedIndex = buildIndex({
      ...
      definitions: indexData.definitions.map((definition) =>
        definition.id === "tool:billing_lookup"
          ? { ...definition, status: "removed" as const }
          : definition,
      ),
    });
    expect(
      mcpCatalogView(removedIndex.byId("tool:billing_lookup")!, removedIndex),
    ).toMatchObject({
      kind: "tool",
      state: "removed",
    });
