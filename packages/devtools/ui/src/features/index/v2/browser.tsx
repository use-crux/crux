...
function buildGroups(idx: IndexIndex, defs: ViewDef[], axis: string): Group[] {
  ...
  if (axis === 'quality') {
    ...
    defs.forEach((d) => {
      const q = d.quality ?? {}
      const k =
        (q.evalIds && q.evalIds.length) || (q.suiteIds && q.suiteIds.length) ? 'covered' : q.runCount ? 'runs' : 'none'
  ...
  return (
    ...
    >
      ...
      <span style={{ minWidth: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {children.length > 0 && (
          <span style={{ fontFamily: T.mono, fontSize: 9, color: T.fgFaint, flexShrink: 0 }}>+{children.length}</span>
        )}
      </span>
      <span style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
        {def.fidelity !== 'resolved' && (
          <span
            style={{ width: 5, height: 5, borderRadius: 99, background: def.fidelity === 'error' ? T.danger : T.warn }}
            title={def.fidelity}
          />
        )}
...
export function IndexBrowser({
  ...
  const defs = idx.standalone.filter((d) => {
    ...
    if (query && !`${d.name} ${d.kind} ${kindMeta(d.kind).label}`.toLowerCase().includes(query.toLowerCase()))
      return false
    return true
  })
                ...
                <div key={g.key} style={{ marginBottom: 4 }}>
                  ...
                  >
                    <Icon
                      ...
                      size={8}
                      color={T.fgFaint}
                      style={{ transform: isCol ? 'rotate(-90deg)' : 'none', transition: 'transform 120ms' }}
                    />
                    <span style={{ width: 7, height: 7, borderRadius: 99, background: gc.fg }} />
                    <span
                      style={{ fontFamily: g.mono ? T.mono : T.sans, fontSize: 11.5, fontWeight: 600, color: T.fg }}
                    >
                      {g.label}
                    </span>
