/**
 * Index v2 — Library / Index master–detail browser (content only).
 *
 * Ported from the design's index-browser.jsx. A compact finder rail
...
function buildGroups(idx: IndexIndex, defs: ViewDef[], axis: string): Group[] {
  const groups: Group[] = [];
  ...
  if (axis === "family") {
    INDEX_FAMILY_ORDER.forEach((fam) =>
      ensure(fam, { label: familyMeta(fam).label, tone: familyMeta(fam).tone }),
    );
    defs.forEach((d) => {
      const fam = (kindMeta(d.kind).family ?? "other") as FamilyId | "other";
      ensure(fam, {
        label: familyMeta(fam as FamilyId).label,
        tone: familyMeta(fam as FamilyId).tone,
      }).items.push(d);
    });
  ...
  if (axis === "health") {
    ...
    (["error", "warning", "info", "clean"] as const).forEach((k) =>
      ensure(k, meta[k]),
    );
    defs.forEach((d) => {
      const ls = idx
        .lintsForDef(d.id)
        .filter((f) => f.primaryDefinitionId === d.id);
        ...
        {def.fidelity !== "resolved" && (
          <span
            style={{
              ...
              height: 5,
              borderRadius: 99,
              background: def.fidelity === "error" ? T.danger : T.warn,
            }}
            title={def.fidelity}
          />
        )}
...
export function IndexBrowser({
  ...
  const defs = idx.standalone.filter((d) => {
    ...
    if (
      query &&
      !`${d.name} ${d.kind} ${kindMeta(d.kind).label}`
        .toLowerCase()
  ...
  return (
    <IndexSelectProvider select={onSelect}>
      <div
        style={{
          display: "flex",
          height: "100%",
          ...
          >
            ...
            >
              <button
                type="button"
                onClick={() =>
                  setFams(allOn ? new Set() : new Set(INDEX_FAMILY_ORDER))
                }
                style={{
                  all: "unset",
                  cursor: "pointer",
              ...
              {INDEX_FAMILY_ORDER.map((fam) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      style={{ fontFamily: T.mono, fontSize: 9, opacity: 0.8 }}
                    >
                      {famCounts[fam] ?? 0}
                    </span>
                  </button>
        ...
        {graphOpen && (
          ...
          >
            ...
            >
              ...
              <span
                style={{ fontFamily: T.sans, fontSize: 12.5, fontWeight: 600 }}
              >
                Architecture graph
              </span>
              <span style={{ fontSize: 11, color: T.fgFaint }}>
