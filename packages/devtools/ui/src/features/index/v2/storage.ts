...
export type StorageComponentSummary = Partial<
  Record<StorageComponentKey, string>
>;
...
function storageKind(value: unknown): StorageDefinitionKind | undefined {
  return typeof value === "string" && STORAGE_KINDS.has(value)
    ? (value as StorageDefinitionKind)
    : undefined;
}
...
function storageRuntime(raw: unknown): StorageRuntimeSummary | undefined {
  ...
  return Object.values(runtime).some((value) => value != null)
    ? runtime
    : undefined;
}
...
function storageCapabilities(
  raw: unknown,
): IndexedStorageCapabilities | undefined {
  if (!isRecord(raw)) return undefined;
  ...
  const filtered: IndexedStorageCapabilities = {
    ...
    ...(capabilities.search ? { search: capabilities.search } : {}),
  };
  return filtered.record || filtered.search ? filtered : undefined;
}

/** Returns true when a view definition represents a first-class Storage Beta definition. */
export function isStorageDef(
  def: ViewDef | undefined,
): def is ViewDef & { kind: StorageDefinitionKind } {
 ...
 */
export function storageSummaryForDef(
  def: ViewDef,
): StorageReadModelSummary | undefined {
  ...
  return {
    ...
    capabilities:
      storageCapabilities(storage?.capabilities) ?? facts?.capabilities,
...
export function storageInventoryForIndex(
  index: Pick<IndexIndex, "defs">,
