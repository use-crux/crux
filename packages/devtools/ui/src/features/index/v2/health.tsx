...
function byKind(
  list: readonly HealthFinding[],
  key: keyof HealthFinding,
      ...
      {shown.map((s) => {
        ...
        return (
          ...
          >
            ...
            <span
              style={{ color: m.solid ? c.fg : T.fgMuted, fontWeight: 600 }}
            >
              {counts[s]}
            </span>
                ...
                {ev.map((e, i) => (
                  ...
                  >
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div
                        style={{ fontSize: 11.5, color: T.fg, lineHeight: 1.4 }}
                      >
                        {e.note}
                      </div>
                    </div>
...
function FindingRow({
  ...
  return (
    <div
      style={{ borderBottom: `1px solid ${T.border}`, opacity: sup ? 0.55 : 1 }}
    >
      <button
        type="button"
        onClick={onToggle}
        ...
        >
          {!isDirect && (
            ...
            >
              via{" "}
              <KindBadge
                kind={fnd.primaryKind}
                label={fnd.primaryDefinitionId ?? "—"}
              />
            </span>
          )}
          {!open && (
      ...
      >
        ...
        <span style={{ marginLeft: "auto", display: "inline-flex", gap: 8 }}>
          <Btn
            ...
            variant="outline"
            disabled
            title="Opening source files isn’t available yet"
          >
            Open in editor
          </Btn>
        </span>
...
export function IndexHealthOverview() {
  ...
  const byDef: Record<
    string,
    { id: string; kind: string; items: HealthFinding[] }
  > = {};
  for (const f of all) {
  ...
  const affected = Object.values(byDef)
    .map((d) => ({
      ...
      max:
        [...d.items.map((f) => f.severity)].sort(
          (a, b) => (SEV_ORDER[b] ?? 0) - (SEV_ORDER[a] ?? 0),
        )[0] ?? "info",
    }))
    .sort(
      (a, b) =>
        b.count - a.count || (SEV_ORDER[b.max] ?? 0) - (SEV_ORDER[a.max] ?? 0),
    )
    .slice(0, 6);
    ...
    <div style={{ background: T.bg, color: T.fg, fontFamily: T.sans }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
        ...
        >
          ...
          >
            ...
            <span
              style={{
                fontFamily: T.mono,
                fontSize: 12,
                color: rules.length ? T.warn : T.ok,
              }}
            >
              {rules.length}
            </span>
          ...
          <Bar
            value={rules.length}
            max={Math.max(1, rules.length)}
            tone={rules.length ? "warn" : "ok"}
            height={8}
          />
                ...
                >
                  <span
                    style={{
                      fontFamily: T.mono,
                      fontSize: 11,
                      color: cat === "safety" ? T.danger : T.fgMuted,
                    }}
                  >
                    {cat}
                  </span>
