...
export function CatSchemaField({
  ...
  return (
    <div style={{ position: 'relative', paddingLeft: indent }}>
      ...
      {depth > 0 && (
        <span style={{ position: 'absolute', left: indent - 8, top: 14, width: 6, height: 1, background: T.border }} />
      )}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {refs.map((r) => {
        ...
        return (
          <div
            key={r.id}
            style={{ background: T.bgElev, border: `1px solid ${T.border}`, borderRadius: 10, overflow: 'hidden' }}
          >
            <button
              type="button"
...
function CatEffectiveInput({ contract }: { contract: ContractView }) {
  ...
  return (
    <IntelCard
      ...
      right={
        <span style={{ fontFamily: T.mono, fontSize: 10.5, color: T.fgFaint }}>
          {total} fields · {authored.length} authored · {contribs.length} contributed
        </span>
      }
    >
      <p
      ...
      >
        What this definition effectively expects once statically-visible injected contexts &amp; injectables are
        followed.{' '}
        <span style={{ color: T.fg }}>
          Always-injected required fields stay required; conditional contributions are included but optional.
        </span>
      </p>
        ...
        return (
          <div key={key} style={{ marginTop: 12, borderLeft: `2px solid ${c.line}`, paddingLeft: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
              ...
              <span style={{ fontFamily: T.mono, fontSize: 11.5, fontWeight: 600, color: c.fg }}>
...
export function IndexContract({ def }: { def: ViewDef }) {
  ...
  const meta = [hasEffective && 'effective input', ...cols.map((s) => s.title.toLowerCase())]
    .filter(Boolean)
    .join(' · ')
  return (
    <>
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: (cols.length || hasEffective) && pair ? '1fr 1fr' : '1fr',
          gap: 16,
          marginBottom: 22,
          alignItems: 'start',
        }}
      >
        ...
        {cols.map((s) => (
          <IntelCard
            ...
            tone={s.tone}
            right={
              <span style={{ fontFamily: T.mono, fontSize: 10.5, color: T.fgFaint }}>{s.fields.length} fields</span>
            }
          >
            {s.fields.map((f, i) => (
        ...
        <IntelCard title="Execution" tone="blue">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            ...
            {ctl.retryPolicy &&
              ...
              kv('non-retryable', ctl.retryPolicy.nonRetryableErrors.join(', '))}
            {ctl.fallbackPolicy &&
              kv(
                'fallback',
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {d.artifacts.map((a, i) => (
                <div
                  key={i}
                  style={{ padding: '7px 10px', background: T.bg, border: `1px solid ${T.border}`, borderRadius: 7 }}
                >
  ...
  return (
    <>
      <SectionHead
        ...
        right={<span style={{ fontFamily: T.mono, fontSize: 11, color: T.fgFaint }}>{keys.length} params</span>}
      />
      <div
        style={{
          background: T.bgElev,
