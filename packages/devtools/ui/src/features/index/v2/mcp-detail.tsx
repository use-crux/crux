...
function transportLabel(view: McpServerCatalogView): string | undefined {
  ...
  if (transport.kind === 'stdio') return [transport.kind, transport.executable].filter(Boolean).join(' · ')
  if (transport.kind === 'streamable-http')
    return [transport.kind, transport.origin, transport.pathname].filter(Boolean).join(' · ')
...
function McpServerDetail({ view }: { view: McpServerCatalogView }) {
  ...
  return (
    <>
      <SectionHead eyebrow="MCP server" right={<StateChip state={view.state} />} />
      <Card>
        ...
        <Row label="failure">{view.failure ? `${view.failure.phase} · ${view.failure.category}` : undefined}</Row>
        <Row label="last success">{view.lastSuccessfulDiscovery?.observedAt}</Row>
        <Row label="implementation">{view.lastSuccessfulDiscovery?.implementation}</Row>
        <Row label="protocol">{view.lastSuccessfulDiscovery?.protocolVersion}</Row>
        <Row label="server-reported">
          {view.lastSuccessfulDiscovery?.server
            ? [view.lastSuccessfulDiscovery.server.name, view.lastSuccessfulDiscovery.server.version, 'untrusted']
                .filter(Boolean)
                .join(' · ')
            : undefined}
        </Row>
      ...
      <SectionHead eyebrow="Provided tools" right={<span style={{ color: T.fgFaint }}>{view.tools.length}</span>} />
      <Card>
        {view.tools.length === 0 ? (
...
function schemaFields(schema: McpToolCatalogView['inputSchema']): string {
  ...
  return properties && typeof properties === 'object' ? Object.keys(properties).join(', ') : 'Schema captured'
}
...
function McpToolDetail({ view }: { view: McpToolCatalogView }) {
  ...
  return (
    <>
      <SectionHead eyebrow="MCP origin" right={<StateChip state={view.state} />} />
      <Card>
        <Row label="server">
        ...
        {view.inputSchema && <Row label="input schema">{schemaFields(view.inputSchema)}</Row>}
        {view.outputSchema && <Row label="output schema">{schemaFields(view.outputSchema)}</Row>}
      </Card>

      {view.annotations && (
        <>
          <SectionHead eyebrow="Untrusted server annotation" right={<Chip tone="warn">Untrusted</Chip>} />
          <Card>
...
export function IndexMcpDetail({ def }: { def: ViewDef }) {
  ...
  if (!view) return null
  return view.kind === 'server' ? <McpServerDetail view={view} /> : <McpToolDetail view={view} />
}
