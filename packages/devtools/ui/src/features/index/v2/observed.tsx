...
function WindowNote({
  ...
  const label =
    window ?? (runCount != null ? `last ${runCount} runs` : "recent traces");
  return (
    <span
      ...
      {keys.map((k) => (
        ...
        >
          <Icon
            name={glyph}
            size={9}
            color={tone === "muted" ? T.fgFaint : c.fg}
          />
          {k}
        </span>
      ))}
...
export function CatRuntimeInput({ observed }: { observed: ObservedInjection }) {
  ...
  return (
    <ObsCard
      ...
      glyph="inbox"
      right={
        <WindowNote window={observed.window} runCount={observed.runCount} />
      }
    >
      <p
        style={{
      ...
      >
        Which keys real calls actually passed, checked against the effective
        schema.{" "}
      ...
      {passRate != null && (
        ...
        >
          ...
          <span
            style={{ fontFamily: T.mono, fontSize: 10.5, color: T.fgMuted }}
          >
            {Math.round(passRate * 100)}% pass
            {input.validateCount != null
              ? ` · ${input.validateCount} calls`
              : ""}
          </span>
        </div>
      )}
                ...
                {tools.map((tl) => (
                  ...
                  >
                    <CatIcon
                      name="tool"
                      size={9}
                      color={toneColor(T, "ok").fg}
                    />
                    {tl.name}
                    {tl.count != null && (
                      <span style={{ color: T.fgFaint }}>· {tl.count}</span>
                    )}
                  </span>
                ))}
              </div>
...
export function CatObservedInjection({
  observed,
}: {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ color: T.fg }}>
          checked ≠ dropped; unobserved ≠ impossible.
        </span>
      </p>
      {sources.map((src, i) => (
        <ObservedSourceRow
          key={src.sourceDefinitionId ?? src.variable ?? i}
          src={src}
        />
      ))}
    </ObsCard>
...
> = {
  ...
  spread: {
    tone: "blue",
    blurb: "A known spread — members resolved, shape stable.",
  },
  dynamic: {
    tone: "muted",
    blurb: "Computed at runtime — could not be resolved statically.",
  },
};
...
export function CatContributes({
  contributions,
}: {
  ...
  return (
    <ObsCard title="Contributes" tone="iris" glyph="layers">
      ...
      >
        The non-tool material this injectable folds in, each tagged by how
        reliably Crux could resolve it.
      </p>
...
export function CatObservedSection({ def }: { def: ViewDef }) {
  ...
  if (
    !observed ||
    (!observed.input && !(observed.sources && observed.sources.length))
  )
    return null;
  return (
    <>
      ...
      >
        ...
        {observed.sources && observed.sources.length > 0 && (
          <CatObservedInjection observed={observed} />
        )}
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          Observed evidence is a quiet annotation over authored truth — authored
          stays primary.
        </div>
      </div>
    </>
