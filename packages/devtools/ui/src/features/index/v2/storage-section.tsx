...
import { storageSummaryForDef, type StorageReadModelSummary, type StorageWarningSeverity } from './storage'
...
function Panel({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div style={{ background: T.bgElev, border: `1px solid ${T.border}`, borderRadius: 9, padding: 14, minWidth: 0 }}>
      <div
        style={{
          fontFamily: T.mono,
          fontSize: 10,
...
function KeyValueList({ rows }: { rows: Row[] }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 7 }}>
      {rows.map((row) => (
        <div key={row.label} style={{ display: 'grid', gridTemplateColumns: '96px minmax(0, 1fr)', gap: 10 }}>
          <span style={{ fontFamily: T.mono, fontSize: 11, color: T.fgFaint }}>{row.label}</span>
          <span style={{ fontFamily: T.mono, fontSize: 11.5, color: T.fg, minWidth: 0, overflowWrap: 'anywhere' }}>
            {row.value}
          </span>
        </div>
      ))}
...
function runtimeRows(summary: StorageReadModelSummary): Row[] {
  ...
  const rows: Array<{ label: string; value: ReactNode }> = [
    ...
    { label: 'latency', value: runtime.avgLatencyMs == null ? undefined : `${runtime.avgLatencyMs}ms` },
...
function warningTone(severity: StorageWarningSeverity): Tone {
  return severity === 'error' ? 'danger' : severity === 'warning' ? 'warn' : 'muted'
}
        ...
        {entries.map(([label, id]) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ fontFamily: T.mono, fontSize: 11, color: T.fgFaint }}>{label}</span>
              <span style={{ fontFamily: T.mono, fontSize: 11.5, color: T.fg, overflowWrap: 'anywhere' }}>{id}</span>
            </button>
          )
        })}
      </div>
      ...
      <div style={{ display: 'grid', gap: 7 }}>
        {summary.usedBy.map((use) => {
          ...
          return (
            ...
            >
              {target ? <KindGlyph kind={target.kind} size={22} /> : <span />}
              <span style={{ fontFamily: T.mono, fontSize: 11.5, color: T.fg, overflowWrap: 'anywhere' }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              {warning.relatedDefinitionIds?.map((id) => (
                <span key={id} style={{ fontFamily: T.mono, fontSize: 10.5, color: T.fgMuted }}>
                  {id}
                </span>
              ))}
            </div>
            <div style={{ fontFamily: T.serif, fontSize: 13, lineHeight: 1.5, color: T.fg }}>{warning.message}</div>
          </div>
        )
      })}
    </div>
...
export function IndexStorage({ def }: { def: ViewDef }) {
  ...
  const visibleOverviewRows = overviewRows.filter((row): row is Row => row.value != null)
      ...
      <SectionHead
        ...
        right={
          summary.warnings.length ? (
            <Chip tone={summary.warnings.some((w) => w.severity === 'error') ? 'danger' : 'warn'} dot>
