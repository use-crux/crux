...
describe('index storage view model', () => {
    ...
    definitions: [
      {
        ...
        metadata: {
          facts: {
            ...
            capabilities: { record: { ttl: 'lazy', filter: 'scan', watch: true, batch: false } },
          },
          storage: {
            kind: 'storage.recordStore',
            backend: 'inMemoryRecordStore',
      ...
      },
      {
        ...
        metadata: {
          facts: {
            ...
            capabilities: { vector: { dense: true, sparse: false, hybrid: false, fusion: [], filter: 'post' } },
          },
        },
      },
      {
        ...
        status: 'active',
        metadata: {
          facts: { kind: 'storage.bundle', records: 'records', vectors: 'vectors' },
          storage: {
            kind: 'storage.bundle',
            components: {
            ...
            warnings: [
              {
                ...
                severity: 'warning',
                message: 'Retriever is wired to a vector store that filters after search.',
                primaryDefinitionId: 'storage.bundle:appStorage',
                relatedDefinitionIds: ['rag.retriever:docs', 'storage.vectorStore:vectors'],
              },
            ],
          },
        },
      },
      {
        ...
        metadata: {
          ...
          intelligence: { confidence: 'static', dependencies: { storage: ['appStorage'], vectorStores: ['vectors'] } },
        },
      },
    ],
    relations: [
  ...
  it('registers storage definitions in the state family', () => {
    expect(kindMeta('storage.bundle')).toMatchObject({ label: 'Storage bundle', family: 'state' })
    expect(kindMeta('storage.vectorStore')).toMatchObject({ label: 'Vector store', family: 'state' })
  })
