...
function toolState(definition: ViewDef): McpCatalogToolState {
  ...
  return definition.facts?.mcp?.provenance === "runtime-discovered" &&
...
function serverView(
  definition: ViewDef,
  index: IndexIndex,
): McpServerCatalogView | undefined {
  const metadata = definition.raw.metadata as
  ...
  const selection =
    allow || deny || prefix ? { allow, deny, prefix } : undefined;
  ...
  const supportedImplementation =
    implementation === "official-client" || implementation === "ai-sdk-native"
      ? implementation
      : undefined;
...
function toolView(
  ...
  const owner = index
    ...
    .incoming.find(
      (relation) =>
        relation.type === "mcp.server.provides_tool" &&
        index.byId(relation.from)?.kind === "mcp.server",
    );
  ...
  return {
    ...
    annotations:
      annotations?.untrusted === true && annotationValue
        ? { untrusted: true, value: annotationValue }
        : undefined,
  };
}
