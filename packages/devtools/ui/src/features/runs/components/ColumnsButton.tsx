...
import { ALL_COLUMN_IDS, COLUMN_DEFS, DEFAULT_VISIBLE_COLUMNS, REQUIRED_COLUMNS } from '../lib/run-columns'
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          columns
          <span style={{ color: 'var(--qw-fg-faint)' }}>· {visible.length}</span>
        </button>
      </PopoverTrigger>
      <PopoverContent
        align="end"
      ...
      >
        <PopoverSection title={`Columns · ${visible.length}/${ALL_COLUMN_IDS.length}`}>
          ...
          >
            <button
              type="button"
              onClick={() => onChange(ALL_COLUMN_IDS.filter((id) => REQUIRED_COLUMNS.includes(id)))}
