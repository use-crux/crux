...
type FilterKind = "status" | "target" | "model" | "last" | "search";
...
export function RunsFilterBar({
  ...
  const addOptions: ReadonlyArray<AddFilterOption<FilterKind>> = [
    { kind: "status", label: "Status", enabled: !activeKinds.has("status") },
    {
      kind: "target",
      label: "Target",
      enabled: !activeKinds.has("target") && distinctTargets.length > 0,
    },
    {
      kind: "model",
      label: "Model",
      enabled: !activeKinds.has("model") && distinctModels.length > 0,
    },
    ...
    >
      ...
      {activeKinds.has("target") && (
        <MultiSelectChip
          ...
          onChange={(next) =>
            update("target", next.length > 0 ? next : undefined)
          }
        />
      )}
      ...
      {activeKinds.has("model") && (
        <MultiSelectChip
          ...
          onChange={(next) =>
            update("model", next.length > 0 ? next : undefined)
          }
      ...
      <AddFilterButton
        ...
        onAdd={(kind) => {
          ...
          else if (kind === "model" && distinctModels[0])
            update("model", [distinctModels[0]]);
