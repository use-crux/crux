...
import { annotateRunRowWithQuality, qualityAnnotationsByRunId, rowFromRunSummary, runsPageOptionsFromFilters } from './run-mappers'
...
describe('runs row mapping', () => {
  ...
  it('indexes Quality annotations by traceId (== observability runId)', () => {
    const quality = { _tag: 'QualityRun', traceId: 'run-1', feedbackCount: 3 } as QualityRunRecord
  ...
  it('maps status/time-range filters onto the server-side page request', () => {
    const opts = runsPageOptionsFromFilters({ status: ['ok', 'error'], last: 'all' })
  ...
  it('reflects a live-to-terminal transition as the same row identity with an updated status', () => {
    ...
    expect(live).toMatchObject({ traceId: 'run_transition', status: 'running' })

    const terminal = rowFromRunSummary({ ...base, status: 'ok', endedAt: '2026-07-03T10:00:01.000Z', revision: 2 })
    expect(terminal).toMatchObject({ traceId: 'run_transition', status: 'ok', revision: 2 })
  ...
  it('reflects suspend/resume as a multi-segment row without treating the pause as terminal', () => {
    ...
    expect(resumed).toMatchObject({ status: 'running', segmentCount: 2, activeSegmentId: 'segment_2' })
  })
  ...
  it('does not let late stream metadata change the already-committed duration or status', () => {
    ...
    const withoutLateUsage = rowFromRunSummary({ ...summaryBase, revision: 5, metrics: { totalTokens: 100 } })
