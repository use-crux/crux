...
export const COLUMN_DEFS: readonly ColumnDef[] = [
  ...
  { id: 'fdbk', label: 'fdbk', width: '60px', align: 'right' },
  { id: 'tools', label: 'tools', width: '60px', align: 'right' },
  { id: 'spans', label: 'spans', width: '60px', align: 'right' },
  { id: 'session', label: 'session', width: '90px' },
  { id: 'cassette', label: 'cassette', width: '88px' },
...
export function loadVisibleColumns(): readonly ColumnId[] {
  ...
  try {
    ...
    const ids = raw.split(',').filter((s): s is ColumnId => (ALL_COLUMN_IDS as readonly string[]).includes(s))
...
export function gridTemplateFor(visible: readonly ColumnId[]): string {
  ...
  const tracks = COLUMN_DEFS.filter((c) => visibleSet.has(c.id)).map((c) => c.width)
