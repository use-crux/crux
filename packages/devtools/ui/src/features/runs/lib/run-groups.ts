...
export function rowsForTab(rows: readonly RunRow[], tab: RunsTab): readonly RunRow[] {
  if (tab === 'live') return rows.filter((run) => run.status === 'running')
  if (tab === 'failures') return rows.filter(isFailureStatus)
  if (tab === 'has-feedback') return rows.filter((run) => run.feedbackCount > 0)
  return rows
}
...
export function summarizeRunGroup(rows: readonly RunRow[]): RunGroupSummary {
  ...
  return {
    totalCost,
    totalTokens,
    avgDurationMs: durationCount > 0 ? totalDuration / durationCount : undefined,
    failCount,
  }
}
...
export function countRuns(rows: readonly RunRow[]) {
  return {
    ...
    feedback: rows.filter((run) => run.feedbackCount > 0).length,
  }
}
