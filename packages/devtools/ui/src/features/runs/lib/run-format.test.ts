...
describe('reliabilityParts / reliabilityTone', () => {
  it('lists each non-trivial signal as a terse fragment', () => {
    expect(reliabilityParts({ segmentCount: 2, gapCount: 1, traceAliasConflict: true, orderingConfidence: 'partial', deliveryHealth: 'degraded' })).toEqual([
      '2 segments',
      '1 sequence gap',
  ...
  it('a trace alias conflict always wins the tone, even alongside degraded delivery', () => {
    expect(reliabilityTone({ traceAliasConflict: true, deliveryHealth: 'degraded' })).toBe('danger')
  })

  it('degraded delivery without a conflict uses the shared deliveryHealthTone', () => {
    expect(reliabilityTone({ deliveryHealth: 'degraded' })).toBe(deliveryHealthTone('degraded'))
  })
...
describe('explainRunReliability', () => {
  it('explains a suspended run, mentioning segment count only when non-trivial', () => {
    ...
    expect(explainRunReliability({ status: 'suspended', segmentCount: 3 })).toBe(
  ...
  it('explains an incomplete run, citing gap count when present', () => {
    ...
    expect(explainRunReliability({ status: 'incomplete', gapCount: 2 })).toContain('2 sequence gaps')
  })

  it('explains a conflicted run differently for a trace alias conflict vs. other terminal evidence conflicts', () => {
    expect(explainRunReliability({ status: 'conflicted', traceAliasConflict: true })).toContain('trace alias')
    expect(explainRunReliability({ status: 'conflicted' })).toContain('stored terminal evidence')
  })

  it('explains degraded delivery health, noting partial ordering when present', () => {
    expect(explainRunReliability({ deliveryHealth: 'degraded' })).toContain('degraded')
    expect(explainRunReliability({ deliveryHealth: 'degraded', orderingConfidence: 'partial' })).toContain(
      'causal display order',
    )
  })

  it('returns undefined for a calm, terminal, healthy run', () => {
    expect(explainRunReliability({ status: 'ok', deliveryHealth: 'healthy' })).toBeUndefined()
  })
})
