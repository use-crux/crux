...
function MemoryOverview() {
  ...
  const details = useMemo(
    () =>
      detailQueries
        .map((q) => q.data)
        .filter((d): d is MemoryStoreDetail => Boolean(d)),
    [detailQueries],
  );
  ...
  const kpis = useMemo(() => {
    ...
    for (const s of list) {
      ...
      byType[s.type as MemoryStoreType] =
        (byType[s.type as MemoryStoreType] ?? 0) + 1;
      ...
      if (tr) {
        if (!readsTrend)
          readsTrend = Array.from({ length: tr.length }, () => 0);
        for (let i = 0; i < tr.length && i < readsTrend.length; i++)
          readsTrend[i] += tr[i] ?? 0;
      }
      if (tw) {
        if (!writesTrend)
          writesTrend = Array.from({ length: tw.length }, () => 0);
        for (let i = 0; i < tw.length && i < writesTrend.length; i++)
          writesTrend[i] += tw[i] ?? 0;
      }
    }
    return {
      reads,
  ...
  const operations = useMemo<readonly MemoryOperationRecord[]>(
    () => opsData ?? [],
    [opsData],
  );
    ...
    >
      <div className="mx-auto w-full max-w-7xl px-8 py-6">
        <div className="mb-5 grid grid-cols-4 gap-3">
          <Kpi
            ...
            sublabel={
              [
                ...
                kpis.byType.episodic
                  ? `${kpis.byType.episodic} episodic`
                  : null,
                kpis.byType.semantic
                  ? `${kpis.byType.semantic} semantic`
                  : null,
                kpis.byType.blackboard
                  ? `${kpis.byType.blackboard} blackboard`
                  : null,
              ]
                .filter(Boolean)
                .join(" · ") || "none"
          ...
          />
          <Kpi
            label="Reads · all stores"
            value={kpis.reads.toLocaleString()}
            trend={kpis.readsTrend ?? undefined}
          />
          <Kpi
            ...
            sublabel={
              kpis.conflicts > 0
                ? `${kpis.conflicts} conflict${kpis.conflicts === 1 ? "" : "s"}`
                : "0 conflicts"
            }
          />
          <Kpi
            label="Carry-forward"
            value={
              kpis.carryEntries > 0 ? kpis.carryEntries.toLocaleString() : "—"
            }
            ...
            <div className="mb-6 grid grid-cols-2 gap-3">
              {filtered.map((s) => (
                <StoreCard
                  ...
                  store={s}
                  onOpen={() =>
                    navigate({ view: "library-memory", memoryId: s.id })
                  }
        ...
        {filter === "all" && (spotlight.working || spotlight.episodic) && (
          ...
          >
            <SpotlightWorking
              ...
              onOpen={(id) =>
                navigate({ view: "library-memory", memoryId: id })
              }
            />
            <SpotlightEpisodic
      ...
      actions={
        <>
          <Btn
            variant="ghost"
            size="sm"
            onClick={() => navigate({ view: "library-memory" })}
          >
            ← All stores
          </Btn>
          <Btn
...
function DetailBody({ store }: { store: MemoryStoreDetail }) {
  return (
    <>
      {store.inspection && (
        <MemoryInspectionNotice inspection={store.inspection} />
      )}
...
function DetailBodyInner({ store }: { store: MemoryStoreDetail }) {
  switch (store.state.type) {
    ...
    default:
      return (
        <EmptyHint>
          Unknown memory type: {(store.state as { type: string }).type}
        </EmptyHint>
      );
  }
}
    ...
    <>
      <LDHeaderStrip
        ...
        stats={[
          {
            label: "Chunks",
            value: fmtCount(index?.chunkCount ?? chunks.length),
          },
          {
            label: "Sources",
            value: fmtCount(index?.sourceCount ?? topSources.length),
          },
          ...
          ...(avgTop != null
            ? [
                {
                  label: "Avg top",
                  value: avgTop.toFixed(2),
                  color: scoreTone(avgTop),
                },
              ]
            : []),
      ...
      >
        ...
        <div className="flex flex-col gap-3.5">
          <LDCard title="Index stats" color={m.color} padding="12px 14px">
            <div className="flex flex-col gap-2">
              {[
                ...
                [
                  "Dimensions",
                  index?.dimensions != null ? String(index.dimensions) : "—",
                ],
          ...
          <LDCard
            title={`Source documents · top ${topSources.length}`}
            padding="12px 14px"
          >
            {topSources.length === 0 ? (
          ...
          <SchemaCard
            schema={store.schema}
            color={m.color}
            authoringHint="semanticMemory({ chunk })"
          />
...
function SemanticChunkRow({
  chunk,
  last,
}: {
  chunk: MemorySemanticChunk;
  last: boolean;
}) {
  return (
    <div
...
function SemanticQueryLog({
  ...
  const hasHits = queries.some(
    (q) => q.hitChunkIds && q.hitChunkIds.length > 0,
  );
      ...
      >
        <TableHeader
          cols={[
            ...
            ...(hasHits
              ? [{ label: "top hits", width: "minmax(0, 1fr)" }]
              : []),
            ...(hasLat
              ? [{ label: "lat", width: "60px", align: "right" as const }]
              : []),
            ...(hasTrace
        ...
        {queries.map((q, i) => (
          ...
          >
            ...
            <span
              ...
              style={{ color: scoreTone(q.topScore) }}
            >
...
function BlackboardDetail({
  store,
  state,
}: {
  store: MemoryStoreDetail;
  state: MemoryBlackboardState;
}) {
  ...
  const conflictFields = useMemo(
    () => fields.filter((f) => (f.conflicts ?? 0) > 0),
    [fields],
  );
  ...
  const mergedLiveFields = useMemo<
    readonly MemoryBlackboardField[] | null
  >(() => {
                  ...
                  tabs={[
                    {
                      value: "live",
                      label: "Live",
                      count: liveFields4Display.length,
                    },
                    {
                      value: "conflicts",
                      label: "Conflicts",
    ...
    <>
      <TableHeader
        cols={[
          ...
          ...(hasWrittenAt
            ? [{ label: "at", width: "70px", align: "right" as const }]
            : []),
        ]}
      />
      {fields.map((f, i) => (
...
function BlackboardChangeLog({
  log,
}: {
  log: readonly MemoryBlackboardChange[];
}) {
  if (log.length === 0) {
    return (
      <section>
      ...
      >
        <TableHeader
          cols={[
            ...
            ...(hasResolved
              ? [{ label: "resolution", width: "minmax(0, 1.4fr)" }]
              : []),
          ]}
        />
        {log.map((l, i) => (
