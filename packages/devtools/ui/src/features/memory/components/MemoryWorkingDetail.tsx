...
import type { MemoryStoreDetail, MemoryWorkingField, MemoryWorkingMutation, MemoryWorkingState } from '@/types'

export function WorkingDetail({ store, state }: { store: MemoryStoreDetail; state: MemoryWorkingState }) {
  ...
  const updates = muts.filter((m) => m.op === 'update' || m.op === 'append').length
      ...
      <LDHeaderStrip
        ...
        stats={[
          ...
          { label: 'Updates', value: updates || 0 },
          {
            ...
            color: (store.stats?.conflicts ?? 0) > 0 ? 'var(--qw-warn)' : 'var(--qw-fg-faint)',
          },
      ...
      >
        <TableHeader
          cols={[
            ...
            ...(hasSpan ? [{ label: 'source span', width: 'minmax(0, 1fr)' }] : []),
          ...
          <div
            ...
            style={{
              gridTemplateColumns: [
                ...
                .filter(Boolean)
                .join(' '),
              borderBottom: i === muts.length - 1 ? 'none' : '1px solid var(--qw-border)',
            }}
          >
            <span style={{ color: 'var(--qw-fg-faint)' }}>{fmtTime(m.timestamp)}</span>
            <LDOpPill op={m.op} />
            <span style={{ color: 'var(--qw-crux)' }}>{m.key}</span>
            <span className="truncate" style={{ color: 'var(--qw-fg-muted)' }} title={fmtValue(m.before)}>
              {fmtValue(m.before)}
            </span>
            <span className="truncate" style={{ color: 'var(--qw-fg)' }} title={fmtValue(m.after)}>
              {fmtValue(m.after)}
            </span>
            {hasSpan && (
              <span
