...
export function DefinitionBindingCard({ store, note }: { store: MemoryStoreDetail; note?: string }) {
  ...
  if (store.captureMode) {
    rows.push(['capture', <span style={{ color: 'var(--qw-fg)' }}>{store.captureMode}</span>])
  }
  if (store.budget) {
    rows.push(['budget', <span style={{ color: 'var(--qw-fg)' }}>{formatBudget(store.budget)}</span>])
  }
  if (store.scope) {
    rows.push([
      'scope',
    ...
    <LDCard title="Binding" padding="12px 14px">
      ...
      {store.blocks && store.blocks.length > 0 && (
        <div className="mt-3 flex flex-col gap-2 border-t border-dashed border-(--qw-border) pt-3">
          <div className="font-mono text-[10px] uppercase tracking-[0.08em]" style={{ color: 'var(--qw-fg-faint)' }}>
            blocks
          </div>
          <div className="flex flex-col gap-1.5">
            {store.blocks.map((block, index) => (
              <MemoryBlockBindingRow key={`${block.id ?? block.kind ?? 'block'}:${index}`} block={block} />
            ))}
          </div>
        </div>
      )}
...
function MemoryBlockBindingRow({ block }: { block: MemoryBlockMetadata }) {
  ...
  return (
    ...
    >
      <span className="mr-1 font-mono text-[11.5px]" style={{ color: 'var(--qw-crux)' }} title={label}>
        {label}
      </span>
      {block.kind && (
      ...
      {block.renderStrategy && (
        <Chip tone={block.renderStrategy === 'disabled' ? 'warn' : block.renderStrategy === 'semantic' ? 'ok' : 'iris'} mono>
...
function formatBudget(budget: Record<string, unknown>): string {
  ...
  if (typeof maxTokens === 'number') return `${maxTokens.toLocaleString()} tokens`
