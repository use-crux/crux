...
export function EpisodicDetail({
  store,
  state,
}: {
  store: MemoryStoreDetail;
  state: MemoryEpisodicState;
}) {
  ...
  const avgConf = useMemo(() => {
    ...
    return (
      withConf.reduce((a, e) => a + (e.confidence ?? 0), 0) / withConf.length
    );
  }, [entries]);

  // Provenance is the rich story here: how many distinct subsystems wrote episodes.
  const sourceCount = useMemo(
    () => new Set(entries.map((e) => e.writtenBy).filter(Boolean)).size,
    [entries],
  );
      ...
      <LDHeaderStrip
        ...
        stats={[
          ...
          ...(sourceCount > 0
            ...
            : []),
          ...(retentionPolicy
            ? [{ label: "Retention", value: retentionPolicy }]
            : []),
          ...(embeddingModel
            ? [
                {
...
function EpisodicEntries({
  ...
  const allTags = useMemo(
    () =>
      Array.from(new Set(entries.flatMap((e) => e.tags ?? []))).sort((a, b) =>
        a.localeCompare(b),
      ),
    [entries],
  );
  ...
  const byTag = useMemo(
    () =>
      activeTag
        ? recent.filter((e) => (e.tags ?? []).includes(activeTag))
        : recent,
    [recent, activeTag],
  );
  ...
  useEffect(() => {
    if ((tab === "conf" && !hasConf) || (tab === "tag" && !hasTags))
      setTab("recent");
  }, [tab, hasConf, hasTags]);
  ...
  const tabs: MemoryTabSpec[] = [
    ...
    ...(hasConf
      ? [{ value: "conf", label: "Highest conf.", count: byConf.length }]
      : []),
    ...(hasTags
      ? [{ value: "tag", label: "By tag", count: allTags.length }]
      : []),
  ];
  ...
  return (
    <Tabs value={tab} onValueChange={setTab}>
      <LDCard
        ...
        color={color}
        right={<MemoryCardTabs tabs={tabs} />}
      >
...
function EpisodicEntryRow({
  entry,
  last,
}: {
  entry: MemoryEpisodicEntry;
  last: boolean;
}) {
...
function EpisodicQueries({
  ...
  const cols: ColDef[] = [
    ...
    ...(hasTop
      ? [{ label: "top", width: "50px", align: "right" as const }]
      : []),
    ...(hasLat
      ? [{ label: "lat", width: "50px", align: "right" as const }]
      : []),
    ...(hasTrace
      ? [{ label: "trace", width: "60px", align: "right" as const }]
      : []),
  ];
...
function EpisodicWrites({
  ...
  const cols: ColDef[] = [
    ...
    ...(hasConf
      ? [{ label: "conf", width: "50px", align: "right" as const }]
      : []),
    ...(hasTrace
      ? [{ label: "trace", width: "60px", align: "right" as const }]
      : []),
          ...
          {writes.map((w, i) => (
            ...
            >
              ...
              {hasConf && (
                <span
                  className="text-right"
                  style={{ color: scoreTone(w.confidence) }}
                >
...
function EpisodicIndexCard({ index }: { index: MemoryEpisodicState["index"] }) {
  ...
  if (index.status) {
    rows.push({
      k: "Vector index",
      v: (
        <span style={{ color: indexStatusColor(index.status) }}>
          {index.status}
        </span>
      ),
      sub:
...
function EpisodicRetentionCard({
  retention,
}: {
  retention: MemoryEpisodicState["retention"];
}) {
  ...
  if (retention.lastGcAt) {
    rows.push({
      ...
      sub:
        retention.lastGcEvicted != null ? (
          <span>evicted {retention.lastGcEvicted} stale</span>
        ) : undefined,
    });
  }
