 ...
 * fields in that case.
 */
export function parseLiveFields(
  inspection: MemoryInspection | undefined,
): readonly LiveField[] | null {
  ...
  for (const entry of entries) {
    ...
    if (content && typeof content === "object" && !Array.isArray(content)) {
      ...
      rows.push({
        name: entry.key,
        value: obj.content ?? entry.value,
        ty: liveTypeName(obj.content ?? entry.value),
        writtenAt,
      });
    }
  }
