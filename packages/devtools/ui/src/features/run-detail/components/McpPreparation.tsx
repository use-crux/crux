    ...
    >
      <CardShell label={title}>
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 px-3.5 py-3">
          ...
          {view.transportKind && (
            <KeyValue k="transport" v={view.transportKind} />
          )}
          {view.implementation && (
            <KeyValue k="materializer" v={view.implementation} />
          )}
          {view.protocolVersion && (
            <KeyValue k="protocol" v={view.protocolVersion} />
          )}
          {view.serverVersion && (
            <KeyValue k="server version" v={view.serverVersion} />
          )}
          {view.serverName && <KeyValue k="server name" v={view.serverName} />}
          {view.discoveredToolCount != null && (
            <KeyValue k="Discovered" v={String(view.discoveredToolCount)} />
          )}
          {view.exposedToolCount != null && (
            <KeyValue k="Exposed" v={String(view.exposedToolCount)} />
          )}
          {view.failurePhase && (
            <KeyValue k="failure phase" v={view.failurePhase} />
          )}
          {view.errorCategory && (
            <KeyValue k="error category" v={view.errorCategory} />
          )}
        </div>
      </CardShell>
      ...
      {view.server && (
        <CardShell label="Catalog">
          <div className="px-3.5 py-3">
            <CatalogReference
              value={view.server.value}
              resolved={view.server.resolved}
  ...
  return (
    <CardShell label="MCP origin">
      <div className="flex flex-col gap-2 px-3.5 py-3">
        ...
        {origin.tool && (
          <CatalogReference
            value={origin.tool.value}
            resolved={origin.tool.resolved}
            onOpenCatalog={onOpenCatalog}
          />
        )}
      </div>
...
export function McpPreparationNode({
  node,
}: {
