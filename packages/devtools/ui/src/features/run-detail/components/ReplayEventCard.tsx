...
export function ReplayEventRow({
  ...
  const useSerif =
    canon === "generate" || canon === "output" || canon === "input";
  ...
  return (
    <div
      ...
      style={{ gridTemplateColumns: "92px 36px 1fr", opacity: dim ? 0.55 : 1 }}
    >
      <div
    ...
    >
      {!inlineJson && (
        <div className="whitespace-pre-wrap break-words">
          {isMarkdownish ? (
            <Streamdown>{e.what}</Streamdown>
          ) : (
            <RichHeadline text={e.what} />
          )}
        </div>
      )}
      {e.detail && (
...
function PayloadSection({
  ...
  return (
    <div className="mt-2.5">
      <div
        className="mb-1 font-mono text-[9.5px] uppercase tracking-[0.08em]"
...
function ToolPayload({
  ...
  payload: Extract<ReplayEventPayload, { type: "tool" }>;
  color: string;
}) {
  const isErr =
    payload.status === "error" || payload.status === "failed" || payload.error;
  return (
    <>
      {payload.args != null && (
...
function MemoryPayload({
  ...
  payload: Extract<ReplayEventPayload, { type: "memory" }>;
}) {
  const op = payload.operation || "";
  const isWrite =
    op === "set" || op === "append" || op === "write" || op === "put";
  ...
  return (
    <>
      ...
      {payload.value != null && (
        <PayloadSection
          label={isWrite ? "Value (written)" : isQuery ? "Result" : "Value"}
        >
          <PayloadCode>
...
function RetrievalPayload({
  ...
  payload: Extract<ReplayEventPayload, { type: "retrieval" }>;
}) {
  ...
  return (
    <>
      ...
      {hits.length > 0 && (
        <PayloadSection
          label={`Hits${payload.k != null ? ` · k=${payload.k}` : ""}`}
        >
          <div className="flex flex-col gap-1.5">
            {hits.slice(0, 8).map((hit: unknown, i: number) => {
              const h = (hit ?? {}) as Record<string, unknown>;
              const source =
                typeof h.source === "object" && h.source !== null
                  ? (h.source as Record<string, unknown>)
                  : undefined;
              const id = String(
                h.id ?? h.chunkId ?? source?.id ?? h.path ?? `hit-${i}`,
              );
...
function HandoffPayload({
  ...
  payload: Extract<ReplayEventPayload, { type: "handoff" }>;
}) {
  return (
    <>
...
function ScorePayload({
  ...
  payload: Extract<ReplayEventPayload, { type: "score" }>;
  color: string;
}) {
...
function ErrorPayload({
  ...
  payload: Extract<ReplayEventPayload, { type: "error" }>;
}) {
  return (
    <>
      {payload.message && (
        <PayloadSection
          label={payload.category ? `Error · ${payload.category}` : "Error"}
          tone="danger"
        >
          <div
