...
function StatusChip({
  ...
  if (meta.hidden)
    return <span style={{ color: "var(--qw-fg-faint)" }}>—</span>;
  ...
  return (
    ...
    >
      ...
      {meta.label}
      {suffix != null && (
        <span style={{ color: "var(--qw-fg-faint)" }}>· {suffix}</span>
      )}
    </span>
  );
}
          ...
          <span
            ...
            style={{
              ...
              boxShadow:
                i < lit
                  ? "none"
                  : `inset 0 0 0 1px ${missing ? "var(--qw-border-strong)" : "var(--qw-border)"}`,
            }}
          />
        ))}
      </span>
  ...
  return (
    ...
    >
      <span
        ...
        style={{
          background:
            m.tone === "muted" ? "var(--qw-fg-faint)" : TONE_VAR[m.tone],
        }}
      />
      {m.label}
    </span>
