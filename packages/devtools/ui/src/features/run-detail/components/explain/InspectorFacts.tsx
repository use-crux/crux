...
function attentionReason(report: RuntimeTurnDecisionReport): string {
  const normalized = normalizeTurnDecisionReport(report);
  if (!normalized || !turnHasWarningSignal(normalized))
    return "No warning signal — opened Output.";
  const reasons: string[] = [];
  if (normalized.turn.status && normalized.turn.status !== "ok")
    reasons.push(normalized.turn.status);
  if (
    normalized.considered.some((c) => c.required && c.disposition === "dropped")
  )
    reasons.push("required context dropped");
  if (normalized.freshness.some((f) => f.status === "stale-used"))
    reasons.push("stale evidence used");
  if (
    normalized.decisions.some((d) =>
      d.reason.code.startsWith("routing.fallback"),
    )
  )
    reasons.push("fallback fired");
  return reasons.length > 0
    ? `Opened Explain · ${reasons.join(", ")}.`
    : "Opened Explain.";
}

/** Compact per-turn facts for the inspector rail. */
export function TurnInspectorFacts({
  report,
}: {
  ...
  const normalized = normalizeTurnDecisionReport(report);
  if (!normalized) return null;
  const checked = normalized.considered.filter(
    (c) => c.disposition === "checked",
  ).length;
  const dropped = normalized.considered.filter(
...
export function RunInsightFacts({
  reports,
}: {
  reports: readonly RuntimeTurnDecisionReport[];
}) {
