...
import {
  resolveOpenTab,
  type ExplainGenTab,
} from "@/features/run-detail/lib/explain/tabs";
...
export function ExplainTab({
  ...
  const sectionRefs = useRef<
    Partial<Record<ExplainSection, HTMLDivElement | null>>
  >({});
  ...
  const jump = useCallback((section: ExplainSection) => {
    ...
    if (el && scroller)
      scroller.scrollTo({
        top: Math.max(0, el.offsetTop - 14),
        behavior: "smooth",
      });
    setFlash(section);
  ...
  const contextLink = resolveOpenTab("Context", availableTabs)
    ? openContext
    : undefined;

  const register = useCallback(
    (id: ExplainSection, el: HTMLDivElement | null) => {
  ...
  const droppedSome = normalized.considered.some(
  ...
  return (
    <div ref={scrollRef} className="min-h-0 flex-1 overflow-auto">
      <div className="mx-auto px-6 py-5" style={{ maxWidth: 960 }}>
        <ReadoutBand
          readout={normalized.turn.readout}
          chips={chips}
          activeJump={flash}
          onJump={jump}
        />
        ...
        <Sec id="saw" flash={flash} register={register}>
          <SecBand
            ...
            right={
              contextLink ? <OpenContext onClick={contextLink} /> : undefined
            }
          />
          <Card>
            {normalized.saw.length === 0 ? (
              <Empty>
                Nothing was recorded as reaching the model for this turn.
              </Empty>
            ) : (
              normalized.saw.map((item, i) => (
                <SawRow key={item.id ?? i} item={item} onOpen={contextLink} />
              ))
            )}
          </Card>
        </Sec>
        ...
        <Sec id="considered" flash={flash} register={register}>
          ...
          <Card>
            {normalized.considered.length === 0 ? (
              ...
              normalized.considered.map((item, i) => (
                <ConsideredRow key={item.id ?? i} item={item} />
              ))
            )}
          </Card>
        </Sec>
        ...
        <Sec id="fresh" flash={flash} register={register}>
          <SecBand
            icon="clock"
            title="Freshness & cache"
            hint="two different questions — kept apart on purpose"
          />
          <FreshCacheBlock
        ...
        <Sec id="decisions" flash={flash} register={register}>
          ...
          <Card>
            {normalized.decisions.length === 0 ? (
              ...
              normalized.decisions.map((d) => (
                <DecisionRow key={d.id} decision={d} onOpen={linkTo(d.tab)} />
              ))
            )}
          </Card>
        </Sec>
        ...
        <Sec id="source" flash={flash} register={register}>
          <SecBand
            icon="link"
            title="What source do I change?"
            hint="definitions that shaped this turn"
          />
        ...
        <Sec id="gaps" flash={flash} register={register}>
          <SecBand
            ...
            count={
              normalized.gaps.length ? `${normalized.gaps.length}` : undefined
            }
