...
import {
  ...
  SummaryMode,
  type SummaryNav,
} from "@/features/run-detail/components/RunDetailModes";
import {
  archetypeHasSummary,
  archetypeStrip,
  runArchetype,
} from "@/features/run-detail/lib/archetype";
import type {
...
function missingRunDetail(traceId: string): InspectRunDetailRecord {
  ...
  return {
    ...
    trace: {
      ...
      status: "error",
      error: {
        message: "Run detail is not available in the observability store.",
      },
    },
    events: [],
    spans: [],
...
export function RunDetailShell({
  ...
  const inspectDetail = useInspectRunDetailSuspense(traceId);
  const canonicalHeader = useObservabilityGraph(traceId);
  const detail = useMemo(
    () => inspectDetail ?? missingRunDetail(traceId),
    [inspectDetail, traceId],
  );
  ...
  const model =
    ...
    detail.run.model;
  const provider =
    modelSummary?.primaryProvider ?? run?.provider ?? detail.run.provider;
  const modelExtraCount = modelSummary?.mixed
    ? Math.max(0, (modelSummary.models?.length ?? 1) - 1)
    : 0;
  ...
  const modelLabel = model
    ? `${model}${modelExtraCount > 0 ? ` +${modelExtraCount}` : ""}`
    : undefined;
  const subtitleText = [primitive, provider, modelLabel]
    .filter(Boolean)
    .join(" · ");
  ...
  const modelTitle = modelSummary?.mixed
    ? modelSummary.models
        ...
        .replace(/[^\x00-\x7F]+/g, "/")
    : undefined;
  const subtitle = modelTitle ? (
    <span title={modelTitle}>{safeSubtitleText}</span>
  ) : (
    safeSubtitleText
  );
  ...
  const showSummary =
    summary === true ||
    (summary == null && lens == null && navSpanId == null && hasSummary);
  const effectiveLens: RunLens = lens ?? "tree";
  const summaryNav = hasSummary
    ? {
        active: showSummary,
        onSelect: () =>
          navigate({ view: "run-detail", traceId, summary: true }),
      }
    : undefined;
  ...
  const failingSpanIds = useMemo(
    () => (spanTree ? collectFailingSpanIds(spanTree) : []),
    [spanTree],
  );
  ...
  useEffect(() => {
    function onKey(e: globalThis.KeyboardEvent) {
      ...
      const lensByKey: Record<string, RunLens> = {
        "1": "tree",
        "2": "timeline",
        "3": "graph",
        "4": "story",
  ...
  useEffect(() => {
    ...
    function onCopyPermalink() {
      void navigator.clipboard?.writeText(
        buildRunPermalink({ traceId, lens: effectiveLens, spanId: navSpanId }),
      );
      toast({
        kind: "ok",
        title: "Permalink copied",
      ...
      <div className="flex h-full min-h-0 flex-col">
        ...
        >
          <SectionBoundary
            ...
            invalidateKeys={[
              qk.inspect.run(traceId),
              qk.observability.run(traceId),
            ]}
          >
            {showSummary ? (
              <SummaryMode
...
function RunDetailSkeleton({ lens }: { lens: RunLens }) {
  ...
  if (lens === "story") {
    return (
      <div
        className="mx-auto flex flex-col gap-4 px-8 py-6"
        style={{ maxWidth: 1120 }}
      >
...
function ReplayMode({
  ...
  const topMeta = useMemo(() => {
    ...
    if (trace?.result?.cost != null)
      items.push({ label: "cost", value: `$${trace.result.cost.toFixed(4)}` });
    return items;
  }, [trace]);
 ...
 */
function spanField(s: InspectRunSpan, ...keys: readonly string[]): unknown {
  const data = s.data;
  if (data) {
    for (const k of keys) {
      const v = data[k];
...
/**
 * Map a `InspectRunSpan` into a replay event when it represents an
 ...
 */
function spanToReplay(s: InspectRunSpan, startMs: number): ReplayEvent | null {
  ...
  const toks =
    s.tokenCount != null ? `${s.tokenCount.toLocaleString()} tok` : "";
  ...
  switch (s.primitive) {
    ...
    case "trace": {
      ...
      const streamingMeta = [
        ...
        .join(" · ");
      const meta =
        [model, baseMeta, streamingMeta].filter(Boolean).join(" · ") ||
        undefined;
      if (!text) return null;
      return {
        who: model ?? s.name,
    ...
    case "retrieval.stage": {
      ...
      return {
        ...
        body: hits,
        meta:
          [baseMeta, k != null ? `k=${k}` : ""].filter(Boolean).join(" · ") ||
          undefined,
        t,
        tMs: offset,
      };
 ...
 */
function classifyNarrativeNoise(n: InspectRunNarrativeEvent): boolean {
  const kind = n.kind || "";
  ...
  const primitive =
    typeof data.primitive === "string" ? (data.primitive as string) : undefined;
  // Convex boundary / internal operation spans flagged detail-presentation
  // by the backend.
  if (kind === "operation") {
    const pres =
      attributes &&
      (attributes.presentation as { display?: string } | undefined);
  ...
  if (primitive === "agent.run" || (kind === "agent" && !attributes))
    return true;
  return false;
}
...
function formatToolArgs(args: unknown, max = 90): string {
  ...
  if (typeof args === "object" && !Array.isArray(args)) {
    ...
    const inner =
      formatted.length > max ? formatted.slice(0, max) + "…" : formatted;
    return `(${inner}${tail})`;
  }
 ...
 */
function narrativeToReplay(n: InspectRunNarrativeEvent): ReplayEvent {
...
function canonicalKind(
  ...
  if (primitive === "tool.call" || primitive === "tool" || rawKind === "tool")
    return "tool";
  if (
    primitive === "memory" ||
    rawKind === "memory" ||
  ...
  if (rawKind === "judge" || rawKind === "score" || label.startsWith("judge."))
    return "score";
...
function extractByKind({
  ...
  switch (kind) {
    case "tool": {
      ...
      const isFailed =
        status === "error" || status === "failed" || data.error != null;
      ...
      const payload: ReplayEventPayload | undefined =
        args != null || result != null || isFailed
          ? {
              type: "tool",
              args,
              result: isFailed ? undefined : result,
              status,
              error: errorMessage,
            }
          : undefined;
      return {
        ...
        payload,
        notes: isFailed
          ? `tool failed${errorMessage ? ` · ${errorMessage}` : ""}`
          : undefined,
    ...
    case "memory": {
      const operation =
        ...
        (label.startsWith("memory.")
          ? label.slice("memory.".length)
          : undefined);
      const blockKind = pickStr(
        data.blockKind,
        attributes?.blockKind,
        data.kind as string,
      );
      const memoryId = pickStr(
        data.memoryId,
        attributes?.memoryId,
        data.key,
        attributes?.key,
    ...
    case "retrieval": {
      ...
      const resultCount =
        pickNum(data.resultCount, attributes?.resultCount) ??
        (Array.isArray(hits) ? hits.length : undefined);
      const source = pickStr(
        data.retrieverName,
        data.name,
        attributes?.retrieverName,
      );
      ...
      const headline = [
        ...
        resultCount != null
          ? `${resultCount} chunk${resultCount === 1 ? "" : "s"} returned`
          : null,
      ]
        .filter(Boolean)
        .join(" · ");
    ...
    case "generate": {
      ...
      const text =
        ...
        (data.output != null && typeof data.output !== "string"
    ...
    case "score": {
      const scorer =
        pickStr(data.scorerName, data.judgeName, data.metricId, data.name) ??
        actor ??
        "judge";
      const score = pickNum(data.score, attributes?.score);
      const rationale = pickStr(
        data.rationale,
        data.reasoning,
      ...
      const headline =
        score != null ? `${scorer} → ${score.toFixed(2)}` : scorer;
      ...
      return {
        ...
        notesTone:
          passed === true ? "ok" : passed === false ? "danger" : undefined,
      };
    }
    case "handoff":
    case "transition": {
      ...
      const head =
        from && to ? `${from} → ${to}` : label || (to ? `→ ${to}` : "Handoff");
      const tail = joinMeta([
    ...
    case "step": {
      const stepLabel =
        pickStr(data.stepLabel, data.name, attributes?.stepLabel) ?? label;
      const message = pickStr(data.message, data.summary);
      return {
        who: actor || stepLabel || "step",
        what: message
          ? `${stepLabel} · ${pickPreview(message, 200)}`
          : stepLabel,
    ...
    case "flow": {
      const flowName =
        pickStr(data.name, attributes?.name, data.flowName) ?? label;
    ...
    case "composition": {
      const compName =
        pickStr(data.name, attributes?.name, data.kind as string) ?? label;
    ...
    case "source": {
      const source =
        pickStr(data.sourceId, data.name, data.path, attributes?.sourceId) ??
        label;
      const score = pickNum(data.score, attributes?.score);
      const status = pickStr(data.status, attributes?.status);
      return {
    ...
    case "error": {
      ...
      const category = pickStr(
        (data.error as { category?: string } | undefined)?.category,
        data.category,
      );
      const stack = pickStr(
        (data.error as { stack?: string } | undefined)?.stack,
        data.stack,
      );
      return {
        who: actor || "error",
...
function buildReplay({
  ...
  if (spans && spans.length > 0) {
    const startMs = spans.reduce<number>(
      (min, s) =>
        s.startedAt != null && s.startedAt < min ? s.startedAt : min,
      Number.POSITIVE_INFINITY,
    );
    const zero = Number.isFinite(startMs)
      ? startMs
      : (trace?.startedAt ?? Date.now());
  ...
  if (trace.result?.text || trace.result?.object) {
    const text =
      trace.result?.text ??
      (trace.result?.object
        ? JSON.stringify(trace.result.object, null, 2)
        : "");
