...
function GeneratedOutputParts({ node }: { node: ObservabilityRunDetailNode }) {
  ...
  const allParts = msgs.length > 0 && msgs.every((m) => isContentPart(m as AnyMessageItem))
  if (!allParts) return null
  return <MessageBlock raw={msgs as unknown} label={`Generated · ${msgs.length} parts`} />
}
...
function OutputGrounding({ node }: { node: ObservabilityRunDetailNode }) {
  ...
  const markers =
    report && typeof report === 'object' && (report as { kind?: unknown }).kind === 'citation.report'
      ? ((report as { markers?: ReadonlyArray<Record<string, unknown>> }).markers ?? [])
      : []
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-px" style={{ background: 'var(--qw-border)' }}>
        {markers.map((m, i) => {
          const sourceId = typeof m.sourceId === 'string' ? m.sourceId : typeof m.chunkId === 'string' ? m.chunkId : '—'
          ...
          return (
            ...
            >
              ...
              {isGrounded && score != null ? (
                ...
                <Chip tone="warn">{typeof m.note === 'string' ? m.note : 'unused'}</Chip>
              )}
            </div>
          )
        })}
...
function SpanSection({ title, right, children }: { title: string; right?: ReactNode; children: ReactNode }) {
  return (
...
function ToolCallBox({
  ...
  const HeaderRow = ({ label, note, right }: { label: string; note?: string; right?: ReactNode }) => (
    <div
      ...
      <div className="overflow-hidden rounded-[8px]" style={{ border: '1px solid var(--qw-border)' }}>
        ...
        <div className="px-3.5 py-3">
          {renderVal(
            args,
            <span className="text-[12px]" style={{ color: 'var(--qw-fg-faint)' }}>
              (args not recorded — the model embeds them in the parent generation's messages artifact)
            </span>,
          )}
        </div>
        <HeaderRow
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <ToolCallBox
        ...
        argsNote={
          [argsOwnerInfo, payload.inputSize != null ? `${payload.inputSize.toLocaleString()} B` : null]
            .filter(Boolean)
            .join(' · ') || undefined
        }
        resultNote={
          [resultOwnerInfo, payload.outputSize != null ? `${payload.outputSize.toLocaleString()} B` : null]
            .filter(Boolean)
            .join(' · ') || undefined
        }
...
function MemoryTab({ node }: { node: ObservabilityRunDetailNode }) {
  ...
  const recallBlocks =
    recall && typeof recall === 'object' && Array.isArray((recall as { blocks?: unknown }).blocks)
  ...
  const memoryRel =
    ((relationItems.find((r) => typeof r.kind === 'string' && r.kind.startsWith('memory.'))?.data as
  ...
  const recalled = results.length || (typeof resultCountAttr === 'number' ? resultCountAttr : 0)
  const topScore = results.reduce((m, r) => (typeof r.score === 'number' ? Math.max(m, r.score as number) : m), -1)
  const isWrite = node.primitive.startsWith('memory.write') || op === 'write' || writeMode != null
  ...
  const diffObj =
    diff && typeof diff === 'object' && !Array.isArray(diff) ? (diff as Record<string, unknown>) : undefined
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {results.length > 0 ? (
        <SpanSection title={`Recalled · ${results.length} block${results.length === 1 ? '' : 's'}`}>
          <div className="flex flex-col gap-1.5">
            {results.slice(0, 20).map((r, i) => (
              <MemoryRecalledRow key={i} r={r} fallbackBlock={blockKind ?? memoryType} />
            ))}
          </div>
        </SpanSection>
      ) : (
        recalled > 0 && (
          <SpanSection title={`Recalled · ${recalled} block${recalled === 1 ? '' : 's'}`}>
            <div className="text-[11.5px]" style={{ color: 'var(--qw-fg-faint)' }}>
            ...
            >
              {stateEntries.map(([label, value], i) => (
                <div key={i} className="flex gap-2 font-mono text-[11.5px]">
                  <span className="shrink-0" style={{ color: 'var(--qw-fg-faint)' }}>
                    {label}
                  </span>
                  <span className="min-w-0 flex-1 break-words" style={{ color: 'var(--qw-fg)' }}>
                    {compactValue(value)}
                  </span>
                </div>
              ))}
...
function compactValue(v: unknown, depth = 0): string {
  ...
  if (typeof v === 'object') {
    ...
    return `{ ${ks
      .slice(0, 4)
      .map((k) => `${k}: ${compactValue((v as Record<string, unknown>)[k], depth + 1)}`)
      .join(', ')}${ks.length > 4 ? ', …' : ''} }`
  }
  return String(v)
}
...
function MemoryRecalledRow({ r, fallbackBlock }: { r: Record<string, unknown>; fallbackBlock?: string }) {
  const block = String(r.block ?? r.blockKind ?? r.kind ?? fallbackBlock ?? 'block')
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="font-mono text-[11px]" style={{ color: 'var(--qw-crux)' }}>
          {key}
        </div>
        {preview && (
          <div
    ...
    return (
      <div className="flex flex-col gap-1">
        {Object.entries(value as Record<string, unknown>).map(([k, v], i) => (
          <span key={i} className="font-mono text-[10.5px]" style={{ color: 'var(--qw-fg-muted)' }}>
            {k}: {compactValue(v, 1)}
          </span>
        ))}
      </div>
...
function ApprovalCard({ node }: { node: ObservabilityRunDetailNode }) {
  const decision = (findAttribute(node, 'decision', 'outcome') as string | undefined)?.toLowerCase()
  const approvedAttr = findAttribute(node, 'approved') as boolean | undefined
  const approved = decision ? /approv|accept|grant|allow/.test(decision) : approvedAttr === true
  const denied = decision ? /den|reject|block|refus/.test(decision) : approvedAttr === false
  const approver = findAttribute(node, 'approver', 'approvedBy', 'decidedBy', 'reviewer') as string | undefined
  const reason = findAttribute(node, 'reason', 'note', 'comment') as string | undefined
  ...
  const bg = approved ? 'var(--qw-ok-soft)' : denied ? 'var(--qw-danger-soft)' : 'var(--qw-warn-soft)'
...
function PlanCard({ node }: { node: ObservabilityRunDetailNode }) {
  const goal = findAttribute(node, 'goal', 'objective', 'description', 'summary') as string | undefined
  ...
  const tasks: { label: string; status: string }[] = Array.isArray(tasksRaw)
    ? (tasksRaw as unknown[]).map((t, i) => {
        ...
        return {
          label: String(o.id ?? o.title ?? o.name ?? o.label ?? o.task ?? `task ${i + 1}`),
            ...
            {tasks.map((tk, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-mono text-[9.5px] uppercase" style={{ color: st.color }}>
                    {st.label}
                  </span>
                </div>
              )
...
function PayloadGrid({ cells }: { cells: { label: string; size?: string; body: unknown }[] }) {
  return (
    <div
      className="grid overflow-hidden rounded-[8px]"
          ...
          <div className="px-3 py-2.5">
            {c.body != null ? (
              typeof c.body === 'string' ? (
                <div className="text-[11.5px] leading-[1.5]" style={{ color: 'var(--qw-fg-muted)' }}>
                  {c.body}
                </div>
              ) : (
...
function HandoffTab({ node, onSelect }: { node: ObservabilityRunDetailNode; onSelect?: (id: string) => void }) {
  const isDelegate = node.primitive.startsWith('delegate.')
  const fromAgent = (findAttribute(node, 'fromAgent', 'from', 'caller') as string | undefined) ?? '—'
  const toAgent = (findAttribute(node, 'toAgent', 'to', 'agent', 'callee') as string | undefined) ?? '—'
  ...
  const maxHops = findAttribute(node, 'maxHops', 'hopCount', 'totalHops') as number | undefined
  const pathRaw = findAttribute(node, 'handoffPath', 'path')
  const handoffPath = Array.isArray(pathRaw) ? (pathRaw as string[]).filter((s) => typeof s === 'string') : []
        ...
        >
          ...
          <div className="flex-1">
            ...
            <div className="mt-0.5 text-[11.5px]" style={{ color: 'var(--qw-fg-muted)' }}>
              ...
              {subRun ? ` Ran its own ${gatherDescendants(subRun).length}-span sub-trace.` : ''}
            </div>
          </div>
          {subRun && onSelect && (
            <button
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <svg width="40" height="12">
          <line x1="0" y1="6" x2="32" y2="6" stroke="var(--qw-iris)" strokeWidth="1.6" strokeDasharray="4 3" />
          <path d="M32 2l6 4-6 4" fill="none" stroke="var(--qw-iris)" strokeWidth="1.6" />
        </svg>
...
function EmbeddingCard({ node }: { node: ObservabilityRunDetailNode }) {
  ...
  const dims = findAttribute(node, 'dimensions', 'dims', 'dimension') as number | undefined
  const total = findAttribute(node, 'inputs', 'inputCount', 'count') as number | undefined
  const hit = findAttribute(node, 'cacheHits', 'cachedInputs', 'cached') as number | undefined
  const truncations = findAttribute(node, 'truncations', 'truncated') as number | undefined
  const retries = findAttribute(node, 'retries', 'retryCount') as number | undefined
  const rateLimitWait = findAttribute(node, 'rateLimitWaitMs', 'rateLimitWait') as number | undefined
  const pct = hit != null && total ? Math.round((hit / total) * 100) : undefined
  ...
  if (rateLimitWait != null) cells.push(['rate-limit wait', fmtDuration(rateLimitWait)])
...
function RetrievalSpanTab({ node }: { node: ObservabilityRunDetailNode }) {
  ...
  const funnel = stages
    .map((s) => (typeof s.outHits === 'number' ? (s.outHits as number) : undefined))
        ...
        >
          <div className="flex flex-wrap gap-1.5">
            {stages.map((s, i) => {
              ...
              const out = typeof s.outHits === 'number' ? (s.outHits as number) : undefined
              const inH = typeof s.inHits === 'number' ? (s.inHits as number) : undefined
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    {status && status !== 'success' && (
                      <span className="size-1.5 rounded-full" style={{ background: 'var(--qw-warn)' }} />
                    )}
                  </div>
                  <div className="font-mono text-[13px] font-semibold" style={{ color: 'var(--qw-ok)' }}>
                    {out ?? '·'}
                    {inH != null && out != null && inH !== out && (
                      <span className="ml-1 text-[10px] font-normal" style={{ color: 'var(--qw-fg-faint)' }}>
                        ← {inH}
                      </span>
                    )}
                  </div>
        ...
        {hits.length === 0 ? (
          ...
          <div className="flex flex-col gap-1.5">
            {hits.slice(0, 20).map((h, i) => (
              <ChunkHitRow key={i} hit={h} rank={typeof h.rank === 'number' ? (h.rank as number) : i + 1} />
            ))}
          </div>
        )}
      </SpanSection>
        ...
        <div className="flex flex-col gap-2">
          ...
          {retrievals.map((r) => {
            ...
            return (
              ...
              >
                <div className="flex flex-col gap-1.5 px-3.5 py-3">
                  {query && <div className="mb-1 font-mono text-[12px]">{query}</div>}
                  {hits.length === 0 ? (
                    <span className="text-[12px]" style={{ color: 'var(--qw-fg-faint)' }}>
                      (no hits)
                    </span>
      ...
      {memory.length > 0 && (
        <div className="flex flex-col gap-2">
          ...
          {memory.map((m) => {
            ...
            const previewKind = recall ? 'recall' : diff ? 'diff' : snapshot ? 'snapshot' : undefined
            return (
              <CardShell
                key={m.id}
                label={
