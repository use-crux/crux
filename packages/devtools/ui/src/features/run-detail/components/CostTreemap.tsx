...
export function CostTreemap({
  ...
  const total =
    totalCost || Object.values(costs).reduce((a, b) => a + b, 0) || 1;

  return (
    <div
      className={cn(
        "flex h-14 gap-px overflow-hidden rounded border border-zinc-800",
        className,
      )}
    >
      {visible.map((seg) => {
        ...
        return (
          <div
            key={seg.key}
            className={cn(
              "flex flex-col justify-center px-2.5 border-l first:border-l-0",
              seg.bg,
              seg.border,
            )}
            style={{ width: `${pct}%` }}
