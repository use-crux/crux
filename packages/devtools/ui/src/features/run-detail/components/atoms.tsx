...
export function StatusPill({ status, className }: { status: RunStatus; className?: string }) {
  const meta = STATUS_META[status] ?? { tone: 'muted' as ChipTone, label: String(status) }
  return (
    <Chip
      tone={meta.tone}
      dot
      className={cn(meta.faint && 'opacity-60 line-through', meta.pulse && 'animate-pulse', className)}
    >
      {meta.label}
    </Chip>
  )
      ...
      {items.map((it) => (
        <div key={it.label} className="flex flex-col leading-tight">
          ...
          <span
            ...
            style={{ fontSize: size, color: it.tone ? TONE_VAR[it.tone] : 'var(--qw-fg)' }}
          >
            {it.value}
          </span>
        </div>
