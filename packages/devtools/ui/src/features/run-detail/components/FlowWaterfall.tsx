...
export function buildSteps(traces: Trace[]): StepData[] {
  ...
  for (const key of order) {
    ...
    const endedAt = Math.max(
      ...stepTraces.map((t) => (t.status === 'running' ? now : t.startedAt + (t.durationMs ?? 0))),
    )
    const totalDurationMs = endedAt - startedAt
    const totalCost = stepTraces.reduce((sum, t) => sum + (t.result?.cost ?? 0), 0)
    const totalTokens = stepTraces.reduce((sum, t) => sum + (t.result?.usage?.totalTokens ?? 0), 0)
    ...
    const models = stepTraces.map((t) => t.model).filter((m) => m !== 'resolve-only')
...
export function layoutSteps(steps: StepData[]): LayoutStep[] {
  ...
  for (let i = 0; i < steps.length; i++) {
    const step = steps[i]!
    const width = Math.round(NODE_MIN_W + (step.totalDurationMs / maxDuration) * (NODE_MAX_W - NODE_MIN_W))

    const isParallel = i > 0 && Math.abs(step.startedAt - colAnchorTime) <= PARALLEL_THRESHOLD_MS
...
function findChildFlows(
  ...
  for (const t of allSessionTraces) {
    if (t.parentFlowId === parentFlowId && t.flowId && t.flowId !== parentFlowId) {
  ...
  for (const [flowId, traces] of childFlowTraces) {
    ...
    const endedAt = Math.max(...traces.map((t) => (t.status === 'running' ? now : t.startedAt + (t.durationMs ?? 0))))
    const totalTokens = traces.reduce((sum, t) => sum + (t.result?.usage?.totalTokens ?? 0), 0)
    ...
    const stepLabels = [...new Set(traces.map((t) => t.stepLabel).filter((l): l is string => l != null))]

    children.push({
      flowId,
      name: nameMap?.get(flowId) ?? (stepLabels.length > 0 ? stepLabels.join(' → ') : 'sub-flow'),
      traces,
...
function measureChildFlow(child: ChildFlowInfo): {
  ...
  const width = CF_PAD_X * 2 + (maxCol + 1) * CF_STEP_W + maxCol * CF_STEP_GAP_X
  const height = CF_PAD_TOP + (maxRow + 1) * (CF_STEP_H + CF_STEP_GAP_Y) - CF_STEP_GAP_Y + CF_PAD_BOTTOM
  return { width, height }
}
...
function buildGraph(
  ...
  if (childFlows.length > 0) {
    const maxRow = laid.length > 0 ? Math.max(...laid.map((s) => s.row), 0) : -1
    const stepsBottom = maxRow >= 0 ? (maxRow + 1) * (NODE_H + NODE_GAP_Y) + CF_GAP_Y : 0
    let childX = 0
    ...
    for (let i = 0; i < childFlows.length; i++) {
      ...
      const cfMaxCol = child.steps.length > 0 ? Math.max(...child.steps.map((s) => s.col), 0) : -1
      ...
      for (const step of child.steps) {
        ...
        const isSelected = step.traces.some((t) => t.traceId === selectedTraceId)
...
const StepNode = memo(function StepNode({ data }: NodeProps<Node<StepNodeData>>) {
  ...
  const shortModel = data.model ? data.model.replace(/^[^/]+\//, '').slice(0, 20) : null
    ...
    >
      ...
      <div className="px-3 py-2">
        <div className="flex items-center gap-1.5 mb-1">
          ...
          />
          <span className="text-xs font-medium text-zinc-200 truncate">{data.stepLabel}</span>
          {data.traceCount > 1 && <span className="text-[10px] text-zinc-600 tabular-nums">{data.traceCount}x</span>}
        </div>

        {shortModel && (
          <div className="mb-1.5">
            <span className="text-[10px] text-zinc-500 font-mono bg-zinc-800/60 rounded px-1 py-0.5">{shortModel}</span>
          </div>
        )}
...
const ChildFlowNode = memo(function ChildFlowNode({ data }: NodeProps<Node<ChildFlowNodeData>>) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 px-2.5 py-1.5">
        <span className={`text-[9px] font-semibold uppercase tracking-wider ${styles.badge} rounded px-1 py-0.5`}>
          Sub-flow
        </span>
        <span className="text-[10px] font-mono text-zinc-300 truncate font-medium">{data.name}</span>
...
const ChildStepNode = memo(function ChildStepNode({ data }: NodeProps<Node<StepNodeData>>) {
  const colors = STATUS_COLORS[data.status]
  const ringClass = data.selected ? 'ring-2 ring-zinc-300/60' : ''
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Left} className="!bg-zinc-600 !w-1 !h-1 !border-0" />
      <Handle type="source" position={Position.Right} className="!bg-zinc-600 !w-1 !h-1 !border-0" />
      ...
      <div className="px-2 py-1">
        <div className="flex items-center gap-1 mb-0.5">
          ...
          />
          <span className="text-[9px] font-medium text-zinc-200 truncate">{data.stepLabel}</span>
        </div>
...
export function FlowWaterfall({
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    const childFlows =
      parentFlowId && allSessionTraces ? findChildFlows(parentFlowId, allSessionTraces, flowNameMap) : []
  ...
  if (nodes.length === 0) {
    return (
      <div className={`h-[300px] flex items-center justify-center text-sm text-zinc-500 ${className ?? ''}`}>
  ...
  return (
    <div className={`rounded-lg border border-zinc-800 bg-zinc-950 ${className ?? ''}`} style={{ height }}>
      <ReactFlow
        nodes={nodes}
        edges={edges}
        nodeTypes={nodeTypes}
