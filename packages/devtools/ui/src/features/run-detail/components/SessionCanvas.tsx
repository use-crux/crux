...
function buildFlowTree(
...
): { roots: FlowNode[]; standalone: Trace[] } {
  ...
  for (const [flowId, flowTraces] of byFlow) {
    ...
    nodeMap.set(flowId, {
      ...
      name:
        nameMap?.get(flowId) ??
        (stepLabels.length > 0 ? stepLabels.join(" → ") : "flow"),
      traces: flowTraces,
      children: [],
      startedAt,
      endedAt,
  ...
  function finalize(node: FlowNode): void {
    ...
    if (node.children.length > 0) {
      ...
      const allEndedAt = Math.max(
        node.endedAt,
        ...node.children.map((c) => c.endedAt),
      );
      node.durationMs = allEndedAt - node.startedAt;
      node.endedAt = allEndedAt;
      if (node.children.some((c) => c.status === "error"))
        node.status = "error";
      else if (
        node.children.some((c) => c.status === "running") ||
        node.status === "running"
      )
        node.status = "running";
    }
...
function measureChildFlow(traces: Trace[]): {
  ...
  const height =
    CF_PAD_TOP + (grid.height > 0 ? grid.height : STEP_H) + CF_PAD_BOTTOM;
...
function buildGraph(
  ...
  for (const root of roots) {
    ...
    // Measure child flows
    const childMeasurements = root.children.map((child) =>
      measureChildFlow(child.traces),
    );
    ...
    for (let i = 0; i < childMeasurements.length; i++) {
      ...
      childrenMaxHeight = Math.max(
        childrenMaxHeight,
        childMeasurements[i]!.height,
      );
    }
    ...
    const gapBetween =
      stepsBlockHeight > 0 && childBlockHeight > 0 ? CF_GAP_Y : 0;
    const rootHeight =
      ROOT_PAD_TOP +
      stepsBlockHeight +
      gapBetween +
    ...
    const ownMaxCol =
      ownSteps.length > 0 ? Math.max(...ownSteps.map((s) => s.col), 0) : -1;
    let xc = ROOT_PAD_X;
    ...
    for (let ci = 0; ci < root.children.length; ci++) {
      ...
      const cfMaxCol =
        cm.steps.length > 0 ? Math.max(...cm.steps.map((s) => s.col), 0) : -1;
      let cfXc = CF_PAD_X;
...
const RootFlowNode = memo(function RootFlowNode({
  data,
}: NodeProps<Node<RootFlowData>>) {
