...
export function RequestMap({ inspect, tools, contextLookup, onSegmentClick, className }: RequestMapProps) {
    ...
    <div className={cn('space-y-2', className)}>
      ...
      <div className="flex h-8 rounded-lg overflow-hidden border border-zinc-800/60 bg-zinc-900">
        {segments.map((seg) => {
          ...
          const rawPct =
            measuredTokens > 0 ? (seg.tokens / measuredTokens) * (hasTools ? 88 : 100) : 100 / segments.length
          ...
          return (
            ...
            >
              {showLabel && (
                <span className="text-[10px] text-white font-medium truncate px-1.5 drop-shadow-sm">{seg.label}</span>
              )}
              ...
              {hoveredSource === seg.source && (
                <div className="absolute -top-9 left-1/2 -translate-x-1/2 z-20 whitespace-nowrap rounded bg-zinc-900 border border-zinc-700 px-2 py-1 text-[10px] text-zinc-200 shadow-lg pointer-events-none">
                  ...
                  {measuredTokens > 0 ? ((seg.tokens / measuredTokens) * 100).toFixed(1) : 0}%
                </div>
              )}
            </button>
          )
        ...
        {hasTools && (
          ...
          >
            ...
            <span className="text-[10px] text-zinc-300 font-medium">{tools!.length}</span>
      ...
      <div className="flex items-center justify-between text-[10px]">
        <div className="flex items-center gap-3">
          <span className="text-zinc-400 tabular-nums">{formatTokens(budgetUsed)} tokens</span>
          {/* Legend: compact segment key */}
          <div className="flex items-center gap-2">
            {segments.length > 1 &&
              segments.slice(0, 5).map((seg) => (
                <span key={seg.source} className="flex items-center gap-1">
                  <span className={cn('w-1.5 h-1.5 rounded-sm', seg.color.bg)} />
                  <span className="text-zinc-600 truncate max-w-[80px]">{seg.label}</span>
                </span>
              ))}
            {segments.length > 5 && <span className="text-zinc-600">+{segments.length - 5}</span>}
          </div>
        </div>

        {budgetTotal && budgetPct != null && (
          <span className={cn('tabular-nums font-medium', budgetColor)}>
            {formatTokens(budgetUsed)} / {formatTokens(budgetTotal)} ({budgetPct.toFixed(1)}%)
          </span>
        )}
      </div>
        ...
        <div className="flex items-center gap-1.5 text-[10px] text-zinc-500">
          ...
          <div className="flex items-center gap-2 overflow-hidden">
            {inspect.excludedContexts.map((ctx) => (
              <span key={ctx.source} className="flex items-center gap-1 text-zinc-500/60">
                <span className="line-through">{ctx.source.replace(/^context:/, '')}</span>
