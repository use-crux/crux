...
export function buildRunPermalink({ traceId, lens, spanId }: RunPermalinkParts): string {
...
export function RunShareButton({ traceId, lens, spanId }: RunPermalinkParts) {
  ...
  return (
    <div className="relative" ref={ref}>
      <Btn size="sm" icon={<Icon name="arrowUp" size={13} />} onClick={() => setOpen((o) => !o)}>
        Share
      </Btn>
      {open && (
        <div
        ...
        >
          ...
          <div
            ...
            title={url}
          >
            /runs/<span style={{ color: 'var(--qw-crux)' }}>{traceId.slice(0, 12)}</span>
            {spanId && (
              <>
                ?spanId=<span style={{ color: 'var(--qw-crux)' }}>{spanId.slice(0, 12)}</span>
              </>
            )}
            {lens !== 'tree' && (
              <>
                {spanId ? '&' : '?'}lens=<span style={{ color: 'var(--qw-crux)' }}>{lens}</span>
              </>
            )}
          </div>

          <div className="mt-2 flex gap-2.5 font-mono text-[10.5px]" style={{ color: 'var(--qw-fg-faint)' }}>
            <span>run</span>
            <span>· selection</span>
          ...
          <div className="mt-3 flex items-center gap-2">
            <Btn size="xs" variant="soft" icon={<Icon name="link" size={12} />} onClick={copy}>
              Copy link
            </Btn>
