...
function reportOfKind<K extends string>(preview: unknown, kind: K): boolean {
  ...
  return (
    ...
    preview !== null &&
    (preview as { kind?: unknown }).kind === kind
  );
}
...
export function EvalRunCard({
  ...
  if (cases.length === 0) {
    return (
      <EmptyHint>
        No eval cases recorded for this run yet — see the Output / structure.
      </EmptyHint>
    );
  }
...
function findOperationReport(
  node: ObservabilityRunDetailNode,
): { kind: string; preview: unknown } | null {
...
export function OperationReportCard({
  node,
}: {
  node: ObservabilityRunDetailNode;
}) {
  ...
  if (!found)
    return (
      <EmptyHint>
        No structured report for this operation yet — see the Output tab.
      </EmptyHint>
    );
  ...
  switch (found.kind) {
    case "routing.report":
      return (
        <RoutingReceiptReport
          report={found.preview as CruxRoutingReportPreview}
        />
      );
    case "cache.report":
    ...
    case "compaction.report":
      return (
        <CompactionReport
          report={found.preview as CruxCompactionReportPreview}
        />
      );
    case "guardrail.report":
      return (
        <GuardrailReport report={found.preview as CruxGuardrailReportPreview} />
      );
    case "constraint.report":
      return (
        <ConstraintReport
          report={found.preview as CruxConstraintReportPreview}
        />
      );
    case "security.report":
      return (
        <SecurityReport report={found.preview as CruxSecurityReportPreview} />
      );
    case "corpus.report":
      return <CorpusReport report={found.preview as CruxCorpusReportPreview} />;
    case "indexing.report":
      return (
        <IndexingReport report={found.preview as CruxIndexingReportPreview} />
      );
    case "ingest.report":
...
export function OperationReportFor({
  node,
  kind,
}: {
  node: ObservabilityRunDetailNode;
  ...
  if (preview === undefined || !reportOfKind(preview, kind)) {
    return (
      <EmptyHint>
        No {kind.replace(".report", "")} report on this span.
      </EmptyHint>
    );
  }
  switch (kind) {
    case "routing.report":
      return (
        <RoutingReceiptReport report={preview as CruxRoutingReportPreview} />
      );
    case "cache.report":
      return <CacheReport report={preview as CruxCacheReportPreview} />;
    case "compaction.report":
      return (
        <CompactionReport report={preview as CruxCompactionReportPreview} />
      );
    case "guardrail.report":
      return <GuardrailReport report={preview as CruxGuardrailReportPreview} />;
    case "constraint.report":
      return (
        <ConstraintReport report={preview as CruxConstraintReportPreview} />
      );
    case "security.report":
  ...
  const pct =
    report.compressionRatio != null
      ...
      : before > 0
        ...
        : undefined;
  const afterW =
    before > 0 ? Math.max(4, Math.min(100, (after / before) * 100)) : 100;
  return (
    <div className="flex flex-col gap-5">
      <Section
  ...
  return (
    <div className="flex flex-col gap-3">
      <CardShell
        ...
        right={
          <Chip
            tone={blocked ? "danger" : report.action === "pass" ? "ok" : "warn"}
            dot
          >
            {report.action}
          </Chip>
                ...
                <div key={a.n ?? i} className="flex gap-3">
                  ...
                  <div className="flex-1 pb-4">
                    <div className="mb-1 flex items-center gap-2">
                      ...
                      <Chip
                        tone={
                          ok ? "ok" : a.status === "fail" ? "danger" : "warn"
                        }
                        dot
                      >
                        {a.status}
...
function PipelineFunnel({ stages }: { stages: CruxSourceStageCountsPreview }) {
  ...
  const extra = Object.keys(stages).filter(
    (k) =>
      !STAGE_ORDER.includes(k as (typeof STAGE_ORDER)[number]) &&
      typeof stages[k] === "number",
  );
        ...
        {ordered.map((name, i) => (
          <Fragment key={name}>
            ...
            >
              ...
              <div className="mt-0.5 font-mono text-[18px] font-bold">
                {(stages[name] ?? 0).toLocaleString()}
              </div>
              <div
                className="text-[10px]"
                style={{ color: "var(--qw-fg-faint)" }}
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1.5 px-3.5 py-3">
        ...
        {report.parts != null && (
          <KeyValue k="parts" v={String(report.parts)} />
        )}
        {report.chunks != null && (
          <KeyValue k="chunks" v={String(report.chunks)} />
        )}
        {report.warningCount != null && (
          <KeyValue k="warnings" v={String(report.warningCount)} />
        )}
  ...
  return (
    <>
      ...
      >
        ...
        />
        <KindTag
          kind={classifyPrimitive(node.primitive)}
...
function agentInstructions(
  ...
  const gen = gatherDescendants(node).find((n) =>
    (n.primitive ?? "").startsWith("generation"),
  );
...
export function AgentCard({
  ...
  const availableRaw = findAttribute(
    node,
    "toolNames",
    "toolsAvailable",
    "tools",
  ...
  const available = Array.isArray(availableRaw)
    ? availableRaw.filter((t): t is string => typeof t === "string")
    : [];
      ...
      {instructionsText && (
        ...
        >
          ...
          >
            ...
            >
              {insLong && !insOpen
                ? instructionsText.slice(0, INSTRUCTIONS_PREVIEW).trimEnd() +
                  "…"
                : instructionsText}
            </div>
            {insLong && (
...
export function CompositionCard({
  ...
  if (!reportOfKind(raw, "composition.report")) {
    return (
      <EmptyHint>
        No composition report — see the structure (tree / graph) for the agents.
      </EmptyHint>
    );
  }
      ...
      <CardShell
        ...
        right={
          report.wallTimeMs != null && report.serialTimeMs != null ? (
            <span>
              wall {fmtDuration(report.wallTimeMs)} vs serial{" "}
              {fmtDuration(report.serialTimeMs)}
            </span>
          ) : report.finalAgentId ? (
        ...
        {report.handoffPath && report.handoffPath.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5 px-3.5 py-3">
            {report.handoffPath.map((a, i) => {
              ...
              const revisited =
                report.handoffPath!.filter((x) => x === a).length > 1;
              return (
                <Fragment key={`${a}-${i}`}>
                  {i > 0 && (
                    <span style={{ color: "var(--qw-fg-faint)" }}>→</span>
...
export function FlowCard({
  ...
  if (steps.length === 0)
    return <EmptyHint>No steps recorded for this flow yet.</EmptyHint>;
...
function nestKindLabel(p: string | undefined): string {
  ...
  if (s.startsWith("retrieval") || s.startsWith("embedding"))
    return "retrieval";
...
function stepRanChild(
  step: ObservabilityRunDetailNode,
...
function nestSummary(node: ObservabilityRunDetailNode): string | null {
  ...
  const kids = node.children ?? [];
  if (p.startsWith("flow.run"))
    return kids.length
      ? `${kids.length} step${kids.length === 1 ? "" : "s"}`
      : null;
  if (p.startsWith("composition")) {
    const report = findArtifact(node, "composition.report")?.preview as
      | CruxCompositionReportPreview
      | undefined;
    const agents = report?.roster?.length ?? (kids.length || undefined);
    const hops = report?.handoffPath
      ? Math.max(0, report.handoffPath.length - 1)
      : undefined;
    return (
      [
        agents != null ? `${agents} agents` : null,
        hops != null ? `${hops} hops` : null,
      ]
        .filter(Boolean)
        .join(" · ") || null
    );
...
function NestedSpan({
  ...
  const isStep =
    (node.primitive ?? "").startsWith("flow.step") || node.kind === "step";
    ...
    >
      <div className="flex flex-wrap items-center gap-2 px-3 py-2.5">
        ...
        <span
          ...
          style={{ background: stepDotColor(node) }}
        />
        <KindTag
          kind={nestKindLabel(node.primitive)}
          primitive={node.primitive}
          size={8.5}
        />
        <button
        ...
        {ranChild && (
          ...
          >
            ran{" "}
            <KindTag
              kind={nestKindLabel(ranChild.primitive)}
              primitive={ranChild.primitive}
              size={8}
            />
        ...
        >
          {kids.map((c, i) => (
            <NestedSpan
              ...
              node={c}
              index={
                (c.primitive ?? "").startsWith("flow.step") ? i + 1 : undefined
              }
              depth={depth + 1}
              onSelect={onSelect}
            />
