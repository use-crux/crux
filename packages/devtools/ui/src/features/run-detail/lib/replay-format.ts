...
const KNOWN_KINDS = new Set([
  ...Object.keys(EVENT_COLOR),
  ...Object.keys(CANON_PRIMITIVE),
]);
...
export function canonicalKind(kind: string | undefined): string {
  ...
  if (k === "agent.run" || k === "agent.step" || k.startsWith("agent."))
    return "agent";
  if (k === "tool.call" || k.startsWith("tool.")) return "tool";
  if (k === "retrieval.stage" || k.startsWith("retrieval.") || k === "retrieve")
    return "retrieval";
  if (k === "flow.step" || k.startsWith("flow.")) return "flow";
  if (k === "generation" || k.startsWith("generation.") || k === "llm")
    return "generate";
  if (k === "handoff.transfer" || k.startsWith("handoff.")) return "handoff";
  if (k === "memory.read" || k === "memory.write" || k.startsWith("memory."))
    return "memory";
...
export function statusTone(status: string): ChipTone {
  if (status === "success" || status === "ok" || status === "passed")
    return "ok";
