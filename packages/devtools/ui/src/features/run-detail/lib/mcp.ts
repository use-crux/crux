...
function linkWithRole(
  links: readonly DefinitionRefLink[],
  role: string,
...
function preparationFromNode(
  ...
  const error =
    node.error && typeof node.error !== "string" ? node.error : undefined;
  return {
    nodeId: node.id,
    spanId: node.spanId,
    ...
    errorCategory:
      stringAttribute(node, "errorCategory") ??
      (typeof error?.category === "string" ? error.category : undefined),
...
export function mcpToolOrigin(
  ...
  if (
    node.primitive !== "tool.call" ||
    stringAttribute(node, "sourceKind") !== "mcp"
  ) {
    return undefined;
  }
