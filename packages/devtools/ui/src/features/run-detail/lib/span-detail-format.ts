...
export function summarizeEvent(event: CorrelatedEvent): string {
  ...
  switch (event.eventType) {
    ...
    case "embed:end": {
      ...
      const cache =
        data.cacheHitCount != null || data.cacheMissCount != null
          ...
          : "";
      const retry =
        data.retryCount != null ? `, ${Number(data.retryCount)} retries` : "";
      const truncated =
        data.truncatedCount != null
          ? `, ${Number(data.truncatedCount)} truncated`
          : "";
    ...
    case "blackboard:update": {
      const fields = Array.isArray(data.fieldsChanged)
        ? (data.fieldsChanged as string[]).join(", ")
        : "";
    ...
    case "tool:end": {
      ...
      const savings =
        typeof data.tokenSavingsEstimate === "number" &&
        data.tokenSavingsEstimate > 0
          ? `, shaped -${data.tokenSavingsEstimate}B`
          : "";
      if (data.error)
        return `✗ ${String(data.toolName ?? "tool")} (${est}${dMs}ms): ${String(data.error)}`;
...
export function getEventColor(event: CorrelatedEvent): string {
  ...
  if (event.eventType === "memory:read" || event.eventType === "memory:write") {
    ...
    if (mt === "episodic")
      return event.eventType === "memory:read"
        ? "bg-violet-400"
        : "bg-violet-500";
    if (mt === "semantic")
      return event.eventType === "memory:read" ? "bg-teal-400" : "bg-teal-500";
  }
