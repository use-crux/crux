...
export function classifyPrimitive(primitive: string | undefined): PrimitiveKind {
...
export function sourceOf(node: ObservabilityRunDetailNode | undefined): NodeSource | undefined {
  if (!node) return undefined
...
export function readMetric(node: ObservabilityRunDetailNode | undefined, key: string): number | undefined {
  if (!node) return undefined
  ...
  const own = (node.metricBuckets?.own as Record<string, number | undefined> | undefined)?.[key]
  if (typeof own === 'number') return own
  const total = (node.metricBuckets?.total as Record<string, number | undefined> | undefined)?.[key]
  if (typeof total === 'number') return total
  const flat = (node.metrics as Record<string, number | undefined> | null | undefined)?.[key]
  ...
  for (const ev of node.events ?? []) {
    if (ev.name === 'usage.observed') {
      const v = (ev.attributes as Record<string, unknown> | null | undefined)?.[key]
      if (typeof v === 'number') return v
    }
  }
  ...
  if (insp?.events) {
    for (const ev of insp.events) {
      if (ev.kind === 'usage.observed' && ev.data && typeof ev.data === 'object') {
...
export function readMetricDeep(node: ObservabilityRunDetailNode | undefined, key: string): number | undefined {
  if (!node) return undefined
  let sum = 0
  let found = false
...
export function tokensPerSecond(node: ObservabilityRunDetailNode | undefined): number | undefined {
  if (!node) return undefined
...
export function finishReasonsFor(node: ObservabilityRunDetailNode | undefined): readonly string[] {
  if (!node) return []
  const out = new Set<string>()
  for (const ev of node.events ?? []) {
    if (ev.name === 'generation.step') {
      const r = (ev.attributes as { finishReason?: unknown } | null | undefined)?.finishReason
 ...
 */
export function resolveModels(node: ObservabilityRunDetailNode | undefined): readonly ModelUse[] {
  if (!node) return []
  const out: ModelUse[] = []
  function readMeta(n: ObservabilityRunDetailNode): { model?: string; responseId?: string } {
    for (const art of n.artifacts ?? []) {
      if (art.kind === 'output' && art.preview && typeof art.preview === 'object') {
        const meta = (art.preview as { meta?: { actualModelId?: unknown; responseId?: unknown; modelId?: unknown } })
          .meta
        if (meta) {
          const m =
            typeof meta.actualModelId === 'string'
              ...
              : typeof meta.modelId === 'string'
                ? meta.modelId
                : undefined
          const r = typeof meta.responseId === 'string' ? meta.responseId : undefined
  ...
  function readProvider(n: ObservabilityRunDetailNode): string | undefined {
    ...
    const p = (n.attributes as { provider?: unknown; providerId?: unknown } | null | undefined)?.provider
    if (typeof p === 'string') return p
    const pid = (n.attributes as { providerId?: unknown } | null | undefined)?.providerId
...
export function nodeTokens(node: ObservabilityRunDetailNode | undefined): number | undefined {
...
export function gatherDescendants(node: ObservabilityRunDetailNode): readonly ObservabilityRunDetailNode[] {
...
export function findArtifact(
  ...
  for (const art of node.artifacts ?? []) {
    if (art.kind === kind && art.preview !== undefined) {
      return { preview: art.preview, sizeBytes: art.sizeBytes, contentType: art.contentType }
    }
  }
...
export function findAllArtifacts(
  node: ObservabilityRunDetailNode | undefined,
  kind: string,
): Array<{ preview: unknown; sizeBytes?: number; contentType?: string; owner: ObservabilityRunDetailNode }> {
  const out: Array<{ preview: unknown; sizeBytes?: number; contentType?: string; owner: ObservabilityRunDetailNode }> =
    []
  if (!node) return out
  function walk(n: ObservabilityRunDetailNode) {
    for (const art of n.artifacts ?? []) {
      if (art.kind === kind && art.preview !== undefined) {
        out.push({ preview: art.preview, sizeBytes: art.sizeBytes, contentType: art.contentType, owner: n })
      }
    }
...
export function gatherResolvedContexts(node: ObservabilityRunDetailNode | undefined): readonly ResolvedContext[] {
  if (!node) return []
  ...
  if (insp?.context && insp.context.length > 0) {
    for (const item of insp.context) {
      ...
      const family =
        ...
        (item.kind?.startsWith('prompt.') ? 'prompt' : item.kind?.startsWith('context.') ? 'context' : 'context')
      const label =
        item.label ??
        (data.attributes as { contextId?: string; promptId?: string } | undefined)?.contextId ??
        (data.attributes as { promptId?: string } | undefined)?.promptId ??
        (data.name as string | undefined) ??
        'context'
      const primitive = (data.primitive as string | undefined) ?? item.kind ?? 'context.resolve'
      const attrs = data.attributes as Record<string, unknown> | null | undefined
      const priority = typeof attrs?.priority === 'number' ? (attrs.priority as number) : undefined
      const source = typeof attrs?.source === 'string' ? (attrs.source as string) : undefined
      let text: string | undefined
      let body: unknown
      let sizeBytes: number | undefined
      const arts = data.artifacts as Array<{ kind?: string; preview?: unknown; sizeBytes?: number }> | undefined
      for (const art of arts ?? []) {
        if (art.kind === 'context' || art.kind === 'system' || art.kind === 'prompt') {
      ...
      byLabel.set(label, {
        ...
        durationMs: (data.timing as { durationMs?: number } | undefined)?.durationMs,
      })
    }
    if (byLabel.size > 0) {
      return Array.from(byLabel.values()).sort((a, b) => (b.priority ?? -1) - (a.priority ?? -1))
    }
  }
  ...
  function visit(n: ObservabilityRunDetailNode) {
    for (const det of n.details ?? []) {
      ...
      const label =
        ...
        (det.attributes as { contextId?: string; promptId?: string } | undefined)?.contextId ??
        (det.attributes as { promptId?: string } | undefined)?.promptId ??
        det.name ??
        'context'
      const primitive = (det as { primitive?: string }).primitive ?? 'context.resolve'
      const attrs = det.attributes as Record<string, unknown> | null | undefined
      const priority = typeof attrs?.priority === 'number' ? (attrs.priority as number) : undefined
      const source = typeof attrs?.source === 'string' ? (attrs.source as string) : undefined
      let text: string | undefined
      let body: unknown
      ...
      byLabel.set(label, {
        ...
        body,
        hasPredicate: primitive === 'context.predicate',
        durationMs: (det as { timing?: { durationMs?: number } }).timing?.durationMs,
      })
    }
...
export interface ResolvedOutput {
  text?: string
  object?: unknown
  meta?: { usage?: Record<string, number>; cost?: number; finishReason?: string; modelId?: string }
  owner?: ObservabilityRunDetailNode
}

export type OutputRenderMode = 'raw' | 'pretty'

export function unwrapOutput(preview: unknown): { text?: string; object?: unknown; meta?: ResolvedOutput['meta'] } {
  ...
  if (typeof preview === 'object') {
    ...
    const wrapped =
      'text' in obj || 'object' in obj || 'meta' in obj
        ? {
            text: typeof obj.text === 'string' ? (obj.text as string) : undefined,
            object: obj.object,
...
export function resolveOutput(
  ...
  if (own?.preview != null) return { ...unwrapOutput(own.preview), owner: node }
  ...
  let best: { result: ResolvedOutput; rank: number; order: number } | null = null
  let order = 0
  function rankOf(primitive: string): number {
    if (primitive.startsWith('generation.')) return 3
    if (primitive.startsWith('agent.')) return 2
    if (primitive.startsWith('flow') || primitive.startsWith('composition.')) return 1
    return 0
  }
  ...
  function walk(n: ObservabilityRunDetailNode) {
    for (const art of n.artifacts ?? []) {
      if (art.kind === 'output' && art.preview != null) consider(n, art.preview)
    }
    for (const d of n.details ?? []) {
      for (const art of d.artifacts ?? []) {
        if (art.kind === 'output' && art.preview != null) consider(n, art.preview)
  ...
  // Fallback to the trace projection from /api/quality/runs/{traceId}
...
export function resolveMessages(node: ObservabilityRunDetailNode | undefined): {
  ...
  if (insp?.messages && insp.messages.length > 0) {
    ...
    if (data && typeof data === 'object') {
      const obj = data as { input?: unknown; messages?: unknown; system?: unknown; prompt?: unknown }
      return {
  ...
  if (typeof preview === 'object') {
    const obj = preview as { input?: unknown; messages?: unknown; system?: unknown; prompt?: unknown }
    return {
      messages: Array.isArray(obj.messages) ? (obj.messages as ChatMessage[]) : [],
      system: typeof obj.system === 'string' ? obj.system : undefined,
      prompt: typeof obj.prompt === 'string' ? obj.prompt : undefined,
...
export function findAttribute(node: ObservabilityRunDetailNode | undefined, ...keys: readonly string[]): unknown {
  if (!node) return undefined
...
export function findNode(node: ObservabilityRunDetailNode, id: string | null): ObservabilityRunDetailNode | null {
  if (!id) return null
...
export function parsePartSource(source: string): { kind: 'context' | 'prompt'; id: string } {
  if (source.startsWith('prompt:')) return { kind: 'prompt', id: source.slice('prompt:'.length) }
  if (source.startsWith('context:')) return { kind: 'context', id: source.slice('context:'.length) }
