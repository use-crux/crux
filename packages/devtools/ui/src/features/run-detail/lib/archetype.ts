    ...
    if (n.primitive === "eval.case" || n.primitive === "scoring.case") {
      ...
      const report =
        (typeof raw === "object" && raw !== null
          ? (raw as ScoreReportShape)
          : undefined) ?? undefined;
      const judges = report?.judges ?? [];
      const judgesPassed = judges.filter(
        (j) =>
          j.status === "passed" ||
          (j.score != null && j.threshold != null && j.score >= j.threshold),
      ).length;
      rows.push({
        id: n.id,
...
export function archetypeStrip(
  ...
  switch (archetype) {
          ...
          ? [
              {
                label: "pass",
                value: `${pct}%`,
                tone: pct >= 80 ? "ok" : pct >= 50 ? undefined : "danger",
              } as StatItem,
            ]
          : []),
        cost,
    ...
    case "indexing": {
      ...
      if (t.sources != null)
        items.push({ label: "sources", value: String(t.sources) });
      if (t.chunks != null)
    ...
    case "swarm": {
      const raw = findArtifact(root, "composition.report")?.preview as
        ...
        | undefined;
      const agents = Array.isArray(raw?.roster)
        ? raw!.roster.length
        : undefined;
      const hops =
        raw?.handoffCount ??
      ...
      return [
        ...(agents != null
          ...
          : []),
        ...(hops != null
          ? [{ label: "hops", value: String(hops) } as StatItem]
          : []),
        durItem,
        tokens,
        cost,
