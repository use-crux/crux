...
describe("definitionRefLinks", () => {
  ...
  it("uses each canonical ref.id directly for generic Catalog navigation", () => {
    const links = definitionRefLinks(
      refs,
      new Set(["agent:router", "flow:onboarding"]),
    );
    expect(links[0]?.to).toEqual({
      view: "library-index",
      promptId: "agent:router",
    });
    expect(links[1]?.to).toEqual({
      view: "library-index",
      promptId: "flow:onboarding",
    });
  });

  it("keeps deleted refs visible with role, kind, source, and no dead link", () => {
    const deleted = definitionRefLinks(
      refs,
      new Set(["agent:router", "flow:onboarding"]),
    )[2];
