...
export function collectToolRequests(
...
export function resolveToolPayload(
  ...
  function walk(n: ObservabilityRunDetailNode) {
    if (n.primitive === "delegate.invoke") {
      ...
      if (typeof attrs?.delegateId === "string")
        out.delegateId ??= attrs.delegateId as string;
      if (typeof attrs?.handoffId === "string")
        out.handoffId ??= attrs.handoffId as string;
      if (typeof attrs?.inputSize === "number")
        out.inputSize ??= attrs.inputSize as number;
      const input =
        findArtifact(n, "input")?.preview ??
        findArtifact(n, "handoff.payload")?.preview;
    ...
    if (n.primitive === "handoff.prepare") {
      const attrs = n.attributes as Record<string, unknown> | null | undefined;
      if (typeof attrs?.fromAgent === "string")
        out.fromAgent ??= attrs.fromAgent as string;
      if (typeof attrs?.toAgent === "string")
        out.toAgent ??= attrs.toAgent as string;
      if (typeof attrs?.handoffId === "string")
        out.handoffId ??= attrs.handoffId as string;
      if (typeof attrs?.outputSize === "number")
        out.outputSize ??= attrs.outputSize as number;
      ...
      if (payload != null) {
        ...
        const summary =
          ...
          payload != null &&
          "data" in (payload as Record<string, unknown>)
            ? ((payload as { data?: { summary?: unknown } }).data?.summary as
                | string
                | undefined)
            : undefined;
