...
function report(
  id: string,
  over: Partial<TurnDecisionReport> = {},
): TurnDecisionReport {
  return {
    schemaVersion: 1,
    reportId: `tdr:run:${id}`,
...
describe("collectTurnReports", () => {
  it("walks the node tree and gathers every decisionReport, root first", () => {
    ...
    expect(collectTurnReports(tree).map((r) => r.turn.id)).toEqual([
      "root",
      "a",
      "b",
    ]);
    ...
    const tree: ReportNode = {
      ...
      children: [
        { id: "gen-1", decisionReport: report("a"), children: [] },
        {
          id: "no-id-no-report",
          children: [
            { id: "gen-2", decisionReport: report("b"), children: [] },
          ],
        },
      ],
    };
...
describe("warningTurnSpanIds", () => {
  it("collects ids of nodes whose turn carries a warning signal", () => {
    const tree: ReportNode = {
      id: "root",
      decisionReport: report("root", {
        turn: { id: "root", kind: "generation.call", status: "ok" },
      }),
      children: [
        {
          id: "warn",
          decisionReport: report("warn", {
            turn: { id: "warn", kind: "generation.call", status: "error" },
          }),
          children: [],
        },
