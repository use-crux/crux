...
function deriveChips(report: TurnDecisionReport): ExplainChip[] {
  ...
  const checked = report.considered.filter(
    (c) => c.disposition === "checked",
  ).length;
  const dropped = report.considered.filter(
  ...
  if (report.freshness.some((f) => f.status === "stale-used"))
    chips.push({
      id: "fresh",
      label: "Freshness stale-used",
      tone: "warning",
      icon: "alert",
      jump: "fresh",
    });
  if (
    report.decisions.some((d) => d.reason.code.startsWith("routing.fallback"))
  )
    chips.push({
      id: "fallback",
      label: "Fallback",
...
export function summaryChips(
  ...
  if (normalized.chips && normalized.chips.length > 0)
    return normalized.chips.map(fromBackend);
...
export function warningChips(
  ...
  return summaryChips(report).filter(
    (c) => c.tone === "warning" || c.tone === "danger",
  );
}
