...
import { collectFailingSpanIds, flatStatuses, nodesOnFailurePath, stepFailure } from './triage'
...
function node(
  ...
  return { id, kind: 'composition', label: id, status, startedAt: 0, children, depth }
}
...
describe('collectFailingSpanIds', () => {
  it('lists failing spans pre-order, top→bottom', () => {
    expect(collectFailingSpanIds(tree)).toEqual(['root', 'sections', 'sec', 'draft'])
  })
})
...
describe('nodesOnFailurePath', () => {
  ...
  it('is empty when nothing failed', () => {
    expect(nodesOnFailurePath(node('ok', 'success', [node('c', 'success', [], 1)])).size).toBe(0)
  })
})

describe('flatStatuses', () => {
  it('emits one status per span, pre-order over the whole tree', () => {
    expect(flatStatuses(tree)).toEqual(['error', 'success', 'error', 'success', 'error', 'error'])
  })
})
