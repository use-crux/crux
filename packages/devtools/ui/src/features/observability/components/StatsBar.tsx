import type { Trace, BudgetSnapshotData, StatsData, TimeseriesBucket } from '@/types'
import { InlineBudgetGauge } from './BudgetGauge'
import { ErrorGroupBar } from './ErrorGroupBar'
import { formatDuration, formatCost } from '@/features/observability/lib/timeline-helpers'
...
export function StatsBar({
  ...
  const successRate = total > 0 ? Math.round((successCount / (successCount + errorCount || 1)) * 100) : 0
  const completed = traces.filter((t) => t.durationMs !== undefined)
  const avgDuration =
    completed.length > 0 ? Math.round(completed.reduce((s, t) => s + (t.durationMs ?? 0), 0) / completed.length) : 0
  const totalCost = stats?.totalCost ?? traces.reduce((s, t) => s + (t.result?.cost ?? 0), 0)
  ...
  const sparkline = (data: number[] | undefined, color: string) => {
    ...
    return (
      <svg width={w} height={h} className="inline-block ml-1 align-middle opacity-60">
        {bandHeight > 0 && <rect x={0} y={bandTop} width={w} height={bandHeight} fill="#3f3f46" opacity={0.4} rx={1} />}
    ...
    <>
      <div className="flex items-center gap-6 bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-2 text-xs">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-zinc-200 font-medium tabular-nums">{total}</span>
          {sparkline(execSparkline, '#a1a1aa')}
          {execSparkline &&
            execSparkline.length >= 4 &&
            (() => {
              const recent = execSparkline.slice(-Math.ceil(execSparkline.length / 2))
              const older = execSparkline.slice(0, Math.ceil(execSparkline.length / 2))
        ...
        {totalCost > 0 && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-zinc-200 font-medium tabular-nums">{formatCost(totalCost)}</span>
