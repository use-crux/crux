...
export function FlowGroupHeader({
  ...
  const statusIcon = group.isRunning
    ? "\u25CF"
    : group.hasError
      ? "\u2717"
      : "\u2713";
  ...
  const hasSelectedTrace = group.traces.some(
              ...
              {stepGroup.stepLabel !== "_ungrouped" && (
                <div className="flex items-center gap-2 px-4 py-0.5 text-[10px]">
                  ...
                  <span className="text-zinc-700">
                    {stepGroup.traces.length}x
                  </span>
                </div>
              )}
              {stepGroup.traces.map((trace, tIdx) => {
                const avgCost = trace.promptId
                  ? anomalies.avgCostByPrompt.get(trace.promptId)
                  : undefined;
                const costDev =
                  avgCost && trace.result?.cost != null
                    ? (trace.result.cost - avgCost) / avgCost
                    : undefined;
                return (
                  <TraceListRow
                    key={trace.traceId}
                    ...
                    isSlow={
                      ...
                      trace.durationMs >= anomalies.durationP90
                    }
