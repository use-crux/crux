...
function TraceHoverPreview({ trace }: { trace: Trace }) {
  ...
  return (
    <div className="space-y-1 text-xs">
      <div className="font-medium text-zinc-200">{trace.promptId ?? 'unnamed'}</div>
...
export function InlineHandoff({ event }: { event: AgentEventData }) {
  ...
  const d = event as Partial<{ fromAgent: string; toAgent: string; summary: string }>
...
export function TraceListRow({
  ...
  const roleColor = trace.role ? (ROLE_COLORS[trace.role] ?? 'text-zinc-500') : 'text-zinc-500'
  const dotColor = trace.role ? (ROLE_DOT_COLORS[trace.role] ?? 'bg-zinc-600') : 'bg-zinc-600'
  const lineColor = trace.role ? (ROLE_LINE_COLORS[trace.role] ?? 'border-zinc-700') : 'border-zinc-700'
      ...
      <TooltipTrigger asChild>
        <div className="relative">
          ...
          {showTreeLines && nestLevel > 0 && (
            <div className="absolute left-0 top-0 bottom-0" style={{ width: nestLevel * 20 }}>
              <div
          ...
          >
            {showTreeLines && nestLevel > 0 && (
              <span className={`w-2 h-2 rounded-full ${dotColor} shrink-0 relative z-10`} />
            )}
            {(!showTreeLines || nestLevel === 0) && (
              <span
            ...
            <span className="text-zinc-500 tabular-nums w-16 shrink-0">{formatTime(trace.startedAt)}</span>
            {roleLabel && <span className={`${roleColor} w-14 shrink-0 font-mono text-[10px]`}>{roleLabel}</span>}
            <span className="font-mono text-zinc-200 truncate">{trace.promptId ?? 'unnamed'}</span>
            {flowBreadcrumb && (
              <span className="text-zinc-600 text-[10px] truncate max-w-[100px]">{flowBreadcrumb}</span>
            )}
            {shortModel && <span className="text-zinc-600 truncate text-[10px] max-w-[100px]">{shortModel}</span>}
            ...
            {showTreeLines && nestLevel > 0 && <span className={`${statusColor} text-[10px]`}>{statusIcon}</span>}
            <span className={`${statusColor} tabular-nums ml-auto flex items-center gap-1.5`}>
              {trace.status === 'running' ? (
