...
export function ParetoChart({
  ...
  const points = useMemo(() => {
    ...
    const byModel = new Map<
      string,
      { costs: number[]; traceIds: Set<string> }
    >();
  ...
  if (points.length < 2) {
    return (
      <div className="text-xs text-zinc-500">
        Need 2+ models with judge scores for Pareto analysis
      </div>
    );
  }
  ...
  const toX = (cost: number) =>
    PAD + ((cost - minCost) / costRange) * (W - PAD * 2);
  const toY = (score: number) =>
    H - PAD - ((score - minScore) / scoreRange) * (H - PAD * 2);
  ...
  const frontierPath =
    frontier.length >= 2
      ? frontier
          .map(
            (p, i) =>
              `${i === 0 ? "M" : "L"}${toX(p.avgCost)},${toY(p.avgScore)}`,
          )
          .join(" ")
      : undefined;
    ...
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-[500px]">
        ...
        <text
          ...
          y={H - PAD + 14}
          textAnchor="middle"
          className="fill-zinc-600 text-[8px] tabular-nums"
        >
          ${minCost.toFixed(4)}
        </text>
        <text
        ...
        {points.map((p) => (
          <div key={p.label} className="flex items-center gap-1.5">
            <span
              ...
              style={{ backgroundColor: colorForScore(p.avgScore) }}
            />
            <span className="font-mono text-zinc-300">{p.label}</span>
            <span className="text-zinc-600">
              score:{p.avgScore.toFixed(2)} cost:${p.avgCost.toFixed(4)} (
              {p.traceCount})
            </span>
          </div>
        ))}
