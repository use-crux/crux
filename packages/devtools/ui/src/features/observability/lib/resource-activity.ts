...
function attrs(activity: ObservabilityResourceActivity): Record<string, unknown> {
...
export function memoryEventsFromResourceActivity(activity: ObservabilityResourceActivity[]): MemoryEventData[] {
  return activity
    ...
    .map((item): MemoryEventData => {
      ...
      const base = {
        ...
        memoryType: (stringValue(a.memoryType) as MemoryEventData['memoryType']) ?? 'block',
      ...
      return {
        ...
        proposalStatus: stringValue(a.proposalStatus) as MemoryWriteEvent['proposalStatus'],
      }
    })
}

export function workspaceEventsFromResourceActivity(activity: ObservabilityResourceActivity[]): WorkspaceOperationEvent[] {
  return activity
...
export function planEventsFromResourceActivity(activity: ObservabilityResourceActivity[]): PlanEventData[] {
  return activity
    .filter((item) => item.family === 'plan')
    .map((item): PlanEventData => {
      const a = attrs(item)
      const preview = artifactPreview(item)
      const p = typeof preview === 'object' && preview !== null ? (preview as Record<string, unknown>) : {}
      const operation = operationFromName(item)
      const planId = stringValue(a.planId) ?? stringValue(p.planId) ?? item.resourceId
      if (operation === 'update') {
        return {
          type: 'plan:updated',
          _kind: 'updated',
          ...
          changes: stringList(a.changes).filter((change): change is 'title' | 'content' | 'status' | 'metadata' =>
...
export function taskEventsFromResourceActivity(activity: ObservabilityResourceActivity[]): {
  ...
  for (const item of activity.filter((entry) => entry.family === 'task')) {
    ...
    const taskListId = stringValue(a.taskListId) ?? stringValue(p.taskListId) ?? item.resourceId
    ...
    if (operation === 'tasklist.create') {
      ...
      taskListEvents.push({
        ...
        _kind: 'discarded',
        taskListId,
        completedCount: numberValue((p.counts as Record<string, unknown> | undefined)?.completed) ?? 0,
        remainingCount: numberValue((p.counts as Record<string, unknown> | undefined)?.remaining) ?? 0,
        timestamp,
      })
      ...
      taskEvents.push({
        ...
        _kind: 'added',
        taskListId,
        taskId: stringValue(a.taskId) ?? stringValue(p.taskId) ?? item.resourceId,
