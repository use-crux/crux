      ...
      {files.length === 0 ? (
        ...
        <div className="px-2 pb-4">
          {tree.children.map((n, i) => (
            <TreeNodeRow key={`${n.type}-${i}`} node={n} selectedPath={selectedPath} onSelect={onSelect} />
          ))}
        </div>
      )}
    ...
    return (
      <div>
        ...
        {node.children.map((c, i) => (
          <TreeNodeRow key={`${c.type}-${i}`} node={c} selectedPath={selectedPath} onSelect={onSelect} />
        ))}
      </div>
    )
  }
  ...
  return (
    <button
      ...
      style={{
        ...
        borderLeft: isErr ? '2px solid var(--qw-danger)' : '2px solid transparent',
        marginLeft: -2,
      }}
    >
      <Icon name="doc" size={11} color={isErr ? 'var(--qw-danger)' : on ? 'var(--qw-crux)' : 'var(--qw-fg-muted)'} />
      <span
        className="truncate font-mono text-[11.5px]"
        style={{
          color: isErr ? 'var(--qw-danger)' : on ? 'var(--qw-crux)' : 'var(--qw-fg)',
