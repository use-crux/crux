/**
 * Catalog v2 — full-width detail view (right pane of the browser).
 *
 * Ported from the design's catalog-detail.jsx. Fixed anatomy:
...
function CatRelations({ def }: { def: ViewDef }) {
  ...
  const Col = ({ title, edges, dir }: { title: string; edges: typeof rels.incoming; dir: 'in' | 'out' }) => (
    <div>
      <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: T.fgFaint, fontWeight: 500, marginBottom: 10 }}>
        {title} · {edges.length}
      </div>
          ...
          edges.map((r) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ fontFamily: T.mono, fontSize: 12, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{otherId}</span>
  ...
  return (
    <>
      <SectionHead eyebrow="Relations" right={<span style={{ fontFamily: T.mono, fontSize: 11, color: T.fgFaint }}>{rels.incoming.length} in · {rels.outgoing.length} out</span>} />
...
function catSectionOrder(def: ViewDef): string[] {
  ...
  if (k.startsWith('routing.')) return ['hero', 'config', 'control', 'deps', 'source', 'observability', 'relations', 'health', 'quality']
  if (k.startsWith('composition.')) return ['hero', 'control', 'config', 'deps', 'data', 'observability', 'relations', 'quality', 'health']
  if (k.startsWith('rag.')) return ['hero', 'config', 'deps', 'data', 'source', 'observability', 'quality', 'relations', 'health']
  if (k.startsWith('eval.')) return ['hero', 'quality', 'config', 'source', 'relations', 'health']
  if (k === 'memory' || k === 'blackboard') return ['hero', 'config', 'contract', 'data', 'source', 'observability', 'relations', 'health']
...
// ── the detail view ──────────────────────────────────────────────────────────
export function CatalogDetail({ def, onExpand }: { def: ViewDef | undefined; onExpand?: () => void }) {
  const idx = useCatalogIndex()
  if (!def) return <div style={{ padding: 40, color: T.fgFaint, fontFamily: T.mono, fontSize: 13 }}>Select a definition</div>
  ...
  if (q && q.passRate != null) {
    ...
    props.push(
      <span key="pr" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: T.mono, fontSize: 11, color: T.fgMuted }}>
        <span style={{ width: 40 }}>
  ...
  if (def.runtimeJoin) {
    props.push(
      <span key="rj" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: T.mono, fontSize: 11, color: T.crux, background: T.cruxSoft, padding: '2px 8px', borderRadius: 5 }}>
  ...
  if (directLints.length) {
    props.push(
      <span key="lint" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: T.mono, fontSize: 11, color: directLints.some((l) => l.severity === 'warning') ? T.warn : T.iris }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ fontFamily: T.mono, fontSize: 23, fontWeight: 600, letterSpacing: '-0.02em' }}>{def.name}</span>
            ...
            {def.description && <p style={{ margin: '8px 0 0', fontFamily: T.serif, fontSize: 14.5, lineHeight: 1.55, color: T.fgMuted, maxWidth: 760 }}>{def.description}</p>}
          </div>
      ...
      <div style={{ padding: '22px 30px 40px' }}>
        ...
        {chips.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
            {chips.map(([key, v]) => (
              <span key={key} style={{ display: 'inline-flex', alignItems: 'baseline', gap: 6, padding: '4px 10px', borderRadius: 6, background: T.bgElev, border: `1px solid ${T.border}`, fontFamily: T.mono, fontSize: 11 }}>
