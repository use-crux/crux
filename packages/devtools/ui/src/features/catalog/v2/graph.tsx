/**
 * Catalog v2 — architecture graph (relations as nodes + edges).
 *
 * Ported from the design's catalog-graph.jsx. Standalone definitions are
...
const GRAPH_STRUCTURAL = /includes_step|includes_route|includes_tier|includes_option|includes_block|includes_case|uses_store/
...
function liftId(idx: CatalogIndex, id: string): string {
  let d = idx.byId(id)
  let guard = 0
  while (d && d.presentation && d.presentation.parentDefinitionId && guard++ < 6) {
    d = idx.byId(d.presentation.parentDefinitionId)
  }
  return d ? d.id : id
}

function buildGraphModel(idx: CatalogIndex): GModel {
  const nodes: GNode[] = idx.standalone.map((d) => ({ id: d.id, kind: d.kind, name: d.name, family: kindMeta(d.kind).family }))
...
function forceLayout(nodes: GNode[], edges: GEdge[], W: number, H: number, iterations: number): Record<string, Pos> {
  const rng = mulberry32(1337)
  const pos: Record<string, Pos> = {}
  const N = nodes.length
  nodes.forEach((n, i) => {
    const a = (i / N) * Math.PI * 2
    pos[n.id] = { x: W / 2 + Math.cos(a) * 260 + (rng() - 0.5) * 40, y: H / 2 + Math.sin(a) * 220 + (rng() - 0.5) * 40, vx: 0, vy: 0 }
  })
  const REP = 5200
  const SPRING = 0.012
  const LEN = 130
...
export function CatalogGraph({ onOpenDetail, initialSelected }: { onOpenDetail?: (id: string) => void; initialSelected?: string | null }) {
  ...
  const visEdges = model.edges.filter((e) => visSet.has(e.from) && visSet.has(e.to))
  const pos = useMemo(() => forceLayout(model.nodes, model.edges, W, H, 320), [model])
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: T.bg, position: 'relative', minHeight: 0 }}>
      {/* controls */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderBottom: `1px solid ${T.border}`, flex: '0 0 auto', flexWrap: 'wrap' }}>
        <Icon name="grid" size={15} color={T.crux} />
        <span style={{ fontSize: 13, fontWeight: 600 }}>Architecture graph</span>
        <span style={{ fontFamily: T.mono, fontSize: 11, color: T.fgFaint }}>{visNodes.length} nodes · {visEdges.length} edges</span>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
          <Btn size="xs" onClick={() => setView((v) => ({ ...v, s: Math.min(3, v.s * 1.2) }))}>
            +
          </Btn>
          <Btn size="xs" onClick={() => setView((v) => ({ ...v, s: Math.max(0.3, v.s / 1.2) }))}>
            −
          </Btn>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex' }}>
          ...
          <div style={{ position: 'absolute', left: 0, top: 0, transform: `translate(${view.x}px,${view.y}px) scale(${view.s})`, transformOrigin: '0 0' }}>
            <svg width={W} height={H} style={{ position: 'absolute', left: 0, top: 0, overflow: 'visible', pointerEvents: 'none' }}>
              {visEdges.map((e, i) => {
                ...
                const c = toneColor(T, fromNode ? kindMeta(fromNode.kind).tone : 'muted')
                ...
                return (
                  <g key={i} opacity={dim ? 0.08 : active ? 1 : 0.34}>
                    <path d={`M${a.x},${a.y} Q${mx},${my} ${b.x},${b.y}`} fill="none" stroke={c.fg} strokeWidth={active ? 1.8 : 1} />
            ...
            {visNodes.map((n) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{ fontFamily: T.mono, fontSize: 11, fontWeight: on ? 600 : 450, color: on ? c.fg : T.fg }}>{n.name}</span>
                </div>
              )
            })}
          </div>
        ...
        {/* drawer */}
        {sel && (
          <aside style={{ width: 320, flex: '0 0 320px', borderLeft: `1px solid ${T.border}`, background: T.bg, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            <div style={{ padding: '16px 18px', borderBottom: `1px solid ${T.border}`, background: toneColor(T, kindMeta(sel.kind).tone).soft }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <KindGlyph kind={sel.kind} size={32} />
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontFamily: T.mono, fontSize: 15, fontWeight: 600 }}>{sel.name}</div>
                  <div style={{ marginTop: 4 }}>
                    <KindBadge kind={sel.kind} />
                  </div>
                </div>
                <Btn size="xs" icon="x" variant="ghost" style={{ marginLeft: 'auto' }} onClick={() => setSelected(null)}>
                  Close
                </Btn>
              </div>
              {sel.description && <p style={{ margin: '10px 0 0', fontFamily: T.serif, fontSize: 12.5, lineHeight: 1.5, color: T.fgMuted }}>{sel.description}</p>}
            </div>
            <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: '14px 18px' }}>
              {(['outgoing', 'incoming'] as const).map((dir) => {
                const es = visEdges.filter((e) => (dir === 'outgoing' ? e.from : e.to) === sel.id)
                if (!es.length) return null
                return (
                  <div key={dir} style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: T.fgFaint, fontWeight: 500, marginBottom: 8 }}>
                      {dir === 'outgoing' ? 'Depends on' : 'Used by'} · {es.length}
                    </div>
                      ...
                      {es.map((e, i) => {
                        ...
                        return (
                          <button
                            ...
                            style={{ all: 'unset', cursor: 'pointer', display: 'grid', gridTemplateColumns: '20px 1fr auto', gap: 8, alignItems: 'center', padding: '6px 8px', background: T.bgElev, border: `1px solid ${T.border}`, borderRadius: 7 }}
                          >
                            <KindGlyph kind={od.kind} size={20} />
                            <span style={{ fontFamily: T.mono, fontSize: 11.5, color: T.fg, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{od.name}</span>
                            <span style={{ fontFamily: T.mono, fontSize: 9, color: T.fgFaint }}>{e.type.replace(/_/g, ' ')}</span>
                          </button>
                        )
                      })}
                    </div>
              ...
              {onOpenDetail && (
                <Btn size="md" variant="primary" iconRight="arrowRight" onClick={() => onOpenDetail(sel.id)} style={{ width: '100%', justifyContent: 'center', marginTop: 4 }}>
                  Open detail
                </Btn>
              )}
            </div>
