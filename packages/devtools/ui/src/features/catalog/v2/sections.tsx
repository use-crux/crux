/**
 * Catalog v2 — mid/low-tier detail sections.
 *
 * Ported from the design's catalog-sections.jsx:
 *   · CatalogQuality       — exhaustive definition.quality (pass rate, run
 *     breakdown, coverage, linked artifacts, fingerprints, drift table)
 *   · CatalogObservability — runtimeJoin span correlation card
 *   · CatalogDiagnostics    — intelligence.diagnostics
 *   · CatalogHealthSection  — lint findings (direct + via deps)
 *   · CatalogProvenance     — the quiet "everything else" card
 *
...
import { useCatalogIndex } from './context'
...
// ── QUALITY (exhaustive) ─────────────────────────────────────────────────────
export function CatalogQuality({ def }: { def: ViewDef }) {
  const q = def.quality
  ...
  return (
    <>
      <SectionHead
        ...
        right={
          <span style={{ display: 'flex', gap: 6 }}>
            {q.lastStatus && (
              <Chip tone={q.lastStatus === 'pass' || q.lastStatus === 'ok' ? 'ok' : q.lastStatus === 'running' ? 'crux' : 'danger'} dot>
                {q.lastStatus}
              </Chip>
            )}
      ...
      />
      <div style={{ background: T.bgElev, border: `1px solid ${T.border}`, borderRadius: 11, overflow: 'hidden', marginBottom: 22 }}>
        {/* headline */}
        <div style={{ display: 'flex', gap: 24, padding: '16px 18px', flexWrap: 'wrap', alignItems: 'center' }}>
          {pr != null && (
            <div style={{ minWidth: 116 }}>
              <div style={{ fontFamily: T.mono, fontSize: 30, fontWeight: 600, letterSpacing: '-0.02em', color: c.fg }}>{Math.round(pr * 100)}%</div>
              <div style={{ fontSize: 11, color: T.fgFaint, fontFamily: T.mono, marginBottom: 7 }}>pass rate{q.caseCount ? ` · ${q.caseCount} cases` : ''}</div>
          ...
          {(comp != null || fail != null || run != null) && (
            <>
              ...
              <div style={{ minWidth: 150 }}>
                <div style={{ fontFamily: T.mono, fontSize: 10, color: T.fgFaint, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>{runs} runs</div>
          ...
          {lastRunAt && (
            <>
              <div style={{ width: 1, background: T.border, alignSelf: 'stretch' }} />
              <div style={{ fontFamily: T.mono, fontSize: 11, color: T.fgMuted, display: 'flex', flexDirection: 'column', gap: 4 }}>
                <span>
        ...
        {(q.evalIds || q.suiteIds || usedArtifacts.length > 0) && (
          <div style={{ borderTop: `1px solid ${T.border}`, padding: '12px 18px', display: 'flex', gap: 26, flexWrap: 'wrap', alignItems: 'center' }}>
            {q.evalIds && (
              <div>
                <div style={{ fontFamily: T.mono, fontSize: 9.5, color: T.fgFaint, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>evals</div>
            ...
            {q.suiteIds && (
              <div>
                <div style={{ fontFamily: T.mono, fontSize: 9.5, color: T.fgFaint, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>suites</div>
                <span style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  {q.suiteIds.map((s) => (
            ...
            {usedArtifacts.length > 0 && (
              <div style={{ marginLeft: 'auto' }}>
                <div style={{ fontFamily: T.mono, fontSize: 9.5, color: T.fgFaint, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 5 }}>linked</div>
                <span style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  {usedArtifacts.map(([label, n, icon]) => (
                    <span key={label} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '2px 8px', borderRadius: 5, background: T.bg, border: `1px solid ${T.border}`, fontFamily: T.mono, fontSize: 10.5, color: T.fgMuted }}>
        ...
        {(q.currentFingerprint || q.baselineFingerprint) && (
          <div style={{ borderTop: `1px solid ${T.border}`, padding: '10px 18px', display: 'flex', gap: 18, alignItems: 'center', fontFamily: T.mono, fontSize: 11 }}>
        ...
        {drift && drift.length > 0 && (
          <div style={{ borderTop: `1px solid ${T.border}` }}>
            <div style={{ display: 'grid', gridTemplateColumns: '24px 1fr 90px 90px 70px 70px', padding: '8px 18px', gap: 10, fontSize: 9.5, color: T.fgFaint, textTransform: 'uppercase', letterSpacing: '0.1em', background: T.bgMuted }}>
              <div />
              <div>check</div>
            ...
            {drift.map((d, i) => (
              <div key={d.id} style={{ display: 'grid', gridTemplateColumns: '24px 1fr 90px 90px 70px 70px', padding: '9px 18px', gap: 10, alignItems: 'center', fontFamily: T.mono, fontSize: 11.5, borderTop: i ? `1px solid ${T.border}` : 'none' }}>
                ...
                <span style={{ textAlign: 'right', color: T.fg, fontWeight: 600 }}>{Math.round(d.passRate * 100)}%</span>
                <span style={{ textAlign: 'right', color: T.fgFaint }}>{d.runs}</span>
                <span style={{ textAlign: 'right', fontWeight: 600, color: d.driftPp < 0 ? T.danger : d.driftPp > 0 ? T.ok : T.fgMuted }}>
                  {d.driftPp > 0 ? '+' : ''}
                  {d.driftPp}pp
                </span>
              </div>
...
export function CatalogObservability({ def }: { def: ViewDef }) {
  ...
  return (
    <>
      ...
      />
      <div style={{ background: T.bgElev, border: `1px solid ${T.border}`, borderLeft: `3px solid ${T.crux}`, borderRadius: 11, padding: '14px 18px', marginBottom: 22 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
          <Icon name="trace" size={15} color={T.crux} />
          <span style={{ fontFamily: T.mono, fontSize: 14, fontWeight: 600, color: T.fg }}>{rjn.spanName || rjn.primitive}</span>
          {rjn.primitive && (
            <Chip tone="crux" mono>
              {rjn.primitive}
            </Chip>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {rjn.correlationAttributes && (
              <div>
                <div style={{ fontFamily: T.mono, fontSize: 9.5, color: T.fgFaint, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }}>correlation attributes</div>
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  {rjn.correlationAttributes.map((a) => (
                    <span key={a} style={{ fontFamily: T.mono, fontSize: 10.5, padding: '2px 7px', borderRadius: 4, background: T.cruxSoft, color: T.crux }}>
                      {a}
                    </span>
                  ))}
                </div>
            ...
            {rjn.spanAttributes && (
              <div>
                <div style={{ fontFamily: T.mono, fontSize: 9.5, color: T.fgFaint, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }}>span attributes</div>
...
export function CatalogDiagnostics({ def }: { def: ViewDef }) {
  ...
  return (
    <div style={{ marginBottom: 22 }}>
      {ds.map((d, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '10px 14px', background: T.bg, border: `1px dashed ${c.line}`, borderRadius: 8, marginBottom: 8 }}>
...
// ── HEALTH (lint) ────────────────────────────────────────────────────────────
export function CatalogHealthSection({ def }: { def: ViewDef }) {
  ...
  return (
    <>
      <SectionHead eyebrow="Health" right={<span style={{ fontFamily: T.mono, fontSize: 11, color: T.fgFaint }}>{direct.length} direct · {transitive.length} via deps</span>} />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 22 }}>
        {[...direct, ...transitive].map((fnd) => {
          ...
          return (
            <div key={fnd.id} style={{ background: T.bgElev, border: `1px solid ${T.border}`, borderLeft: `3px solid ${lc.fg}`, borderRadius: 10, padding: '12px 16px', opacity: isDirect ? 1 : 0.92 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontFamily: T.mono, fontSize: 9.5, color: lc.fg, background: lc.soft, padding: '1px 6px', borderRadius: 3 }}>{fnd.category}</span>
              ...
              <div style={{ fontFamily: T.serif, fontSize: 13, color: T.fgMuted, lineHeight: 1.5, marginBottom: 8 }}>{fnd.rationale}</div>
              {fnd.fix && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px', background: T.bg, border: `1px solid ${T.border}`, borderRadius: 7 }}>
                  <span style={{ fontFamily: T.mono, fontSize: 9.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: lc.fg, fontWeight: 600 }}>fix</span>
                  <span style={{ fontSize: 12, color: T.fg }}>{fnd.fix}</span>
                  {fnd.suppression && <span style={{ marginLeft: 'auto', fontFamily: T.mono, fontSize: 10, color: T.fgFaint }}>{fnd.suppression.directive}</span>}
                </div>
              )}
            </div>
          )
...
export function CatalogProvenance({ def }: { def: ViewDef }) {
  const idx = useCatalogIndex()
  const m = kindMeta(def.kind)
  const indexing = idx.indexing
  const Row = ({ k, children }: { k: string; children?: ReactNode }) =>
    children != null && children !== '' ? (
      <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: 12, padding: '6px 0', borderTop: `1px solid ${T.border}` }}>
        <span style={{ fontFamily: T.mono, fontSize: 10.5, color: T.fgFaint, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{k}</span>
        <span style={{ fontFamily: T.mono, fontSize: 11.5, color: T.fg }}>{children}</span>
      </div>
    ) : null
  return (
    <>
      <SectionHead eyebrow="Provenance & indexing" right={<span style={{ fontFamily: T.mono, fontSize: 11, color: T.fgFaint }}>read-model metadata</span>} />
      <div style={{ background: T.bgElev, border: `1px solid ${T.border}`, borderRadius: 11, padding: '6px 18px 14px', marginBottom: 8, display: 'grid', gridTemplateColumns: '1fr 1fr', columnGap: 36 }}>
        <div>
        ...
        <div>
          ...
          {indexing && <Row k="ast index">{indexing.ast.status}{indexing.ast.indexedAt ? ` · ${indexing.ast.indexedAt}` : ''}</Row>}
          {indexing && <Row k="semantic index">{indexing.semantic.status}{indexing.semantic.indexedAt ? ` · ${indexing.semantic.indexedAt}` : ''}</Row>}
        </div>
      </div>
    </>
  )
