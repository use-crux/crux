...
function PlansOverview() {
  ...
  const featured = useMemo(
    () => pickActivePlan(filtered) ?? pickActivePlan(list),
    [filtered, list],
  );
      ...
      actions={
        <>
          <Btn
            ...
            disabled
            title="Diff versions — pending backend"
          >
            Diff versions
          </Btn>
          <Btn
      ...
      <div className="mx-auto w-full max-w-7xl px-8 py-6">
        ...
        <div className="mb-6 grid grid-cols-5 gap-3">
          ...
          <Kpi
            label="Avg task dur"
            value={
              kpis.avgDur != null ? (fmtDuration(kpis.avgDur) ?? "—") : "—"
            }
            sublabel={
              kpis.p99Dur != null
                ? `P99 · ${fmtDuration(kpis.p99Dur)}`
                : undefined
            }
          />
        </div>
        ...
        {list.length === 0 ? (
          <EmptyHint>
            No plans have been observed yet. They appear here as soon as your
            app creates a plan through the Crux plan primitive.
          </EmptyHint>
        ) : filtered.length === 0 ? (
            ...
            <div className="flex min-w-0 flex-col gap-3.5">
              {featured && (
                <FeaturedPlanCard
                  ...
                  onOpen={() =>
                    navigate({ view: "library-plans", planId: featured.id })
                  }
                />
              )}
              {featured && (
              ...
              {filtered.length > 1 && (
                <OtherPlansList
                  ...
                  onOpen={(id) =>
                    navigate({ view: "library-plans", planId: id })
                  }
                />
              )}
            </div>
...
function FeaturedPlanCard({
  ...
  return (
    ...
    >
          ...
          <div className="mb-1 flex flex-wrap items-center gap-2">
            ...
            >
              {plan.versionCount != null
                ...
                : ""}
              {plan.lastUpdatedAt
                ? ` · last ${fmtTime(plan.lastUpdatedAt) ?? fmtRelative(plan.lastUpdatedAt)}`
                : ""}
            </span>
          </div>
          {plan.title && (
      ...
      <div
        className="grid gap-5 px-4 py-3.5"
    ...
    >
      ...
      {tasks.length === 0 ? (
        ...
        >
          ...
          starts emitting <span className="font-mono">task.added</span> events
          at runtime.
        </div>
      ) : (
        <>
...
function TimelineRow({
  ...
  return (
    <div
      ...
      style={{ gridTemplateColumns: "54px 18px minmax(0, 1fr)" }}
    >
      <span
