/**
 * Insights — derived diagnoses from backend Quality/observability records.
 */
...
import { useQualityInsights, useQualityInsightSilences, useQualityRuns } from '@/shared/hooks/useQualityApi'
import { useInsightMutation, useSilenceMutation, useUnsilenceMutation } from '@/shared/hooks/useQualityMutations'
...
import type { QualityInsightRecord, QualityInsightSilence, QualityRunRecord } from '@/types'
...
export type InsightsGroupBy = 'none' | 'severity' | 'target' | 'status' | 'title' | 'tag'
...
export function InsightsView({ filters, groupBy }: InsightsProps) {
  ...
  const { data: bffInsights, loading: insightsLoading, reload } = useQualityInsights()
  const initialLoading = insightsLoading && !bffInsights
  const { data: silenceList } = useQualityInsightSilences()
  const activeSilences = useMemo(() => (silenceList ?? []).filter((s) => !s.deletedAt), [silenceList])
  ...
  const { data: allRuns } = useQualityRuns({})
  const traceLookup = useMemo(() => {
    const m = new Map<string, QualityRunRecord>()
  ...
  const statusFilter =
    filters.status && filters.status.length > 0 ? new Set(filters.status as readonly string[]) : new Set(['open'])
  ...
  const insights = useMemo(() => {
    return (bffInsights ?? []).filter((i) => {
      ...
      if (targetFilter.size > 0 && (!i.targetId || !targetFilter.has(i.targetId))) return false
      if (titleFilter.size > 0 && !titleFilter.has(i.title)) return false
      if (tagFilter.size > 0 && !i.tags.some((t) => tagFilter.has(t))) return false
      if (deferredSearch) {
        const hay = `${i.title} ${i.summary} ${i.targetId ?? ''} ${i.tags.join(' ')}`.toLowerCase()
        if (!hay.includes(deferredSearch)) return false
      }
      return true
    })
  }, [bffInsights, hidden, statusFilter, severityFilter, targetFilter, titleFilter, tagFilter, deferredSearch])
  ...
  const groups = useMemo(() => {
    ...
    const map = new Map<string, QualityInsightRecord[]>()
  ...
  return (
    <QwShell
      ...
      connected={connected}
      badges={{ insights: insights.length > 0 ? { count: insights.length, tone: 'danger' } : undefined }}
      filterBar={
        <InsightsFilterChips
          filters={filters}
    ...
    >
      ...
      >
        {activeSilences.length > 0 && <SilencesStrip silences={activeSilences} onUnsilence={handleUnsilence} />}
        ...
        {!initialLoading && insights.length === 0 && (
          ...
          >
            No insights match the active filters. Diagnoses show up when failed experiment cases, repeated tool loops,
            or cassette mismatches are detected locally.
          </div>
        )}
...
function SilencesStrip({
  ...
  silences: readonly QualityInsightSilence[]
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5">
        {visible.map((s) => (
          ...
          >
            <span style={{ color: 'var(--qw-fg-faint)' }}>{s.pattern.targetId ? 'pattern:' : 'title:'}</span>
            <span>{s.pattern.title}</span>
            {s.pattern.targetId && <span style={{ color: 'var(--qw-fg-faint)' }}>· {s.pattern.targetId}</span>}
...
function InsightsFilterChips({
  ...
  function update<K extends keyof InsightsFilters>(key: K, value: InsightsFilters[K]) {
  ...
  const addOptions: ReadonlyArray<AddFilterOption<Kind>> = [
    { kind: 'title', label: 'Type', enabled: !has.title && distinctTitles.length > 0 },
    { kind: 'tag', label: 'Tag', enabled: !has.tag && distinctTags.length > 0 },
    { kind: 'severity', label: 'Severity', enabled: !has.severity },
    { kind: 'target', label: 'Target', enabled: !has.target && distinctTargets.length > 0 },
  ...
  return (
    ...
    >
      ...
      {has.severity && (
        <MultiSelectChip
          ...
          onChange={(next) =>
            update(
              'severity',
              next.filter((s) => s === 'high' || s === 'medium' || s === 'low') as readonly (
                | 'high'
                | 'medium'
                | 'low'
              )[],
      ...
      {has.status && (
        <MultiSelectChip
          ...
          onChange={(next) =>
            update(
              'status',
              next.filter((s) => s === 'open' || s === 'dismissed' || s === 'resolved') as readonly (
                | 'open'
                | 'dismissed'
                | 'resolved'
              )[],
      ...
      <AddFilterButton
        options={addOptions}
        onAdd={(kind) => {
          if (kind === 'title' && distinctTitles[0]) update('title', [distinctTitles[0]])
          else if (kind === 'tag' && distinctTags[0]) update('tag', [distinctTags[0]])
