...
function run(partial: Partial<ObservabilityRunSummary> & Pick<ObservabilityRunSummary, 'runId'>): ObservabilityRunSummary {
  return {
    runId: partial.runId,
...
describe('searchRuns', () => {
  it('matches revisioned Runs-page rows by id, name, model, and prompt', () => {
    const rows = [
      run({ runId: 'run_alpha', name: 'greeter', promptId: 'prompt:hello', model: 'gpt-test' }),
      run({ runId: 'run_beta', name: 'other', rootPrimitive: 'flow.run', model: 'claude-test' }),
    ]

    expect(searchRuns(rows, 'hello').map((r) => r.id)).toEqual(['run_alpha'])
    expect(searchRuns(rows, 'gpt-test').map((r) => r.id)).toEqual(['run_alpha'])
    expect(searchRuns(rows, 'flow.run').map((r) => r.id)).toEqual(['run_beta'])
  })

  it('caps results and navigates to run-detail by run id', () => {
    const rows = Array.from({ length: 8 }, (_, i) => run({ runId: `run_${i}`, name: `match-${i}` }))
