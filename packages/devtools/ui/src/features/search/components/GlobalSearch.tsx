...
import { useInspectInsights } from "@/shared/hooks/useInspectApi";
...
function searchPrompts(prompts: PromptMeta[], query: string): SearchResult[] {
  ...
  for (const p of prompts) {
    ...
    if (
      matches(query, p.id, p.description) ||
      p.tags.some((tag) => matches(query, tag))
    ) {
      results.push({
        category: "prompts",
        id: p.id ?? "unknown",
...
function searchInsights(
  insights: readonly InspectInsightRecord[],
  query: string,
): SearchResult[] {
  const results: SearchResult[] = [];
...
function searchJudgeEvents(
  events: JudgeEventData[],
  query: string,
): SearchResult[] {
  const results: SearchResult[] = [];
...
export function GlobalSearch({
  ...
  const { data: insightsData } = useInspectInsights();
            ...
            {entries.length > 0 && (
              <div className="flex items-center gap-3 border-t border-(--qw-border) px-3 py-1.5 text-[10px] text-(--qw-fg-faint)">
                ...
                <span className="ml-auto hidden sm:inline">
                  everything here is also reachable by pointer
                </span>
              </div>
            )}
          </div>
