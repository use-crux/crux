...
export const runtimeService = {
  ...
  inspect(
    workId: string,
    signal?: AbortSignal,
  ): Promise<RuntimeInspectResponse> {
    return fetchJson<RuntimeInspectResponse>(
      `/api/runtime/work/${encodeURIComponent(workId)}`,
      signal,
    );
  },
  ...
  async retry(workId: string): Promise<RuntimeRetryResponse> {
    const response = await postJson(
      `/api/runtime/work/${encodeURIComponent(workId)}/retry`,
      {},
    );
  ...
  async cancel(workId: string): Promise<RuntimeCancelResponse> {
    const response = await postJson(
      `/api/runtime/work/${encodeURIComponent(workId)}/cancel`,
      {},
    );
