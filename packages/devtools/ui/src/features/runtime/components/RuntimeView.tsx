...
export function RuntimeView() {
  ...
  const deadLetters = useMemo(() => workRows.filter((row) => row.status === 'dead-letter'), [workRows])
  const filteredWork = useMemo(() => filterRuntimeWork(workRows, filters), [workRows, filters])
  ...
  async function retryWork(workId: string) {
    try {
      ...
      toast({
        ...
        title: result.retried ? 'Runtime work retried' : 'Runtime work was not retryable',
        message: workId,
      })
    } catch (error) {
      toast({ kind: 'danger', title: 'Retry failed', message: errorMessage(error) })
    }
  }
  ...
  async function cancelWork(workId: string) {
    try {
      ...
      toast({
        ...
        title: result.cancelled ? 'Runtime work cancelled' : 'Runtime work was already terminal',
        message: workId,
      })
    } catch (error) {
      toast({ kind: 'danger', title: 'Cancel failed', message: errorMessage(error) })
    }
  }
  ...
  return (
    <QwShell
      ...
      title="Runtime"
      subtitle={status ? `${status.namespace} namespace · ${workRows.length} work items` : 'Runtime state'}
      connected={connected}
      actions={
        <Btn
          size="sm"
      ...
      tabs={[
        { label: 'Work', count: workRows.length, active: tab === 'work', onClick: () => setTab('work') },
        { label: 'Timers', count: timers.length, iconName: 'clock', active: tab === 'timers', onClick: () => setTab('timers') },
        { label: 'Outbox', count: outbox.length, iconName: 'inbox', active: tab === 'outbox', onClick: () => setTab('outbox') },
        { label: 'Dead-letter', count: deadLetters.length, iconName: 'alert', active: tab === 'dead-letter', onClick: () => setTab('dead-letter') },
      ]}
      filterBar={<RuntimeFilterBar filters={filters} namespaces={namespaces} targets={targets} onChange={setFilters} />}
    >
...
function RuntimeFilterBar({
  ...
  return (
    <div className="flex flex-shrink-0 flex-wrap items-center gap-2 px-8 py-2" style={{ borderBottom: '1px solid var(--qw-border)' }}>
      <Icon name="filter" size={11} />
      <Select value={filters.status ?? 'all'} onChange={(status) => onChange({ ...filters, status: status as RuntimeWorkStatus | 'all' })} options={STATUSES} />
      <Select value={filters.namespace ?? 'all'} onChange={(namespace) => onChange({ ...filters, namespace: namespace === 'all' ? undefined : namespace })} options={['all', ...namespaces]} />
      <Select value={filters.targetId ?? 'all'} onChange={(targetId) => onChange({ ...filters, targetId: targetId === 'all' ? undefined : targetId })} options={['all', ...targets]} />
      {filters.status && filters.status !== 'all' && <Chip tone="crux" mono>{filters.status}</Chip>}
    </div>
  )
}
