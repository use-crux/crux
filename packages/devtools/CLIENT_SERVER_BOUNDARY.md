...
| What                                            | Owner  | Notes                                                                                                                                                 |
| ----------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| Cell lookup by experiment/case/variant/trial    | Server | `GET /api/quality/experiments/{experimentId}/cell-evidence` validates coordinates and returns 404 for missing cells.                                  |
| Assertion/check normalization                   | Server | `checks` is derived from assertion outcomes, score thresholds, gates, and runtime errors.                                                             |
| Authored source frames                          | Server | Outcomes and `code.primaryFrame` use stored source-frame snapshots or explicit unavailable reasons; clients must not show compiled output as source.  |
| Values at check                                 | Server | Curated, redacted values only: input/output/expected, score values, matcher thresholds, trace refs, usage/cost/latency, and explicit metadata.        |
| Baseline output/deltas                          | Server | Old baselines degrade honestly with `kind: 'unavailable'`; clients do not backfill or guess missing output diffs.                                     |
...
| Citation detection from output text  | Server | Regex/structured detection is fine on either side, but the _list of canonical citation ids_ should be on the trace record so it survives across UI rewrites. Currently client-side regex. |
...
## 4. Insights (`/api/quality/insights`)
