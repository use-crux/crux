...
describe('discoverBinary', () => {
  it('uses an executable configured path before every fallback', async () => {
    await expect(
      discoverBinary(
        './bin/crux',
        folder,
        host(['/workspace/crux/bin/crux'], '/usr/bin/crux'),
      ),
    ).resolves.toEqual({
      path: '/workspace/crux/bin/crux',
      source: 'configured',
    })
  })
  ...
  it('rejects a configured non-executable path without falling through', async () => {
    await expect(
      discoverBinary('/missing/crux', folder, host([], '/usr/bin/crux')),
  ...
  it('checks make local and platform bundle outputs before PATH', async () => {
    ...
    await expect(
      discoverBinary('', folder, host([local], '/usr/bin/crux')),
    ).resolves.toEqual({ path: local, source: 'workspace' })

    const bundle = '/workspace/crux/packages/local/dist/crux-linux-x64/bin/crux'
    await expect(
      discoverBinary('', folder, host([bundle], '/usr/bin/crux')),
    ).resolves.toEqual({ path: bundle, source: 'workspace' })
  })

  it('discovers a project-local npm shim before PATH on Unix', async () => {
    const shim = '/workspace/crux/node_modules/.bin/crux'

    await expect(
      discoverBinary('', folder, host([shim], '/usr/bin/crux')),
    ).resolves.toEqual({ path: shim, source: 'workspace' })
  })

  it('discovers project-local Windows npm shims and native executables', async () => {
    const windowsHost: DiscoveryHost = {
      ...host([]),
      platform: 'win32',
      isExecutable: async (path) =>
        path.endsWith('node_modules\\.bin\\crux.cmd'),
      findOnPath: async (command) => {
        expect(command).toBe('crux')
        return 'C:\\global\\crux.cmd'
      },
    }

    await expect(
      discoverBinary('', 'C:\\workspace\\crux', windowsHost),
    ).resolves.toEqual({
      path: 'C:\\workspace\\crux\\node_modules\\.bin\\crux.cmd',
      source: 'workspace',
    })
  })
  ...
  it('falls back to PATH and then reports no match', async () => {
    await expect(
      discoverBinary('', folder, host(['/usr/bin/crux'], '/usr/bin/crux')),
