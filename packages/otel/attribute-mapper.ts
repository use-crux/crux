...
import {
  CRUX_COST,
  CRUX_PROMPT_ID,
  CRUX_RETRIEVAL_RECIPE_ID,
  CRUX_RETRIEVAL_STEP_ID,
  CRUX_RETRIEVAL_STEP_INPUT_HIT_COUNT,
  CRUX_RETRIEVAL_STEP_INPUT_QUERY_COUNT,
  CRUX_RETRIEVAL_STEP_KIND,
  CRUX_RETRIEVAL_STEP_OUTPUT_HIT_COUNT,
  CRUX_RETRIEVAL_STEP_OUTPUT_QUERY_COUNT,
  CRUX_RETRIEVAL_STEP_WARNING_COUNT,
  CRUX_TOOL_CALL_ID,
...
export function attributesFor(
  ...
  const retrievalStepAttributes = isRetrievalStepAttributeSet(attributes)
  for (const [key, value] of Object.entries(attributes)) {
    if (isPayloadAttributeKey(key)) continue
    const normalizedKey = attributeKeyFor(
      key,
      workspaceAttributes,
      retrievalStepAttributes,
    )
...
/** Retrieval recipe step attributes use domain-specific keys for OTel export. */
function isRetrievalStepAttributeSet(
  attributes: Record<string, unknown>,
): boolean {
  return (
    typeof attributes.recipeId === 'string' &&
    typeof attributes.stepId === 'string'
  )
}
...
function attributeKeyFor(
  ...
  switch (key) {
    ...
    case 'tokensPerSecond':
      return CRUX_GEN_OUTPUT_TOKENS_PER_SECOND
    case 'gen.time_per_output_chunk_ms':
      return CRUX_GEN_TIME_PER_OUTPUT_CHUNK_MS
    case 'inputTokens':
    ...
    case 'recipeId':
      return CRUX_RETRIEVAL_RECIPE_ID
    case 'stepId':
      return retrievalStepAttributes ? CRUX_RETRIEVAL_STEP_ID : `crux.${key}`
    case 'stepKind':
      return retrievalStepAttributes ? CRUX_RETRIEVAL_STEP_KIND : `crux.${key}`
    case 'inputQueryCount':
      return retrievalStepAttributes
        ? CRUX_RETRIEVAL_STEP_INPUT_QUERY_COUNT
        : `crux.${key}`
    case 'outputQueryCount':
      return retrievalStepAttributes
        ? CRUX_RETRIEVAL_STEP_OUTPUT_QUERY_COUNT
        : `crux.${key}`
    case 'inputHitCount':
...
function attributeValue(
  ...
  if (
    key === GEN_AI_CLIENT_OPERATION_DURATION ||
    key === GEN_AI_SERVER_TIME_TO_FIRST_TOKEN
  ) {
...
function primitiveArrayValue(values: readonly unknown[]): {
  readonly value?: OtelAttributeValue
  ...
  if (
    ...
    return { value: values }
  if (values.every((item): item is boolean => typeof item === 'boolean'))
    return { value: values }
  return jsonAttributeValue(values)
}

function jsonAttributeValue(value: unknown): {
