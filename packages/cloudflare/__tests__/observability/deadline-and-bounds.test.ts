...
describe("Workers bounded deadline reporting", () => {
  it("truthfully reports a deadline-exceeded drain instead of claiming success", async () => {
    ...
    const response = await worker.fetch(
      new Request("http://fixture.test/emit?requestId=slow&flushTimeoutMs=5"),
      env,
      ctx,
    );
    ...
    expect(latestDrainResult()).toEqual(
      expect.objectContaining({ status: "deadline", deadlineExceeded: true }),
    );
    // The kernel flush follows the bounded wrapper report and can still
    // deliver records within its own retention budget.
  ...
  it("drains fully when the transport is fast relative to the budget", async () => {
    ...
    await worker.fetch(
      new Request(
        "http://fixture.test/emit?requestId=fast&flushTimeoutMs=5000",
      ),
      env,
      ctx,
    );
...
describe("bounded byte/record overflow", () => {
  ...
  it("drops overflow instead of growing the queue unboundedly for an oversized burst", async () => {
    ...
    await Promise.all(
      contexts.map((ctx, i) =>
        worker.fetch(
          new Request(`http://fixture.test/emit?requestId=burst-${i}`),
          env,
          ctx,
        ),
      ),
