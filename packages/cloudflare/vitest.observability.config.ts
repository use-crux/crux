 ...
 *
 * Runs `__tests__/observability/**` inside a real workerd isolate via
 ...
 * and `@use-crux/cloudflare` import graphs do not require it.
 *
...
const resumerEntry = join(
  here,
  "__tests__/observability/fixtures/resumer-worker.ts",
);
const moduleScopeEntry = join(
  here,
  "__tests__/observability/fixtures/module-scope-worker.ts",
);
const channelModulePath = join(here, "../core/src/observability/channel.ts");
...
export default defineConfig({
  plugins: [
    cloudflareTest({
      wrangler: {
        configPath: "./__tests__/observability/fixtures/wrangler.jsonc",
      },
      miniflare: {
        workers: [
          {
  ...
  test: {
    include: ["__tests__/observability/**/*.test.ts"],
  },
});
