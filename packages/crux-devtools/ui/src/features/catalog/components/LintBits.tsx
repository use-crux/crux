 ...
 * (`Catalog.tsx`) and the sweep-view detail panel (`CatalogHealth.tsx`)
...
import type { CatalogLintFinding, ProjectDefinition } from '@/types'

type Severity = CatalogLintFinding['severity']
type Fix = CatalogLintFinding['fixes'][number]
type Evidence = CatalogLintFinding['evidence'][number]
...
export function LintMetaChips({ finding }: { finding: CatalogLintFinding }) {
  ...
  items.push(
    <MetaChip key="cat" label={finding.category} title={`category · ${finding.category}`} />,
  )
...
export function resolveDocsHref(finding: CatalogLintFinding): string | null {
...
export function resolveSuppressDirective(finding: CatalogLintFinding): string | null {
 ...
 *  and (when applicable) links into the catalog or shows file:line. */
    ...
    <div className="flex flex-col gap-1.5">
      {items.map((ev, i) => {
        ...
        const fileLabel = ev.source
          ? `${stripRoot(ev.source.file, projectRoot)}:${ev.source.line}`
          : undefined
        return (
          <div
            key={`${ev.kind}-${i}`}
...
export function FixCard({
  ...
  fix: Fix
  severity: Severity
  source: CatalogLintFinding['source']
  projectRoot: string | undefined
}) {
  const c = lintTone(severity)
  const docsHref = fix.kind === 'docs' ? cruxDocsUrl(fix.docsUrl) : null
  const fileLabel = source
    ? `${stripRoot(source.file, projectRoot)}:${source.line}`
    : undefined
  return (
    <div
