/**
 * Catalog · Health — sweep view for catalog lint findings.
 *
 * Opt-in surface, opened from the `Health · N` button on the Catalog
 * header. The design intent (see `v4-catalog-lints.jsx` → V4CatalogLintsSweep)
 ...
 *
 * All data comes from the same `/api/catalog.lintFindings` payload as
...
import { glyphFor, KindBadge } from '@/features/catalog/components/CatalogKind'
import { normalizeKind, stripRoot } from '@/features/catalog/components/CatalogTree'
import {
  AffectedDefList,
  EvidenceList,
  FixCard,
...
type Severity = CatalogLintFinding['severity']
...
interface CatalogHealthProps {
  ...
  connected: boolean
  /** Shared tab strip — owned by `CatalogView` so both surfaces stay
...
export function CatalogHealth({
  ...
  const totalUnsuppressed = useMemo(
    () => lintFindings.filter((f) => !f.suppressed).length,
    [lintFindings],
  )
  ...
  const groups = useMemo(() => {
    const m = new Map<string, CatalogLintFinding[]>()
    ...
    const arr = Array.from(m.entries()).map(([key, items]) => {
      const worst = items.reduce<Severity>((acc, f) => (SEV_RANK[f.severity] < SEV_RANK[acc] ? f.severity : acc), 'info')
  ...
  return (
    <QwShell
      ...
      breadcrumb="Library / Catalog / Health"
    ...
    >
      <div className="px-8 pt-5 pb-10">
        ...
        >
          ...
          {(['warning', 'info'] as Severity[]).map((sev) => {
            ...
            const counts = lintFindings.filter(
              (f) => !f.suppressed && (sev === 'warning' ? f.severity === 'warning' || f.severity === 'error' : f.severity === 'info'),
            ).length
            return (
              <button
                key={sev}
          ...
          <span className="ml-auto flex items-center gap-1.5">
            ...
            {(['rule', 'file', 'kind'] as GroupKey[]).map((g) => (
              <Btn
                key={g}
                size="xs"
                variant={groupBy === g ? 'primary' : undefined}
        ...
        {visible.length === 0 ? (
          ...
          >
            {totalUnsuppressed === 0
              ? 'Nothing to clean up — every catalog definition is in good shape.'
          ...
          <div className="grid gap-[18px]" style={{ gridTemplateColumns: '1.45fr 1fr' }}>
            ...
            <div className="flex flex-col gap-3">
              {groups.map(({ key, items, worstSeverity }) => {
                ...
                return (
                  ...
                  >
                    ...
                    {items.map((f, i) => {
                      const targetId =
                        f.primaryDefinitionId ?? f.relatedDefinitionIds?.[0]
                      const target = targetId ? defsById.get(targetId) : undefined
                      const sel = selectedFinding?.id === f.id
                      const g = target ? glyphFor(target.kind) : { icon: 'doc' as const, color: 'var(--qw-fg-muted)', tone: 'muted' as const, label: 'unknown' }
                      return (
                        <button
                          key={f.id}
                          type="button"
                          ...
                          <span className="flex items-center gap-1">
                            {target && (
                              <Btn
                                ...
                                onClick={(ev) => {
                                  ev.stopPropagation()
                                  navigate({ view: 'library-catalog', promptId: target.id })
                                }}
                                title={`Open ${target.name} in the catalog`}
                              >
                                Inspect
                              </Btn>
                            )}
              ...
              >
                ...
                <span>
                  ...
                  <span style={{ color: 'var(--qw-fg-muted)' }}>// crux-lint-disable</span>
                  {' '}· suppressed findings stay out of the inbox but still show on their
                  definition page.
                </span>
              </div>
            </div>
            ...
            {selectedFinding && (
              <DetailPanel
                ...
                onNavigate={(id) => navigate({ view: 'library-catalog', promptId: id })}
              />
            )}
          </div>
        )}
    ...
    >
      ...
      <div className="flex flex-col gap-3.5 px-4 py-3.5">
        ...
        <Section eyebrow="What">
          <span style={{ fontFamily: 'var(--qw-serif, Georgia, serif)' }}>
            {finding.message}
          </span>
        </Section>
