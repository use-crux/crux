...
function planDescription(type: string, title?: string, version?: number): string {
...
function taskDescription(type: string, label?: string, status?: string): string {
  const l = label ?? 'task'
  switch (type) {
    case 'added':
      return `Task added: ${l}`
...
export function PlanTaskTimeline({ planEvents, taskEvents }: PlanTaskTimelineProps) {
