...
export function FileInspector({
  ...
  const { data: detail, loading, error } = useWorkspaceFile(workspaceId, filePath)
  const fileOps = useMemo(() => ops.filter((o) => o.path === filePath), [ops, filePath])
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col px-8 py-6">
      <div className="mb-3 flex flex-wrap items-center gap-2.5">
        <Icon name="doc" size={16} color={isErr ? 'var(--qw-danger)' : 'var(--qw-crux)'} className="shrink-0" />
        <span className="min-w-0 truncate font-mono text-[18px] font-semibold" title={filePath}>
      ...
      <div
        ...
        style={{ background: 'var(--qw-bg)', border: '1px solid var(--qw-border)', padding: 2 }}
      >
        {(['preview', 'operations', 'versions'] as const).map((id) => {
          const on = tab === id
          const count = id === 'operations' ? combinedOps.length : id === 'versions' ? versions.length : null
          return (
            <button
              key={id}
              type="button"
            ...
            >
              {id}
              {count != null && (
                <span className="font-mono text-[10px]" style={{ color: on ? 'var(--qw-crux)' : 'var(--qw-fg-faint)' }}>
                  {count}
                </span>
              )}
            </button>
      ...
      {error && <ErrorBanner message={`Couldn't load file: ${error.message}`} />}

      {tab === 'preview' && (
        <PreviewBlock filePath={filePath} detail={detail} loading={loading && !detail} summary={summary} />
      )}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span>{filePath}</span>
        {summary?.lastOpAt && (
          <span style={{ color: 'var(--qw-fg-faint)' }}>· last touched {fmtRelative(summary.lastOpAt)}</span>
        )}
      ...
      >
        ...
        {ops.map((o, i) => (
          ...
          >
            ...
            {hasActor && <span style={{ color: 'var(--qw-fg)' }}>{o.actor ?? '—'}</span>}
            {hasSpan && (
              <span
      ...
      >
        ...
        {versions.map((v, i) => (
          ...
          >
            ...
            <span style={{ color: 'var(--qw-fg-muted)' }}>{fmtRelative(v.timestamp) ?? '—'}</span>
