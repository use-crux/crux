...
export function buildToolExecutionMap(correlatedEvents: readonly CorrelatedEvent[]): Map<string, ToolExecutionInfo> {
    ...
    if (event.eventType === 'tool:start') {
      ...
      if (key) {
        map.set(key, {
          ...
          tokenSavingsEstimate: event.data.tokenSavingsEstimate as number | undefined,
    ...
    if (event.eventType === 'delegate:start') {
      ...
      for (const [, value] of map) {
        if (value.handoffId === event.data.handoffId) {
          ...
          if (!value.outputSize) value.outputSize = event.data.outputSize as number | undefined
        }
      }
    }
  }
