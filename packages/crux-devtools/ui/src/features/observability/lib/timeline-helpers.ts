...
export function filterTraces(
  ...
  let filtered = traces
  if (sessionFilter) filtered = filtered.filter((t) => t.sessionId === sessionFilter)
  if (promptFilter) filtered = filtered.filter((t) => t.promptId === promptFilter)
...
export function buildSessionGroups(traces: Trace[]): {
  ...
  for (const [sessionId, sessionTraces] of bySession) {
    ...
    const endedAt = Math.max(...sessionTraces.map((t) => t.startedAt + (t.durationMs ?? 0)))
    const totalTokens = sessionTraces.reduce((sum, t) => sum + (t.result?.usage?.totalTokens ?? 0), 0)
    const totalCost = sessionTraces.reduce((sum, t) => sum + (t.result?.cost ?? 0), 0)
...
export function buildFlowGroup(flowId: string, flowTraces: Trace[], nameMap?: Map<string, string>): FlowGroup {
  ...
  const endedAt = Math.max(...flowTraces.map((t) => (t.status === 'running' ? now : t.startedAt + (t.durationMs ?? 0))))
  ...
  const name = nameMap?.get(flowId) ?? flowId.replace(/^flow-\d+-[a-z0-9]+$/, 'flow')
...
export type ListItem = { kind: 'trace'; trace: Trace } | { kind: 'flow'; group: FlowGroup }

export function buildMixedChronological(traces: Trace[], nameMap?: Map<string, string>): ListItem[] {
...
export function computeTraceAnomalies(traces: Trace[]): TraceAnomalies {
  ...
  const durationP90 =
    completedDurations.length > 0 ? completedDurations[Math.floor(completedDurations.length * 0.9)]! : Infinity
  ...
  for (const [promptId, costs] of costByPrompt) {
    avgCostByPrompt.set(promptId, costs.reduce((a, b) => a + b, 0) / costs.length)
  }
