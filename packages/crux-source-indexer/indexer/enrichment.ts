...
async function resolvedAgentDefinition(
  ...
  const definitionItem = await resolvedDefinition(root, file, id, 'agent', agentValue.id, agentValue.description, {
    exportName,
    promptId: agentValue.prompt?.id,
    toolNames,
    handoffs,
  }, expected)
  const relations = relationsFromExpected(expected, id, file).filter((relationItem) => relationItem.type !== 'agent.uses_prompt' && relationItem.type !== 'agent.uses_tool')
  ...
  for (const handoff of handoffs) relations.push(relation('agent.can_handoff_to', id, `agent:${safeId(handoff.id)}`, file))
...
async function resolvedFlowDefinition(
  ...
  return {
    definition: await resolvedDefinition(root, file, id, 'flow', flowValue.name, expected.description, {
      exportName,
...
async function resolvedRetrieverDefinition(
  ...
  return {
    definition: await resolvedDefinition(root, file, `rag.retriever:${safeId(retrieverValue.id)}`, 'rag.retriever', retrieverValue.id, expected.description, {
      exportName,
...
async function resolvedRetrievalPipelineDefinition(
  ...
  const pipelineValue = value as { id?: string; namespace?: string; mode?: string; base?: { id?: string }; stages?: Array<{ name?: string; phase?: string; kind?: string }> }
  const id = expected.id
  const retrieverId = pipelineValue.base?.id
  const definitionItem = await resolvedDefinition(root, file, id, 'rag.pipeline', expected.name || exportName, expected.description, {
    exportName,
    retrieverId,
    namespace: pipelineValue.namespace,
    mode: pipelineValue.mode,
    stageNames: (pipelineValue.stages ?? []).map((stage) => stage.name).filter((name): name is string => typeof name === 'string'),
  ...
  const relations = relationsFromExpected(expected, id, file).filter((relationItem) => relationItem.type !== 'rag.pipeline.uses_retriever')
  if (retrieverId) relations.push(relation('rag.pipeline.uses_retriever', id, `rag.retriever:${safeId(retrieverId)}`, file))
  return { definition: definitionItem, relations }
}
...
async function resolvedMemoryDefinition(
  ...
  return {
    definition: await resolvedDefinition(root, file, `memory:${safeId(memoryValue.id)}`, 'memory', memoryValue.id, expected.description, {
      exportName,
      blocks: (memoryValue.blocks ?? []).map((block) => ({ id: block.id, kind: block.kind, priority: block.priority })),
...
async function resolvedBlackboardDefinition(
  ...
  return {
    definition: await resolvedDefinition(root, file, `blackboard:${safeId(blackboardValue.id)}`, 'blackboard', blackboardValue.id, expected.description, {
      exportName,
    }, expected),
    relations: [],
  }
...
async function resolvedConstraintDefinition(
  ...
  return {
    definition: await resolvedDefinition(root, file, `constraint:${safeId(constraintValue.name)}`, 'constraint', constraintValue.name, expected.description, {
      exportName,
...
async function resolvedGuardrailDefinition(
  ...
  return {
    definition: await resolvedDefinition(root, file, `guardrail:${safeId(guardrailValue.name)}`, 'guardrail', guardrailValue.name, expected.description, {
      exportName,
...
async function resolvedScorerDefinition(
  ...
  return {
    definition: await resolvedDefinition(root, file, `scorer:${safeId(scorerValue.id)}`, 'scorer', scorerValue.id, expected.description, {
      exportName,
    }, expected),
    relations: [],
  }
...
function isJudgeInstance(value: unknown): boolean {
  return Boolean(value && typeof value === 'object' && typeof (value as { id?: unknown }).id === 'string' && typeof (value as { score?: unknown }).score === 'function')
}
...
function toolNamesFromRuntime(tools: unknown): string[] {
  ...
  if (Array.isArray(tools)) {
    return tools
      .map((tool) => (tool && typeof tool === 'object' && typeof (tool as { name?: unknown }).name === 'string' ? (tool as { name: string }).name : undefined))
