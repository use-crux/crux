...
// ----------------------------------------------------------------
// Flow eval event types.
// ----------------------------------------------------------------

// FlowStartEvent is the incoming event for flow:start.
...
// FlowCaseEvent is the incoming event for flow:case.
type FlowCaseEvent struct {
	FlowID       string           `json:"flowId"`
...
// FlowEndEvent is the incoming event for flow:end.
...
// FlowStart creates a new flow eval run.
func (s *Store) FlowStart(event FlowStartEvent) {
	s.mu.Lock()
	...
	s.flowRunList = append([]*FlowRun{run}, s.flowRunList...)
	s.flowByID[event.FlowID] = run

	// Evict oldest if over capacity.
	for len(s.flowRunList) > s.maxFlowRuns {
		evicted := s.flowRunList[len(s.flowRunList)-1]
		s.flowRunList = s.flowRunList[:len(s.flowRunList)-1]
		delete(s.flowByID, evicted.FlowID)
	}
...
// FlowCase adds a completed case to a flow eval run.
func (s *Store) FlowCase(event FlowCaseEvent) {
	s.mu.Lock()

	run := s.flowByID[event.FlowID]
	if run == nil {
		s.mu.Unlock()
		return
	}

	run.CompletedCases = append(run.CompletedCases, FlowCaseData{
...
// FlowEnd marks a flow eval run as completed.
func (s *Store) FlowEnd(event FlowEndEvent) {
	s.mu.Lock()
...
// GetFlowRuns returns all flow eval runs in newest-first order.
func (s *Store) GetFlowRuns() []FlowRun {
	s.mu.RLock()
	defer s.mu.RUnlock()

	out := make([]FlowRun, len(s.flowRunList))
...
// GetFlowRun returns a single flow eval run by ID, or nil if not found.
func (s *Store) GetFlowRun(flowID string) *FlowRun {
	s.mu.RLock()
	defer s.mu.RUnlock()
