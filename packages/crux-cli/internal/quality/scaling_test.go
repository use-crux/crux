...
func TestOverviewReadModelGolden(t *testing.T) {
	...
	assertQualityGoldenJSON(t, "testdata/overview_generation.golden.json", got)
}

func BenchmarkServiceOverviewFromObservability(b *testing.B) {
	obs := newQualityBenchmarkObservability(b)
...
func newQualityBenchmarkObservability(tb testing.TB) *observability.Service {
	tb.Helper()
...
func assertQualityGoldenJSON(t *testing.T, path string, got any) {
	t.Helper()
