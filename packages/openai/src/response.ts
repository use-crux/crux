...
export interface OpenAIUsageShape {
  ...
  readonly completion_tokens_details?: { readonly reasoning_tokens?: number } | null
}

/** Normalize OpenAI usage counts into the canonical token-usage shape. */
export function openAIUsage(usage: OpenAIUsageShape | undefined): NativeResponseMetadata['usage'] {
...
/**
 * Normalize an OpenAI `finish_reason` (and any explicit refusal) into the
 * provider-neutral finish reason. A non-empty `message.refusal` reports a
 * model-side refusal even when `finish_reason` is `stop`.
 */
