 ...
 * ```
 */
export type OpenAIGenerateImage = GenerateImage<
  string,
  OpenAIImageExtra,
  ImagesResponse
>;
type OpenAIImageInput = GenerateImageOptions<
  string,
  OpenAIImageExtra,
  ImagePrompt
>;
...
export function createOpenAIGenerateImage(client: OpenAI): OpenAIGenerateImage {
  return bindCompletedOperation({
    definition: createOpenAIImageOperation(client),
    provider: "openai",
    operation: "generateImage",
  });
}

/** Define OpenAI image mechanics for first-class provider-runtime compilation. */
export function createOpenAIImageOperation(client: OpenAI) {
  const definition = defineCompletedOperation({
    async normalize(input: OpenAIImageInput, context) {
        ...
        ? client.images.edit(
            {
              ...
              image: await Promise.all(
                prompt.images.map((asset, index) =>
                  dataAssetFile(asset, `image-${index}`),
                ),
              ),
              ...(prompt.mask === undefined
                ? {}
                : { mask: await dataAssetFile(prompt.mask, "mask") }),
    ...
    validate(raw, { options }) {
      const mediaType = outputMediaType(
        raw.output_format ?? options.extra?.output_format,
      );
      const images: NativeGeneratedImage[] = (raw.data ?? []).flatMap(
        (image) =>
...
async function dataAssetFile(
  ...
  if (asset.type !== "data")
    throw new TypeError("OpenAI edits require data assets.");
