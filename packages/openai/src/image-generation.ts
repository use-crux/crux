...
export function createOpenAIGenerateImage(client: OpenAI): OpenAIGenerateImage {
  return async (options): Promise<GeneratedImage<ImagesResponse>> => {
    validateGenerateImageOptions(options)
    const prompt = await lowerImagePrompt(options, { adapter: 'openai', model: options.model })
    ...
    const raw = prompt.images.length > 0 || prompt.mask
...
function openAIImageIssues(
  ...
  if ((prompt.images.length > 0 || prompt.mask) && options.extra?.quality === 'hd') issues.push(issue('image.edit.quality'))
  prompt.images.forEach((asset, index) => {
    if (asset.type !== 'data') issues.push(issue('image.edit.reference', `prompt.images[${index}]`))
  })
  if (prompt.mask?.type !== undefined && prompt.mask.type !== 'data') issues.push(issue('image.edit.mask', 'prompt.mask'))
  return issues
}

function issue(capability: string, path?: string): UnsupportedCapabilityIssue {
  return { capability, ...(path === undefined ? {} : { path }), remediation: 'Use native OpenAI image controls and byte assets.' }
}

async function dataAssetFile(asset: Awaited<ReturnType<typeof lowerImagePrompt>>['images'][number], name: string): Promise<Uploadable> {
