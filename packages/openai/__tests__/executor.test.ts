...
import { z } from "zod";
import { createOpenAI, embedding as makeEmbedding } from "../src";
...
describe("OpenAI adapter via adapter", () => {
  ...
  it("maps neutral toolChoice settings to OpenAI tool_choice", async () => {
    const { adapter, mockCreate } = createAdapter();
    const prompt = makePrompt({
      id: "openai-tool-choice",
      system: "Use tools.",
      settings: { toolChoice: { tool: "search" } },
      tools: {
        search: {
          description: "Search",
    ...
    await adapter.generate(prompt, { model: "gpt-4o" });
