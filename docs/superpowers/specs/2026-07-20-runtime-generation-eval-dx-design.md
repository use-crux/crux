...
The local artifact manifest v3 records every arm's identity and placement. Each
...
- New code reads local artifact manifest v3 exactly. An unsupported local
...
- local manifest v3, runtime-arm registry allowlisting, and exact-arm admission;
- byte-compatible host-wire v1 and local-artifact v3 fixtures;
...
- local artifact manifest v3 rejects unsupported versions as specified;
