...
cache namespace is `static-parse-v57`. Go Static Index cache replay is pinned to
the same namespace through the shared fixture. Semantic facts use
`semantic-facts-v22`, and the Go Project Index snapshot cache lives under
`.crux/cache/index-v2/epoch-28/` so restart warm loads cannot mask renamed or
