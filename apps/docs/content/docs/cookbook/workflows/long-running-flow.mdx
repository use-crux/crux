...
const onboardingFlow = flow('onboarding', async (flow, input: OnboardingInput) => {
  ...
  const plan = await flow.step('first-week-plan', async () => {
    const result = await generate(onboardingPlanPrompt, {
      model: openai('gpt-4o'),
      input: { name: input.name, signupGoal: input.goal },
    })
    return result.text
  })
  ...
  return flow.step('schedule-checkins', async () => {
    return scheduleEmails(input.email, plan)
  })
})
...
// Resume happens automatically on the next handle.resume(flowId), usually
...
Use `flow.waitUntil('name', conditionFn)` to suspend until a condition function returns `true`. Core rechecks the
condition only when the flow is explicitly resumed; if you want periodic checks, schedule resume attempts in your
