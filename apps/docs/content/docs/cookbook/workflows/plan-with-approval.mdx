...
This recipe builds a workflow that proposes a plan, suspends for human review, and resumes only after a user approves (or modifies) it. The plan persists in `RecordStore` so the UI can render it live; the flow suspends so server resources don't sit idle.

## Primitives used

- `plan()` from `@use-crux/core/plan` for the persistent plan document
- `flow()` from `@use-crux/core/flow` with `flow.suspend()` for the approval pause
- `signalFlow()` (or `handle.signal()`) to resume once the user clicks Approve
- `usePlan()` from `@use-crux/react` for live UI subscription
...
```ts
import { signalFlow } from "@use-crux/core/flow";
import { plan } from "@use-crux/core/plan";
...
export async function approvePlan(
  ...
  await plan.ref(planId).update({ metadata: { status: "approved" } });
...
export async function rejectPlan(planId: string, flowId: string) {
  await plan.ref(planId).update({ metadata: { status: "rejected" } });
...
1. **The flow drafts the plan and persists it.** The plan document lives in `RecordStore`; the UI subscribes via `usePlan()` and re-renders when the content changes.
...
Use `flow()` from `@use-crux/convex/server` so the flow handle exposes `.action`, `.handler`, `.args`, and `.signal()` for Convex-aware start/resume.
...
<Cards>
  <Card
    ...
    description="The full plan and task-ledger primitive set."
  ...
  <Card
    title="Reactive UI"
    description="usePlan, useTasks, transports."
