...
const summaryMentionsArea = constraint({
  ...
  severity: 'assert',
  run: async (ticket) => {
    if (!ticket.summary.toLowerCase().includes(ticket.affectedArea.toLowerCase())) {
...
<Cards>
  <Card title="Validation retry guide" description="Text-repair, schema-retry tiers, and bounded retry budgets." href="/docs/guides/safety/validation-retry" />
  <Card title="Constraints guide" description="Add business-rule validation that retries with feedback." href="/docs/guides/safety/constraints" />
  <Card title="Eval-driven prompts" description="Test extraction quality across a model matrix in CI." href="/docs/cookbook/production" />
</Cards>
