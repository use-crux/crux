...
If the schema is large and you want progressive UI updates, stream the prompt and
read `partialOutputStream`:

```ts
const result = await stream(classifyTicket, {
  model: openai("gpt-4o"),
  input: { body },
});
for await (const partial of result.partialOutputStream) {
  // Canonical partial input, published only once each value is release-ready.
}
const { object } = await result.completion;
```

Do not reach for the AI SDK's `streamObject` directly to get partials. It resolves
before terminal Safety and sees only one attempt, so its partials can carry
content a guardrail, constraint, or validation retry was about to reject.
`partialOutputStream` is a projection of the same canonical text Crux publishes
on `textStream`, so it can only ever describe committed output.
