...
For source material that already lives in a retriever, MCP server, project
index, connected drive, or app document service, use a source-backed mount
instead of copying everything into the workspace. Provider mounts are read-only
by default and can opt into writes through custom source hooks. See
[Federated workspace sources](/docs/cookbook/basics/workspace-source-mounts).
