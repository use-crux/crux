...
description: Implement durable Crux RecordStore, pgvector VectorStore, and bytea AssetStore adapters on Postgres.
---
...
This recipe starts with a `RecordStore`, then shows optional Postgres-backed vector and asset storage. You can use only the pieces you need:
...
Postgres owns JSON record hydration. `VectorStore` owns similarity search. `AssetStore` owns bytes for workspaces. Keeping those capabilities separate makes each adapter simpler and avoids pretending Postgres is also object storage or vector infrastructure unless you explicitly add those capabilities.
...
<Cards>
  ...
  <Card
    ...
    description="Pair Postgres records with vector search."
