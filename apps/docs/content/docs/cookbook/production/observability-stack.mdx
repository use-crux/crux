...
- A custom `CruxPlugin` implementing `install(hooks)` with hooks
...
```ts
import type { CruxPlugin, CruxHooks } from '@use-crux/core'
...
export function withCostTracker(options: CostTrackerOptions): CruxPlugin {
  return {
    ...
    install(hooks: Readonly<CruxHooks>) {
      return {
        observability subscribers: {
          ...hooks.observability subscribers,
          onGenerateEnd: async (event) => {
            // Forward to existing handlers (fan-out)
            await hooks.observability subscribers?.onGenerateEnd?.(event)
...
<Cards>
  ...
  <Card
    title="Plugins guide"
    description="CruxPlugin interface and mergeHooks semantics."
