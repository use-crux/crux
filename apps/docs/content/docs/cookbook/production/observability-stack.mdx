...
export default config({
  devtools: { serverUrl: process.env.DEVTOOLS_URL }, // dev only — empty in prod
  plugins: [
...
2. **`withDevtools` is auto-prepended.** When `devtools.serverUrl` is set, Crux installs the devtools plugin first. In production where the env var is unset, the plugin is a no-op.
...
### Devtools in production via tunnel

For staging environments or for debugging customer issues, run `crux dev --tunnel` on your local machine and point `DEVTOOLS_URL` at the public tunnel. Production traffic streams to your local devtools UI.
...
<Cards>
  ...
  <Card
    ...
    description="@crux/otel setup, attributes, span hierarchy."
