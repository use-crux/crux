...
```ts
import { retrievalRecipe, retrieve, rerank, retriever } from '@use-crux/core/retrieval'
...
export const docsAnswer = retrievalRecipe({
  ...
  steps: [retrieve({ limit: 20 }), rerank({ engine: judgeReranker({ name: 'docs-judge', model: retrievalModel }), topK: 5 })],
})

const { hits, trace } = await docsAnswer.retrieveWithTrace('enterprise SSO setup')
```
...
<Cards>
  ...
  <Card title="@use-crux/upstash reference" description="Upstash VectorStore and Redis RecordStore APIs." href="/docs/reference/storage/upstash" />
</Cards>
