---
title: Local Files RAG
description: Index a local docs directory, compose a retrieval recipe, and answer questions with citations.
---

This recipe is the simplest full retrieval stack that still feels like a real product setup. It loads a local docs folder, indexes it into a store-backed corpus, composes a named retrieval recipe, and injects the selected evidence into a prompt with citations.

It is a good starting point when your source material already lives on disk and you want the cleanest path from "folder of docs" to "answer questions against it."

## Primitives used

- `@use-crux/ingest/files` for loading local documents
- `@use-crux/core/indexing` for corpus sync, chunking, and write-time embedding
- `@use-crux/core/retrieval` for query-time search, recipes, and grounding
- `@use-crux/ai` for prompt execution
- `inMemoryRecordStore()` and `inMemorySearchStore()` for a lightweight local corpus

## Full code

### `lib/rag/embedding.ts`

```ts
import { embedding } from "@use-crux/core/embedding";
import { embedMany } from "ai";
import { openai } from "@ai-sdk/openai";

export const dense = embedding({
  kind: "dense",
  name: "text-embedding-3-small",
  dimensions: 1536,
  maxInputTokens: 8191,
  batch: { maxSize: 100, concurrency: 3 },
  embed: async (inputs) => {
    const { embeddings } = await embedMany({
      model: openai.embedding("text-embedding-3-small"),
      values: inputs.map((input) => {
        if (input.type !== "text") throw new Error("text-only provider invariant");
        return input.text;
      }),
    });
    return { embeddings };
  },
});
```

### `lib/rag/store.ts`

```ts
import {
  inMemoryRecordStore,
  inMemorySearchStore,
} from "@use-crux/core/storage";

export const records = inMemoryRecordStore();
export const search = inMemorySearchStore();
```

### `lib/rag/index.ts`

```ts
import { corpus, indexer } from "@use-crux/core/indexing";
import { filesSource } from "@use-crux/ingest/files";

import { dense } from "./embedding";
import { records, search } from "./store";

export const docsIndexer = indexer({
  id: "local-docs",
  namespace: "local-docs",
  records,
  search,
  dense,
});

export const docsCorpus = corpus({
  id: "local-docs",
  namespace: "local-docs",
  records,
  indexer: docsIndexer,
});

export async function syncDocs() {
  const source = filesSource(
    { directory: "./docs", recursive: true },
    { namespace: "local-docs" },
  );

  return docsCorpus.sync(source.load(), {
    sourceSet: "complete",
    stale: "delete",
  });
}
```

### `lib/rag/retriever.ts`

```ts
import {
  fanout,
  retrievalRecipe,
  retrieve,
  retriever,
} from "@use-crux/core/retrieval";

import { dense } from "./embedding";
import { retrievalModel } from "./retrieval-model";
import { records, search } from "./store";

export const docs = retriever({
  id: "local-docs",
  namespace: "local-docs",
  records,
  search,
  dense,
  limit: 10,
});

export const docsAnswer = retrievalRecipe({
  id: "local-docs-answer",
  retriever: docs,
  model: retrievalModel,
  steps: [fanout({ maxQueries: 3 }), retrieve({ limit: 10 })],
});

const debug = await docsAnswer.retrieveWithTrace("how do refunds work?");
```

Start with `docs`. Switch prompts to `docsAnswer.asRetriever()` when you need query expansion or trace output while debugging retrieval.

### `lib/rag/answer.ts`

```ts
import { prompt } from "@use-crux/core";
import { generate } from "@use-crux/ai";
import { grounding, citationSchema } from "@use-crux/core/citations";
import { openai } from "@ai-sdk/openai";
import { z } from "zod";

import { docsAnswer } from "./retriever";

const groundedDocs = grounding({
  id: "local-docs",
  retriever: docsAnswer.asRetriever(),
  query: ({ input }) => input.question,
  limit: 5,
  citations: {
    required: true,
    quotes: "required",
  },
});

const answerPrompt = prompt({
  id: "answer-from-local-docs",
  use: [groundedDocs],
  input: z.object({
    question: z.string(),
  }),
  output: z.object({
    answer: z.string(),
    citations: z.array(citationSchema),
  }),
  system: `Answer using only the retrieved context. Cite the chunks you used.
     If the docs do not contain the answer, say so clearly.`,
  prompt: ({ input }) => input.question,
});

export async function answerFromDocs(question: string) {
  const result = await generate(answerPrompt, {
    model: openai("gpt-4.1"),
    input: { question },
  });

  return result.object;
}
```

## How it works

The important pattern here is the handoff between layers. `filesSource()` turns the docs folder into a stream of `IngestDocument` values. `docsCorpus.sync()` compares those sources with the previous source ledger, only sends changed sources through the indexer, and deletes stale sources because this loader represents the complete folder. The retriever becomes the reusable query surface for the corpus. `grounding()` then turns that retriever into prompt evidence with a citation contract, so the model has to cite chunks that actually came from retrieval.

That split is what makes the recipe easy to evolve. If you later move from local files to URLs, only the loading layer changes. If you later move to persistent storage, the indexing and retrieval code stays almost identical.

## Variations

If you want the cheapest possible version, use the dense retriever directly and skip the recipe.

If you want a persistent local-dev workflow, swap `inMemoryRecordStore()` and `inMemorySearchStore()` for production adapters and keep the same indexer and retriever shapes.

If you only need a throwaway fixture, you can call `docsIndexer.indexDocuments()` directly. For an app or background job that runs more than once, prefer `docsCorpus.sync()` so unchanged files do not rewrite the whole corpus.

## Related

<Cards>
  <Card
    title="Semantic RAG with Upstash"
    description="The same architecture on top of a fused-search-capable production store."
    href="/docs/cookbook/rag/semantic-rag"
  />
  <Card
    title="Retrieval Architecture"
    description="How the layers fit together."
    href="/docs/guides/retrieval"
  />
  <Card
    title="Corpus Sync"
    description="Why `corpus.sync()` is the production indexing loop."
    href="/docs/guides/retrieval/corpus-sync"
  />
</Cards>
