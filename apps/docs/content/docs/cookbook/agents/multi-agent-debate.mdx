...
const optimist = agent({
  ...
  prompt: prompt({
    ...
    output: VoteSchema,
    system:
      "You favor shipping. Argue for ship unless there is a clear blocker.",
...
await debateBoard.patch({
  proposal: "Release auth v2 next Tuesday",
  risks: [],
});

const decision = await consensus({
  id: "multi-agent-debate-consensus-1",
...
<Cards>
  <Card
    title="Compositions: Consensus"
    description="Quorum semantics, executor signatures, error policies."
    href="/docs/guides/compositions/consensus"
  />
  <Card
    title="Blackboards"
    description="Shared typed state across agents: set, patch, subscribe."
