...
const writer = agent({
  ...
  prompt: prompt({
    ...
    system:
      "Write a concise answer with [n] citations referencing the findings.",
    prompt: ({ input }) =>
      input.results
        .map(
          (r) =>
...
<Cards>
  <Card
    title="Compositions"
    description="Pipeline, parallel, consensus, swarm: when to use which."
    href="/docs/guides/compositions"
  />
  <Card
    title="Long-running flow"
    description="Make this pipeline suspendable across action boundaries."
