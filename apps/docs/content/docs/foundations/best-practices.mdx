...
Memory blocks can run on the in-memory store first, then move to a Convex store contract or a custom store for production. The block API does not change; only the store does.
