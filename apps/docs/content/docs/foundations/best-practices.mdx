...
Priority orders authorized alternatives; it does not make a plain Context
droppable. Keep critical instructions exact, and wrap only content whose
fidelity may change:

```ts
const rules = context({ priority: 90, system: `Today is ${date}.` });
const examples = droppable(
  context({ priority: 30, system: "## Examples\n..." }),
);
```

<Callout type="idea">
  Use `preview()` during development to verify prospective selection under
  different input budgets.
</Callout>
...
<Callout type="info">
  `when` and `match` exclusion is stronger than token-budget dropping. Excluded
  contexts contribute nothing, not even tools. Combine both: use `when` for
  relevance, `priority` for graceful degradation.
</Callout>
...
<Callout type="info">
  Contexts with `memo` require an `id` for cache key derivation. Static string
  contexts cannot use `memo`; use `cache: true` when stable static content
  should participate in provider prefix caching.
</Callout>
...
For user-facing input, rely on Crux's auto-escaping (enabled by default). Use
`escapeFields` for validated nested objects or arrays, and only use
`rawFields` when you intentionally need unescaped top-level content like HTML.
...
Keep the production task callable, then define an inert Eval beside it. Run the
same command locally and in CI; use scorers for subjective quality and
assertions for structural behavior.
...
export default evaluate({
  ...
  expect: ({ output, expected, expect }) => {
    expect(output.sentiment).toBe(expected.sentiment);
...
<Callout type="info">
  Run `crux eval --plan` to inspect work before inference, then `crux eval`.
  Eval runs and their linked production runs appear in Devtools automatically.
</Callout>
...
<Cards>
  ...
  <Card
    title="Recipes"
    description="Full working examples combining multiple primitives."
    href="/docs/cookbook"
  />
  <Card
    title="FAQ"
