...
| `@use-crux/convex`    | Convex RecordStore adapter, dense VectorStore adapter, workspace BlobStore, server boundaries, Agent bridge, compaction, swarm component | Your backend is Convex                                 |
| `@use-crux/upstash`   | Upstash VectorStore, Redis RecordStore adapter, compatibility stores                                                                 | You need managed vector search or Redis-backed storage |
...
| `@use-crux/core/storage`        | `RecordStore`, `VectorStore`, `BlobStore`, `storage`, `inMemoryStorage`              |
...
Persistent storage is split by capability. `RecordStore` holds JSON records, `VectorStore` handles dense/sparse/hybrid search, and `BlobStore` holds workspace bytes.
