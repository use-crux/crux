...
This page is the index of Crux primitives. Each section is a one-paragraph hook plus an example so you can recognize the primitive when you need it. Follow the links for the deep guide.
...
const tl = await tasklist({ planId: p.id, title: 'Roadmap tasks', tasks: [...] })
```
...
const research = skill.fromRegistry('skills.sh:user/skills/research')
```
...
- **`@crux/otel`** — OpenTelemetry spans for prod (Datadog, Honeycomb, Grafana)
...
<Cards>
  <Card title="Mental Model" description="How these primitives compose at runtime." href="/docs/foundations/mental-model" />
