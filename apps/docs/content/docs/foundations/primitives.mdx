...
- **`quality()`** stores local experiments, variants, comparisons, feedback, and replay data
- **`suite()`** defines Git-friendly regression cases
- **`target()`** wraps prompts, retrievers, flows, tools, or app code
- **`expect()`** checks outputs, retrieval hits, tool calls, and flow steps
- **`judge()`** for LLM-as-a-judge scoring with pre-built quality metrics
- **Quality experiments** persist suites, variants, scores, and local comparison history
...
<Cards>
  ...
  />
  <Card title="Get Started" description="Pick a framework and ship something." href="/docs/getting-started" />
  <Card title="Cookbook" description="Full working examples that combine many primitives." href="/docs/cookbook" />
</Cards>
