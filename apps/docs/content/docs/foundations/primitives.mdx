...
`history()` keeps a summarized prefix and an exact recent suffix when a
conversation no longer fits the resolved model. Canonical messages remain
unchanged; generated summaries are reusable derived artifacts.
