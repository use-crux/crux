...
Plugins install themselves into these hooks via `mergeHooks()`. The same event fans out to all installed plugins — OTel spans, custom telemetry, policy hooks — without primitives ever knowing they're being watched. Devtools tracing uses the canonical `@use-crux/core/observability` graph transport so the Go backend owns tree and relation assembly for both the web UI and TUI.
