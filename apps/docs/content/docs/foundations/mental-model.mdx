...
2. **Sanitize input** with the optional prompt `sanitize` hook, then auto-escape top-level string fields and selected `escapeFields` (configurable)
...
Every interesting moment in the pipeline emits a **canonical graph record**
(`run:start` / `span:start` / `span:event` / `artifact` / `edge` / `span:end` /
`run:end`) rather than a swarm of ad-hoc callback names. Built-in primitives open
spans such as `generation.call`, `tool.call`, `memory.read`, `flow.step`,
`scoring.judge`, and `composition.parallel`.

Plugins and integrations observe that stream with `subscribeObservability()`,
the Node `crux:observability` diagnostics channel, the HTTP devtools transport,
or `@use-crux/otel`, all consumers see the same sanitized records. Custom metrics
plugins subscribe to graph records and clean up in `dispose`; they do not install
per-event callback maps on `CruxHooks`.
...
Testing is the loop around the runtime pipeline. An Eval points at a callable
production task, adds typed Cases and checks, and produces an Eval run. Current
is always present; Variants contain only differences. A complete accepted run
arm becomes a Baseline, while production feedback creates separate Review work.

Exact task and managed-scorer evidence is reused automatically. `--offline`
guarantees zero network access, `--plan` shows admitted work without writes,
and `--fresh` explicitly bypasses reusable external evidence.
...
- **Evals** run typed Cases against callable production tasks, compare Variants, and optionally use LLM judges.
...
<Callout type="info">
  Crux already supports typed prompts, composable context, local devtools,
  tracing, and quality foundations. Some deeper debugging and test helpers are
