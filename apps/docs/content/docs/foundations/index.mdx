...
Prompts, contexts, memory blocks, retrievers, guardrails, routes, and Evals live in your codebase. They can be typed, reviewed, reused, and changed deliberately.
...
Evals keep typed Cases and checks close to the production tasks they protect. The goal is to catch AI regressions before users do: not only "was the answer good?", but "did the system use the right ingredients to produce it?"
...
<Callout type="info">
  **Using an AI assistant?** Start with [AI coding
  assistants](/docs/developer-tools/ai-assistants), or point it directly at
  [`/llms.txt`](/llms.txt) for a machine-readable index of these docs.
</Callout>
