...
The first call runs the resolver and memoizes the result. Subsequent calls with the same `orgId` within 5 minutes skip the resolver entirely. Adapters mark the block for provider caching; see the [Anthropic adapter reference](/docs/reference/adapters/anthropic) for exact marker placement.
...
Resolved contributions carry freshness facts through compiler inspection and
