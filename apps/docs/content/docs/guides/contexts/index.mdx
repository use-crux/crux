...
Contexts contribute system text, additional input fields, and optional tools.
They can also bundle richer primitives such as
[memory](/docs/guides/memory), [retrieval](/docs/guides/retrieval),
[grounding](/docs/guides/retrieval/grounding),
[skills](/docs/guides/skills), and
[blackboards](/docs/guides/agents/blackboard).
...
1. **Filter** contexts whose own `when` returns false, falsy entries, and
   unmatched `when()` or `match()` branches.
2. **Resolve** each active `system()` callback, including nested
   `context({ use })` entries before the context that owns them within the same
   cache tier.
3. **Compose** the prompt's own system text first, followed by cached contexts
   as a stable prefix and then uncached contexts as the droppable tail. Each
   tier preserves `use` order.
4. **Apply the token budget** by dropping the lowest-priority uncached contexts
   until the text fits. The prompt's own system text and cached prefix remain.

Priority is `0–100` (default `50`). Higher priority means dropped later under
token pressure. It does not reorder contexts within a cache tier. See [Context
Caching](/docs/guides/contexts/caching) for prefix behavior and [Best
Practices](/docs/foundations/best-practices) for priority guidelines.
