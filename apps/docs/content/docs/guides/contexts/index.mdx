...
4. **Retain exact contributions** unless `prefer()`, `summarizable()`,
   `offloadable()`, or `droppable()` explicitly authorizes another
   representation.
...
[Context Caching](/docs/guides/contexts/caching) for prefix behavior and
[Context planning](/docs/guides/context-planning) for representation rules.
...
The nested memory still behaves like memory. It renders its context, exposes
...
<Cards>
  ...
  <Card
    title="Token budget"
    description="Set whole-request limits and authorize explicit representation ladders."
