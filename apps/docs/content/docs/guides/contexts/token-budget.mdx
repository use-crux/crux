...
When the assembled system message exceeds a token budget, Crux preserves the prompt's own system text and any provider-cache prefix, then drops the lowest-priority uncached contexts first. Provider-specific adaptations let you tweak prompts per model without breaking portability.
