...
<Callout type="info">
See the [Quick Start](/docs/guides/plans-and-tasks) for `CruxStore` setup (`inMemoryCruxStore()` for dev, `defineConvexStoreContract().store(ctx)` for Convex production).
</Callout>
