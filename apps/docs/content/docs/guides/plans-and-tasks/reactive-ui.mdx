...
function WorkPanel({ planId, taskListId }: { planId?: string; taskListId?: string }) {
  const planState = usePlan(planId)
  const tasksState = useTasks(taskListId)

  if (!planId || !taskListId) return null
  if (planState.status === 'loading' || tasksState.status === 'loading') return <p>Loading...</p>

  return (
    <section>
      <h2>{planState.data?.title}</h2>
      <ul>
        {tasksState.data.map((item) => (
          <li key={item.id}>
...
```tsx
function WorkPanel({ roadmap, work }: { roadmap?: PlanHandle; work?: TasksHandle }) {
