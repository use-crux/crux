...
Task IDs must be unique within the ledger. A duplicate `add()` rejects with `DuplicateTaskIdError`, including when concurrent writers race to add the same ID. Results and metadata must be JSON-safe.
