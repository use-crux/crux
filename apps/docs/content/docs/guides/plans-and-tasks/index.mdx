...
<Callout type="info">
**Prerequisites:**
- `@use-crux/core` — plans, tasks, and reactive hooks
- An AI SDK adapter (`@use-crux/ai` for Vercel AI SDK, or `@use-crux/openai`/`@use-crux/google`/`@use-crux/anthropic`)
- A `CruxStore` adapter for persistence (`inMemoryCruxStore()` for dev, `defineConvexStoreContract().store(ctx)` for Convex, `cruxRedisStore()` for Redis)
</Callout>
...
import { openai } from '@ai-sdk/openai' // Use any AI SDK model — see @use-crux/ai docs
