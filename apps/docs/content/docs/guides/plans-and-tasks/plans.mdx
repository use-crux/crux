...
Plans capture what the agent intends to do. They are freeform documents with title, content, metadata, timestamps, and a version that increments when title or content changes.
...
- Human-reviewable agent intent before execution.
- A shared document that worker prompts can read.
- Traceable revisions to the objective or approach.

Use `flow()` or an external durable runner for execution guarantees such as queues, retries, schedules, and resume behavior.
