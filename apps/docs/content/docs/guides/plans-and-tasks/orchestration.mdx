...
import { pipeline, generate } from '@use-crux/ai'
...
<Callout type="info">
  **Convex apps:** Use `flow()` from `@use-crux/convex/server`. It exposes `.action`, `.handler`, `.args`, and `.signal(ctx,
...
import { parallel, generate } from '@use-crux/ai'
