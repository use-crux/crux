...
Dry runs read and populate stage entries even though they do not write indexed
chunks or corpus ledger records. This makes a later real run reusable. Cache
hits and misses appear as `embedding` stages with `embeddingKind: "dense"` or
`"sparse"`.
