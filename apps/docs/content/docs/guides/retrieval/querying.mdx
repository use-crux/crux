...
Store-backed retrievers read indexed chunk and parent records written by `indexer()` or `knowledgeBase().index()`. If the retriever id differs from the writer id, pass `indexerId` so hydration uses the correct read-model keys.
