...
Store-backed retrievers read the indexed chunk/parent records written by `indexer()`. If the retriever id differs from the writer id, pass `indexerId`:
...
| --- | --- | --- |
| `dense` | `dense` | `vectors.search({ mode: 'dense', dense })` |
| `sparse` | `sparse` | `vectors.search({ mode: 'sparse', sparse })` |
| `hybrid` | `dense` and `sparse` | `vectors.search({ mode: 'hybrid', dense, sparse, fusion? })` |
...
```ts
import { reranker } from '@use-crux/core/retrieval'
