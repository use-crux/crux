...
| `rerank()` | hits -> hits | Uses a model to reorder and optionally trim retrieved hits. |
...
const recipe = retrievalRecipe({
  ...
  steps: [
    fanout({ maxQueries: 4 }),
    retrieve({ limit: 30 }),
    rerank({ topK: 8, model: rankingModel }),
  ],
})
```
...
There is one recipe step: `rerank()`. You can run it three ways.

Use the recipe model for the zero-config judge path:
...
const recipe = retrievalRecipe({
  ...
  steps: [
    retrieve({ limit: 30 }),
    rerank({ engine: reranker({ model: cohere.reranking('rerank-v3.5') }), topK: 8 }),
  ],
})
```
...
const supportAndDocs = retrievalRecipe({
  ...
  steps: [retrieve({ limit: 25 }), rerank({ topK: 8, model })],
})
```

Federated retrieval runs sources concurrently, fuses duplicate hits by `namespace/sourceId/chunkId`, and records per-source attribution in hit provenance and recipe traces.
