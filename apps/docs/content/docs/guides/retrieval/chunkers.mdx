...
The retriever searches child chunks. Each child stores a parent reference; `parentExpand({ store: data })` resolves that reference through the indexed read model at query time and adds parent content while keeping the child chunk as the citation.
