...
The retriever searches child chunks. Each child stores a parent reference; `parentExpand({ records })` resolves that reference through the indexed read model at query time and adds parent content while keeping the child chunk as the citation.

```ts
import { parentExpand, retrievalPipeline } from '@use-crux/core/retrieval'
...
- [Pipelines](/docs/guides/retrieval/pipelines) — `parentExpand` and other retrieval-time transforms
