...
| PDF      | `.pdf`, `application/pdf`, PDF bytes | `page`           | Prefers native layout-aware Markdown and page blocks, while retaining every physical page. |
...
Today that covers plain text, Markdown, HTML, PDF, CSV, JSON, DOCX, and XLSX files. PDFs become page parts with layout-aware Markdown and, on reliable native pages, ordered heading, narrative, code, list, and table blocks. CSV and spreadsheet content becomes table/sheet parts. JSON becomes path-addressed parts. DOCX files are converted through Mammoth so headings, paragraphs, and tables can move through the same document model as everything else.
...
The native PDF backend marks pages whose text is unreliable. Only those pages are sent to the application-owned `media.describe` operation; reliable pages never incur that model call. Crux does not ship OCR, and model-derived page text intentionally has no trusted page blocks.
...
const loader = filesSource(
  { directory: "./pdfs" },
  {
    namespace: "docs",
    media: {
      describe: async ({ sourceLocation }) => ({
        text: `Visual description for page ${sourceLocation?.type === "page" ? sourceLocation.pageNumber : "unknown"}`,
      }),
    },
  },
);
...
If the native package cannot load, extraction throws, or its result does not match the physical page count, Crux falls back for the whole document to `pdfjs-dist`. A successful downgrade emits exactly one `parser_warning` with `primaryParser: "pdf-inspector"`, `fallbackParser: "pdfjs-dist"`, and a bounded `reason` of `backend_unavailable`, `extraction_failed`, or `invalid_result`. It never includes native diagnostics. Textless pages may additionally carry their existing located `partial_extraction` warning. If fallback extraction also fails, the source returns the existing `parse_failed` result instead of a document warning.

PDF ingestion requires Node 22 and a supported native platform; browser and Edge runtimes are not supported. Application bundlers must externalize `@firecrawl/pdf-inspector` and its selected N-API platform package so the binary remains resolvable at runtime. Unsupported native platforms still use the safe `pdfjs-dist` fallback.

Structured parts carry their own provenance too. Page parts include `pageNumber`; reliable native PDF pages can include stable page-local block IDs and exact page-relative source ranges. Table parts include rows and columns, sheet parts include `sheetName`, and JSON parts include `path`. The default structured and parent-child chunkers consume PDF blocks; explicitly selected text and semantic chunkers retain their existing flat or semantic behavior.
