...
| --- | --- | --- |
| Reuse vectors anywhere the embedding primitive is called | `cache: embeddingCache(...)` on `embedding()` | One normalized text |
...
preprocessors, truncation, and the optional `version`. It deliberately excludes
