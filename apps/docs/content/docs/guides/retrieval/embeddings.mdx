...
| Reuse final indexing work across generations, retries, or dry runs | `cache: true` on `indexer()` | One ordered source bundle per dense/sparse kind |
...
hit does not call `embedMany()` at all; `refresh` recomputes and replaces the
bundle, while `bypass` reads and writes no bundle.
