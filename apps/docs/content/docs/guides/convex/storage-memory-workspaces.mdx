...
`convexRecordStore()` creates Convex-backed Crux records for memory, blackboards, plans, prompt cache entries, flow state, task state, and workspace metadata. Use `convexStorage()` when a feature also needs vectors or blobs.
...
```ts
import { convexRecordStore, createInMemoryConvexStoreDocumentComponent, createConvexTransport } from '@use-crux/convex'
...
Use `convexWorkspaceBlobStore()` for binary and oversized workspace payloads. Keep metadata in the Convex record store.
...
- Use `convexRecordStore()` for JSON records; use `convexWorkspaceBlobStore()` for file bytes.
