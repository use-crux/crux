...
Without a `SearchStore`, blocks configured with `embed` still persist embeddings in their records, but semantic recall falls back to the block's recency/list behavior. Pass an explicit search backend when you need similarity-ranked recall. For example, pair Convex records with Upstash Vector:
...
Convex bundled storage supplies records, not search. Pair it with an explicit pre-filter-capable `SearchStore` for semantic cache lookup, and isolate that search namespace from RAG chunks and memory entries.
...
- Pass an explicit `SearchStore` for semantic recall and use a dedicated search namespace for semantic cache.
