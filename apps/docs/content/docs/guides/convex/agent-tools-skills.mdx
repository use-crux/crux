...
Use `convexAgent()` or `crux.convexAgent()` when Crux should own the profile-backed prompt lifecycle. It keeps the Convex Agent call shape, but lets the Crux prompt define dynamic context, memory, skills, and tools. Use the lower-level `Agent` facade when you want the normal `@convex-dev/agent` constructor and method shape with Crux tool/runtime propagation and observability.
