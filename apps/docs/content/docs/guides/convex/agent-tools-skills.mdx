...
`convexTools()` returns Convex Agent tools that are already Crux-observed. Pass them directly to `Agent`; do not pass them through `wrapConvexTool()` again.
...
Skills are core definitions. With `convexAgent()`, put skills in prompt `use[]` and let the wrapper persist active skill ids:
...
External agent frameworks still need persistence callbacks because they own the tool loop:
