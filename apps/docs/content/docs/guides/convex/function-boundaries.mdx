...
Convex runs each action, query, and mutation in its own worker, so Crux run/span context does not survive `ctx.runAction()`, `ctx.runQuery()`, or scheduled work. The drop-in wrappers in `@use-crux/convex/server` preserve it: they keep Convex's normal function shape while restoring incoming context, exposing `ctx.crux` helpers inside handlers, and flushing observability before action workers exit.
...
Context travels in a hidden optional `__crux` argument added by the wrappers; see the [`@use-crux/convex` reference](/docs/reference/storage/convex) for the propagation envelope details.
...
const answer = await ctx.crux.runAction(
  "write answer",
  internal.writer.answer,
  {
    threadId: args.threadId,
    plan,
  },
...
```ts
const inputs = await ctx.crux.runQuery(
  "load research inputs",
  internal.research.data.getResearchInputs,
  {
    threadId,
  },
);
