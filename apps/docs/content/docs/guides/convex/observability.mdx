...
`@use-crux/convex/server` actions and internal actions flush automatically before returning, because Convex can freeze or tear down the worker after the handler completes. The flush is bounded to a short fixed window (3 seconds) so a slow devtools connection cannot stall the action; see the [observability reference](/docs/reference/crux-core/observability) for the flush timeout constant, the structured `ObservabilityFlushResult`, and the `onDrain` hook.
