...
| Bridge command has only a message      | Older peer or malformed command response          | Use current `@crux/convex` bridge setup so `command.error.details` carries normalized error details |
