...
`@use-crux/convex/server` actions and internal actions flush automatically before returning. This matters because Convex can freeze or tear down the worker after the handler completes. The bound is a short fixed window (`DEFAULT_CONVEX_OBSERVABILITY_FLUSH_TIMEOUT_MS`, 3 seconds) rather than a derived remaining-time budget, since Convex exposes no per-invocation deadline API. Every flush returns a structured `ObservabilityFlushResult`; pass `onDrain` to `flushObservability()`/`withObservabilityFlush()` to inspect it, or rely on the default reporter's console warning whenever a drain does not fully complete.
...
| Generation stays running past timeout  | Provider/worker never delivered terminal span end | Set `timeout.totalMs`; core emits a terminal timeout span and backend reconciles gaps                   |
