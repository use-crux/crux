...
The bridge exposes the same `storage(ctx)`, `run(ctx, target, fn)`, and `bridge(http, cruxConfig)` methods as the runtime part of `createCruxConvex()`. `createCruxConvex()` delegates those methods to this lower-level bridge.
