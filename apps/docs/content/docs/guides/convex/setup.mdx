...
Use `crux.convexAgent({ prompt, model, ... })` for agents, `crux.store(ctx)` for a request-scoped store, and `crux.withRuntime(ctx, target, fn)` only for lower-level integrations that need Convex-bound memory/tools outside the high-level wrapper.
