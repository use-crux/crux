...
1. Run `crux dev` and open the definition in **Catalog / Index**.
...
- **Quality-primary**: scorers correlate through the Quality join; a live `scoring.judge` span still surfaces when an authored `judge({ id })` ran.
- **No runtime**: static or declarative definitions show no fabricated counts and no dead links.

## Run Detail links back

Run Detail lists every definition reference on the run or span, and resolved ids link into Catalog. If a definition was renamed or deleted since the run, the row keeps its role, kind, id, and any sanitized source, and is marked **unresolved** instead of becoming a dead navigation target. Runs whose refs point at a since-deleted definition still appear in the filtered Runs list.
