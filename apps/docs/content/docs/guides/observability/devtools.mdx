...
`POST /api/observability/records` is intentionally partial-accepting. Parseable batches return `202`
with accepted and rejected record counts; malformed JSON returns `400`; transient local storage
failures return `503` with `Retry-After: 1`. The local read models keep broad dashboard/resource
queries bounded, including latest-500 caps for full-history devtools pulls and resource activity.
...
The Go process owns the server and read models. Node.js is spawned only for bounded helper workers: Project Index indexing, eval execution, and lazy source resolution. The server stores the raw payload for every accepted graph record, including forward-compatible unknown record types, so newer SDKs can preserve data even when an older local read model skips normalized projection. Local observability storage is retained by default for 14 days and 2000 runs, with 256KB artifact preview caps. Use `CRUX_OBSERVABILITY_RETENTION_DAYS`, `CRUX_OBSERVABILITY_RETENTION_RUNS`, and `CRUX_OBSERVABILITY_PREVIEW_MAX_BYTES` to tune those limits during local development.

Quality evaluation graphs are first-class local traces. The embedded eval runner emits one `eval.run` umbrella run plus separate `eval.case` runs in the same trace, links them with `eval.case_of`, and represents baseline comparisons and experiment diffs through `comparison.report` artifacts with `comparison.candidate` and `comparison.baseline` edges. Successful baseline promotion emits `baseline.promotion` linked to the promoted eval run. Cassette replays link the replaying case run to the originally recorded run with `replay.of`. Edges may point at runs recorded in another local database or older baseline file; the server preserves those external references and does not treat absent target nodes as read-model errors.

### The SDK

Your application connects to the devtools server through `config()` or `enableDevtools()`. When `serverUrl` is set, the SDK installs the canonical observability transport, posts ordered graph record batches to `POST /api/observability/records`, and sends runtime prompt/context/tool snapshots through `POST /api/index/snapshot` to enrich the Project Index. Delivery is non-blocking, batches records on a short timer, chunks requests in the SDK delivery engine, caps queued records with oldest-record drops, retries only failed chunks on a capped backoff without waiting for another emitted record, and swallows synchronous or asynchronous transport failures so devtools never slow down or break application code. For tunneled/cloud runtimes, prefer `observability.token` or `CRUX_DEVTOOLS_TOKEN`; the Go server accepts that bearer token only on the observability ingest route. Tokenized tunnel URLs such as `https://example.ngrok.app?t=...` are still supported as `serverUrl` values for compatibility; the token is preserved on the final ingest endpoint. The Go server keeps list/dashboard/lifecycle reads on cheap run-summary paths and reserves exact graph projection for single-run inspection. Generation streams close their spans from the raw stream's completion or error signal, so stopped streams and failed streams do not stay visually running while later usage metadata is collected. In serverless runtimes, call `observe.flush()` or use the `@use-crux/convex` helpers before the action returns so queued records are not killed with the worker.
