...
Explain leads with a one-line **verdict** and a row of scan chips, then a
scannable body:
...
- **How this is protected** — a coverage scorecard plus read-only suggested
  assertions (tests are authored in code and run from the CLI).
...
failing-first **Insight** roll-up across every turn. Status (disposition,
freshness, cache, coverage) is the only saturated colour; coverage gaps and
missing evidence are never styled as failures.
