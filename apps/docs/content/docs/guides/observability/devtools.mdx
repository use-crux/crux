...
The Storage detail view is privacy-safe by default. It shows wiring, record/vector capabilities, warnings, and runtime operation summaries, but it does not display record values, vector contents, or asset bodies unless raw debug data is explicitly enabled by the application.
