...
Select a generation turn and the **Explain** tab answers, in one place, *why this
model call behaved the way it did*. It is a read-out: it summarises the turn and
...
- **Freshness & cache** — two deliberately separate columns. Freshness answers
  *was the evidence current enough for this turn?* (`fresh` / `refreshed` /
  `stale-used` / `stale-rejected`); cache answers *what did reuse save?*
