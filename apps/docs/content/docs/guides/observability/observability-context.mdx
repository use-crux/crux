...
When one workflow intentionally owns several run roots, pass `traceId` to `observe.openRun()` for the child roots. Quality uses this for evaluation traces: the `eval.run` umbrella and every `eval.case` run share one trace while staying separate runs.
...
await propagateAttributes(
  ...
  () => observe.run({ name: 'checkout assistant', rootPrimitive: 'agent.run' }, runCheckout),
)
```
