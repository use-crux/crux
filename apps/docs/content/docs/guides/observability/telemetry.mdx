...
<Callout type="warning">
  Install and start your `TracerProvider` before `withTelemetry()`. If Crux sees invalid OTel span contexts, it warns once and falls back to lightweight span tracking so application code keeps running.
</Callout>
...
if (extracted.carrier) {
  const resumed = observe.resumeRun(extracted.carrier, { reason: 'worker-start' })
  // ...
  resumed.end()
}
```
...
<Cards>
  ...
  />
  <Card title="Runtime setup" description="Node, serverless, Workers, Convex flush bindings." href="/docs/guides/observability/runtime-setup" />
  <Card title="Privacy" description="What OTel keeps off by default." href="/docs/guides/observability/privacy" />
