...
| `streamImage()` / `streamSpeech()` | media primitive logical + attempt spans | `crux.operation`, `crux.streamingRole`, route/commit, counts, bytes, MIME facts, first-event/duration, terminal |
...
Bounded media streams keep one logical span and child physical-attempt spans.
They export progressive counts and timing, not progressive content. Prompts,
media bytes/base64, URLs, filenames, hashes, refs, held Safety payloads, and
provider-native events are dropped before OTel mapping.
...
Qualified `evidence.for` relationships bypass generic edge projection and
become dedicated `crux.evidence` events on the protected producer span. Their
exact attributes are `crux.evidence.id`, `crux.evidence.role`,
`crux.evidence.kind`, optional `crux.evidence.conclusion`, and
`crux.evidence.subject_kind`. Application `custom.*` kinds normalize to
`custom`. Coverage maps separately to `crux.evidence.coverage` with role and
coverage status; coverage conflicts expose only role.

The evidence ID is for event correlation with Local and delivery diagnostics.
Never promote it to a span/resource attribute or use it as a metric dimension.
Payloads, graph endpoints, producer identity, paths, raw keys, digests, and
arbitrary evidence attributes are not exported. See the complete
[positive and negative allowlists](/docs/guides/observability/execution-evidence#privacy-and-opentelemetry).
...
Successful declarative pattern application adds only privacy-safe graph
evidence: `applied: true` plus affected broad telemetry surfaces. Local uses it
for Runs badges and labels. The OTel subscriber receives the same already
redacted graph record, but the OTel mapper does not export the evidence as a
new semantic-convention attribute. Rules, matched values, replacements, paths,
hashes, and counts are never included.
