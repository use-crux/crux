...
| Cloudflare Workers | Install defer and drain through `ctx.waitUntil` (no added response latency; no `nodejs_compat` required). | `withCrux()` from `@use-crux/core/observability/workers` |
...
export const handler = withObservableInvocation(
  ...
  () => ({
    ...
    onDrain(result) {
      if (result.status !== 'drained') console.warn('incomplete Crux drain', result)
    },
  }),
)
```
