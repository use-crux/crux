...
<Callout type="info">
  When `devtools.serverUrl` is set in `config()` and no explicit `observability` transport is configured, Crux
  auto-installs the local devtools transport before your custom plugins.
</Callout>
...
<Cards>
  <Card
    ...
    description="Send traces to Datadog, Honeycomb, or Grafana via @crux/otel."
