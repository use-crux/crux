...
config({
  prompts,
  plugins: [
    withDevtools({ prompts: [...], serverUrl: '...' }),
    withTelemetry({ serviceName: 'my-app' }),
    myCustomPlugin,
  ],
})
...
<Callout type="info">
When `devtools.serverUrl` is set in `config()`, the devtools plugin is auto-prepended to the plugins array. You don't need to add it explicitly unless you want full control over plugin ordering.
</Callout>
...
<Cards>
  <Card title="Production Telemetry" description="Send traces to Datadog, Honeycomb, or Grafana via @crux/otel." href="/docs/guides/observability/telemetry" />
  <Card title="Middleware" description="Global middleware for logging, timing, and error handling." href="/docs/guides/observability/middleware" />
  <Card title="CruxPlugin Reference" description="Full API reference for the plugin interface." href="/docs/reference/crux-core#cruxplugin" />
</Cards>
