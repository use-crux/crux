...
Later plugins wrap earlier ones for middleware (the outer middleware controls when `next()` is called), other hook fields merge by `mergeHooks` rules, and `dispose()` runs in reverse install order. Every subscriber, diagnostics-channel listener, transport, and `@use-crux/otel` projection observes the same sanitized record stream, with subscriber errors isolated from user code. Details: [Plugin composition](/docs/reference/crux-core/observability#plugin-composition).
