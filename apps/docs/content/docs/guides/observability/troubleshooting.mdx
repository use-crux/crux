...
UI tooltip: _The server has not observed enough delivery evidence to call this run healthy or degraded._
...
UI copy: _Telemetry ended without a run:end record: the run may still be executing out of view, or its process exited before reporting a terminal status._ (Plus gap wording when sequence gaps exist.)
...
- _A trace alias identifies more than one logical run…_ when `traceAliasConflict` is set
- Otherwise _stored terminal evidence … conflicts with what was previously recorded_
...
Catalog zero-activity copy: _No runs have referenced this definition yet._
