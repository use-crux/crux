...
| Since-deleted definitions | Runs still filter by the historical id; Catalog may show unresolved links from old runs. |
...
Links come from runtime `definitionRefs` on the run/span. If refs were never emitted (anonymous handle, non-instrumented path), there is nothing to link. If refs exist but the Project Index no longer contains the id, the row shows **unresolved** with: *Not found in the current Project Index: the source definition may have been renamed or deleted.*
