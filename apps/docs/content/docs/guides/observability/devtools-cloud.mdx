...
When your backend runs locally, devtools just works — `DEVTOOLS_URL` defaults to `http://localhost:4400`. But when your backend runs in the cloud (Convex, Vercel, AWS Lambda), the devtools server needs a **public URL** so your backend can reach it.
...
Your `crux.config.ts` picks it up automatically:
...
When runtime step labels match step IDs from a `flowEvaluation()`, the Timeline shows delta indicators comparing runtime duration/tokens against the eval baseline average.
