  ...
  hooks: {
    onPrepare: (args) => {
      ...
      if (args.droppedContexts.length > 0) {
        console.warn('Dropped:', args.droppedContexts.map(c => c.source))
      }
    },
    onGenerate: (args) => {
...
<Callout type="info">
This approach captures `generate()` and `stream()` calls. For full observability covering memory operations, compaction, tool calls, and flows, OpenTelemetry integration is planned — see the [Devtools guide](/docs/guides/observability/devtools) for current instrumentation coverage.
</Callout>

## Next steps

<Cards>
  <Card title="Devtools" description="Visual tracing UI for generations, memory, evals, and quality experiments." href="/docs/guides/observability/devtools" />
  ...
  <Card title="Scoring" description="LLM-as-a-judge scoring and pre-built quality metrics." href="/docs/guides/eval/scoring" />
  <Card title="API Reference" description="PromptHooks, PrepareHookArgs, and middleware type signatures." href="/docs/reference/crux-core" />
</Cards>
