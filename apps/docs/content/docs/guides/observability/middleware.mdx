...
For deeper observability into memory, scoring, and agent operations, Crux provides `InstrumentationHooks`. These are automatically wired up when devtools are enabled via `config({ devtools: { serverUrl } })` — you don't need to configure them manually.
...
<Callout type="info">
  ...
  compaction, tool calls, and flows, OpenTelemetry integration is planned — see the [Devtools
  guide](/docs/guides/observability/devtools) for current instrumentation coverage.
</Callout>
