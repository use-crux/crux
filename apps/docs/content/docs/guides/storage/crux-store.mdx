...
It is not a vector index and it is not an asset store. If a feature needs similarity search, give it a [`VectorStore`](/docs/guides/storage/vector-store). If a workspace needs binary files, give it an [`AssetStore`](/docs/guides/storage/asset-storage).
...
If your database also supports vectors or assets, implement [`VectorStore`](/docs/guides/storage/vector-store) or [`AssetStore`](/docs/guides/storage/asset-storage) as separate capabilities and bundle them with `storage()`.
