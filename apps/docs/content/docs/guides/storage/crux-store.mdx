...
Use `cruxConvexStore()` when your app already runs on Convex. Its component list query returns `{ docs, cursor }` pages from the `by_key` index while the store adapter owns TTL suppression and decoded-value filters. See the [Convex guide](/docs/guides/convex) for setup and boundary rules.
