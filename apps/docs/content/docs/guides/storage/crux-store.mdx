...
It is not a retrieval index and it is not an asset store. If a feature needs similarity search, give it a [`SearchStore`](/docs/guides/storage/search-store). If a workspace needs binary files, give it an [`AssetStore`](/docs/guides/storage/asset-storage).
...
Retrieval systems usually pair records with search storage:
...
| `postgresRecordStore()`     | Lazy expiry suppressed by every read/list path   |
...
Use PostgreSQL for durable records with native JSONB filters, batches, and
versioned CAS:
...
PostgreSQL setup is explicit. Application reads and writes never run DDL.
Prefix scans treat `%`, `_`, and `\\` literally, and `null` filters match an
explicit JSON `null` rather than a missing property.
...
If your database also supports search or assets, implement [`SearchStore`](/docs/guides/storage/search-store) or [`AssetStore`](/docs/guides/storage/asset-storage) as separate capabilities and bundle them with `storage()`.
