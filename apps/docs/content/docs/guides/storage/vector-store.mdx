...
Use first-party PostgreSQL storage when pgvector is available:

```ts
import { postgresVectorStore } from "@use-crux/postgres";
...
Omit `sparseDimensions` for a dense-only store. When configured, PostgreSQL
supports sparse and hybrid search with normalized RRF. It does not support
DBSF. Metadata filters run before candidate ranking, writes are strongly
consistent, and HNSW indexes provide approximate cosine search. The existing
Postgres cookbook remains useful for custom schemas and exact-scan designs;
most applications should start with `postgresVectorStore()`.
