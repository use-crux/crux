...
The workspace stores file metadata in `RecordStore`: path, MIME type, size, timestamps, preview, and blob URI. The blob store keeps the actual bytes.
...
```ts
import { inMemoryBlobStore } from '@use-crux/core/storage'
...
const blobs: BlobStore = {
  ...
  async head(uri): Promise<BlobRef | null> {
    // return metadata without reading the content
    return null
  },
  ...
  capabilities() {
    return { multipart: false, signedUrls: false }
  },
}
```
...
3. Preserve `mimeType`.
4. Implement `delete(uri)`; beta blob stores require a lifecycle delete operation.
