...
<Callout type="info">
  Each delegation validates data at three points: the LLM's tool args, the
  subagent's return value, and the final transformed output. See the [API
  reference](/docs/reference/crux-core/agent#delegateconfig) for the
...
// ---cut---
const result = await researchDelegate.run(
...
<Callout type="info">
  All delegate executions are instrumented automatically: `delegate:start` and
  `delegate:complete` events appear in
  [Devtools](/docs/guides/observability/devtools) with input/output payload
  snapshots and duration.
</Callout>
...
<Cards>
  ...
  <Card
    title="Agents"
    description="Multi-agent coordination patterns at a glance."
