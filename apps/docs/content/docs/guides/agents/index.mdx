...
You have a prompt that works. Now you need _two_ prompts to work together, or three, or ten. Maybe a researcher feeds a writer, reviewers run in parallel, or a triage agent decides who handles a customer issue.
...
<Callout type="info">
  Each SDK adapter (`@use-crux/ai`, `@use-crux/openai`, `@use-crux/anthropic`,
  `@use-crux/google`) re-exports pre-bound versions of all patterns. Import from
  your adapter, not from `@use-crux/core`.
</Callout>
...
const result = await pipeline({
  ...
  steps: [
    { agent: researcher, name: "research" },
    {
      agent: writer,
      name: "write",
      input: (ctx) => ({ findings: ctx.research.output.sources }),
    },
    {
      agent: editor,
      name: "edit",
...
**Still not sure?** _"Do I know the sequence ahead of time?"_
...
[Deep dive →](/docs/guides/agents/handoff)
    ...
    lookupInvoices: {
      ...
      execute: async ({ customerId }) => {
        ...
        const invoiceLookup = flow("invoice-lookup", async (flow) => {
          const invoices = await flow.step(
            "fetch",
...
<Cards>
  <Card
    title="Agent"
    description="Bundle prompt + model + tools into a reusable agent for compositions."
  ...
  <Card
    title="Flows"
    description="Named steps with retries, fallbacks, suspend/resume, and tracing."
