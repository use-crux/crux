...
<Callout type="warn">
  Board writes are in-process only. For cross-process coordination (e.g.,
  separate serverless invocations), provide persistent `records`, see
  [Memory](/docs/guides/memory) for storage options.
</Callout>
...
<Cards>
  ...
  <Card
    title="Recipes"
    description="Full working example of a multi-agent pipeline with blackboard."
    href="/docs/cookbook"
  />
</Cards>
