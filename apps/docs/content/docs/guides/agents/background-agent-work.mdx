...
status, wait briefly for a result, request cancellation, or detach it. Do not
define your own `work` tool in that Agent or its Prompt: the collision fails
during preparation.
...
The registry, result, and control capability exist only in the current
JavaScript process. Process exit, a serverless freeze, a crash, or a request
landing on another worker loses them. There is no crash recovery, remote
transport, API route, cross-process or Session rejoin, durable replay, or
automatic application-level persistence.
