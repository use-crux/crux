...
Agent-as-Tool children derive a stable occurrence identity from the parent
execution owner, turn/step, tool call, and binding key so adapter/provider
replay reconnects the existing child instead of starting another. Conflicting
reuse rejects deterministically. Separate parent runs do not share occurrence
identity even when tool-call ids collide.
