...
During `crux dev`, Crux shows a fast first view quickly, then enriches it with deeper TypeScript understanding when available. A snapshot may briefly include an `index.static_only` diagnostic while that deeper pass is unavailable or still running.
