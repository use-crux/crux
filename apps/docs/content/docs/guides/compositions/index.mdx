...
Most real LLM workloads aren't one-shot. You want to _plan then execute_, _generate three drafts and pick one_, _fan out to specialists and merge_, or _let the model itself route the next step_. Hand-coding these patterns leads to ad-hoc orchestration that's hard to test, hard to instrument, and hard to swap.
...
```ts
import { agent, createPipeline } from "@use-crux/core/agent";
...
<Cards>
  <Card
    title="Pipeline"
    description="Sequential chaining with typed context accumulation."
    href="/docs/guides/compositions/pipeline"
  />
  <Card
    title="Parallel"
    description="Concurrent agents, results keyed by name."
    href="/docs/guides/compositions/parallel"
  />
  <Card
    title="Consensus"
    description="Voting with a quorum threshold."
    href="/docs/guides/compositions/consensus"
  />
  <Card
    title="Swarm"
    description="Peer-to-peer LLM-routed handoffs."
