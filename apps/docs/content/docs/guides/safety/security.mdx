...
<Callout type="warn">
  Auto-escape only rewrites top-level string fields in the parsed `input`
  object. Nested strings inside objects or arrays emit a diagnostic and must be
  escaped explicitly. If you inject untrusted content via `rawFields` or
