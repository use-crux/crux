...
Top-level string input fields are XML-escaped by default, and a `safe` tagged template gives you per-value control over escaping, truncation, and wrapping.
...
Security warnings are emitted as canonical `security.warning` graph records
(and related `security.report` artifacts) with full trace correlation — each
warning links to the `generate()` call that triggered it. Subscribe to the graph
stream for custom integrations:

```ts
import { subscribeObservability } from '@use-crux/core/observability'

const unsubscribe = subscribeObservability((record) => {
  if (record.type === 'span:event' && record.name === 'security.warning') {
    // attributes include pattern / field / severity when present
