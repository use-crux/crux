...
These appear in the Constraints view in the devtools dashboard, are emitted as OTel spans when `@use-crux/otel` is active, and are wired through `graph-record subscribers` (`constraint.checked records`, `constraint.retry records`, `constraint failure records`).
