...
Use `guardrail.media()` with `boundary.input.media()` for a declarative
attachment policy. Crux checks the real canonical image, audio, video, or file
part before provider normalization or provider I/O:
...
and model-produced media safety remains tracked in
