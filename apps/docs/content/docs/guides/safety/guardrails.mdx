...
import { guardrail, createSafetyPlugin } from '@use-crux/core/safety'
...
The guards themselves are also standalone — `guardrail()` has no adapter dependencies. You can use `@use-crux/core/safety` purely for the safety primitives without the rest of Crux.
...
```ts
import { isGuardrail } from '@use-crux/core/safety'
...
- Are emitted as OTel spans when `@use-crux/otel` is active
- Are wired through `graph-record subscribers` (`guardrail.action records`)

Use `graph-record guardrail events` plus the audit on `result._meta.guardrails` for application-level logging and EU AI Act audit trails. The optional `category` field on each policy lets reporting aggregate by risk type (`pii`, `jailbreak`, `toxicity`, ...) instead of by policy name.
