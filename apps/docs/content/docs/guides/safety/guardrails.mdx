...
import { guardrail, createGuardrailPlugin } from '@crux/core/safety'
...
// Install as plugin — runs on every generate() call
config({ plugins: [createGuardrailPlugin([injectionGuard, piiGuard])] })
```
...
Compose guards into an executable pipeline. Mixed phases are accepted — the pipeline auto-splits them.

```ts
import { createGuardrailPipeline } from '@crux/core/safety'

const pipeline = createGuardrailPipeline(
  [injectionGuard, piiGuard, safetyGuard, jsonFixer],
  {
    onBlock: (guard, detail) => log.warn(`Blocked by ${guard.name}`, detail),
    onRedact: (guard, detail) => log.info(`Redacted by ${guard.name}`),
    onTransform: (guard, detail) => log.info(`Transformed by ${guard.name}`),
    onWarn: (guard, detail) => log.info(`Warning: ${guard.name}`, detail),
  },
)
```
...
**Input guards** (`pipeline.runInput(content, ctx)`):
- Execute in declaration order
- Redacted/transformed content flows forward to subsequent guards
- First `block` short-circuits — remaining guards skip
- Output guards are ignored

**Output guards** (`pipeline.runOutput(content, ctx)`):
- Same sequential execution and content chaining
- Input guards are ignored
...
Guardrails never re-call the model. If you need to validate output quality and retry with feedback until requirements are met, use [`constraint()`](/docs/guides/safety/constraints) instead. Constraints run inside the adapter's generate loop with parallel-check and combined-retry.

### Audit trail

Both `runInput` and `runOutput` return a result with an audit trail:

```ts
const result = await pipeline.runOutput(text, ctx)

result.content                    // final (possibly modified) content
result.audit.blocked              // false (would have thrown if true)
result.audit.applied[0].guard     // 'pii'
result.audit.applied[0].action    // 'redact'
result.audit.applied[0].original  // original content before this guard
result.audit.applied[0].durationMs // 2.3
```
...
const guardedStream = llmStream.pipeThrough(transform)
```

Mixed strategies work in the same pipeline. Each guard runs independently — `buffer: 'none'` guards process chunks in real-time while `buffer: 'full'` guards wait for completion.

### Works with any text stream

`createStreamGuardrailTransform` returns a standard Web Streams API `TransformStream<string, string>` — it's not tied to Crux adapters. Pipe any text stream through it:

```ts
// Vercel AI SDK
const result = streamText({ model, prompt })
result.textStream.pipeThrough(transform)

// OpenAI SDK direct
const stream = await openai.chat.completions.create({ stream: true, ... })
// pipe text deltas through transform
...
Use the pipeline callbacks (`onBlock`, `onRedact`, `onWarn`) for application-level logging and EU AI Act audit trails.
