...
Input guardrails evaluate the text projection returned by `messageText()`.
Text parts pass through verbatim. Media parts become bounded descriptors:
...
For example, this guard receives both the text and the image URL:
...
`block` and `warn` apply to the complete projection. `rewrite` applies only to
...
media-only message, or cannot be redistributed unambiguously. If a policy must
act on a media source, such as a signed URL containing a token, use `block`
instead. Media bytes, pixels, and other media content are not evaluated. See
the [media rewrite actions RFC](https://github.com/use-crux/crux/issues/212)
for planned media-specific rewrite behavior.
