...
Rung 3 adds **scorers** (graded 0–1 measures, reported with honest
...
**Model-backed** (use the `quality.setup()` providers):
...
| `scorers.embeddingSimilarity()` | Cosine similarity to `expected` (needs `setup().embed`) |
...
`score.metadata.rationale`. Judge model resolution: `opts.model` →
`setup().judgeModel` → `setup().model` — string refs resolve through
...
which the `consistency` gates read. Comparisons (next rung) are paired per
case, not pooled.
