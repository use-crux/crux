...
A pass/fail assertion cannot tell you that a prompt got 10% worse this week.
**Scorers** can: graded 0–1 measures, reported with honest statistics.
**Gates** turn those measures into the declarative pass policy that drives the
exit code.
...
### Can I trust this judge?

Judges are useful regression signals, not ground truth. Crux makes them as
repeatable and auditable as the provider allows: judge calls pin deterministic
generation settings, wrap task output/reference/context as untrusted data in
the prompt, record/replay through the same cassette boundary as task model
calls, and stamp each judge score with model, prompt version, rubric
fingerprint, and rationale metadata.

Use human labels to calibrate the signal:

```bash
crux quality label <experiment-id> --case refund-policy --verdict fail --score helpful --note "misses refund window"
crux quality judge-report support.refunds --json
```

`judge-report` joins human labels with judge-backed scores and returns
agreement, precision, recall, Cohen's kappa, and disagreements you can inspect.
Treat low agreement as a scorer-design bug: refine the rubric, add examples, or
replace the judge with a code scorer for that requirement.
