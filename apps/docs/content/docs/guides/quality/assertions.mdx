...
Scorers tell you how good an answer is; assertions tell you what is flatly
wrong for a specific case. Add `expect:`, an assertion callback that runs for
**every case**. It receives the case's full context and a bound,
capability-typed `expect` surface:
...
Use `afterScores:` when a check needs scorer results. It runs after all scorers,
...
| `ctx.recordScore(name, score, metadata?)` | Ad-hoc per-case score (0–1): joins the same score model as scorers                   |
...
Scores produced by dynamic scorer functions or pre-score `ctx.recordScore(name, …)`
...
available. `assertions.outcomes` is the durable assertion ledger; consumers
should filter failed or uncaptured outcomes when they need a failure list. When the runner can resolve authored source, outcomes can also
...
it were authored code. If a helper throws a plain `Error` before a matcher
records an outcome, the cell is still marked errored, but local evidence records a
...
## Asserting on traces (tool calls, routing, safety)
...
ctx.expect.decisionReport.safety.toHaveOutcome('pii-guardrail', 'rewrite', {
...
Assert that a Safety policy intervened through the canonical decision report:

```ts
expect: (ctx) => {
  ctx.expect.decisionReport.safety.toHaveOutcome('refund-approval', 'request_approval', {
    reasonCode: 'tool.eligible.request',
  })
}
```
