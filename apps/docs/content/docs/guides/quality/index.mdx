...
convention, persists records under `.crux/quality`, and skips colocated
`prompt({ tests })` because there is no prompt registry to import.
...
`quality.setup()` remains compatible when a project intentionally wants
project-wide defaults, shared named models, judge-scorer providers, redaction,
or run defaults:
...
`setup()` provides compatibility defaults: `generate` (adapter generate fn),
`model` (default for tasks that don't specify one), `models` (named models
referenceable as strings in params/variants), `judgeModel` (for judge
scorers), and `embed` (for `scorers.embeddingSimilarity`). `redact` dot-paths
scrub persisted records and cassettes; `defaults` fills
`trials`/`concurrency`/`timeoutMs`/`replay` for evaluations that don't declare
them.
