...
Accepts Google-native options: `tools`, `temperature`, `maxOutputTokens`, `topP`, `topK`.
...
For agent frameworks that manage their own model calls, the agent adapter returns composed instructions and a wrapped model instead of executing directly. This works with any AI SDK-based framework:
...
<Callout type="info">
  See [Framework Guides](/docs/getting-started) for specific integration examples with Convex Agent and other frameworks.
</Callout>
...
<Callout type="info">
The Vercel AI SDK adapter exports pre-bound functions (`generateObjectFn`, `generateTextFn`) since the model is passed per-call. The OpenAI, Google, and Anthropic adapters use factory functions that bind the client and model upfront.
</Callout>

## Next steps

<Cards>
  <Card title="Flows" description="Chain multiple generate() calls into structured pipelines with named steps." href="/docs/guides/flows" />
  <Card title="Tools" description="Declare and compose tools across contexts and prompts." href="/docs/guides/tools" />
  <Card title="Middleware & Hooks" description="Global middleware and per-prompt lifecycle hooks." href="/docs/guides/observability/middleware" />
  <Card title="Examples" description="Full working examples combining execution with memory and agents." href="/docs/cookbook" />
  <Card title="API Reference" description="@crux/ai, @crux/openai, @crux/google, and @crux/anthropic adapter signatures." href="/docs/reference/adapters/ai" />
</Cards>
