...
<Callout type="warn">
  The Google adapter validates structured output with Zod after generation. If the model returns invalid JSON, you'll
  get a Zod error, not a Google SDK error.
</Callout>
...
<Callout type="info">
  See [Framework Guides](/docs/getting-started) for specific integration examples with Convex Agent and other
  frameworks.
</Callout>
...
```ts
import { createGenerateObjectFn, createGenerateTextFn } from '@use-crux/anthropic'
...
<Callout type="info">
  These helpers are provider-native, lightweight wrappers. They pass the schema to the provider structured-output API
...
<Cards>
  ...
  />
  <Card title="Tools" description="Declare and compose tools across contexts and prompts." href="/docs/guides/tools" />
  <Card
