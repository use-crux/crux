...
| CI regression testing | `tests` or [`evaluatePrompt()`](/docs/guides/eval) |
...
- **[Skills](/docs/guides/skills)** inject a lightweight catalog plus `LoadSkill` and `LoadReference` tools so the model can pull Markdown instructions on demand.
...
- **[Evals](/docs/guides/eval)** run prompt and RAG regression suites in CI and devtools.
