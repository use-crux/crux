...
- `transportBindingHealth({ store, namespace, program })` projects one secret-free
  ...
  Memory and PostgreSQL implement the transports port; Convex does not claim
  managed-transport accept/checkpoints and rejects managed-binding workers with
  `CAPABILITY_MISSING`.
