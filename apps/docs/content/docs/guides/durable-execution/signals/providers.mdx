...
| SSE / WebSocket thin adapters | **Not shipped** — follow-on adapters must compile to `stream({ open })`, not a second worker |
...
- you need first-party SSE or WebSocket helpers — those remain follow-on thin
  adapters over `stream({ open })`, not a second supervision path.
...
| SSE / WebSocket | Not shipped as first-party helpers. When added, they must be thin authoring adapters over this `stream({ open })` seam. |
