...
Crux adapters have four execution rungs. They all share the same prompt
resolution, tool policy, validation, safety, memory, and observability pipeline;
they only differ in who makes the provider call.
...
They are translation helpers only.
