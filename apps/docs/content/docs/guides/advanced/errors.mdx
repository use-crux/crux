...
## Flow evaluation errors

### `Flow step "{id}" has not been executed yet`

Inside a flow step's `input` or `skip` function, you're referencing a previous step that hasn't completed. Steps execute in the order defined in the `steps` array — you can only access steps that come before the current one.

```ts
flowEvaluation({
  steps: [
    { id: 'research', prompt: researchPrompt },
    {
      id: 'write',
      prompt: writePrompt,
      input: (ctx) => ({
        findings: ctx.step('research').output.findings, // OK — research runs first
        review: ctx.step('review').output,               // throws — review hasn't run yet
      }),
    },
    { id: 'review', prompt: reviewPrompt },
  ],
})
```

**Fix:** Reorder steps so dependencies come first.

### `Flow step "{id}" does not exist. Available: {stepIds}`

You called `trace.step(id)` with a step id that doesn't exist in the flow definition. The error lists all available step ids.

### `No model configured for step "{id}" in config "{configName}"`

A flow config doesn't include a model for one of the steps. Every step needs a model in the config's `models` map.
...
A flow step threw an error during execution. The original error message is preserved. This wraps errors from the underlying `generate()` call — check the inner message for the actual cause (API error, timeout, etc.).

### `Intermediate assertion failed for turn {n}: "{userMessage}"`

In a multi-turn flow step, an intermediate assertion returned `false`. The error includes the turn number and the user message that triggered it.

### `No steps completed`

Flow execution failed before any step could complete. This usually indicates a configuration error — check that your flow definition, configs, and generate function are valid.

## Testing errors

### `evaluatePrompt: No test cases provided and prompt "{id}" has no tests defined`

You called `evaluatePrompt()` without passing `cases` and the prompt has no inline `tests` defined.

```ts
// Bad — no cases anywhere
await evaluatePrompt({ prompt: myPrompt, generate, models: [model] })
...
await evaluatePrompt({
  ...
  models: [model],
  cases: [
    { name: 'basic', input: { text: 'hello' }, assert: (r) => r.text.length > 0 },
  ],
})

// Good — inline tests on the prompt
const myPrompt = prompt({
  id: 'greet',
  tests: [
    { name: 'basic', input: { name: 'World' }, assert: (r) => r.text.includes('World') },
  ],
  // ...
})
```
