...
Runtime Engine errors use stable `CruxRuntimeError.code` values and have one page per code under [Runtime Errors](/docs/errors).

## Configuration errors

### `configure: all prompts must have an id`

Every prompt passed to the legacy registry helper `configure()` must have an `id` field. Prompt ids are
also required for stable local discovery and devtools joins.
...
Two or more prompts share the same `id`. Each prompt in a prompt bundle or legacy `configure()` call
...
const missingGeneratorJudge = judge({
  id: 'quality',
  criteria: 'Does the output satisfy the quality bar?',
  scale: { min: 1, max: 5 },
})
await missingGeneratorJudge.score({ input: '...', output: '...' }) // throws

// Good: provide in config
const configuredJudge = judge({
  id: 'quality',
  criteria: 'Does the output satisfy the quality bar?',
  scale: { min: 1, max: 5 },
...
await configuredJudge.score({ input: '...', output: '...' })

// Good: provide per-call
await missingGeneratorJudge.score({ input: '...', output: '...' }, { generate: generateObjectFn, model })
```
...
All three adapters (`@use-crux/ai`, `@use-crux/openai`, `@use-crux/google`) catch errors from the underlying SDK, fire the `onError` hook if configured, then re-throw the original error unchanged. This means you'll see native error types:
...
```
[@use-crux/core] Suspicious pattern detected in input field "{field}": {description}
```
