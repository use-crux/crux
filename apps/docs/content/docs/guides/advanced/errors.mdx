...
await missingGeneratorJudge.score(
  { input: "...", output: "..." },
  { generate: generateObjectFn, model },
);
```
...
Eval discovery, task-contract, host-readiness, and Baseline errors fail before
spend where possible and include the exact remedy. See [Eval errors and
remedies](/docs/guides/evals/errors) for the canonical messages.
