...
const acmeDialect: ProviderTranscriptDialect<YourSDKMessage, AcmeResponse> = {
  ...
  encodeToolResults: ({ results }, helpers) => results.map((r) => encodeToolResult(r, helpers)),
  decodeMessage: (msg) => decodeToUnits(msg),
  readAssistant: (raw) => ({ text: raw.message.content, toolCalls: raw.message.toolCalls }),
}
...
export const acmeSdkRuntime = defineProviderRuntime({
  ...
  loop: {
    ...
    bind: (client: AcmeGateway) => ({
      ...
      runStructuredAttempt: (request) => runAcmeStructuredAttempt(client, request),
