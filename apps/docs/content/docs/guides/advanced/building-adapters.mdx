...
Step 3 is identical across all adapters. Adapter profiles compile into a private execution facade, which wires prompt resolution, tool lifecycle, safety policy, retries, stream safety, metadata stamping, and memory capture around the SDK-specific call. If your SDK has ordinary chat text/structured/stream calls, start with `defineAdapterProfile({ driver: nativeChat(...) })`; use the lower-level shared orchestration functions only when you are writing a new profile driver.

## Fast path: native chat providers

Use `@crux/core/adapter/profile` when your SDK boundary looks like OpenAI Chat Completions, Anthropic Messages, or Google GenAI content generation: Crux should own the loop, and your package should own provider wire facts.
...
export const acmeProfile = defineAdapterProfile({
  id: 'acme',
  driver: nativeChat({
    bind: bindAcme,
    request: acmeRequest,
...
export const createAcme = acmeProfile.create
```

This keeps tests small: bind `acmeProfile.create(scriptedClient)`, run prompts through the public adapter methods, and assert on the scripted client's captured request bodies. Add provider-specific transcript tests for unusual message conversion behavior.
...
import type { NativeTranscriptCodec } from '@crux/core/adapter/profile'
...
Profile hooks should be narrow and typed. Request hooks build SDK-native input; response hooks normalize metadata only; transcript hooks own assistant text/tool calls.

```ts
import type { GenerationSettings } from '@crux/core'
import type { NativeChatRequestArgs, NativeResponseMetadata } from '@crux/core/adapter/profile'
...
Use `sdkLoop()` when the SDK owns the tool loop. Implement typed hooks for the SDK boundary and let Crux steer the policy layer through the executor request:

```ts
import { defineAdapterProfile, sdkLoop } from '@crux/core/adapter/profile'

export const acmeSdkProfile = defineAdapterProfile({
  id: 'acme-sdk',
