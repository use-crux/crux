...
Every prompt has a `.resolve()` method. The standalone `preview()` function
observes whole-request planning without executing any model call.
...
**`preview()`**: measures the complete prospective request and reports whether
it fits:

```ts
import { preview } from "@use-crux/core";

const result = await preview(editDraft, {
  input: { ... },
  model,
  inputBudget: { max: 2000 },
});

result.status       // "fits" | "over-limit" | "unknown"
result.inputTokens  // complete-request estimate, when available
result.adaptations  // redacted selected or prospective representations
result.diagnostics  // content-free reasons and remedies
```
