...
<Callout type="warn">
  Indexer extension authoring is experimental. Extractors and relation specs can
  be loaded from allowlisted packages. Executable third-party lint rules, custom
  parsers, custom resolvers, custom emitters, public queries, and sandboxed
  ...
  extractors: [
    {
      ...
      patterns: [
        {
          kind: "call",
          name: "defineWorkflowTool",
          importFrom: ["@acme/workflow"],
        },
      ],
      extract(ctx) {
        const name =
          ctx.config?.string("id") ??
          ctx.args.string(0) ??
          ctx.source.localName;
...
```ts
patterns: [
  { kind: "call", name: "defineWorkflowTool", importFrom: ["@acme/workflow"] },
];
```
...
return facts({
  definitions: [
    ctx.define.definition({ variableName, id, kind: "tool", name }),
  ],
  references: [
    ctx.ref.variable("@acme/crux-indexer/workflow.uses_tool", "searchTool"),
  ],
});
```
...
export default {
  name: "@acme/crux-indexer",
  version: "1",
  crux: { indexer: "^0.5.0 || ^1.0.0", projectIndexSchema: 1 },
  relations: [
    {
...
describe("@acme/crux-indexer", () => {
  it("extracts workflow tools from authored source", async () => {
    ...
    await expect(
      assertDeterministicExtraction(
        fixture,
        `export const t = defineWorkflowTool({ id: 't' })`,
      ),
