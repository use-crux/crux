...
Quality judges accept text, an `Asset`, or canonical content parts. Binary outputs are not written implicitly to cassettes or output caches, and safe descriptor projections (not media bytes or locators) enter experiment artifacts.
