...
A budget manager tracks token usage across multiple sources (system, history, tools, retrieved context) and reports a pressure level: `normal`, `warning`, or `critical`. It does **no** compaction itself. It tells you _when_ to act, not _how_.

## What problem does this solve?

Different parts of a request consume tokens at different rates: a long-running chat history grows linearly, retrieved RAG context arrives in chunks, tools come and go between turns. To compact intelligently you need to know _what_ is taking up the budget, not just that you're close to the limit.
...
```ts title="budget.ts"
import { createBudgetManager } from "@use-crux/core/compaction";
...
<Cards>
  ...
  <Card
    ...
    description="Stateful rolling compression triggered by your decisions."
    href="/docs/guides/compaction/sliding-window"
  />
  <Card
    title="Token budget"
    description="Drop low-priority contexts under pressure."
