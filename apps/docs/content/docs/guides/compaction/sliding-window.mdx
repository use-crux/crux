...
A sliding window keeps the most recent N messages verbatim and automatically summarizes older messages when the window overflows. The summary lives in a `RecordStore` keyed by the window's id, so the running summary survives across requests.
...
- You need **structured data** out of older messages — use [extract key facts](/docs/guides/compaction/extraction) instead, possibly _before_ sliding-window evicts them
...
const window = createSlidingWindow({
  ...
  storage: myStorage, // optional records + media assets
});
...
<Callout type="idea">
  Use a cheap, fast model for `generate` here — summaries don't need your best
  model. `gpt-4o-mini`, `claude-haiku`, or `gemini-flash` keeps compaction fast
  and inexpensive.
</Callout>
...
2. If `window.length > windowSize`, the oldest messages above the threshold are _evicted_
3. Evicted messages get passed to `summarizeMessages()` along with the existing running summary
4. The merged summary and recent-message window commit together under `compact:{id}:state`
...
Without storage, the running summary is lost on process restart. For a chat app,
pass one `Storage` bundle. Its asset store keeps media usable across reloads:
...
const assets = convexAssetStore({ ctx });
...
The `id` becomes the record key prefix — different threads must use different ids.
...
const window = createSlidingWindow({
  ...
  onCompactEnd: ({ summaryTokens, ratio }) =>
    metrics.gauge("summary.ratio", ratio),
});
```
...
Sliding window calls the LLM _only when the window overflows_. With `windowSize: 20`, summarization runs roughly every Nth turn (depending on message length). Each summarization call replaces the previous summary — there's no log-O(N) overhead.
...
<Cards>
  <Card
    ...
    description="All four primitives at a glance."
    href="/docs/guides/compaction"
  />
  <Card
    title="Budget manager"
    description="Decide when to compact based on token pressure."
    href="/docs/guides/compaction/budget"
  />
  <Card
    title="Extract key facts"
    description="Pull structured data before sliding-window evicts it."
