...
// Then use the registry name as the prefix
...
`config()` registers the registry globally for the runtime. Once registered, any prompt that uses `skill.fromRegistry('internal:...')` can resolve it.
...
| Unknown registry prefix | Prefix not registered via `config({ registries })` |
