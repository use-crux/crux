...
Memory blocks emit `memory:read` and `memory:write` events with `memoryType: 'block'`, `blockId`, `blockKind`, and a privacy-safe `namespaceHash`. Devtools groups them in the Memory Explorer, the CLI includes them in memory stats, and `@use-crux/otel` adds block attributes to memory spans.
...
<Cards>
  <Card title="Working state" description="Typed task state with Zod validation." href="/docs/guides/memory/working" />
  <Card title="Episodes" description="Append-only event memory with dense recall." href="/docs/guides/memory/episodic" />
  <Card title="Facts and procedures" description="Long-term extracted knowledge and operating memory." href="/docs/guides/memory/semantic" />
  <Card title="Storage" description="DataStore persistence, vector recall compatibility, TTL, and adapters." href="/docs/guides/storage/crux-store" />
  <Card title="Embeddings" description="Dense, sparse, and hybrid vector setup." href="/docs/guides/retrieval/embeddings" />
</Cards>
