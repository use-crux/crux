...
On every call, Crux renders each block into prompt context. After the model finishes, the adapter captures the user and assistant turn and forwards each tool call to blocks' `captureToolEvent` hooks, so `episodes()` records tool activity. Streaming works the same way after `stream().completion()`.

`capture.mode` defaults to `afterResponse` and controls when capture work is awaited:

- `inline` awaits capture before `captureTurn()` or adapter generation resolves.
- `afterResponse` starts capture after generation. With `capture.waitUntil`, the work is handed to that hook and the response does not wait. Without `waitUntil`, the adapter awaits capture before resolving, which is the safe serverless default.
- `detached` starts capture in the background and the adapter does not await it. Call `flush()` when you need to await the work.
