...
For production, pass a durable `DataStore`. Convex apps use `cruxConvexStore()`; see the [Convex guide](/docs/guides/convex) for setup and boundary rules.
...
| Convex app state and reactivity       | `cruxConvexStore()` as a `DataStore`; see [Convex](/docs/guides/convex) |
