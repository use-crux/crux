...
For general store APIs, TTL, adapters, subscriptions, and custom database implementations, use the [store reference](/docs/reference/crux-core/store). For vector search, use the [VectorStore guide](/docs/guides/storage/vector-store). For workspace files and generated binary outputs, use `AssetStore`, not memory storage.
