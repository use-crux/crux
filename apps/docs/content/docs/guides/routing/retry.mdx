...
`retry()` never retries `input_limit` failures. If a request is too large for a model, retrying the same model cannot make it fit; use `fallback()` to move to a larger candidate or `cascade()` with `escalateOn: ["input_limit"]` to leave a capacity-limited tier.
