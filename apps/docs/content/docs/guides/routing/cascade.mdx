...
Use `escalateOn: ['input_limit']` when a tier contains a fallback chain whose candidates all fail with `REQUEST_TOO_LARGE`, and the next cascade tier has a larger context window.
