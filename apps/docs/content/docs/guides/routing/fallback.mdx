...
Use fallback for provider outages, rate limits, slow starts, validation exhaustion, input-limit failures, or response-aware rejection.
...
| `schema_incompatible` | local schema/profile incompatibility or an evidenced provider schema rejection |
| `input_limit`         | `REQUEST_TOO_LARGE` request composition failure                                |
...
By default, `fallback()` tries the next candidate for any classified category,
including `schema_incompatible` and `input_limit`. Schema compilation happens
inside each concrete attempt, so a later provider can receive its own wire
schema and decode manifest.
...
Failed attempts record their error category and bounded error text in the same receipt. Run Detail uses those fields to explain why it moved to the next model without exposing unbounded provider payloads.
