...
By default, `fallback()` tries the next candidate for any classified category, including `input_limit`. Use this when a smaller model cannot accept the complete authorized request and a later candidate has more capacity.
