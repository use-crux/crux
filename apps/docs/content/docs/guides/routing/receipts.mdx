...
The same receipt is what devtools renders: each routed resolution attaches it as a `routing.report` observability artifact. See the [Observability reference](/docs/reference/crux-core/observability) for the artifact shape and how devtools presents TTFT, attempt errors, and cascade notes from it.
