...
The Project Index records every authored routing definition with its folded child rows, and stable `id` values join those definitions to runtime spans, receipts, and decision reports: see [Routing definitions and catalog facts](/docs/reference/crux-core/project-index#routing-definitions-and-catalog-facts).
