...
A live MCP task can read or change real systems. For an Eval, bind the same
callable managed production task to a deterministic in-process or spawned MCP
fixture with no network dependency. Use a stable fixture revision so identity
changes when behavior changes. If a dynamic dependency cannot be proven, Crux
executes it normally but does not reuse its task output.
...
observability, Eval identity, and Project Index contracts.
