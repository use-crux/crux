...
Prompt-time tool names must be unique. Crux checks collisions across these prompt-owned sources:

1. **Skill tools** — loader tools from `skill()`
2. **Context tools** — from `context({ tools })` via `use`
...
Call-site tools from `generate(prompt, { tools })` and `stream(prompt, { tools })` are applied later and intentionally override.
...
If `toModelOutput()` is omitted, Crux uses the standard default: strings become `{ type: 'text' }`, everything else becomes JSON. `@use-crux/ai` passes model output through to the AI SDK. Native adapters preserve supported media natively where the provider allows it: Google uses function-response media parts, Anthropic uses native `tool_result` content blocks, and OpenAI Chat Completions receives deterministic text references for non-text parts because that API only accepts text tool results.
