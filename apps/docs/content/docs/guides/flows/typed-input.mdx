...
`flow` infers typed input from the handler's second parameter. This lets you pass structured data into a flow at call time and access it directly in the flow body. The same input is also available on `flow.input` for reusable step helpers that receive the scope.
