...
A handoff validates and transforms data between agents. Use it to connect the output of one flow to the input of another: the handoff sits _between_ the two flows as a validated bridge.
...
const researchFlow = flow("research", async (flow) => {
  const plan = await flow.step("plan", () =>
    generate(researchPlanner, {
      model,
      input: { query: "cloud migration strategies" },
    }),
  );
...
<Callout type="info">
  All agent primitive events, blackboard updates, handoff preparations, delegate
  executions, are automatically correlated to the current flow step in devtools
  via observability context. No extra wiring needed.
</Callout>
