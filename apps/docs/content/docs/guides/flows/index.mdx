...
<Callout type="info">
  **Before reaching for `flow`, check if a composition pattern already covers your use case.**
  [`pipeline()`](/docs/guides/compositions/pipeline) handles sequential agent chains with typed context accumulation,
  [`parallel()`](/docs/guides/compositions/parallel) handles concurrent execution with named results,
  [`consensus()`](/docs/guides/compositions/consensus) handles voting, and
  [`swarm()`](/docs/guides/compositions/swarm) handles dynamic LLM-decided routing. These are simpler, require less
...
- **Quality↔runtime comparison**: flow step labels show up as typed `steps` signals in Quality evaluations, so test assertions and production traces speak the same names
...
```ts
const topicResearch = flow('research', async (flow, input: { query: string }) => {
  const plan = await flow.step('plan', () => generate(planner, { model, input: { query: input.query } }))
  await flow.step('search', () => generate(searcher, { model, input: { query: input.query, plan } }))
})

await withSession(sessionId, () => Promise.all(queries.map((q) => topicResearch.run({ query: q }))))
```
...
See the [Quality guide](/docs/guides/quality) for variants, gates, and replay.
