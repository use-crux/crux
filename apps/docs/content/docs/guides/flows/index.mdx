...
Prefer `pipeline.signal(flowId, name, payload)` on the flow handle. The bare `signalFlow()` export is a low-level primitive that only writes the signal payload — see [Suspend and resume](/docs/guides/flows/suspend-and-resume#signaling-a-suspended-flow) for the full picture.
