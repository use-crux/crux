...
<Callout type="warn">
  This API is experimental. It is production-used by Crux first-party internals
  and supports explicit allowlisted package loading, but it is not a sandboxed
  third-party plugin ecosystem yet.
</Callout>
...
import {
  ...
  none,
  projectDefinition,
} from "@use-crux/indexer/extensions";
```
...
const out = await extractFixtureSource(
  fixture,
  `export const wf = defineWorkflow({ id: 'release' })`,
);
```

`extractFixtureSource(...)` runs the production static extraction engine over in-memory source text with cache disabled. The testing harness uses the in-process TypeScript syntax-record producer by default and reports the producer on `out.trace.syntaxFrontend`. Use it for source-text-to-facts tests instead of constructing parser-native contexts. `assertDeterministicExtraction(...)` double-runs a fixture and throws when the canonical output changes.
...
export default config({
  indexer: {
    extensions: [
      { package: "@acme/crux-indexer", export: "default", version: "^1.0.0" },
    ],
...
export const extractor: IndexExtractor = {
  ...
  extract(ctx) {
    ...
    const name =
      ctx.config.string("name") ?? ctx.args.string(0) ?? ctx.source.localName;
