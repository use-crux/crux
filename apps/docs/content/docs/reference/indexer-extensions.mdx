...
`@crux/indexer/extensions` exposes the authoring primitives for Crux Indexer extensions. Extensions contribute immutable Project Index facts; the compiler owns parsing, relation resolution, validation, cache invalidation, and snapshot or patch emission. Degraded extractor diagnostics and declared source-file dependencies are preserved by the compiler, but runtime construction, rule execution, and parser adapters remain internal.
...
- Guide: [Crux Indexer Extensions](/docs/guides/advanced/indexer-extensions)
- Reference: [@crux/indexer](/docs/reference/indexer)
