...
`@crux/source-indexer/extensions` exposes the authoring primitives for Source Indexer extensions. Extensions contribute immutable Project Catalog facts; the compiler owns parsing, relation resolution, validation, cache invalidation, and snapshot or patch emission. Degraded extractor diagnostics and declared source-file dependencies are preserved by the compiler, but runtime construction, rule execution, and parser adapters remain internal.
...
`extractors` are the only production-executed v1 public slot. `relations` lets an extension declare relation semantics for references it emits. Custom sources, parsers, resolvers, rules, emitters, queries, compiler profiles, compiler intrinsics, and external plugin loading are internal/reserved until the extension system is stabilized.
