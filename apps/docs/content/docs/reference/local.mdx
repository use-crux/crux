...
Experiment records persist to `.crux/quality/experiments/` (gitignored — the runner scaffolds the `.gitignore`; committed baselines and cassettes arrive in later phases). `crux eval` remains as an alias that forwards its arguments to `crux quality run`.
