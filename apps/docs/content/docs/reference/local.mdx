...
plus colocated `prompt({ tests })`), discovered via the `quality:` block of
...
| `--ci` | Force plain, non-animated output and no color, even on a TTY (explicit override for piping to a file while attached to a terminal). Off a TTY this is detected automatically |
