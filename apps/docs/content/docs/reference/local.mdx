...
Selectors match Eval ids or source paths. Every run includes Current; repeat
`--variant` to add blocking Variants and `--case` to select Cases.
...
| `--variant <name>`    | Add a blocking Variant alongside Current; repeatable                |
...
Crux writes private run and evidence data under `.crux/quality/`; keep that
directory gitignored. Commit authored Eval files, sibling `.cases.jsonl`
datasets, explicit files referenced by `caseFile()`, and `.baseline.json`
