...
`boundary.input.media()` visits every non-text part of every user message in
caller order. Its callback receives the original canonical part object plus
`messageIndex` and `partIndex`; both indexes refer to the caller's arrays before
any earlier strip. This happens before invocation validation converts media to
a provider dialect and before the provider request begins.

Media callbacks see the values the caller supplied. Crux does not infer a MIME
type or size when that information is unavailable, so unknown values remain
unknown. In enforce mode, `strip` removes only the current part while retaining
sibling text and media. In report mode, the same result records a would-strip
decision without changing provider input. Removing the final part of a
media-only message blocks the call immediately instead of sending an empty
message.

A media guardrail can target only `boundary.input.media()` and cannot enable
streaming or mix media with text boundaries in one `on` tuple. It supports
`allow`, `warn`, `block`, and `strip`; `warn`, `block`, and `strip` require a
reason. Model-produced media is outside this input boundary; follow
...
hold stream segments. Input media guardrails use `strip` instead of `rewrite`
or `hold`.
...
Creates a callback restricted to `boundary.input.media()`. At least one rule
group is required. Configured rules run in stable `mediaTypes` → `size` →
...
provider/file identifiers. Model-produced media is out of scope; see
