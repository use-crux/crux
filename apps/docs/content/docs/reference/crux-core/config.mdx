---
title: Config
description: Full crux.config.ts reference for project policy, local tooling, runtime hooks, storage, observability, plugins, and experimental options.
---

`config()` is the single public API for Crux project configuration. Use it for policy, local tooling,
runtime hooks, storage, observability, plugins, and explicit experimental opt-ins.
...
Outside index mode, config application is transactional: storage and explicit observability are
...
| `experimental`  | `CruxExperimentalConfig`  | no experimental flags                                    | Evals, `@use-crux/indexer`, Local         |
| `runtime`       | `RuntimeEngineDefinition` | no durable Runtime Engine                                | Runtime Engine APIs, wake handlers, CLI   |
| `storage`       | `Storage`                 | no global store                                          | flows, plans, tasks, Runtime Bridge reads |
...
`experimental.eval.pricing` is a model-keyed record of conservative USD
ceilings for one provider call:
...
Each ceiling must be finite and non-negative. The optional `default` covers
otherwise unmatched model IDs. This is deliberately not input/output token
pricing: `crux eval --max-cost` needs a maximum it can reserve before making a
call. Model lookup follows the existing exact, provider-stripped, then
version-stripped normalization.
...
`experimental.indexer.native` controls semantic enrichment only. When enabled, TypeScript-Go owns
semantic project setup, checker calls, declaration lookup, and AST traversal. Static Index has no
frontend selector: the local runtime always uses its Go-orchestrated Rust/Oxc compiler. Node can
still start for config inspection or trusted TypeScript extension work.
...
| `generation.autoEscape`       | `boolean`          | `true`                                                | Escapes top-level string input fields and selected `escapeFields` before context gates and system/prompt functions receive them. |
...
See [Runtime Engine](/docs/reference/crux-core/runtime-engine) for the API reference and [Runtime guides](/docs/guides/durable-execution) for deployment shapes.

## `storage`

Storage config installs the standard capability bundle. Crux does not infer durability, tenancy, or data
locality from source discovery.

| Option            | Type          | Default     | Notes                                                                      |
| ----------------- | ------------- | ----------- | -------------------------------------------------------------------------- |
| `storage.records` | `RecordStore` | required    | Used by durable flows, plans, tasks, and Runtime Bridge record reads.      |
| `storage.search`  | `SearchStore` | `undefined` | Optional retrieval-search capability.                                      |
| `storage.assets`  | `AssetStore`  | `undefined` | Optional media and oversized-payload capability.                           |
...
The former `persistence` key is rejected with migration guidance. Move its
`records` member directly into `storage`.
...
| `observability.feedbackDestination`           | `CruxFeedbackDestination`    | transport capability                 | Single durable destination used by `feedback()`; overrides a capable transport.                      |
| `observability.redactPaths`                   | `readonly string[]`          | `[]`                                 | Dot paths shared by feedback, Eval evidence, Review, sidecars, and generated Runtime hosts.          |
| `observability.redactPatterns`                | `readonly CruxObservabilityRedactionPattern[]` | `[]`                    | Deployment-wide patterns removed from captured observability payload values.                         |
...
Local's config-aware Project Index publishes only whether
`observability.redactPatterns` is non-empty. Devtools shows that effective
project-level state in Catalog and shows successful application separately in
Runs. It never publishes pattern sources, flags, replacements, rule counts,
or a description of `redactRecord`; a deployed environment may load different
effective config.

## `plugins`

Plugins install hooks in order. Each plugin receives the cumulative hook state from all previous
plugins. Cleanup runs when the returned Crux object is disposed. Cleanup hooks may return a promise;
Crux starts cleanup in reverse install order, and plugin runners that expose async disposal wait for
those promises before reporting shutdown complete.
        ...
        const unsubscribe = subscribeObservability(
          ...
          (record) => {
            if (record.primitive === "tool.call") {
              console.log(record.attributes?.toolName);
            }
          },
        );
