...
| `indexer` | `CruxIndexerConfig` | no third-party extensions | `@crux/indexer` |
| `experimental` | `CruxExperimentalConfig` | no experimental flags | `@crux/indexer`, `@crux/local` |
...
Indexer config is inert data stored by `@crux/core` and enforced by `@crux/indexer`. Loading an
...
Persistence config installs an explicit runtime store. Crux does not infer durability, tenancy, or data
locality from source discovery.

| Option | Type | Default | Notes |
| --- | --- | --- | --- |
| `persistence.store` | `CruxStore` | `undefined` | Required for durable flows, plans, tasks, and Runtime Bridge store reads. |
...
Use `devtools` when application runtime code should send local or tunnel devtools records, or when it
should expose a Runtime Bridge peer.

| Option | Type | Default | Notes |
| --- | --- | --- | --- |
| `devtools.serverUrl` | `string` | `undefined` | Installs the local devtools transport plugin for this process. |
...
Use `observability` for deliberate export or custom transport behavior. For ordinary local devtools,
prefer `devtools.serverUrl` or `withDevtools()`.
...
transport to that server. When `observability.transport` is set, that custom transport wins.
