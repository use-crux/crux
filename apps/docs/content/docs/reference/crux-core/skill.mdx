...
| ------------------------------------ | ------------------ | ---------------------------------------------------------------------------------------------- |
| `skill.fromRegistry(identifier)`     | `string`           | Prefixed registry identifier, such as `skills.sh:owner/repo/skill` or `acme:brand-guidelines`. |
| `skill.fromRegistry(registry, path)` | `Registry, string` | Preferred custom-registry form. Binds the registry by reference at the call site.              |
...
**Throws:** `SkillLoadError` on missing prefix, missing path, network failure, 404, or unknown registry prefix.
...
`registry()` returns a registry value and also registers the custom registry name in the current process for string-prefix compatibility. Keep the exported registry value in normal TypeScript source so local tooling can inspect it; do not repeat registries in `crux.config.ts`.
