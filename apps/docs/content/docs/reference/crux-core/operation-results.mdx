...
A terminal failure reaches every surface, not just `completion`: each live stream
replays its already committed prefix and then errors with the exact same
normalized error object that `completion` rejects with. A caller reading only
`textStream` therefore cannot observe a clean success for an operation a terminal
guard, constraint, or provider failure rejected.
