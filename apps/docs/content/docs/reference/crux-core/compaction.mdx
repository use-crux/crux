...
Use this when a compaction, scoring, retrieval, or quality API needs the `GenerateObjectFn` shape and the call should still run through adapter runtime behavior such as prompt hooks, validation retry, safety, memory, or instrumentation.
