...
<Callout type="info">
  For usage patterns and when to use each primitive, see the [Compaction
  guide](/docs/guides/compaction).
</Callout>
...
| `storage`       | `Storage?`       | Persistent records and media assets |
...
**Returns:** `Promise<CompactionResult>` with `summary`, `tokensBefore`,
`tokensAfter`, `ratio`, and the exact outer `compaction.run` pair in `_meta`.
Direct `summarizeMessages()` owns its summary span.

`compactConversation()` applies the same observed result contract to a
stateless evicted-message batch, including both zero-work paths. Generated work
may call `summarizeMessages()` as a child, but the returned envelope points at
the outer conversation compaction span. `@use-crux/convex` re-exports this Core
implementation under the same name. See
...
Use this when a compaction, scoring, retrieval, or Eval API needs the `GenerateObjectFn` shape and the call should still run through adapter runtime behavior such as prompt hooks, validation retry, safety, memory, or instrumentation.
