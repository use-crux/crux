...
```ts
import {
  createSlidingWindow, createBudgetManager,
  extractKeyFacts, summarizeMessages,
...
```ts
import type {
  GenerateTextFn, GenerateObjectFn, CompactionResult,
  SlidingWindow, SlidingWindowConfig, SlidingWindowStats,
  BudgetManager, BudgetConfig, BudgetState,
} from '@crux/core/compaction'

type GenerateTextFn = (opts: { model: unknown; system?: string; prompt: string }) => Promise<{ text: string }>
type GenerateObjectFn = <T>(opts: { model: unknown; system?: string; prompt: string; schema: ZodType<T> }) => Promise<{ object: T }>
```
