...
Crux emits this finding when a router route, split route, retry target, cascade tier, or fallback option points at a target that is not visible as an index definition.
...
export const modelRouter = router({
  ...
  classify,
  routes: {
    default: { model: strongAgent, maxTokens: 1200 },
  },
});
```
