...
`ErrorCategory` = `'rate_limit' | 'timeout' | 'server_error' | 'connection_error' | 'auth_error' | 'invalid_response' | 'input_limit'`
...
`fallback()` tries the next candidate on `input_limit` by default. `cascade()` only escalates a tier for that category when the tier sets `escalateOn: ["input_limit"]`. `retry()` does not accept or retry `input_limit`, because repeating the same model cannot change its context capacity.
