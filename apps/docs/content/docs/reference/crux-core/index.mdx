...
`@crux/core/routing` exports `router`, `cascade`, `fallback`, the type guards `isRouter`, `isCascade`, `isFallback`, and `resolveModel`. Stable `id` fields on router/cascade/fallback configs are optional for execution but recommended for catalog and trace joins. See the [Routing guide](/docs/guides/advanced/routing) for full coverage.
