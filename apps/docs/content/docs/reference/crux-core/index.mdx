...
<Cards>
  ...
  <Card
    ...
    description="session, getSession, turn handles, inspection, stats, and SessionRuntimeReadModel."
    href="/docs/reference/crux-core/session"
  />
  <Card
    title="GenerationModel"
    description="Adapter-bound models for Agents, Sessions, Runtime programs, and Eval identity."
    href="/docs/reference/crux-core/generation-model"
  />
  <Card
    title="Plan"
...
`ErrorCategory` = `'rate_limit' | 'timeout' | 'server_error' | 'connection_error' | 'auth_error' | 'invalid_response' | 'schema_incompatible' | 'input_limit'`
...
| local schema incompatibility or evidenced provider schema rejection | `schema_incompatible` |
| `REQUEST_TOO_LARGE` request composition                             | `input_limit`         |
| HTTP 400, unknown                                                   | `null` (no fallback)  |

`fallback()` tries the next candidate on `schema_incompatible` and `input_limit`
by default. `cascade()` only escalates a tier for `input_limit` when the tier
sets `escalateOn: ["input_limit"]`. `retry()` does not accept or retry
`input_limit`, because repeating the same model cannot change its context
capacity.
