...
`experimental.indexer.nativeAst` is a separate beta for the static AST/source pass. Set it to `true`
or `{ frontend: 'oxc' }` to let `@crux/local` use the Go-orchestrated Rust/Oxc native static
compiler when the native worker is available. Node may still run for config/static-plan inspection,
TypeScript-authored extensions, or rule compatibility work, and this flag does not enable the native
semantic backend.
