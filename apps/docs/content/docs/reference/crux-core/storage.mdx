...
description: RecordStore, VectorStore, BlobStore, storage(), and in-memory Storage Beta implementations.
---
...
| `BlobStore` | Binary and oversized payload storage with `put`, `get`, `head`, and `delete` |
| `Storage` | A capability bundle: `{ records, vectors?, blobs? }` |

Use this module for new application code. `@use-crux/core/store` is the legacy compatibility module and should not be used for Storage Beta wiring.
...
interface BlobStore {
  ...
  createReadUrl?(uri: string, options?: { expiresInMs?: number }): Promise<string>
  capabilities(): BlobStoreCapabilities
}
```

Blob URIs are internal application references, not automatic public URLs. Signed URLs are app-authorized and are not shown in devtools by default.
...
In-memory stores are process-local and not durable. They are contract-correct reference implementations: records use lazy TTL and scan filtering, vectors pre-filter metadata before scoring, and blobs support `head()` and `delete()`.

## Related

<Cards>
  <Card title="RecordStore guide" description="JSON record storage, TTL, filters, watches, and custom database implementations." href="/docs/guides/storage/crux-store" />
  ...
  <Card title="Postgres RecordStore" description="A complete cookbook implementation for SQL-backed record storage." href="/docs/cookbook/basics/postgres-data-store" />
</Cards>
