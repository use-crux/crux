...
| Interface     | Purpose                                                                                                      |
| ------------- | ------------------------------------------------------------------------------------------------------------ |
| `RecordStore` | JSON records with reads, writes, linearizable single-key mutation, listing, TTL, filters, and watches       |
...
Use `mutateRecord()` for portable atomic read-modify-write behavior; it selects
the adapter's native transaction or bounded versioned-CAS seam:
