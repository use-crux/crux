...
| Export                                                                   | Use                                                                                                                                                                                                           |
| ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `node(options?)`                                                         | In-process local/test runtime. Defaults to an in-memory store and microtask wake. Pass `store: postgres()` for long-lived Node processes that need durable state. Accepts `program` and `retention` (including `effectEnvelopes`). |
| `createRuntimeProgram({ targets, transports, effectTargets? })`          | Pure immutable program: flow/task targets, managed transports, and exact Effect recovery `(id, version)` declarations. No global registry and no stored closures. |
...
| `createRuntimeProgram({ targets, transports })`                          | Validate and freeze the immutable target and managed-transport truth shared by generated and hand-written hosts.                                                                                              |
| `createRuntimeWorker({ runtime, program, pollIntervalMs? })`             | Start one process-local maintenance worker for an immutable program. See the [worker reference](/docs/reference/crux-core/runtime-worker).                                                                    |
...
The [Runtime work records](/docs/reference/crux-core/runtime-work-records)
reference defines every `RuntimeWorkItem` field, every `RuntimeWorkState`, the
kernel/store ownership boundary, and the supported declaration-merging path for
...
Durable Signal adapter authors should follow the exact
[Runtime Signal adapter surface](/docs/reference/crux-core/runtime-adapter),
including its record, codec, atomicity, and certification requirements.
...
- Reference: [Runtime work records](/docs/reference/crux-core/runtime-work-records)
