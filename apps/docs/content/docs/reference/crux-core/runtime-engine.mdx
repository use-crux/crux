...
Use them for store adapters and full runtime adapters. Tests cover namespace isolation, idempotency, leases, waiter races, timers, outbox recovery, retention pruning, retry source guards, and idle-counter invariants.
