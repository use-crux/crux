...
can see `RecordStore`, `VectorStore`, `BlobStore`, `storage()`, and `storage.scope()` wiring. The UI
shows capabilities and warnings without exposing record values, vector contents, blob bodies, or signed
URLs by default.
