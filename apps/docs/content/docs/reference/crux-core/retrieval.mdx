...
The embedding cache is per normalized text, not per batch. Crux builds policy-aware cache keys from the embedding kind, name, dense dimensions, `maxInputTokens`, preprocessing fingerprints, truncation policy, and input hash. Changing any part of the policy creates different keys, so stale vectors are not reused accidentally.
