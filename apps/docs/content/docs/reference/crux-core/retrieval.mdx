...
Core config fields are `id`, optional `source` or `corpus`, storage or explicit stores, dense/sparse embeddings, chunking, metadata schema, lifecycle retention, retrieval search plan, and indexing cache policy.
...
Store-backed retrievers require `id`, `namespace`, record/search storage, and the embeddings needed by the selected retrieval plan. `indexerId` points hydration at a writer id when it differs from the public retriever id.
...
`RetrieveRequest<TFilter>` includes `query`, `limit`, `threshold`, `filter`, `search`, `trace`, and `caller`. The `search` plan composes dense, sparse, and lexical legs with optional per-leg candidate counts and RRF fusion.
...
dense, sparse, lexical, or fused multi-leg search. Media queries run the dense branch only;
...
search. A mismatch throws `EmbeddingSpaceMismatchError` with stored and
...
If Crux modeled hybrid as an embedding kind, the primitive would have to own search orchestration concerns that belong in stores or retrievers. Instead, multi-leg retrieval is a query strategy: one dense embedding plus one sparse embedding, optionally combined with lexical search content through `retriever()` or `SearchStore.search()`.

This split is designed to hold as Crux grows:

- retrieval APIs compose dense and sparse embeddings on top of `SearchStore.search()`
