...
interface TasksHandle<TItems = undefined> {
  ...
  add(input: AddTaskInput): Promise<Task>
  ...
  complete(id: TaskId<TItems>, result?: JsonValue): Promise<Task>
...
Defined ledgers narrow `TaskId<TItems>` to the keys in `items`. Dynamic ledgers accept arbitrary string IDs.
