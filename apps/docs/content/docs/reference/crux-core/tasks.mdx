...
Dynamic `add()` calls require each task ID to be unique within the ledger. Duplicate IDs reject with `DuplicateTaskIdError`; CruxStore adapters perform this insert atomically so concurrent writers cannot both create the same task ID.
