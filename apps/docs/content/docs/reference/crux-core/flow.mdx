...
| `handler` | `(flow: FlowScope<TInput>, input?: TInput) => T \| ...` | Flow function with access to steps, suspension, cancellation, Effect rollback, and optional input data |
...
| `effects`                                | `EffectScopeRef`          | In-process reference to the run's passive Effect rollback boundary      |
| `rollback(options?)`                     | `Promise<RollbackResult>` | Explicitly recover completed units and make the boundary terminal       |
...
| ----------- | ------------------------------------ | ---------------------------------------------------- |
| `completed` | `output: T`, `flowId`, `effects`     | Flow finished normally                               |
| `suspended` | `flowId`, `suspendedAt`, `effects`   | Flow paused at a suspend point                       |
| `cancelled` | `flowId`, `cancelReason?`, `effects` | Flow cancelled via `flow.cancel()` or `cancelFlow()` |
| `expired`   | `flowId`, `suspendedAt`, `effects`   | Suspended flow timed out before a signal arrived     |
...
The `effects` ref is JSON-safe but resolves only while the current process
retains the in-memory receipt ledger and recovery handlers. A non-completed
flow does not roll back automatically; recovery always requires
`flow.rollback()` or `rollback(result.effects)`.
...
| `effects`        | `EffectScopeRef?`                        | In-process rollback boundary retained for resume |
