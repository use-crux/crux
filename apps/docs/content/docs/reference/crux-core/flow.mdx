...
**Returns:** `FlowHandle<T, TInput>` -- frozen handle with `.run()`, `.resume()`, `.signal()`, and `.name`

```ts
import { flow } from '@use-crux/core/flow'
import { generate } from '@use-crux/ai'

const researchFlow = flow('research', async (flow, input: { topic: string }) => {
  const plan = await flow.step('plan', () => generate(planner, { model, input: { topic: input.topic } }))
...
Buffer an executable runtime task to run independently. The task becomes durable at the next suspension or completion barrier, so replay cannot enqueue it twice.

```ts
const { workId } = await flow.defer(embedDocument, { documentId })
```

The target must be a runtime `task()` from `@use-crux/core/runtime`, not a plan-ledger task.
...
<Callout type="warn">
  **Low-level primitive.** `signalFlow()` only writes the signal to the RecordStore -- it does NOT trigger the flow to
  ...
  For flows with local signal maps, prefer `handle.signal()` so declared schemas validate before persistence.
</Callout>
...
const approvalFlow = flow('content-approval', async (flow, input: { topic: string }) => {
...
import { FlowSuspendedError, FlowCancelledError, FlowExpiredError } from '@use-crux/core/flow'
```
