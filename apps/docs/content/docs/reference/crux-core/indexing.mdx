...
- `RecordStore` and `SearchStore` writes
  ...
  documents: [
    transform.document({
      ...
      run(document) {
        return {
          ...document,
          content: document.content.replace(/<!-- draft -->/g, ""),
        };
      },
    }),
  ],
...
Set `cache: true` on the indexer to cache expensive document transforms,
chunkers, chunk transforms, and final dense/sparse source bundles in the same
record store. Per-call options can override behavior:
...
Preparation-stage keys include source and stage identity. Embedding-stage keys
include source id, vector kind, embedding fingerprint, and ordered chunk-content
hashes. Cache entries are reused only after strict count and vector-shape
validation. Cached embeddings produce `SourceStageRecord` entries with
`kind: "embedding"`, `embeddingKind`, and the cache outcome.
...
Successful writes return `IndexResult` with the exact `indexing.pipeline`
operation pair in `_meta`. A dry run returns `IndexDryRunResult` with the same
ownership rule for its own pipeline invocation.
...
Dry runs prepare chunks and embeddings but do not delete or write indexed chunks
or corpus source records. With indexer caching enabled, they may read and write
stage-cache entries so a later real run can reuse the completed work. They return
chunk and embedding counts so callers can preview work before committing it.

Dry runs also return prepared parent records and pipeline stages when the configured pipeline produces them.

Chunk arrays, stored source records, progress items, and numeric
`deleteSource()`/`clear()` results remain ordinary domain values without result
metadata. This content-indexing contract does not change Project Index compiler
facts, snapshots, HTTP read models, or caches. See
[Operation Result Correlation](/docs/reference/crux-core/operation-results#compatibility-matrix).
...
The pair identifies the outer `corpus.sync` operation; per-source progress and
ledger records remain unobserved domain data.
...
Dense indexing accepts text and any media modality declared by the embedding.
`CruxDocument.asset` is single-media shorthand; explicit `kind: "media"` parts
carry an asset, optional caption, inferred or explicit modality, and page/time
source location. Each media part produces one chunk and one dense vector.
Sparse embedding skips media chunks.

When storage includes an `AssetStore`, media is materialized before provider
execution and persisted records retain only its `AssetRef`, MIME type, source
location, and allowlisted scalar attribution. Bytes and provider locators are
never written to record/vector metadata or pipeline caches. Dry runs perform no
asset, cache, record, vector, or namespace-space writes.

Before any vector mutation, indexing compares the configured dense space with
the namespace record. `EmbeddingSpaceMismatchError` requires `clear()` plus a
full reindex, or a new namespace. `deleteSource()` preserves the space record.
...
With `cache: true`, Crux stores one ordered vector bundle per source and kind.
Dense runs complete before sparse runs. Hits scatter into the current invocation
order; misses across sources flatten into one `embedMany()` call for that kind.
Embeddings without `fingerprint` are always recomputed and never receive a stage
cache outcome.
