...
const brandVoiceJudge = judge({ id: 'brand-voice', criteria, scale, generate, model })
...
For eval suites, `scorers.judge()` in
[`@use-crux/core/quality`](/docs/reference/crux-core/quality) reuses the same
