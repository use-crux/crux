...
**Returns:** `Constraint` — pass it anywhere constraints are accepted (per-call, per-prompt, or `createSafetyPlugin()`).

Check `metadata.judge` on audit entries carries `{ metricId, score, min, reasoning, detail? }`.
...
import type {
  ...
  JudgeScoreOptions, JudgeFewShot, JudgeConstraintOptions,
