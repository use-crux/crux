...
The session owns: tool merge precedence (call tools shadow prompt tools), middleware chain order (prompt middleware before call middleware), the full approval protocol (deterministic `approval_<toolCallId>` ids, anti-forgery tokens, decision matching, idempotent resume replay), per-call instrumentation (spans, artifacts, `onToolStart`/`onToolEnd`/`onToolApprovalRequest` hooks), output normalization (`toModelOutput`, default shaping, rendering), the `LoadSkill` side effect, and memory-capture fan-out. Both regimes drive the same private gate → execute → settle verdict kernel, so dialect parity is structural — the cross-dialect parity suite verifies it mechanically.
