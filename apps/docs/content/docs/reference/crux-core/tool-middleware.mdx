...
Tool middleware wraps tools before an adapter executes them. Use it for behavior that should apply across many tools: audit logging, timing, validation, matcher-based approval callbacks, and consistent error handling. Tool definitions themselves stay policy-free; explicit approval policy belongs on `context({ toolApproval })`, `prompt({ toolApproval })`, or call-site `generate()` / `stream()` options.
...
When a tool declares `contextSchema`, middleware receives the parsed value on
`call.context` and `call.options.context`. Every hook also receives
`call.runtimeContext`, the shared per-run value supplied by `generate()` /
`stream()`.
...
`approvalMiddleware()` is a convenience wrapper over `toolMiddleware()` for human-in-the-loop approval. It gates matched tool calls and wires approval responses back to callbacks on the next request. For explicit policy that should be inspectable by tool name, prefer `toolApproval`; middleware is useful for broad matcher-based gates and audit callbacks.
...
Apps never call this — both built-in dialect factories (`adapter()` for core-driven loops, `loopRuntimeAdapter()` for SDK-driven loops) construct one session per `generate()`/`stream()` call. You only need it when building a custom adapter outside the factories.
...
const outcome = await sdk.runTextLoop({ tools: lifecycle.tools /* … */ })
