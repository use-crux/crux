...
a new trace format. It summarises and links into the existing detail views rather
than duplicating their payloads.

The local Go projection attaches one report to each generation node and folded
generation detail, plus the run root, exposed as the optional `decisionReport`
field on `CruxRunDetailNode` and `CruxRunDetailDetail`. Consumers read it directly;
when it is absent the existing detail tabs are unaffected.

| Field        | Purpose                                                                                          |
| ------------ | ------------------------------------------------------------------------------------------------ |
| `turn`       | Turn metadata and a short, evidence-bound `verdict` sentence.                                     |
...
| `cache`      | Reuse/write/miss behaviour, with `acceptedByFreshness` / `rejectedByFreshness` as the only join.  |
| `decisions`  | Routing, fallback, guardrail, security, constraint, cache, and compaction outcomes + deep-tab targets. |
| `source`     | Grouped source-definition joins ("what do I change?") with file:line and resolution fidelity.     |
| `coverage`   | Quality coverage scorecard across six areas, with optional read-only suggested assertions.        |
...
`evidenceLevel: "missing"`. Reason codes are stable strings (e.g.
