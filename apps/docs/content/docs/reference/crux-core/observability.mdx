...
Generation and streaming orchestration record operation deadlines when a timeout is configured. `@use-crux/ai` passes `timeoutMs` into core orchestration, which records `timeoutMs` / `deadlineAt` and emits `operation.deadline` before the provider call begins.
...
await observe.run({ name: 'support reply', rootPrimitive: 'agent.run' }, async () => {
  await observe.span({ name: 'retrieve docs', primitive: 'retrieval.query' }, async () => {
...
queueMicrotask(() => {
  void observe.withContext(context, async () => {
    await observe.span({ name: 'delegate', primitive: 'delegate.invoke' }, async () => {
      // delegated work
    })
  })
})
...
When disabled, input-family artifacts (`messages`, `input`, `system`, `prompt`, `tool.args`, `tool.request`) and output-family artifacts (`output`, `stream.timeline`, `tool.result`) are still emitted, but as `encoding: 'reference'` with `sizeBytes` and `hash` instead of `preview`.
