...
The Project Index answers *what is authored*; the observability graph answers *what ran*. Catalog
...
| Quality-primary | `quality-owned` (and dual-tagged cases) | Correlates through the Quality ↔ observability join; secondary direct runtime (e.g. live `scoring.judge` for `scorer`) still surfaces when present. |
| No runtime | `static-only` / fallback, or other kinds with no runtime path | *Static/declarative: never the target or subject of a runtime primitive.* No fabricated counts, no dead View Runs link, no arbitrary owner selection. |
...
Quality edges may point at runs recorded in another local database or an older baseline file; the server preserves those external references and does not treat absent target nodes as read-model errors. Cached or replayed cells may link `eval.case_of` to a historical case run rather than a run created during the current evaluation; consumers should treat the edge as provenance, not same-run membership.
