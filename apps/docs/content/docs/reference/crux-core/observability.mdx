...
interface DefinitionRef {
  ...
  role: DefinitionRefRole; // e.g. resolved-prompt, invoked-agent, invoked-media-operation
...
| Eval-primary               | `eval-owned`                                                        | Correlates through the Eval ↔ observability join; secondary direct runtime evidence still surfaces when present.                                                                                                                                   |
...
- Deferred work, media operations, and ingest sources emit real runtime
  primitives, but their normal records do not observe the compiler-local
  authored id; Catalog labels them runtime-observed but unjoined. A caller that
  genuinely holds an exact authored media identity may attach
  a DefinitionRef with `kind: "media.operation"` and
  `role: "invoked-media-operation"` for Run Detail navigation, but Crux never
  synthesizes it from operation/provider/model/name. A missing or different
  role remains unavailable; conflicting valid refs remain ambiguous.
...
`RunDetail.currentProjectHealth` is a separate optional read-time projection.
Crux Local compares the run's primary, related, affected, and propagated
definition refs with lint findings in one current Project Index snapshot. Each
projected finding preserves its id, rule, severity, title/message, finding
source, matched definition ids/kinds/roles, relationship kinds, and strict
active/suppressed state. Suppressed findings retain directive source, scope,
and optional reason. The projection is omitted when no materialized Index is
available; a present Index with no matches returns an empty findings list.

This field is current authored context, not a runtime record or historical
claim. It does not add a span, alter stored observability evidence, or affect
the run status. Consumers should label it **Current project health** or
**Current Index context** and keep active and suppressed counts separate.

## Bounded media stream records

`streamImage()` and `streamSpeech()` reuse `media.generate_image` and
`media.generate_speech`. The `operation` attribute distinguishes the bounded
stream from the completed call. `streamingRole` distinguishes ownership: one
`streamingRole: "logical"` span owns the
public operation and same-primitive `streamingRole: "attempt"` children own
physical provider attempts. Read models must not render the logical root as an
attempt.

| Attribute        | Logical | Attempt | Meaning                                      |
| ---------------- | ------- | ------- | -------------------------------------------- |
| `operation`      | yes     | yes     | `streamImage` or `streamSpeech`              |
| `route`          | optional| —       | Selected routing expression/route            |
| `attempt`        | —       | yes     | One-based physical attempt number            |
| `attemptCount`   | yes     | —       | Total physical attempts                      |
| `committed`      | yes     | yes     | Whether public provider media committed route|
| `previewCount`   | yes     | yes     | Published/observed complete previews         |
| `deltaCount`     | yes     | yes     | Progressive byte-event count                 |
| `finalCount`     | yes     | yes     | Final asset-event count                      |
| `byteCount`      | yes     | yes     | Aggregate media bytes                        |
| `mediaTypes`     | yes     | yes     | Declared MIME facts                          |
| `firstEventMs`   | optional| optional| Time to first provider candidate/public event|
| `durationMs`     | yes     | yes     | Logical or attempt duration                  |
| `terminal`       | yes     | yes     | `ok`, `error`, `cancelled`, or `timeout`     |

The logical report can additionally carry calls and safe final media-report
counts. Output-media `guardrail.report` artifacts use operation origins with
`operationPhase: "preview" | "final"`, `outputIndex`, and preview `sequence`
where applicable. They never contain the image/audio bytes.

All consumers receive only sanitized scalar facts. Prompts, bytes, base64,
URLs, filenames, hashes, refs, native events, provider envelopes, and held
media do not become span attributes or the bounded-stream Runs presentation.
Local and `@use-crux/otel` consume this same sanitized record stream; OTel keeps
the existing media primitive mapping and exports safe `crux.*` attributes.
...
Model-ingress guardrail records identify the semantic target and privacy-safe
origin only: source `user`, `tool`, or `retrieval`, plus stable coordinates or a
safe tool/retriever identifier when available. They do not contain original or
rewritten text, raw tool values, retrieval hits/queries, media sources, provider
objects, or the private retrieval-fold carrier.
...
`evidence.record()` projects qualified execution evidence into schema v5 using
the canonical `evidence` family and `evidence.record` primitive. Inline
authoring prepares one artifact followed by one `evidence.for` edge; linking an
existing source emits only the edge.

The edge carries a strict allowlist: evidence ID, role, kind, normalized
conclusion, safe timestamps, superseded evidence IDs, protected run-or-span
producer, source mode, and capture state. Idempotent relationships also carry
an all-or-none hash/digest group that Local validates without exposing it.
Inline data and raw idempotency keys never appear in edge attributes. Artifact
and edge privacy are prepared as one batch, so rejecting either half suppresses
both rather than publishing an orphan or tombstone.

The graph contract makes evidence transportable, but does not itself provide a
durable query implementation. Crux Local supplies the restart-safe canonical
read model consumed by Core inspection, Devtools, and agent-facing Local
queries. See [Execution evidence](/docs/reference/crux-core/evidence) for the
API and the [end-to-end guide](/docs/guides/observability/execution-evidence)
for automatic producers, retention, missing states, and conflict behavior.
...
and a `guardrail.action` span event with `action: 'strip'`. Report mode preserves
the canonical input or result; it changes mutation behavior, not whether the
action is observed. `guardrail.stripped` is only the Turn Decision Report reason
code. When an enforced strip violates a required group—an empty media-only
message, final generated image, required speech/transcription audio, or retained
image-edit mask without a reference—the strip report/action are followed by a
`guardrail.blocked` edge and the audit entry records escalation. Transcript
rewrites record when timed segments/words were removed. Media records expose
only the boundary, mode, action, primitive/phase/field, canonical part type,
stable original indexes, safe model id when known, and escalation/detail-loss
flags. Sources, bytes, URLs, provider file ids, filenames, raw responses, and
provider options are never included.
...
`cache.freshness.rejected`) suitable for Eval assertions, while human-facing
...
Successful public operation envelopes use `{ traceId, spanId }` to point at
their exact producing span. They do not include `runId` or `segmentId`, and a
provider `_meta.responseId` remains a separate identifier. See
[Operation Result Correlation](/docs/reference/crux-core/operation-results) for
the result, streaming, Eval evidence, and adapter-author contracts.
...
| Cloudflare Workers          | Pass the Worker's `ExecutionContext` once.                                              | `withCrux()` (`@use-crux/cloudflare`) binds defer and drain to `ctx.waitUntil()`, no `nodejs_compat` required. |
...
`@use-crux/cloudflare`) registers the final drain with
...
`ctx` on its own. `@use-crux/cloudflare` needs no `nodejs_compat` compatibility
flag; `WorkersExecutionContext` is a structural type satisfied by anything
exposing `waitUntil(promise)`. The provider-neutral low-level alternative is
`withWaitUntilDefer()` from `@use-crux/core/defer/serverless`.
...
Trace batches fan out to every leg. Feedback is a durable mutation, so the tee
delegates it to exactly one feedback-capable leg. Construction fails clearly if
multiple legs expose `submitFeedback()` instead of guessing which destination
owns the Review record.
...
`redactPatterns` rewrites string values in artifact previews and URIs, nested
record attributes, and run/span error messages. Bare expressions replace every
non-empty match with `[REDACTED]`. Object-form replacements are literal:
JavaScript replacement tokens such as `$&`, `$1`, `$<name>`, and `$$` are not
expanded. Rules run in declaration order.
...
`tool.result`. `full` retains arbitrary previews. `safe` also retains the
preview and is a producer-side assertion that its artifact contract is already
safe by default; the capture layer does not classify or sanitize arbitrary
text. Use `evidence` or `off` for user-authored payloads that are not safe to
retain.
...
The privacy gate applies patterns before capture derives evidence hashes, sizes,
or reference previews. The existing `redactRecord` hook then receives the
post-capture, pre-sanitization record. After JSON-safe sanitization, Crux runs
the patterns again before validation and final fan-out, covering strings
created or changed by sanitization and the custom hook.

When declarative patterns change a record, the validated graph record includes
privacy-safe `privacy.redaction` evidence with `applied: true` and a
canonical, non-empty union of `artifact.preview`, `artifact.uri`,
`attributes`, and `error.message`. It includes no rules, matches, values,
replacements, nested paths, hashes, or counts. Custom `redactRecord` changes
alone do not claim this evidence. Local projects the same evidence into Run
Detail; the OTel mapper does not add a new semantic-convention attribute for
it.

When an ordinary generation resolves a `PromptText` user prompt, its existing
`messages` input preview can include `userPrompt`: exact resolved text, ordered
authored/dynamic segments, and static/dynamic/total token attribution.
Concatenating the retained segment text must reproduce the exact resolved
prompt. String prompts do not receive invented provenance. The record passes
through the same input capture level, size limits, JSON-safe normalization,
pattern redaction, and last-mile `redactRecord` hook as the rest of the
messages preview; disabling or redacting input capture does not leave a
separate PromptText copy.

The emit path also strips payload-shaped attributes, converts JSON-hostile values, and drops invalid optional metrics before fan-out, so subscribers, the diagnostics channel, plugins, and transports all consume the same sanitized JSON-safe record. A failure inside pattern redaction or a configured `redactRecord` hook drops the record (fail-closed) instead of emitting it unredacted or failing application code. The shared helper `toSafeJsonValue()` truncates long strings (default 8000 characters) and redacts sensitive object keys.

Pattern redaction intentionally excludes structural graph IDs, names,
definition and source references, deployment identity, relation endpoints, and
all object keys. Do not place raw personal data in structural IDs or keys. Use
`redactRecord` when an application deliberately needs to rewrite or drop one
of those fields. Patterns are trusted application configuration; avoid
expressions susceptible to catastrophic backtracking.
