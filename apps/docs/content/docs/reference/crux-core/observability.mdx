...
| --- | --- |
| `context.contribution` | Context inclusion state, source, priority, tokens, cache status, and exclusion reason. |
...
| `citation.report` / `score.report` | Grounding markers, score verdicts, judge rationales, and pass/fail detail. |
...
| `routing.report` / `cache.report` | Router/cascade/fallback choices, tier verdicts, semantic cache hits/writes, and saved work. |
| `compaction.report` / `memory.snapshot` | Before/after token counts, summary previews, memory type, block kind, operation, mode, and status. |
...
- `prompt.resolve()` opens `prompt.resolve`, records conditional context inclusion/exclusion as `context.predicate`, and records resolved context text as `context.resolve` spans with `context.contribution` artifacts. Generation spans that consume the resolved prompt link back to those included context artifacts, so `inspection.context` can show the exact context used for a call. Token-budget decisions emit `prompt.budget` artifacts with dropped contribution previews.
