...
| [`skill()`](/docs/reference/crux-core/skill) | A skill catalog plus `LoadSkill` and `LoadReference` tools. Loaded skill instructions are re-injected at system-prompt level. |
...
When observability is enabled, prompt resolution emits composition artifacts for devtools. Resolved contexts use `context` artifacts with a `context.contribution` preview (`active`, `checked-not-included`, or `dropped-budget` state plus source, tokens, priority, cache status, and reason/branch when relevant). Calls with `tokenBudget` also emit a `prompt` artifact with a `prompt.budget` preview showing `usedTokens`, `totalTokens`, and dropped contributions.
