...
Entries created by `contributor()` are structurally backward-compatible with `InjectableEntry`.
...
const reply = prompt({
  ...
  use: [
    locale,
    when(({ brandVoice }) => Boolean(brandVoice), brand),
  ],
...
## `createPromptResolver(ports?)`

Bind the resolution pipeline to explicit ports instead of process globals. Returns `{ resolvePrompt, inspectArgs }` with behavior identical to `.resolve()` / `.inspect()` — only where ambient effects land changes.

```ts
import { createPromptResolver } from '@crux/core'
import { recordingObservability, fixedClock, inMemoryContextCache } from '@crux/core/testing'
...
const resolved = await resolver.resolvePrompt(config, { input }, schema)
observability.contributionPreviews('checked-not-included') // exclusions, no transport needed
```

Ports: `observability`, `skills`, `cache`, `clock`, `policy`, `diagnostics`, `instrumentation`. Omitted ports use the production runtime adapters, so `createPromptResolver()` with no arguments is exactly the default pipeline. In-memory fakes for every port ship from [`@crux/core/testing`](/docs/reference/crux-core/testing#prompt-resolver-fakes).

For adapter and primitive authors, the lowered contributor contract behind the pipeline is also exported: `lowerEntry(entry, index)` (the only union-aware code), `resolveUse(entries, input, promptId, ports)` (the driver), `collectSchemaContributions(entries)` (definition-time schema collection), and the contract types (`LoweredContributor`, `Contribution`, `GateResult`, `MergedResolution`, …). Application code never needs these.
...
- Reference: [Testing](/docs/reference/crux-core/testing)
