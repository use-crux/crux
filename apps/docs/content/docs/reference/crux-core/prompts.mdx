---
title: Prompts
description: prompt(), createPrompts(), Prompt, and prompt inspection.
---
...
| `.inspect(options)` | Return composition, source, and token-budget details without calling a model. |
...
`.inspect()` runs the same resolution path and adds debugging metadata:
...
Use it to understand what was included, excluded, cached, or dropped.
