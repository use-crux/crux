...
```ts
import { compilePrompt, recordingObservability, fixedClock, inMemoryContextCache } from '@use-crux/core'
...
`PromptResolution` is exported from `@use-crux/core`. The older `Resolution` type name remains as a deprecated compatibility alias.
