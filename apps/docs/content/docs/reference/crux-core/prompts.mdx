...
`md` builds an opaque, immutable `PromptText` value with native tagged-template syntax. The name describes the Markdown-oriented authoring convention; Crux does not parse, validate, or render Markdown. Resolution lowers the value to provider-neutral plain text before adapters, caches, and RPC boundaries.
...
the content trusted, sanitized, or safe.
...
V1 does not include Markdown parsing/rendering, a prompt DSL or compiler transform, standalone prompt files, asynchronous interpolation, message/multimodal construction, editor runtime behavior, semantic section tags, `md.use`, or context-placement placeholders. Context authority and placement remain declared by `use: []`.
