...
Use `RecordStore` for memory records, cache entries, flow snapshots, plans, tasks, blackboards, workspace metadata, and other structured Crux state. Use [`VectorStore`](/docs/guides/storage/vector-store) for similarity search and [`AssetStore`](/docs/guides/storage/asset-storage) for binary or oversized payloads.
