...
Use [`@crux/core/storage`](/docs/reference/crux-core/storage) for new application code. It exposes the canonical `DataStore`, `VectorStore`, and `BlobStore` interfaces.
...
| `defineConvexStoreContract().store(ctx)` | `@crux/convex`     | Production with Convex backend      |
