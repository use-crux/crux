...
Use `RecordStore` for memory records, cache entries, flow snapshots, plans, tasks, blackboards, workspace metadata, and other structured Crux state. Use [`VectorStore`](/docs/guides/storage/vector-store) for similarity search and [`BlobStore`](/docs/guides/storage/blob-storage) for binary or oversized payloads.
...
interface RecordStore<T extends JsonObject = JsonObject> {
  ...
  putMany?(entries: readonly { key: string; value: T; options?: { ttlMs?: number } }[]): Promise<void>
...
Bundle records with vectors and blobs through `storage({ records, vectors, blobs })` when a primitive needs multiple capabilities.
