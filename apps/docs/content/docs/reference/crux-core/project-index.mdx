...
`prompts`, `contexts`, and `tools` are runtime snapshot summaries. The canonical authored read model is `definitions` plus `relations`, `diagnostics`, `lintFindings`, `ruleCatalog`, and `sources`; web devtools and the TUI should render authored primitives from `definitions`.
