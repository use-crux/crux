...
Runtime snapshots emitted by `withDevtools()` can enrich prompt/context/tool metadata, but source files and `.crux/quality` records are authoritative for authored definitions. See [@crux/indexer](/docs/reference/indexer) for the indexing package and [@crux/local](/docs/reference/local) for CLI/API access.
