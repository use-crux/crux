...
diagnostic ids. Definition and relation ids are branded separately. The brands disappear in JSON, but
TypeScript callers must construct ids at a boundary with `createProjectModelDefinitionId()`,
`createProjectModelRelationId()`, or `createProjectModelDiagnosticId()` instead of passing arbitrary
strings through the public contract.
