...
| `@use-crux/local`   | Persists and serves the index read model, enriches it with quality state, and broadcasts updates to UIs. |

Runtime snapshots emitted by `withDevtools()` can enrich prompt/context/tool metadata, but source files and `.crux/quality` records are authoritative for authored definitions. See [@use-crux/indexer](/docs/reference/indexer) for the indexing package and [@use-crux/local](/docs/reference/local) for CLI/API access.
