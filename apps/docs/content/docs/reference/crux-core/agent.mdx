...
```ts
import { agent, isAgent, createCompositions } from "@use-crux/core/agent";

// Pre-bound versions from adapters:
import { parallel, pipeline, consensus, agent } from "@use-crux/ai";
```
...
### Agent entries in `tools`

An Agent's `tools` map may contain direct child Agent values alongside ordinary
Tools:
...
`research` is the exported model-tool name. `researchAgent.id` remains the
child's identity and Work target; it does not rename the Tool. Crux uses the
child Agent's `description`, falling back to its Prompt's `description`. If
neither exists, preparation throws
`Foreground Agent tool "research" requires an Agent or Prompt description.`
before provider dispatch or child Work acceptance. Crux does not synthesize a
description.

| Child Prompt input | Generated Tool parameters | Input passed to the child |
| ------------------ | ------------------------- | ------------------------- |
| Object schema | The object schema unchanged | The validated object unchanged |
| No input schema | Empty object schema | `{}` |
| Non-object or mixed-root schema | `{ input: childSchema }` | The validated `input` value |

There is no text-input shortcut. Tool arguments are validated before Work
acceptance, so invalid input creates no child Work. Direct Agent entries are
bound after authored and call-level tools are merged and before ordinary Tool
preparation. The winning map entry follows the same middleware, approval, and
guardrail path as any other Tool, under its map-key name; no separate collision
or approval behavior is introduced.

Execution accepts exactly one foreground child Work and awaits it. The Tool
result is the child's exact validated structured or multimodal `output`, not an
`AgentResult`, summary, transcript, or execution handle. The child runs with
only its own Agent definition—its prompt, `use`, and tools—and receives only
its declared input. Parent-only and sibling tools, parent history and request
details, and parent runtime or control handles are not inherited.

An ambient Work attachment links the child to its execution ancestry. Without
one, the child receives cancellation linkage only. In either case, that linkage
does not grant authority over the parent. Parent cancellation propagates
through the child Work boundary; child failure, cancellation, or timeout rejects
the ordinary Tool execution and follows the normal Tool-loop error path.

For child Agents that can continue independently, see
[Backgroundable Agent Work](/docs/reference/crux-core/backgroundable-agent-work).
...
| `usage`      | `TokenUsage \| undefined` | Token usage if available                         |
| `threadCommit` | `ThreadCommit \| undefined` | Atomic Thread publication receipt, when the Agent Prompt uses a Thread |
| `_meta`      | `OperationResultMeta`      | Exact `agent.run` trace and span owned by Core   |
...
```ts
type AgentExecutor = (agent: Agent, options: ExecuteOptions) => Promise<AgentResultPayload>
```

Executors are adapter/application ports and return ID-free payload facts. Core
finalizes the public `AgentResult` at `agent.run`. Composition parent results
point at their own `composition.*` spans while nested agent results retain
their child spans. See [Operation Result Correlation](/docs/reference/crux-core/operation-results#media-and-higher-level-results).
...
A conformant in-memory `AgentExecutor` for **testing** how compositions drive the executor without an SDK (the agent-layer analogue of the resolver fakes). Exported from `@use-crux/core/agent` and the package root.

```ts
import { createFakeAgentExecutor, createPipeline } from "@use-crux/core/agent";
...
| `store`  | `RecordStore?` | Persistent storage      |
...
```ts
const research = blackboard({
  id: "research",
  schema,
  tools: { prefix: "research" },
});
const writing = blackboard({
  id: "writing",
  schema,
  tools: { prefix: "writing" },
});
...
const tools = {
  getSharedState: {
    ...readBlackboard,
    description: "Read the current analysis results from the shared workspace.",
  },
  updateSharedState: {
    ...writeBlackboard,
    description: "Write your analysis results to the shared workspace.",
  },
};
```
...
| `store`        | `RecordStore?`                  | Persistent storage, enables `send()` and `receive()` for distributed agents |
...
prompt({
  tools: {
    requestResearch: {
      ...delegate,
      description:
        "Assign a research task to the research team. Provide the topic to investigate.",
    },
  },
});
```
...
**Three-layer validation.** Each delegation validates data at three points:
...
Events are correlated to the enclosing trace when emitted within a `generate()` or agent step. For handoffs, canonical traces also attach the validated handoff input to `inspection.input` and the prepared handoff payload to `inspection.output`.
