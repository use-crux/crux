...
`CellAssertionStatus`, `CellAssertionValue`, `RunOverrides`,
...
the failed-outcome compatibility projection for older consumers. Scorers
still run on expect-failed cells.
