...
`EvaluationManifest`, `QualityConfig`, `QualitySetupResult`, and the
...
→ target defaults → project config `quality.setup()`. Prefer eval-local
imports for model choices; `quality.setup()` remains the project-wide
compatibility/default hook. A model-backed task with no generate available is
a definition error (exit 2).
...
Model-backed (judge/rag scorers use `quality.setup()` providers; pass
`model` to override the judge model):
...
scorers.embeddingSimilarity() // cosine vs expected; pass embed or setup().embed
