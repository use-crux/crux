...
the failed-outcome compatibility projection for older consumers. Outcomes may
also include `sourceFrame`: either a narrow authored-source snapshot with line
roles and `contentHash`, or `{ kind: 'unavailable', reason }` when the runner
cannot prove authored source. Outcomes from post-score `assert` callbacks use
...
The local backend derives two server-owned read models from experiment,
baseline, source, and observability records:
...
- `QualityCellEvidence` for one experiment cell (`caseId`, `variantName`,
  ...
  hotspots, repro command, and provenance.
