---
title: Quality
description: '@use-crux/core/quality, evaluate(), target, scorers, dataset(), cassette(), UncapturedSignalError, and the Experiment/Manifest/Baseline records.'
---

`@use-crux/core/quality` is the Quality system's public surface: five
authoring values, the `UncapturedSignalError` runtime error class, and types.
Authoring lives in `*.eval.ts` files (or colocated
...
```ts
import { evaluate, target, scorers, dataset, cassette, UncapturedSignalError } from '@use-crux/core/quality'
```
...
`Experiment`, `ExperimentCell`, `CellAssertionOutcome`,
`CellAssertionPhase`, `CellAssertionStatus`, `CellAssertionValue`,
...
inference utilities `InputOf`, `OutputOf`, `ParamsOf`, `CapsOf`, and `CaseOf`.

Machine-readable contracts live in the separate
`@use-crux/core/quality/schemas` subpath:
...
import {
  ...
  judgeReportSchema,
  toJsonSchema,
} from '@use-crux/core/quality/schemas'

const schema = toJsonSchema('experiment')
const diffSchema = toJsonSchema('diff')
```

The subpath also exports the `ExperimentRecord`, `FailureArtifact`,
`ExperimentDiff`, `RunSummary`, and `JudgeReport` types. It is the supported
validation surface for agents and custom dashboards that consume persisted
records or CLI JSON.
...
| `afterScores`               | Evaluation-level post-score assertion callback over [`AssertContext`](#assertcontext), runs after scorers and can assert typed score values                                              |
...
Timeouts are cooperative. Crux records a timeout cell and quarantines later
trace, cassette, and score writes from that cell's async context; it does not
forcibly kill arbitrary JavaScript user code.

Inference contract: `data`, `scorers`, and `expect` are `NoInfer` positions, so
...
```ts
{
  ...
  afterScores?: (ctx) => void    // case-SPECIFIC post-score assertions
  trials?: number                // wins over evaluation-level trials
  tags?: string[]
  metadata?: Record<string, unknown> // portable row metadata, preserved on the cell
  only?: boolean
...
target.retrievalRecipe(recipe, { id?, query?, options? })        // maps eval input into a retrieval recipe input
...
| `agent`     | `modelCalls`, `toolCalls`, `steps`, `handoffs`, `retrieval`, `citations`, `safety`, `memory`, `routing`, `decisionReport` |
| `retriever` | `retrieval`                                                                                                             |
...
Effective params for a cell merge as: target defaults ← evaluation `params`
("variant zero") ← the variant entry. Equivalently, `generate`/`model`
resolution order is variant override → evaluation `params` → target defaults.
Bind live model work in eval code or eval-local helper modules. A model-backed
task with no generate available is a definition error (exit 2).
...
```ts
{
  ...
  recordScore(name, score, metadata?) // ad-hoc per-case score; joins the scorer score model
...
The argument to post-score `afterScores` callbacks:
...
```ts
{
  ...
  output: TOutput
  expected: TExpected | undefined
  expect: BoundExpect            // same matcher surface, records phase: "afterScores"
...
- Capability namespaces (exist only when the task captures that signal):
  ...
  - `decisionReport.safety`: `toHaveOutcome(policyId, outcome, { reasonCode? })`
...
passed, failed, uncaptured, and not-evaluated assertions. Consumers that need
a failure list should filter failed or uncaptured outcomes. Outcomes
...
`ctx.expect.soft(...)`. Plain errors thrown from `expect`/`afterScores` callbacks are
...
frame. Outcomes from post-score `afterScores` callbacks use
`phase: "afterScores"`; outcomes from pre-score callbacks use `phase: "expect"`.
...
Experiment records also embed `failures: FailureArtifact[]` for failed or
errored cells. Each artifact includes the cell identity, redacted input,
expected and output snapshots, failing assertion outcomes, scores and deltas
where available, source/trace references, coverage targets from `covers`, and
core-owned `suggestedFixSurfaces` such as `prompt`, `retriever`,
`tool-schema`, `handoff`, `judge`, or `flake`.
...
Rows may include `metadata`. Quality preserves object metadata on the
persisted cell. When a dataset row has `metadata.provenance`, the cell also
gets `metadata.datasetProvenance` with the dataset path, the file content
fingerprint, and the row provenance. Failure artifacts, run-summary failures,
and experiment diffs expose the path and fingerprint so agents can trace a
regression back to the exact imported dataset content.
...
`Experiment` is the persisted result: `cells` (one `ExperimentCell` per case
...
(`.crux/quality/experiments/<ulid>.json`, gitignored, readable with
`crux quality show <experiment-id> --json`), the
...
`crux quality run --json` emits a compact run summary that points at these
records and failing-cell evidence commands. Manifest + Experiment record are the
complete agent-facing contract: list evaluations, run one, read the assertion
outcome ledger with `sourceRef` + `traceIds`, edit, rerun, no parsing of
...
`InputOf<T>` / `OutputOf<T>` / `ParamsOf<T>` / `CapsOf<T>`
