...
`CellAssertionFailure`, `CellAssertionOutcome`, `CellAssertionPhase`,
`CellAssertionStatus`, `CellAssertionValue`, `QualitySourceFrame`,
`QualitySourceFrameSnapshot`, `QualitySourceUnavailable`, `RunOverrides`,
`EvaluationManifest`, `QualityConfig`, `QualitySetupResult`, and the inference
utilities `InputOf`, `OutputOf`, `ParamsOf`, `CapsOf`, `CaseOf`, `ExpectedOf`.
...
cannot prove authored source. Scorers still run on expect-failed cells.
