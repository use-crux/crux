...
Canonical conversation history is not a MemoryBlock. Supply caller-owned
messages or a Thread, and project it with `history.recent()` or managed
`history()` in Prompt composition.
