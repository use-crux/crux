...
Adapters also forward each tool call to blocks' `captureToolEvent` hooks, so `episodes()` records tool activity.
...
`capture.mode` defaults to `afterResponse`:
...
- `detached` starts capture in the background and the adapter does not await it. Call `flush()` to await the work.
