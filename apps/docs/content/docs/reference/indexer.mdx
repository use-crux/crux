...
`experimental.indexer.tsserverPath` provenance when those flags are present.

## Static Index compiler

Static Project Index extraction always runs through the Go-orchestrated Rust/Oxc compiler. The
compiler emits Project Index patch facts directly; Node may still start for config inspection and
TypeScript-authored extension or rule compatibility work. This is independent of semantic backend
selection: use `experimental.indexer.native` only for TypeScript-Go semantic enrichment.

Static Index release readiness is gated by the Rust-owned golden and Go host behavior. The release
gate builds the Rust/Oxc worker, runs Rust tests, runs the full
...
extension compatibility is part of that gate: a configured TypeScript extension extractor may run
through the trusted Node host, but its facts still finalize through the same Project Index patch
contract as Rust first-party output.

If a Static Index run fails or degrades, inspect the indexing diagnostics before treating missing source
facts as absent code. Common causes are a missing Rust worker binary, failed worker startup,
configured TypeScript extension work that requires Node, or config import failures. A missing worker
is a setup error; Crux does not publish an apparently healthy empty index.
...
- emit authored-graph lint findings, retained source-suppression evidence, and available rule descriptor metadata through the index snapshot
