...
`@crux/indexer` is the Crux package that executes TypeScript-family compiler runtimes for Project
...
`experimental.indexer.nativeAst: true` or `{ frontend: 'oxc' }` lets `@crux/local` run the
...
See [Writing an Indexer Extension](/docs/guides/advanced/writing-indexer-extension) for the author walkthrough and [`@crux/indexer/extensions`](/docs/reference/indexer-extensions) for the API reference.
