...
- emit authored-graph lint findings, suppression metadata, and available rule catalog metadata through the index snapshot
...
- `index.ruleCatalog` for backend-owned rule metadata, including built-in rules and extension-provided rules that did not fire a concrete finding.
