...
TypeScript 7 native-preview support is intentionally separate from the public package type-surface preview lane. The native preview is useful for watching future compiler behavior, but indexer runtime support should not be claimed until the TypeScript 7 compiler API is stable enough for source-intelligence use.
