...
AI SDK helpers (`tool`, `stepCountIs`, `hasToolCall`) and AI SDK types are no longer re-exported — import them from `'ai'` directly. Agent compositions come from `@use-crux/core/agent` or from `executorAdapter()`. The legacy `toMessages`/`fromMessages`/`createAIExecutor` exports were removed (RFC [use-crux/crux#28](https://github.com/use-crux/crux/issues/28)).
