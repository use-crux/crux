...
`createGoogle()` binds `googleProviderRuntime.create(client, { cacheResolver })` after resolving CachedContent options. The package-owned `googleTranscript` owns `Content[]` conversion plus assistant text/function-call extraction. The Google package still owns the SDK request shape, function-call normalization, response metadata normalization, and CachedContent lifecycle; the provider runtime supplies the adapter shell and default tool-round formatting.
