...
All three operations use the portable completed-operation Safety contract;
canonical audits are returned in `result.safety`, while provider-native `raw`
values remain unguarded. See the
