...
Start one stateless finite `v1beta` Interactions request. Initial model support
is exactly:
...
and sends no `previous_interaction_id`, background execution, tools, or
webhook. The adapter binds only the current SDK 2.x
`interaction.created`/`step.delta`/`interaction.completed` schema.

Each native image step index receives a dense `outputIndex` on first sight.
Its ordered `image-delta` bytes and assembled final `image` keep that identity;
individual deltas may not be renderable. `completion.raw` is the exact
`InteractionCompletedEvent` envelope, while payload-free provider metadata is
derived from its interaction resource.

### `adapter.streamSpeech(options)`

Start finite `models.generateContentStream()` TTS. Initial support is exactly
`gemini-3.1-flash-tts-preview`; completed-only 2.5 TTS and unknown model ids
fail before provider I/O.

Each `audio-delta` contains decoded native bytes with the exact first
`audio/*` MIME string. Later chunks must declare the same MIME. Google emits
headerless 24 kHz, mono, 16-bit raw PCM; Crux assembles the final `Blob`
without adding a WAV header. Successful completion requires
`candidates[0].finishReason === "STOP"`. `completion.raw` is that exact final
`GenerateContentResponse`; a missing/different reason or a response after it
fails without publishing final `audio` or `finish`.

Both bounded streams share Core's replay, cancellation, timeout, Safety, and
routing laws. Every `fullStream` reader replays the same canonical events.
`cancel(reason)` or `abortSignal` stops the logical operation. Enforcing
output-media Safety retains incomplete deltas until final validation; report
mode may publish them live. The first live delta commits routing, so later
provider failure is terminal. Crux never persists chunks or completion media;
persist the final asset explicitly.
...
retry, safety, tools, memory capture, or instrumentation. Use
`createGenerateObjectFnFromGenerate(generate)` from
`@use-crux/core/compaction` only when full adapter runtime behavior is needed;
do not call that bridge from a guardrail attached to the same lifecycle.
