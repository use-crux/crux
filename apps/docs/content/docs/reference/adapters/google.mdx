...
`createGoogle()` is the package's mapped single-turn bundle factory: it resolves CachedContent options, passes a narrow cache resolver into the provider runtime, and returns the bound adapter. The package-owned `googleTranscript` owns `Content[]` conversion plus assistant text/function-call extraction. The Google package still owns the SDK request shape, function-call normalization, response metadata normalization, and CachedContent lifecycle; the provider runtime supplies the adapter shell and default tool-round formatting.

| Field                | Type                                                             | Description                                                                                                                     |
| -------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `client`             | `GoogleGenAI`                                                    | The Google GenAI client instance                                                                                                |
| `opts.cachedContent` | `false \| GoogleCachedContentOptions \| GoogleCachedContentPort` | CachedContent configuration. Omit for defaults, pass `false` to disable, or pass a custom port to resolve cache names yourself. |

**`GoogleCachedContentOptions`:**

| Field               | Type                    | Default      | Description                                         |
| ------------------- | ----------------------- | ------------ | --------------------------------------------------- |
| `enabled`           | `boolean`               | `true`       | Enable/disable the built-in cache manager           |
...
| `onError`           | `'fallback' \| 'throw'` | `'fallback'` | Whether cache creation failures fall back or reject |

**`GoogleCachedContentPort`:**

```ts
interface GoogleCachedContentPort {
  resolve(args: {
    model: string
...
<Callout type="info">
  ...
  resolver is passed to the native chat profile as provider-owned dependencies, so the same planner is used for
