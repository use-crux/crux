...
const profile = googleModelCapacity("gemini-2.5-flash");
```
...
The adapter does not call a provider token-count endpoint. Crux estimates the
complete request and reports the applied margin in request inspection. Cached
input can reduce provider billing, but it does not reduce the model context
capacity used for fit.
