...
```ts
import { createGoogle, embedding, googleTranscript, toMessages, fromMessages } from '@crux/google'
...
`@crux/google` is built from the shared `defineNativeChatProvider()` profile used by the other native chat adapters.
The package-owned `googleTranscript` owns `Content[]` conversion plus assistant text/function-call extraction. The
Google package still owns the SDK request shape, function-call normalization, response metadata normalization, and the
CachedContent lifecycle; the shared core profile only supplies the adapter shell, structured/text helper plumbing, and
default tool-round formatting.

| Field        | Type                                                          | Description                                                                      |
| ------------ | ------------------------------------------------------------ | -------------------------------------------------------------------------------- |
| `client`     | `GoogleGenAI`                                                | The Google GenAI client instance                                                 |
| `opts.cache` | `GoogleCacheConfig \| false \| GoogleCachedContentLifecycle` | CachedContent configuration (optional). Omit for defaults, `false` to disable, or pass a custom lifecycle. |
...
| `onError`           | `'fallback' \| 'throw'`          | `'fallback'`  | Behavior when a cache operation throws. `'fallback'` sends a plain `systemInstruction`; `'throw'` surfaces the error. |
...
const strict = createGoogle(client, { cache: { onError: 'throw' } })

// Disable caching entirely
const adapter = createGoogle(client, { cache: false })

// Back caching with a custom cache port, or a fully custom lifecycle
const custom = createGoogle(client, { cache: { port: myCachePort } })
