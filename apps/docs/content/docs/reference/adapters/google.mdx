...
Google content support maps image/file data to `inlineData` and URL-backed image/file parts to `fileData` when `mediaType` is present. URL-backed parts without a MIME type degrade before the provider call unless strict unsupported-content mode is enabled. Assistant inline/file media decodes back into canonical content, and function-response media uses the same part table as messages.
