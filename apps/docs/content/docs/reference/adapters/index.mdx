...
| ----------------------------------------------------------------------------------------- | -------------------------- | ------------------------------------------------- |
| Exposes text/structured/stream chat calls and leaves tool execution to Crux               | `singleTurn`               | `@crux/openai`, `@crux/anthropic`, `@crux/google` |
...
Under the hood, `singleTurn` compiles to `AdapterSpec`: core calls `call()` once per loop turn and `appendToolRound()` formats results. `loop` compiles to `ExecutorSpec`: the SDK owns the loop and core steers each step through a `StepObserver`:
...
## `defineProviderRuntime({ singleTurn })`
...
Create a core-driven adapter from a compiled `AdapterSpec`. Provider packages should implement `defineProviderRuntime({ singleTurn })`; use this only when testing core adapter internals or building a bespoke runtime compiler.
...
The core-step execution IR produced by the `singleTurn` provider-runtime branch.
