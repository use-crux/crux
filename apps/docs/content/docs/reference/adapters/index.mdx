...
| Native single-turn adapters (`@use-crux/openai`, `@use-crux/anthropic`, `@use-crux/google`) | `router()`, `split()`, `retry()`, `fallback()`, `cascade()` | `router()`, `split()`, `retry()`, `fallback()` | The adapter loop resolves wrappers before each provider call.             |
