...
Built-in adapters are built on this abstraction: `@crux/anthropic`, `@crux/openai`, and `@crux/google` implement `AdapterSpec`; `@crux/ai` (Vercel AI SDK) implements `ExecutorSpec`. Native provider packages own provider wire codecs and request bodies, while core owns the canonical `Message[]`, `CallArgs`, `AdapterResponse`, `ToolResultEntry`, and small tool-output helpers those codecs share. Use this abstraction when creating a custom adapter for a provider not yet supported.
