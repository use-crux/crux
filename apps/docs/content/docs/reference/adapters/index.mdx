...
import type { NativeChatProfile, NativeProviderPort } from '@crux/core/adapter/native-chat'
...
Built-in adapters are built on this abstraction: `@crux/openai`, `@crux/anthropic`, and `@crux/google` use `NativeChatProfile`, while `@crux/ai` (Vercel AI SDK) implements `ExecutorSpec`. Native provider packages own provider wire codecs, request bodies, cache dependencies, and provider-specific feature flags, while core owns the canonical `Message[]`, `CallArgs`, `AdapterResponse`, `ToolResultEntry`, and small tool-output helpers those codecs share. Use this abstraction when creating a custom adapter for a provider not yet supported.
...
The profile owns provider wire facts: request bodies, SDK method binding through `NativeProviderPort`, response extraction, stream delta extraction, settings/schema mapping, transcript codecs, and provider-specific dependencies. Crux owns prompt resolution, tool execution, default canonical tool-round appends, validation retry, safety, memory capture, and observability.
