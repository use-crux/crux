...
The adapter package provides the public authoring surface for AI provider adapters. `defineProviderRuntime()` binds a stable runtime id to an explicit `ProviderOwnership`: `ownership: 'single-turn'` with `turn`, where Crux owns the provider step loop, or `ownership: 'loop-owned'` with `loop`, where the SDK owns the loop and Crux steers policy around it. Both branches compile into the same execution engine and share the same policy modules (validation retry, tool instrumentation, approvals) plus the same per-call [`Safety` session](/docs/reference/crux-core/safety), so behavior does not diverge between runtimes.
...
| ----------------------------------------------------------------------------------------- | ------------- | --------- | ------------------------------------------------- |
| Exposes text/structured/stream chat calls and leaves tool execution to Crux               | `single-turn` | `turn`    | `@use-crux/openai`, `@use-crux/anthropic`, `@use-crux/google` |
...
Use `adapterSpecConformance()` and `executorSpecConformance()` only when you are testing lower-level compiler output or core execution IR directly. `fakeExecutor()` remains the in-memory reference implementation for policy tests that need a loop-owned runtime without loading an SDK.

## `defineProviderRuntime({ ownership: 'single-turn', turn })`
...
Create a core-driven adapter from a compiled `AdapterSpec`. Provider packages should implement `defineProviderRuntime({ ownership: 'single-turn', turn })`; use this only when testing core adapter internals or building a bespoke runtime compiler.
...
Create an SDK-loop adapter from a compiled `ExecutorSpec`. Provider packages should implement `defineProviderRuntime({ ownership: 'loop-owned', loop })`; use this only when testing executor internals or building a bespoke runtime compiler.
