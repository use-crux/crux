...
Start with the [Media and multimodal guide](/docs/guides/media) to choose
between ordinary media messages and specialized image, transcription, speech,
or streaming operations. This reference owns adapter-specific support; the
guide owns application workflow.
...
const myProvider = defineSingleTurnProviderBundle({
  ...
  profile: {
    ...
    structuredOutput: { accepts: myStructuredCapabilities },
  },
});
...
| `structuredOutput?`   | `{ accepts: StructuredOutputCapabilities }`                             | Inert description of the JSON Schema dialect this provider accepts. Core compiles the authored schema against it and supplies the result as `ctx.outputSchema`. Absent means structured output is unsupported and fails before I/O. |
...
| `stream(prompt, opts)`   | `Promise<StreamResult<TOutput, TPartial>>`     | Execute a prompt and return one managed logical stream                   |
...
`CruxAdapter.stream()` publishes it as `StreamResult<TOutput, TPartial>`; the
provider chunk protocol stops at that boundary.
...
| `inputBudget`     | `{ optimizeAt?: number; max?: number }?`                     | Whole-request soft watermark and strict input maximum       |
...
One managed logical stream. A logical stream may use several physical provider
attempts, but that is never observable: provider framing, discarded attempts,
and the provider stream object are all private.

| Field                 | Type                                        | Description                                                                              |
| --------------------- | ------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `runId`               | `CruxRunId`                                 | Authoritative logical run, stable across physical attempts                               |
| `textStream`          | `AsyncIterableStream<string>`               | Published, Safety-final text; canonical `z.input` JSON for a structured prompt           |
| `fullStream`          | `AsyncIterableStream<StreamEvent<TPartial>>`| Every published logical event in canonical order                                         |
| `partialOutputStream` | `AsyncIterableStream<TPartial>`             | Canonical partial `z.input`; closes empty for a text prompt                              |
| `completion`          | `Promise<StreamCompletion<TOutput>>`        | Completion facts with the same operation pair as the immediate handle                    |
| `cancel`              | `(reason?: unknown) => void`                | Abort the whole logical operation, including the active provider attempt                 |
| `_meta`               | `OperationResultMeta`                       | Exact `generation.stream` identity, available immediately and stable through completion  |

All three streams project one shared append-only event log: each has an
independent cursor, they may be read concurrently, and a surface first read late
replays from logical `start`. Retention never delays publication, so `completion`
settles without any stream being drained.

There is deliberately no `raw`, `acceptedRaw`, `providerResult`, or `unsafe`
escape hatch. Provider-specific request options remain supported, and
provider-specific terminal facts belong on `completion.providerMetadata`.

Adapter payloads and provider stream handles do not manufacture Crux IDs. Core
finalizes managed results after provider normalization. See
[Operation Result Correlation](/docs/reference/crux-core/operation-results) for
payload/result types, middleware timing, and streaming failure behavior.
