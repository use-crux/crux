...
import type { NativeChatProfile, NativeProviderPort, NativeTranscriptCodec } from '@crux/core/adapter/native-chat'
import { adapterSpecConformance, transcriptCodecConformance } from '@crux/core/adapter/testing'
```

## Overview

The adapter subpath provides shared infrastructure for building AI provider adapters, in two dialects: `adapter()` + `AdapterSpec` for raw provider SDKs where core drives the tool loop, and `executorAdapter()` + `ExecutorSpec` for orchestrating SDKs that drive their own. Raw chat SDKs can use `defineNativeChatProvider()` from `@crux/core/adapter/native-chat` to compile a provider profile into an `AdapterSpec`, public `createX()` factory, and lightweight compaction helpers. Both factories share the same policy modules (validation retry, tool instrumentation, approvals) and construct the same per-call [`Safety` session](/docs/reference/crux-core/safety) for guardrails and constraints, so behavior never diverges between dialects.

Built-in adapters are built on this abstraction: `@crux/openai`, `@crux/anthropic`, and `@crux/google` use `NativeChatProfile`, while `@crux/ai` (Vercel AI SDK) implements `ExecutorSpec`. Native provider packages own a `NativeTranscriptCodec` for provider message conversion, assistant text/tool-call extraction, and provider-specific tool-round appends. Core injects transcript-produced `providerMessages` into request builders and composes `transcript.readAssistant(raw)` with response-level metadata. Core owns the canonical `Message[]`, `CallArgs`, `AdapterResponse`, `ToolResultEntry`, and small tool-output helpers those codecs share. Use this abstraction when creating a custom adapter for a provider not yet supported.

## Which contract do I implement?

| Your SDK…                                                                                 | Contract            | Factory                      | Example                                           |
| ----------------------------------------------------------------------------------------- | ------------------- | ---------------------------- | ------------------------------------------------- |
| Exposes text/structured/stream chat calls and leaves tool execution to you                | `NativeChatProfile` | `defineNativeChatProvider()` | `@crux/openai`, `@crux/anthropic`, `@crux/google` |
| Exposes single-turn provider calls but needs a bespoke non-chat boundary                  | `AdapterSpec`       | `adapter()`                  | custom provider                                   |
...
Test native adapters with `adapterSpecConformance()` and SDK-shaped fake clients. The harness scripts provider emissions, captures request bodies, and verifies the `AdapterSpec` boundary without network calls:

```ts
import { adapterSpecConformance } from '@crux/core/adapter/testing'

const violations = await adapterSpecConformance(myNativeSpec, myProviderHarness)
...
## `defineNativeChatProvider(profile)`

Compile a raw chat SDK profile into Crux's native adapter contract.

```ts
import { defineNativeChatProvider } from '@crux/core/adapter/native-chat'

const nativeProvider = defineNativeChatProvider({
...
export const myProviderSpec = nativeProvider.specFor(bindClient)
export const createMyProvider = nativeProvider.createFor(bindClient)
export const { createGenerateTextFn, createGenerateObjectFn } = nativeProvider.helpers(bindClient)
```

The profile owns provider wire facts: request bodies, SDK method binding through `NativeProviderPort`, transcript conversion, assistant-turn extraction, response metadata extraction, stream delta extraction, settings/schema mapping, and provider-specific dependencies. New profiles should use `transcript: NativeTranscriptCodec` plus `response.meta(raw)`; `response.text(raw, assistant)` is only needed when the SDK exposes parsed structured output separately from assistant message text. `messages` and profile-level `appendToolRound` remain compatibility shims for older profiles. Crux owns prompt resolution, tool execution, default canonical tool-round appends, validation retry, safety, memory capture, and observability.
...
`helpers(bind)` generates `GenerateTextFn` and `GenerateObjectFn` factories from the same request/response path. These helpers preserve provider SDK errors and stay intentionally smaller than adapter `generate()`: no prompt resolution, tool loop, safety, cassettes, memory capture, or instrumentation.
