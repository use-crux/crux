...
`createAnthropic()` is built from the shared native chat profile helper in `@crux/core/adapter/native-chat`. Anthropic-specific request params, `tool_use` / `tool_result` blocks, cache-control system blocks, and response normalization stay owned by `@crux/anthropic`; Crux still owns prompt resolution, tool loops, safety, validation retry, memory capture, and observability.
