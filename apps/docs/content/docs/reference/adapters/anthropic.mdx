...
`toMessages()` and `fromMessages()` are provider-history converters, not a generic cross-provider transcript API. They delegate to the same Anthropic-owned codec that `createAnthropic()` uses for request messages, assistant `tool_use` extraction, and second-call tool-loop payloads.
...
This is a provider-native helper. It uses Anthropic's structured parse surface and returns the parsed `{ object }`, preserving provider errors. It does not run Crux prompt resolution, validation retry, safety, cassettes, tools, memory capture, or instrumentation. Use `createGenerateObjectFnFromGenerate(generate)` from `@crux/core/compaction` when a `GenerateObjectFn` needs full adapter runtime behavior.
