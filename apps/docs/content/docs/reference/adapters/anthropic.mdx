...
`anthropicModelCapacity(model)` helper. Both return a frozen profile without
provider I/O.

```ts
import { anthropicModelCapacity } from "@use-crux/anthropic";

const profile = anthropicModelCapacity(modelId);
```

Known prefix profiles cover the instant and 2-series families, the 2.1 family,
the 3-series family including 3.5 and 3.7, and the 4-series Opus, Sonnet, and
Haiku families. Versioned aliases inherit the matching family profile. Their
context windows range from 100,000 to 200,000 tokens, with an output reserve
chosen for that family.

Unknown identifiers use a conservative 100,000-token window with an
8,192-token default output reserve. Known profiles report
`countingConfidence: "estimated"`; the fallback reports `"conservative"`.

The adapter does not call a provider counting endpoint. Crux estimates the
complete request and exposes the applied confidence and margin through request
inspection. Set `inputBudget` when your application needs a stricter input
boundary.
...
Use `step(response)` when the model may request tools or validation retry may
...
These codecs are translation-only. They do not run tool middleware, approvals,
validation retry, memory capture, safety, or observability.
...
must supply a non-empty Anthropic model id.
...
`createAnthropic()`. It accepts exactly one of `prompt` or canonical
`messages`, uses Anthropic's structured parse surface, and returns `{ object }`
while preserving provider errors. It does not run Crux prompt resolution,
validation retry, safety, tools, memory capture, or instrumentation.
