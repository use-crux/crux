...
`createOpenAI()` is built from the shared native chat profile helper in `@crux/core/adapter/native-chat`. That internal profile owns request assembly, response normalization, stream deltas, settings/schema mapping, and message codecs; Crux still owns prompt resolution, tool loops, safety, validation retry, memory capture, and observability.
