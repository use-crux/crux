...
Start one finite, eagerly executing Images API stream. Initial model support is:
...
The request is image-generation-only and single-output in this release.
Omitting `n` means one; `n !== 1`, edits/references/masks, unsupported portable
controls, and DALL·E models fail capability preflight before SDK I/O.
`extra.partial_images` accepts `0` through `3`.

OpenAI partial-image events become complete `image-preview` replacements at
`outputIndex: 0`; the native partial index is the zero-based sequence. The
final `image` event shares its guarded asset with `completion.image`.
`completion.raw` is the exact `ImageGenCompletedEvent`, including usage. It
does not contain or synthesize DALL·E 3's completed-only `revised_prompt`.

### `adapter.streamSpeech(options)`

Start one finite Speech API response-body stream. Initial model support is:
...
The adapter fixes native `stream_format: "audio"` and publishes each response
body chunk as an ordered `audio-delta`. Portable `outputFormat` selects `mp3`,
`opus`, `aac`, `flac`, `wav`, or raw `pcm`; the final `audio` asset is assembled
from those retained chunks. `completion.raw` is the exact SDK `Response`.
...
output-media Safety guards complete previews and holds incomplete deltas until
the final asset passes; report mode may leave deltas live. The first public
preview or live delta commits the selected route, so later provider failure is
terminal. Crux never persists previews, chunks, or completion media; persist
the final asset explicitly.
...
instrumentation. Use `createGenerateObjectFnFromGenerate(generate)` from
