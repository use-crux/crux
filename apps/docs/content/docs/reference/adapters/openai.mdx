...
```ts
import { createOpenAI, embedding, openaiProfile, openAITranscript, toMessages, fromMessages } from '@crux/openai'
...
`createOpenAI()` is `openaiProfile.create`. The package-owned `openAITranscript` owns message conversion plus assistant text/tool-call extraction; response metadata normalization, stream deltas, and settings/schema mapping stay in this package too. Crux still owns prompt resolution, tool loops, safety, validation retry, memory capture, and observability.
