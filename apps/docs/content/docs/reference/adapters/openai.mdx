...
```ts
import { createOpenAI, embedding, toMessages, fromMessages } from '@crux/openai'
...
This is a provider-native helper. It uses OpenAI's structured parse surface and returns the parsed `{ object }`, preserving provider errors. It does not run Crux prompt resolution, validation retry, safety, cassettes, tools, memory capture, or instrumentation. Use `createGenerateObjectFnFromGenerate(generate)` from `@crux/core/compaction` when a `GenerateObjectFn` needs full adapter runtime behavior.
...
```ts
import type { OpenAISettings, OpenAIGenerateOptions } from '@crux/openai'
```
