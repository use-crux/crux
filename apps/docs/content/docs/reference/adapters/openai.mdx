...
OpenAI Chat content support maps image parts to `image_url`, wav/mp3 `file-data` parts to `input_audio`, and supported file data/file ids to OpenAI file parts. PDF URLs and unsupported role/content combinations degrade to deterministic placeholders unless strict unsupported-content mode is enabled. Assistant/user media returned in provider content arrays decodes back into canonical `ContentPart[]`. Chat Completions tool-result messages remain text-only, so non-text tool results become deterministic text references instead of raw base64.
