...
The helper fingerprint always includes the model and explicit dimensions, then
appends `version` when supplied. A user version never replaces model identity,
so either change invalidates cached vectors.
