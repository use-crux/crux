...
Create a transport that polls a `RecordStore` at a regular interval. Universal fallback -- works with any backend that implements `RecordStore`, even without `subscribe()` support.

| Parameter            | Type          | Description                                      |
| -------------------- | ------------- | ------------------------------------------------ |
| `store`              | `RecordStore` | The RecordStore to poll                          |
...
```tsx
import { createMockTransport, CruxProvider, usePlan } from "@use-crux/react";
