...
The root also re-exports the core multimodal content surface unchanged: `ContentPart`, `MessageContent`, `textPart`, `imagePart`, `filePart`, `contentText`, `messageText`, `hasMediaParts`, and `UnsupportedContentError`. Use these identical re-exports in Convex code when message construction or projection lives inside Convex actions.
