...
Create only the Crux-in-Convex runtime bridge: request-scoped storage creation, ambient runtime binding, namespace defaults, and Runtime Bridge HTTP setup. Use this when custom Convex actions or normal Convex Agent-shaped code need Crux runtime plumbing without adopting the profile-backed `convexAgent()` helper.
...
`createCruxConvex()` delegates its `storage()`, `run()`, and `bridge()` methods to this lower-level bridge. Prefer the profile when you also want `crux.convexAgent()` to own prompt resolution, memory, skills, tools, and post-turn persistence.
...
const mem = memory({ id: 'assistant', records, namespace: 'thread:1', blocks: [history] })
```
...
<Callout type="info">
  Plans, task lists, and tasks stored via Convex records are automatically reactive through the Convex transport —
