...
| `@use-crux/convex/skill`   | Identical re-export + Convex adapter | Same skill authoring/session API as `@use-crux/core/skill`; adds `convexSkillActivationPersistence()` for Convex store snapshots. |
| `@use-crux/convex/memory`  | Convex-bound drop-in                 | Re-exports memory block helpers and changes only `memory()` defaults.                                                             |
...
Use `run()` when lower-level integration code needs a Convex-bound store or mirrored `@use-crux/convex/*` helpers outside the high-level agent wrapper:
...
Create only the Crux-in-Convex runtime bridge: request-scoped store creation, ambient runtime binding, namespace defaults, and Runtime Bridge HTTP setup. Use this when custom Convex actions or normal Convex Agent-shaped code need Crux runtime plumbing without adopting the profile-backed `convexAgent()` helper.
...
`createCruxConvex()` delegates its `store()`, `run()`, and `bridge()` methods to this lower-level bridge. Prefer the profile when you also want `crux.convexAgent()` to own prompt resolution, memory, skills, tools, and post-turn persistence.

### `defineConvexStoreContract(options)`

Define the shared Convex store document contract for a crux Convex component.
...
**Returns:** `ConvexStoreContract`

| Method                     | Description                                          |
| -------------------------- | ---------------------------------------------------- |
| `.codec`                   | Shared current-format store document codec           |
| `.store(ctx)`              | Create a server-side `CruxStore` for a Convex ctx    |
| `.transport({ useQuery })` | Create a React transport for Convex reactive queries |

The contract's server store is intentionally dense-only today. It throws explicit errors for sparse or hybrid `searchVectors()` queries instead of silently degrading.

Store values are written as `_cruxDoc` records with JSON `content`, optional top-level `embedding`, `updatedAt`, and an `_expiresAt` value inside the JSON payload when `ttl` is provided. The component I/O contract stays narrow: `components.crux.memory.insert` atomically inserts a document only when the key is absent, and `components.crux.memory.list` accepts `prefix`, `limit`, and `cursor`, reads the `by_key` index, and returns `{ docs, cursor }`.

The same document contract is used by server stores, the HTTP bridge, and React transport: imperative reads suppress expired entries and lazily delete them, vector hits map Convex `_score` to `ScoredEntry.score`, and list/vector filters use top-level exact-match semantics. Filtered `list()` calls with a `limit` fill from additional component pages so `limit` means "up to N matching entries." Server and React reads are strict about current `_cruxDoc` records so malformed documents fail clearly.
...
const transport = cruxDocuments.transport({
  useQuery: component.useQuery,
})

await store.set('memory:alpha', {
  content: 'Alpha',
  namespace: 'kb',
})
...
const io: ComponentDocumentPort = {
  ...
  list: async ({ prefix }) => ({
    docs: [...rawDocs.values()].filter((doc) => String(doc.key).startsWith(prefix)),
  }),
  put: async (doc) => {
...
```ts
import { convexRecordStore, convexStorage, convexVectorStore, convexWorkspaceBlobStore } from '@use-crux/convex'
...
| `convexVectorStore()` | Dense search | `true` |
| `convexVectorStore()` | Sparse search | `false` |
| `convexVectorStore()` | Hybrid search | `false` |
| `convexVectorStore()` | Vector filters | `'post'` |
| `convexVectorStore()` | Consistency | `'strong'` |
| `convexWorkspaceBlobStore()` | Multipart | `false` |
| `convexWorkspaceBlobStore()` | Signed URLs | `false` |
...
Workspace metadata stays in the Convex-backed `RecordStore`; binary and oversized payloads go to Convex file storage. If the current Convex runtime cannot read blobs, `get()` throws clearly instead of silently dropping file content.
...
const result = await supportMetadataAgent.generateObject(ctx, { threadId, userId }, { input: { message } })
```
...
Create a `CruxTransport` from the same Convex store contract used by server functions.
...
<Callout type="info">
  Plans, task lists, and tasks stored via the contract store are automatically reactive through the Convex transport —
...
import type { ConvexWorkspaceBlobStoreConfig } from '@use-crux/convex/workspace'
```
