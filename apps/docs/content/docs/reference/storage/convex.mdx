...
The returned profile exposes `store(ctx)`, `withRuntime(ctx, target, fn)`, and `convexAgent(config)`. Prefer `convexAgent()` for normal agent execution; use `withRuntime()` when lower-level integration code needs to resolve Convex-profile memory or tools outside the high-level wrapper.
