...
`convexStorage()` contains `records` and includes `assets` only when its `assets` option is configured. It does not contain `search`. Embeddings inside record values are mirrored to the component document's top-level `embedding` field, but the bundled component does not declare a retrieval index or perform search ranking.

Memory blocks configured with `embed` therefore fall back to record listing/recency when only Convex storage is present. For similarity-ranked recall, pass an explicit `SearchStore` such as Upstash:
