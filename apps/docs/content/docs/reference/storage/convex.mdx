...
await supportAgent.turn(ctx, { threadId, userId }).generateText({ input: { message } })
```
...
```ts
const { thread } = await supportAgent.turn(ctx, { threadId, userId }).continueThread({ input: { message } })
