...
| `convex.fns.insert` | `FunctionReference` | Convex mutation for atomic insert-if-absent |
...
| `.setIfAbsent(key, entry)` | Atomically store an entry only if the key is absent |
...
Most applications should call `retriever()` instead of copying internal filters. If you are building adapter-level tooling and call the store directly, Crux maps dense, sparse, and hybrid queries to Upstash's native shape:
...
When Crux indexers write chunks, the Upstash adapter mirrors read-model fields into vector metadata for internal filtering and hydration:

```ts
{
  _key,
  // Crux read-model fields for namespace, source, chunk identity,
  // generation state, and active child-chunk filtering.
  ...readModelMetadata,
  ...chunk.metadata,
}
```
...
| `.setIfAbsent(key, value)` | Atomically store a document only if the key is absent. Publishes a `set` event when inserted. |
