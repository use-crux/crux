...
Canonical [`md` PromptText](/docs/reference/crux-core/prompts#md-and-prompttext)
...
The recommended installation is the npm CLI plus the VSIX from the same
release:
...
In VS Code, Cursor, or another compatible editor, you can instead run
**Extensions: Install from VSIX...** and select the downloaded file. A VSIX
installed from GitHub does not update automatically; download and install the
new VSIX when changing Crux versions.
...
confirmation before dispatch. Dispatch invokes `Prompt.inspect()` once. It
