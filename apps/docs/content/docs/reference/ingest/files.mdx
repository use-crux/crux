...
| `.pdf`             | PDF        | Retains every physical page; warns only when native text is empty and `media.describe` is unavailable, returns empty content, or fails |
| image formats      | Image      | Derives text through `media.describe`                               |
| audio formats      | Audio      | Derives text and time segments through `media.transcribe`           |
| video formats      | Video      | Uses explicitly bound `media.describe`, `media.transcribe`, or both |
| `.csv`             | CSV        | Extracts a table part                                               |
| `.json`            | JSON       | Extracts JSON path parts                                            |
| `.docx`            | DOCX       | Extracts text and tables through Mammoth                            |
| `.xlsx`            | XLSX       | Extracts source coordinates and displayed values through ExcelJS   |
...
| `media`     | `IngestMediaOperations?` | Bound semantic description/transcription operations for media sources.         |
...
const source = filesSource(
  { glob: "docs/**/*.{md,html}", cwd: process.cwd() },
  { namespace: "product-docs" },
);
