...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash npm install @use-crux/core @use-crux/ai @ai-sdk/openai zod ```
  </Tab>
  <Tab value="pnpm">
    ```bash pnpm add @use-crux/core @use-crux/ai @ai-sdk/openai zod ```
  </Tab>
  <Tab value="yarn">
    ```bash yarn add @use-crux/core @use-crux/ai @ai-sdk/openai zod ```
  </Tab>
  <Tab value="bun">
    ```bash bun add @use-crux/core @use-crux/ai @ai-sdk/openai zod ```
  </Tab>
</Tabs>
{/* prettier-ignore-end */}
...
<Callout type="idea">
  This is a recommendation, not a requirement. Crux has no required project
  structure: organize however fits your codebase.
</Callout>
...
<Callout type="info">
  The Vercel AI SDK adapter's `stream()` returns a canonical Crux stream result.
  Pass it to `createUIMessageStreamResponse(result)` for seamless integration
  with `useChat` on the client.
</Callout>
...
<Cards>
  ...
  <Card
    title="Memory"
    description="Add chat history, agent state, or semantic recall."
