...
- [**Memory capture modes (Breaking):**](#memory-capture-modes-are-honored) `afterResponse` and `detached` no longer block the response. Convex vector storage is removed.
...
`--max-cost` now fails closed on conservative per-call USD ceilings. Managed tasks, routing trees, bounded tool loops, and judges estimate from `experimental.eval.pricing`. If a path has no model key, you get the missing key and a remedy before any billable work, not an overrun afterwards.
...
- `capture.mode: 'afterResponse'` and `'detached'` are now genuinely non-blocking. If your code relied on memory being written before the response returned, set `'inline'` explicitly.
