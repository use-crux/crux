...
const reply = prompt({
  id: 'reply',
  use: [
    history.recent({ messages: 12 }),
    chat,
    docs.grounding({ query: ({ input }) => input.question }),
  ],
...
Nothing exotic. Caller-owned recent history, learned memory, retrieval, one prompt. And yet: wrong launch.
