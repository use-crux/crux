...
export default evaluate('reply.launch-freshness', {
  covers: ['prompt:reply'],
  task: target.prompt(reply, { generate, model }),
  data: [{ input: { userId: 'u1', question: 'What did we decide about the launch plan?' } }],
  expect: (ctx) => {
...
Run both in CI with `crux quality run --ci`. If the sync job breaks again, the pipeline fails with "stale evidence rejected on prompt:reply" instead of a customer screenshot three weeks later.
