...
Each call renders the block into prompt context before generation. After the model finishes, the adapter captures both turns and flushes the write. Streaming works the same way; capture runs after `stream().completion()` resolves, so nothing blocks the token stream.
...
`capture: { mode: 'afterResponse' }` moves capture work off the response path and hands it to `waitUntil` on serverless. Use `detached` plus an explicit `flush()` when you manage background work yourself, or the default `inline` when you need read-your-writes semantics in tests.
