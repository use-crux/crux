...
Each call renders the block into prompt context before generation. After the model finishes, the adapter captures both turns according to the configured capture mode. Streaming works the same way; capture runs while `stream().completion()` finalizes, after the tokens have already streamed. In `inline` mode (or `afterResponse` without a `waitUntil` hook) capture can delay when `completion()` resolves, but it never blocks token delivery.
