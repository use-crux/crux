...
```ts
export default evaluate('safety.injection-contract', {
  covers: ['prompt:support'],
  task: target.prompt(supportPrompt, { generate, model }),
  data: knownInjectionCases,
  expect: (ctx) => {
...
*Safety docs (guardrails, constraints, validation retry, the security model): [cruxjs.dev/docs/guides/safety](https://cruxjs.dev/docs/guides/safety). Crux is open source and alpha.*
