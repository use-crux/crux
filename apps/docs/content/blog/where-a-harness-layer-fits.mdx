...
3. **First evals: an afternoon per prompt.** `crux quality init` scaffolds from what the Project Index sees, and importing traces beats writing cases by hand.
...
- **You need boring-stable today.** Crux is alpha, its quality system is beta, and parts of the proof layer (the full per-turn decision report, richer routing rationale) are still being completed. The docs label shipped vs. in-progress honestly, but alpha is alpha.
