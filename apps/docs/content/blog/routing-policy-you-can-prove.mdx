...
//   cost: 0.018,
//   trace: [
//     { kind: 'router', id: 'support-model', route: 'deep' },
...
```ts
export default evaluate('support.routing', {
  covers: ['prompt:support'],
  task: target.prompt(supportPrompt, { generate, model: supportModel }),
  data: productionShapedCases,
  expect: (ctx) => {
