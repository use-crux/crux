...
const heroOutputs = [
  {
    name: "constrain()",
    sub: "zod · retry with feedback",
    soft: true,
    span: "span-3",
  },
  {
    name: "generate()",
    sub: "your SDK · your model",
    strong: true,
    span: "span-4",
  },
  { name: "evaluate()", sub: "cases · baselines", soft: true, span: "span-2" },
...
const compositionCode: CodeLine[] = [
  ...
  {
    text: `import { memory, recentMessages, facts } from '@use-crux/core/memory'`,
    type: "import",
  },
  {
    text: `import { retriever } from '@use-crux/core/retrieval'`,
    type: "import",
  },
  ...
  {
    text: `  blocks: [recentMessages(), facts({ id: 'about-user' })],`,
    type: "code",
  },
  { text: `})`, type: "code" },
  { text: ``, type: "blank" },
  {
    text: `const docs = retriever({ store, query: q => q.message })`,
    type: "code",
  },
  { text: ``, type: "blank" },
  ...
  {
    text: `  system: 'Answer using memory and retrieved docs.',`,
    type: "code",
  },
  { text: `})`, type: "code" },
  { text: ``, type: "blank" },
...
const evalCode: CodeLine[] = [
  { text: `import { evaluate } from '@use-crux/core/eval'`, type: "import" },
  { text: `import { replyTask } from './reply'`, type: "import" },
  { text: ``, type: "blank" },
  {
    text: `const rubric = 'Does the answer fit the support request?'`,
    type: "code",
  },
  { text: ``, type: "blank" },
  { text: `export default evaluate({`, type: "code" },
  { text: `  id: 'reply',`, type: "code" },
  { text: `  task: replyTask,`, type: "code" },
  { text: `  cases: [`, type: "code" },
  {
    text: `    { id: 'memory', input: { message: 'What did we agree on?' }, expected: 'demo' },`,
    type: "code",
  },
  {
    text: `    { id: 'off-topic', input: { message: 'Tell me a joke' }, expected: 'cannot help' },`,
    type: "code",
  },
  { text: `  ],`, type: "code" },
  { text: `  expect: ({ output, expected, expect }) => {`, type: "code" },
  { text: `    expect(output.answer).toContain(expected)`, type: "highlight" },
  { text: `  },`, type: "code" },
  {
    text: `  scorers: (scorers) => [scorers.judge({ name: 'support_fit', rubric })],`,
    type: "highlight",
  },
...
const judges = [
  {
    name: "faithfulness",
    desc: "Does the answer stay grounded in the retrieved context?",
  },
  {
    name: "relevance",
    desc: "Is the model addressing what the user actually asked?",
  },
  {
    name: "safety",
    desc: "Pre-built checks for refusal, leakage, off-topic drift.",
  },
];
...
const tradeRows = [
  ...
  {
    l: "Manual string concatenation",
    r: "Reusable context blocks with priority and budget",
  },
  {
    l: "Provider-specific call sites",
    r: "Bring your SDK, keep the structure",
  },
  {
    l: "console.log of request bodies",
    r: "A clear view of what the model saw",
  },
    ...
    <main>
      ...
      <section className="relative overflow-hidden px-6 pt-28 pb-24 sm:pt-36 sm:pb-32">
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 80% 50% at 50% -20%, var(--crux-glow), transparent)",
          }}
        />
        ...
        <div className="mx-auto max-w-[72rem]">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="mt-6 max-w-2xl text-[1.05rem] leading-relaxed text-fd-muted-foreground sm:text-lg">
              AI failures usually hide in what gets sent to the model: stale
              context, missing memory, unsafe inputs, silent fallbacks, weak
              tests. Crux gives you typed building blocks for those pieces
              around the{" "}
              <span className="text-fd-foreground">SDK you already use</span>,
              so you can see what the model saw and fix the right layer.
            </p>
          ...
          <div className="mx-auto mt-24 max-w-5xl">
            ...
            <div className="grid grid-cols-12 gap-2.5">
              ...
              <div className="col-span-12 sm:col-span-2">
                <Tile name="evaluate()" sub="cases · baselines" soft />
              </div>
              <div className="col-span-12 sm:col-span-3">
      ...
      <section className="relative border-t border-fd-border bg-fd-muted/20 px-6 py-20">
        <div className="mx-auto grid max-w-6xl items-center gap-14 lg:grid-cols-[0.95fr_1.05fr]">
          <div>
            <p className="text-xs font-medium tracking-[0.22em] uppercase text-crux">
              Why Crux
            </p>
            ...
            <p className="mt-5 max-w-md text-[0.95rem] leading-relaxed text-fd-muted-foreground">
              The fix usually isn&apos;t the prompt and isn&apos;t the model.
              It&apos;s the missing memory, stale retrieval, dropped
              instruction, or test that should have caught the regression. Crux
              makes those parts explicit.
            </p>
            <Link
              href="/why"
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {[
              {
                k: "Steerability",
                v: "Guardrails, constraints, and fallbacks are declared before the call.",
              },
              {
              ...
              {
                k: "Type safety",
                v: "Zod schemas in. Typed objects out. Refactors stay real.",
              },
              {
                k: "Observable by default",
                v: "See what the model saw before you start guessing.",
              },
            ].map((p) => (
              <div
                key={p.k}
                className="rounded-xl border border-fd-border bg-fd-card/50 px-5 py-4"
              >
                <h3 className="text-sm font-semibold tracking-[-0.005em]">
                  {p.k}
                </h3>
                <p className="mt-1.5 text-[12.5px] leading-relaxed text-fd-muted-foreground">
                  {p.v}
                </p>
              </div>
            ))}
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div
                key={c.k}
                className="rounded-xl border border-fd-border bg-fd-card/50 px-5 py-6"
              >
                <span className="font-mono text-[10px] tracking-[0.2em] text-crux">
                  {c.k}
                </span>
      ...
      <section className="relative border-t border-fd-border px-6 py-24">
        <div className="mx-auto max-w-6xl">
          <div className="grid items-start gap-16 lg:grid-cols-[0.9fr_1.1fr]">
            <div
              className="lg:sticky lg:top-0 lg:-my-24 lg:py-24"
              style={{ zIndex: 1 }}
            >
              ...
              <p className="mt-5 text-[0.95rem] leading-relaxed text-fd-muted-foreground">
                Memory, retrieval, guardrails. One prompt. A Crux prompt has a
                single{" "}
                <code className="rounded bg-fd-card/80 px-1 font-mono text-[0.9em]">
                  use:
                </code>{" "}
                array. Drop any combination of blocks into it; they add context,
                tools, and checks without scattering logic across the app. The
                SDK still makes the call.
              </p>
              <div className="mt-8 space-y-3.5">
                {[
                  [
                    "Resolve",
                    "Blocks become the prompt the model actually sees.",
                  ],
                  [
                    "Adapt",
                    "Translated to the provider or runner you point at.",
                  ],
                  [
                    "Validate",
                  ...
                  >
                    <code className="font-mono text-[12.5px] font-semibold text-crux">
                      {k}
                    </code>
                    <span className="text-[13.5px] leading-[1.55] text-fd-muted-foreground">
                      {v}
                    </span>
                  </div>
                ))}
          ...
          <div className="flex flex-col items-center gap-3.5">
            ...
            <div className="grid w-full grid-cols-2 gap-3 sm:grid-cols-5">
              {adapters.map((a) => (
                <div
                  key={a.pkg}
                  className="rounded-xl border border-fd-border bg-fd-card/50 p-4 text-center"
                >
                  <p className="font-mono text-[11px] text-fd-muted-foreground/60">
                    {a.pkg}
                  </p>
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            >
              <div className="mb-4 flex items-center gap-3">
                ...
                <span className="text-[11px] font-medium tracking-[0.15em] uppercase text-crux/60">
                  Development
                </span>
              </div>
              <h3 className="text-lg font-semibold">Visual devtools</h3>
              <p className="mt-2 text-[13px] leading-relaxed text-fd-muted-foreground">
                Live trace timelines, resolved prompts, Eval runs, Baselines,
                and Review. The browser and CLI expose the same durable
                evidence.
              </p>
            ...
            >
              ...
              <p className="mt-2 text-[13px] leading-relaxed text-fd-muted-foreground">
                Send spans to Datadog, Honeycomb, Grafana, or any
                OTel-compatible platform. Works in Lambda, Convex, and
                Cloudflare Workers.
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                {["Datadog", "Honeycomb", "Grafana", "New Relic"].map(
                  (platform) => (
                    <span
                      key={platform}
          ...
          <div className="mt-6 rounded-xl border border-dashed border-crux/20 bg-crux-soft/30 px-6 py-4 text-center">
            <p className="text-[13px] text-fd-muted-foreground">
              ...
              <Link
                ...
                className="font-medium text-crux hover:text-crux-hover"
              >
                plugin system
              </Link>
              . Composable, zero-overhead when disabled, extensible with custom
              plugins.
            </p>
          </div>
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-fd-border bg-fd-card/50 p-6">
              ...
              <div className="grid gap-3.5">
                {judges.map((j) => (
                  <div
                    key={j.name}
                    className="grid items-start gap-3.5 sm:grid-cols-[7rem_1fr]"
                  >
                    <code className="font-mono text-[12.5px] font-semibold text-crux">
                      {j.name}
                    </code>
                    <span className="text-[13px] leading-[1.5] text-fd-muted-foreground">
                      {j.desc}
                    </span>
                  </div>
                ))}
        ...
        <div className="mx-auto max-w-4xl">
          ...
          <div className="border-t border-fd-border">
            {tradeRows.map((r, i) => (
              ...
              >
                <span className="text-[15px] text-fd-muted-foreground/80 sm:text-right">
                  {r.l}
                </span>
                <span className="font-mono text-[14px] text-crux sm:text-center">
                  →
                </span>
      ...
      <section className="relative border-t border-fd-border px-6 py-28">
        ...
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-6 text-[1.05rem] text-fd-muted-foreground">
            Add the rest when you need them. Bring your SDK. No runtime to
            adopt. Nothing to migrate away from.
          </p>
