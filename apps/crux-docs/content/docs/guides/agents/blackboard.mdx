...
<Callout type="warn">
Board writes are in-process only. For cross-process coordination (e.g., separate serverless invocations), provide a persistent `store` — see [Memory](/docs/guides/memory) for store options.
</Callout>
