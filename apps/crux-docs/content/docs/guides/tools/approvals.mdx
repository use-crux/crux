...
Native adapters return Crux message history on `result.messages`. Use the browser-safe helpers from `@crux/core/tool-approvals` to find approval requests and append decisions.
...
`@crux/core/tool-approvals` is safe to import in browser components. It only exports message-shape helpers and types. Keep `approvalMiddleware()` itself on the server with your prompt/tool definitions.
