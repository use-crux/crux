...
config({
  prompts,
  contexts,
  tools, // optional — registers tool definitions in the devtools catalog
  devtools: {
...
<Callout type="idea">
  Leave the `config()` call in production code — all instrumentation is zero-overhead when devtools are disabled, so
  there is no need to conditionally remove it.
</Callout>
...
- `evaluatePrompt()` runs — per-case results across models
- `evaluateContext()` and `evaluateCompaction()` runs
- `evaluateFlow()` runs — results matrix with per-step detail
- Quality experiments
...
crux catalog               # list Project Catalog definitions
...
crux catalog reindex       # force source/.crux quality reindex
crux catalog <id>          # detail for a specific item

crux inspect <prompt-id>   # token breakdown from most recent trace

crux evals                 # list past eval runs
crux evals <id>            # eval detail with per-model breakdown
...
```bash
crux eval                  # discover and run all evals
crux eval --filter <name>  # filter by eval name
crux eval --ci             # CI mode: no color, JSON summary, exit codes
crux eval --json report.json   # export JSON report
crux eval --report analysis.md # export LLM analysis prompt
```
...
- **Session grouping** — traces inside `withSession()` appear as collapsible groups
...
- **Tool calls** — each invocation with arguments, raw result preview, model-facing `toModelOutput()` preview, duration, and output-size savings
...
- **Errors** — full error message and stack trace when a call fails
...
Browse discovered prompts, contexts, tools, suites, evals, source locations, snippets, authored paths, relations, and diagnostics before the first run. `crux dev` rebuilds this Go-owned read model from source files and `.crux/quality` JSON on startup; runtime snapshots only enrich it. Use `definition.source.file` for file-tree grouping and `definition.path` for user-authored prompt/context/tool tree grouping. Memory and blackboard definitions include authored store bindings and Zod schemas when the indexer can safely resolve them, including `workingState({ schema })` blocks nested in a `memory({ blocks })` definition. Click any item to inspect source metadata and relations. Use `crux catalog reindex` when you want to force a rebuild from the CLI.

### Memory explorer

The **Memory** nav item opens a dedicated view for browsing memory activity and inspectable snapshots. Runtime memory and blackboard reads/writes come from canonical `memory.read` / `memory.write` spans, `memory.snapshot` artifacts, and semantic memory edges. The Go backend joins those records with Project Catalog definitions to expose store details, schemas, source links, read/write trends, and cross-store operation history when the runtime captured the relevant metadata. When Runtime Bridge live inspection is available, the same memory detail response includes `inspection.status: "ok"`, `inspection.source: "mixed"`, and live store entries. When the bridge is unavailable, ambiguous, or fails, the response keeps the projected state and includes `inspection.status: "partial"` with a reason, message, and docs link. Missing optional fields mean "not captured yet", not zero or empty.
...
| `c`       | Browse catalog                              |
| `Tab`     | Switch catalog tab (prompts/contexts/tools) |
| `Esc`     | Go back                                     |
| `q`       | Quit                                        |
...
The `crux` CLI is a native Go binary. It owns the HTTP API, WebSocket/SSE subscriptions, SQLite persistence, graph/resource read-model assembly, the TUI, and the embedded web UI assets.
...
4. Builds the Project Catalog by indexing source files and `.crux/quality` JSON with a bounded embedded Node worker
