...
3. **`@crux/google`** creates server-side `CachedContent` objects via Google's caching API, then references them in `generateContent()` calls. Handles creation, reuse, TTL, concurrency dedup, and graceful error fallback automatically.
...
- **Instrumentation hooks**: `onContextCacheHit` and `onContextCacheMiss` fire from `composeSystem()` with contextId, cache key, and age/resolution timing
...
<Cards>
  <Card title="Token Management" description="Token budgets, priority-based dropping, and custom tokenizers." href="/docs/guides/contexts/token-budget" />
  <Card title="Best Practices" description="Guidelines for composing contexts effectively." href="/docs/foundations/best-practices" />
  <Card title="Compaction" description="Summarization and sliding windows for long conversations." href="/docs/guides/compaction" />
</Cards>
