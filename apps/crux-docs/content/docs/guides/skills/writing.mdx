...
|---|---|---|---|
| `name` | yes | yes | Becomes the skill `id` and the catalog entry name |
| `description` | yes | yes | One-line summary shown in the catalog — this is what the LLM sees when deciding to load |
...
**Optimize the description for routing, not marketing.** The model loads a skill based on the catalog description alone. Make it answer the question *"what kind of task would I use this for?"* in one line.
...
| `skill.fromRegistry()` | Community skills (`skills.sh:...`) or skills hosted on a custom registry. Fetched lazily on first `prompt.resolve()` and cached in memory. See [Registries](/docs/guides/skills/registries). |
