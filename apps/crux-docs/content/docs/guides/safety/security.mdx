...
```ts
config({ prompts, securityWarnings: false }) // disable in dev
config({ prompts, securityWarnings: true })  // enable in prod
```
...
<Cards>
  ...
  <Card title="Type System" description="Input merging, conditional return types, and settings." href="/docs/guides/advanced/type-system" />
  <Card title="Middleware & Hooks" description="Global middleware for logging and error handling." href="/docs/guides/observability/middleware" />
  <Card title="API Reference" description="Full security utility signatures — safe, escapeXml, limit, wrap, detectSuspiciousPatterns." href="/docs/reference/crux-core" />
</Cards>
