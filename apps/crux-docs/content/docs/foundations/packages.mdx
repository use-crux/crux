...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------ |
| `@crux/core`      | Prompts, contexts, memory, plans, tasks, compaction, scoring, agents, catalog contracts, lint contracts, runtime bridge, observability | Always — this is the base                              |
| `@crux/react`     | React provider, hooks, transports, and SSE server helper                                                                              | You render Crux plans/tasks/memory in React             |
| `@crux/ai`        | Vercel AI SDK adapter (`generate`, `stream`) + stream integration for plans/tasks                                                    | You use `ai` / `@ai-sdk/*`                             |
...
| `@crux/local`     | Local Go runtime, dev server, web UI host, TUI, eval runner, catalog, lint, and trace services                                       | Development and debugging                              |
...
| `@crux/core/catalog`     | Project Catalog contracts and serializers                                           |
...
| `@crux/core/ai-agent`   | `resolve()` for non-Crux agent frameworks                                            |
...
config({ prompts, devtools: { serverUrl: process.env.DEVTOOLS_URL } })
```
...
crux eval --ci          # CI mode with exit codes
```
