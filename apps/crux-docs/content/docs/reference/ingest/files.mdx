...
`fileSource()` and `filesSource()` use the same bundled parser catalog as the rest of `@crux/ingest`:
...
For parser output details, see the [@crux/ingest overview](/docs/reference/ingest#bundled-parsers).
